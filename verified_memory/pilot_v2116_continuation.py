"""Zero-provider V2.11.6 continuation import and dispatch authority.

V2.11.6 is deliberately not a retry of the terminal V2.11.5 A/C cells.  It
imports one immutable *authority view* of that release, registers a fresh
87-cell ledger (one operational import plus the 86 untouched D/B/cross cells),
and seals the remaining provider matrix before credentials may be loaded.
...
from .pilot_contract import PilotContract, PilotRunSpec, canonical_sha256, load_pilot_contract
...
V2116_SOURCE_MANIFEST_PATH = PurePosixPath(
    "experiments/pilot_v2_11_6_source_manifest.json"
)
V2116_RAW_ROOT = PurePosixPath("experiment_results/pilot-v2.11.6/raw")
V2116_PARENT_IMPORT_SCHEMA_VERSION = "finevo-pilot-v2.11.6-parent-import-v1"
V2116_SOURCE_MANIFEST_SCHEMA_VERSION = "finevo-pilot-v2.11.6-source-manifest-v1"
V2116_CURRENT_AUTHORITY_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.6-continuation-observed-p95-authority-v1"
)
V2116_CURRENT_PROJECTION_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.6-continuation-projection-v1"
)
V2116_ACCEPTANCE_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.6-scientific-dispatch-acceptance-v1"
)
V2116_ACCEPTANCE_FILENAME = "scientific_dispatch_acceptance.json"
...
V2116_REMAINING_CELL_COUNT = 86
V2116_LEDGER_CELL_COUNT = 87
V2116_REMAINING_PROVIDER_COMPLETIONS = 3_256
V2116_REMAINING_PROJECTED_COST_USD = 149.3301875
V2116_REMAINING_PROJECTED_STORAGE_BYTES = 1_020_000_000
V2116_PROJECTED_CUMULATIVE_COST_USD = 212.4498325625
V2116_PROJECTED_CUMULATIVE_COMPLETIONS = 6_696
V2116_PROJECTED_CUMULATIVE_STORAGE_BYTES = 1_290_188_235
...
_PARENT_CONTRACT_PATH = PurePosixPath("experiments/pilot_v2_11_5.yaml")
_PARENT_SOURCE_MANIFEST_PATH = PurePosixPath(
    "experiments/pilot_v2_11_5_source_manifest.json"
)
_PARENT_RAW_ROOT = PurePosixPath("experiment_results/pilot-v2.11.5/raw")
_PARENT_GATE_PATH = _PARENT_RAW_ROOT / "long-context-preflight/post_gate_authority.json"
_CURRENT_AUTHORITY_PATH = (
    V2116_RAW_ROOT / "parent-import/current_authority/post_gate_authority.json"
)
...
class PilotV2116ContinuationError(RuntimeError):
    """Raised before V2.11.6 may construct a provider."""
...
def _json_copy(value: Any) -> Any:
    try:
        ...
        raise PilotV2116ContinuationError("value is not canonical JSON") from exc


def _strict_json(path: Path, *, name: str) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise PilotV2116ContinuationError(f"{name} must be a regular non-symlink file")

    def pairs(rows: list[tuple[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for key, value in rows:
            if key in result:
                raise PilotV2116ContinuationError(
    ...
    def nonfinite(value: str) -> None:
        raise PilotV2116ContinuationError(f"{name} contains non-finite {value}")
    ...
    try:
        ...
        raise PilotV2116ContinuationError(f"{name} is not strict UTF-8 JSON") from exc
    if not isinstance(value, dict):
        raise PilotV2116ContinuationError(f"{name} must contain an object")
    return value
...
def _write_once(path: Path, value: Mapping[str, Any]) -> None:
    ...
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != encoded:
            raise PilotV2116ContinuationError(f"immutable artifact drifted: {path}")
...
def _real_root(value: str | Path, *, name: str) -> Path:
    ...
    try:
        ...
        raise PilotV2116ContinuationError(f"{name} is unavailable") from exc
    if stat.S_ISLNK(mode) or not stat.S_ISDIR(mode):
        raise PilotV2116ContinuationError(f"{name} must be a real directory")
    return root


def _verify_parent_git(root: Path) -> dict[str, str]:
    tag_ref = f"refs/tags/{V2115_SCIENCE_TAG}"
    observed = {
    ...
    expected = {
        ...
        "head_ref": "HEAD",
        "tag_type": "tag",
        "tag_object": V2115_SCIENCE_TAG_OBJECT,
        "peeled_commit": V2115_SCIENCE_COMMIT,
        "status": "",
    }
    if observed != expected:
        raise PilotV2116ContinuationError(
            "V2.11.5 parent is not the exact clean detached annotated science tag"
        )
...
def _raw_inventory(root: Path) -> dict[str, Any]:
    raw_root = root.joinpath(*_PARENT_RAW_ROOT.parts)
    if raw_root.is_symlink() or not raw_root.is_dir():
        raise PilotV2116ContinuationError("V2.11.5 raw root is unavailable")
    rows: list[dict[str, Any]] = []
    for path in sorted(raw_root.rglob("*"), key=lambda item: item.as_posix()):
        if path == raw_root / ".real-stage-execution.lock":
            continue
        if path.is_symlink():
            raise PilotV2116ContinuationError("V2.11.5 raw tree contains a symlink")
        if path.is_file():
            rows.append(
                {
    ...
    result = {
        "root": _PARENT_RAW_ROOT.as_posix(),
    ...
    if result != {
        ...
        "canonicalization": "json-sort-keys-compact-utf8-v1",
        "excluded_operational_paths": [".real-stage-execution.lock"],
        "file_count": V2115_RAW_INVENTORY_FILE_COUNT,
        "storage_bytes": V2115_RAW_INVENTORY_STORAGE_BYTES,
        "inventory_sha256": V2115_RAW_INVENTORY_SHA256,
    }:
        raise PilotV2116ContinuationError("V2.11.5 parent raw inventory drifted")
    return result
...
def _normalized_d_plan_receipts(
    ...
    for seed in child_contract.seeds["sets"]["main"]:
        ...
        if child_value != parent_value:
            raise PilotV2116ContinuationError(
                f"V2.11.6 D plan differs from V2.11.5 at seed {seed}"
            )
        rows.append(
            {
                "seed": seed,
    ...
    if len(rows) != 5:
        raise PilotV2116ContinuationError("D plan equivalence lacks five seeds")
    return rows
...
def _implementation_equivalence(
    ...
    *,
    ...
    if (
        ...
        or new_functions != _REVIEWED_NEW_ORCHESTRATOR_FUNCTIONS
...
def _current_runtime_source_bindings(root: Path, parent: Path) -> dict[str, Any]:
    full_hash_paths = (
        ...
        "verified_memory/pilot_v2116_continuation.py",
        "scripts/render_pilot_v2116_contract.py",
        "scripts/render_pilot_v2116_source_manifest.py",
    )
...
def build_v2116_source_manifest(
    *,
    contract: PilotContract,
    repo_root: str | Path,
    parent_repo_root: str | Path,
) -> dict[str, Any]:
    """Build the deterministic current/parent source and raw identity."""

    if contract.contract_id != V2116_CONTRACT_ID:
        raise PilotV2116ContinuationError("source manifest requires V2.11.6")
    child = _real_root(repo_root, name="V2.11.6 repository")
    parent = _real_root(parent_repo_root, name="V2.11.5 parent repository")
    release = _verify_parent_git(parent)
    parent_contract_path = parent.joinpath(*_PARENT_CONTRACT_PATH.parts)
    parent_contract = load_pilot_contract(parent_contract_path)
    if (
        parent_contract.contract_id != V2115_CONTRACT_ID
        or parent_contract.canonical_hash != V2115_CONTRACT_SHA256
        or _file_sha256(parent_contract_path) != V2115_CONTRACT_FILE_SHA256
    ):
        raise PilotV2116ContinuationError("V2.11.5 parent contract identity drifted")
    source_path = parent.joinpath(*_PARENT_SOURCE_MANIFEST_PATH.parts)
    source = _strict_json(source_path, name="V2.11.5 source manifest")
    if (
        _file_sha256(source_path) != V2115_SOURCE_MANIFEST_FILE_SHA256
        or source.get("integrity", {}).get("content_sha256")
        != V2115_SOURCE_MANIFEST_CONTENT_SHA256
        or _content_sha256(source) != V2115_SOURCE_MANIFEST_CONTENT_SHA256
    ):
        raise PilotV2116ContinuationError("V2.11.5 source manifest drifted")
    return _seal(
        {
            "schema_version": V2116_SOURCE_MANIFEST_SCHEMA_VERSION,
            ...
            "parent_release": {
                ...
                "source_manifest_path": _PARENT_SOURCE_MANIFEST_PATH.as_posix(),
            ...
            "parent_raw_inventory": _raw_inventory(parent),
            ...
            "observation_boundary": {
                "parent_a_c_outcomes_are_frozen_external_evidence": True,
                "parent_a_c_rows_imported_into_child_ledger": 0,
                "parent_scheduled_cells_mapped_to_child": 86,
...
def _boundary(contract: PilotContract) -> Mapping[str, Any]:
    value = getattr(contract, "v2116_continuation_boundary", None)
    if not isinstance(value, Mapping):
        raise PilotV2116ContinuationError("V2.11.6 continuation boundary is absent")
    return value


def validate_v2116_source_manifest(
    *,
    contract: PilotContract,
    repo_root: str | Path,
    parent_repo_root: str | Path,
) -> dict[str, Any]:
    root = _real_root(repo_root, name="V2.11.6 repository")
    path = root.joinpath(*V2116_SOURCE_MANIFEST_PATH.parts)
    observed = _strict_json(path, name="V2.11.6 source manifest")
    expected = build_v2116_source_manifest(
        contract=contract,
        repo_root=root,
        parent_repo_root=parent_repo_root,
    )
    if observed != expected:
        raise PilotV2116ContinuationError("V2.11.6 source manifest replay drifted")
    boundary = _boundary(contract)
    declared = boundary.get("source_manifest")
    if not isinstance(declared, Mapping):
        raise PilotV2116ContinuationError("contract source-manifest boundary is malformed")
    expected_binding = {
        "path": V2116_SOURCE_MANIFEST_PATH.as_posix(),
    ...
    if {key: declared.get(key) for key in expected_binding} != expected_binding:
        raise PilotV2116ContinuationError("contract source-manifest identity drifted")
    return observed


def parent_budget_debit_for_v2116(contract: PilotContract) -> ParentBudgetDebit:
    if contract.contract_id != V2116_CONTRACT_ID:
        raise PilotV2116ContinuationError("parent debit requires V2.11.6")
    boundary = _boundary(contract)
    ...
    if not isinstance(declared, Mapping) or any(
        ...
        raise PilotV2116ContinuationError("V2.11.6 parent budget debit drifted")
...
def current_projection_path(raw_root: str | Path, model_id: str) -> Path:
    if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
        raise PilotV2116ContinuationError(f"unsupported continuation model {model_id}")
...
def _normalize_parent_spec(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not run_id.startswith(prefix):
        raise PilotV2116ContinuationError("parent scheduled run id is malformed")
    value["run_id"] = f"{V2116_CONTRACT_ID}--{run_id[len(prefix):]}"
    value["contract_id"] = V2116_CONTRACT_ID
    value["budget_bucket"] = "hosted_v2116"
    return value
...
def _parent_state(
    *,
    ...
    manifest = validate_v2116_source_manifest(
        contract=contract,
        repo_root=repo_root,
        parent_repo_root=parent_repo_root,
    )
    ...
    if (
        ...
        raise PilotV2116ContinuationError("V2.11.5 run-ledger terminal prefix drifted")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("V2.11.5 budget-ledger terminal prefix drifted")
    current_rows = [
        row
        for row in budget.get("runs", {}).values()
        if isinstance(row, Mapping) and row.get("stage_bucket") == "hosted_v2115"
    ]
    current_actual = {
        "cost_usd": sum(float(row["actual"]["cost_usd"]) for row in current_rows),
        "completions": sum(int(row["actual"]["completions"]) for row in current_rows),
        "storage_bytes": sum(int(row["actual"]["storage_bytes"]) for row in current_rows),
    }
    if current_actual != {
        ...
        raise PilotV2116ContinuationError("V2.11.5 current-release actual debit drifted")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("V2.11.5 A/C terminal disposition drifted")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("V2.11.5 gate authority binding drifted")
    ...
    if normalized_parent != expected_child or len(expected_child) != V2116_REMAINING_CELL_COUNT:
        raise PilotV2116ContinuationError(
            "V2.11.6 remaining denominator is not 1:1 with parent scheduled specs"
        )
...
def _build_current_authority(
    *,
    ...
    if not isinstance(source_reservations, Mapping):
        raise PilotV2116ContinuationError("parent p95 authority is malformed")
    ...
    for runtime_model, by_kind in source_reservations.items():
        if not isinstance(by_kind, Mapping) or set(by_kind) != {"action", "semantic"}:
            raise PilotV2116ContinuationError("parent p95 call-kind denominator drifted")
        ...
        for call_kind in ("action", "semantic"):
            ...
            if not isinstance(authority, Mapping) or not isinstance(reservation, Mapping):
                raise PilotV2116ContinuationError("parent p95 reservation is malformed")
            stable = {
                key: value
    ...
    authority = _seal(
        {
            "schema_version": V2116_CURRENT_AUTHORITY_SCHEMA_VERSION,
            "contract_id": contract.contract_id,
    ...
    for model_id in ("gpt52_main", "gpt56_diagnostic"):
        ...
        projection = _seal(
            {
                "schema_version": V2116_CURRENT_PROJECTION_SCHEMA_VERSION,
                "model_id": model_id,
...
def _verify_seal(value: Mapping[str, Any], *, name: str) -> None:
    ...
    if (
        ...
        raise PilotV2116ContinuationError(f"{name} self-hash mismatch")
...
def _release_binding(contract: PilotContract, paid: Any) -> dict[str, Any]:
    ...
    if (
        ...
        or expected_tag != V2116_SCIENCE_TAG
        ...
        raise PilotV2116ContinuationError(
            "V2.11.6 continuation requires its exact clean annotated release"
        )
    return {
        "git_tag": paid.git_tag,
        "git_commit": paid.head_commit,
...
def _tracked_source_manifest(
    ...
    *,
    ...
    root = _real_root(repo_root, name="V2.11.6 repository")
    path = root.joinpath(*V2116_SOURCE_MANIFEST_PATH.parts)
    value = _strict_json(path, name="V2.11.6 source manifest")
    _verify_seal(value, name="V2.11.6 source manifest")
    declared = _boundary(contract).get("source_manifest")
    if (
        not isinstance(declared, Mapping)
        or declared.get("path") != V2116_SOURCE_MANIFEST_PATH.as_posix()
        or declared.get("schema_version") != V2116_SOURCE_MANIFEST_SCHEMA_VERSION
        or declared.get("file_sha256") != _file_sha256(path)
        or declared.get("content_sha256")
        != value["integrity"]["content_sha256"]
        or value.get("schema_version") != V2116_SOURCE_MANIFEST_SCHEMA_VERSION
        or value.get("contract_id") != contract.contract_id
        or value.get("release_tag") != V2116_SCIENCE_TAG
    ):
        raise PilotV2116ContinuationError(
            "V2.11.6 tracked source-manifest binding drifted"
        )
    return path, value
...
def _capability_summary(wrapper: Mapping[str, Any], *, model_id: str) -> dict[str, Any]:
    ...
    if not isinstance(capability, Mapping) or not isinstance(integrity, Mapping):
        raise PilotV2116ContinuationError("parent capability wrapper is malformed")
    expected_hash = V2115_CAPABILITY_CONTENT_SHA256[model_id]
    if integrity.get("content_sha256") != expected_hash:
        raise PilotV2116ContinuationError("parent capability wrapper identity drifted")
    result = {
        "model_id": model_id,
...
def _dispatch_authority_source(parent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(gate, Mapping):
        raise PilotV2116ContinuationError("parent gate binding is malformed")
    rows = gate.get("reservations")
    if not isinstance(rows, Mapping):
        raise PilotV2116ContinuationError("parent gate reservations are malformed")
    numeric: dict[str, dict[str, Any]] = {}
    stable: dict[str, dict[str, Any]] = {}
    for runtime_model, by_kind in rows.items():
        if not isinstance(by_kind, Mapping) or set(by_kind) != {"action", "semantic"}:
            raise PilotV2116ContinuationError("parent gate call-kind set drifted")
        numeric[str(runtime_model)] = {}
        stable[str(runtime_model)] = {}
        for kind in ("action", "semantic"):
            entry = by_kind[kind]
            if not isinstance(entry, Mapping) or set(entry) != {"authority", "reservation"}:
                raise PilotV2116ContinuationError("parent gate reservation row drifted")
            authority = entry["authority"]
            if not isinstance(authority, Mapping):
                raise PilotV2116ContinuationError("parent authority row is malformed")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("parent p95 authority set drifted")
    return result


def build_v2116_parent_import_receipt(
    *,
    contract: PilotContract,
    repo_root: str | Path,
    raw_root: str | Path,
    ...
    """Deep-verify V2.11.5 and seal a zero-provider continuation view."""

    require_v2116_provider_keys_absent()
    repository = _real_root(repo_root, name="V2.11.6 repository")
    raw = _real_root(raw_root, name="V2.11.6 raw root")
    if raw != repository.joinpath(*V2116_RAW_ROOT.parts):
        raise PilotV2116ContinuationError("V2.11.6 raw namespace drifted")
    release = _release_binding(contract, paid)
    parent_root = _real_root(parent_repo_root, name="V2.11.5 parent repository")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("parent import authority identity drifted")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("parent calibration authority drifted")
    capabilities = parent_import.get("capability_wrappers")
    if not isinstance(capabilities, Mapping) or set(capabilities) != {
        "gpt52_main",
        "gpt56_diagnostic",
    }:
        raise PilotV2116ContinuationError("parent capability denominator drifted")
    ...
    receipt = _seal(
        {
            "schema_version": V2116_PARENT_IMPORT_SCHEMA_VERSION,
            "status": "complete",
            "go": True,
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            ...
            "parent_release": _json_copy(
                _boundary(contract)["parent_release"]
            ),
            "parent_terminal_prefix": _json_copy(
                _boundary(contract)["parent_terminal_prefix"]
            ),
            "parent_stage_receipts": _json_copy(parent["stage_receipts"]),
            "parent_import_authority": {
                "path": (
                    _PARENT_RAW_ROOT / "parent-import/parent_import_receipt.json"
                ).as_posix(),
            ...
            "denominator_continuation": {
                "parent_registered_rows": 136,
                "parent_terminal_rows": 50,
                "parent_scheduled_rows": 86,
                "child_registered_rows": V2116_LEDGER_CELL_COUNT,
                "child_operational_rows": 1,
                "child_scientific_rows": V2116_REMAINING_CELL_COUNT,
                "source_to_child_mapping": "normalized-one-to-one",
                "normalized_mapping_sha256": parent["scheduled_mapping_sha256"],
                "parent_a_c_rows_copied_to_child": 0,
                "parent_a_c_redispatched": False,
            },
            "cumulative_parent_budget_debit": (
                parent_budget_debit_for_v2116(contract).to_dict()
            ),
            "import_policy": {
                "provider_construction": False,
                "provider_calls": 0,
                ...
                "parent_raw_tree_copied": False,
                "validation_before_provider_construction": True,
            },
            "scientific_evidence": False,
            "claim_boundary": (
                "Immutable V2.11.5 terminal-prefix and non-effect authority "
                "import only. A/C outcomes remain parent evidence and no "
                "terminal parent cell is reclassified or redispatched."
            ),
        }
    )
...
def verify_v2116_parent_import_receipt(
    ...
    *,
    ...
    if (
        raw != repository.joinpath(*V2116_RAW_ROOT.parts)
        or path != raw / "parent-import/parent_import_receipt.json"
    ):
        raise PilotV2116ContinuationError("parent-import receipt path drifted")
    receipt = _strict_json(path, name="V2.11.6 parent-import receipt")
    _verify_seal(receipt, name="V2.11.6 parent-import receipt")
    release = _release_binding(contract, paid)
    ...
    parent_stages = receipt.get("parent_stage_receipts")
    denominator = receipt.get("denominator_continuation")
    policy = receipt.get("import_policy")
    if (
        receipt.get("schema_version") != V2116_PARENT_IMPORT_SCHEMA_VERSION
        ...
        or receipt.get("parent_release") != boundary["parent_release"]
        or receipt.get("parent_terminal_prefix")
        != boundary["parent_terminal_prefix"]
        or not isinstance(parent_stages, Mapping)
        or set(parent_stages) != set(V2115_STAGE_RECEIPT_BINDINGS)
        or any(
            parent_stages[stage_id]
            != {
                "path": (
                    _PARENT_RAW_ROOT / stage_id / "stage_receipt.json"
                ).as_posix(),
                **_json_copy(V2115_STAGE_RECEIPT_BINDINGS[stage_id]),
            }
        ...
        or receipt.get("parent_import_authority")
        != {
            "path": (
                _PARENT_RAW_ROOT / "parent-import/parent_import_receipt.json"
            ).as_posix(),
        ...
        raise PilotV2116ContinuationError("V2.11.6 parent-import receipt drifted")
    expected_debit = parent_budget_debit_for_v2116(contract).to_dict()
    if receipt.get("cumulative_parent_budget_debit") != expected_debit:
        raise PilotV2116ContinuationError("parent debit receipt binding drifted")
    return receipt


def verify_v2116_current_authority(
    *,
    contract: PilotContract,
    repo_root: str | Path,
    raw_root: str | Path,
    ...
    repository = _real_root(repo_root, name="V2.11.6 repository")
    raw = _real_root(raw_root, name="V2.11.6 raw root")
    parent = verify_v2116_parent_import_receipt(
    ...
    authority = _strict_json(path, name="V2.11.6 current p95 authority")
    _verify_seal(authority, name="V2.11.6 current p95 authority")
    dispatch = parent["dispatch_authority_source"]
    release = _release_binding(contract, paid)
    if (
        authority.get("schema_version") != V2116_CURRENT_AUTHORITY_SCHEMA_VERSION
        ...
        or authority.get("parent_release")
        != {
            "contract_id": V2115_CONTRACT_ID,
            "contract_sha256": V2115_CONTRACT_SHA256,
            "git_tag": V2115_SCIENCE_TAG,
        ...
        raise PilotV2116ContinuationError("V2.11.6 current p95 authority drifted")
    return authority


def verified_v2116_projection(
    contract: PilotContract,
    model_id: str,
    *,
    repo_root: str | Path,
    ...
    value = _strict_json(path, name=f"V2.11.6 {model_id} p95 projection")
    _verify_seal(value, name=f"V2.11.6 {model_id} p95 projection")
    profile = contract.provider_profiles[model_id]
    runtime_model = (
        f"{profile.transport}/{profile.served_model}"
        if profile.transport != "openrouter"
    ...
    if (
        value.get("schema_version") != V2116_CURRENT_PROJECTION_SCHEMA_VERSION
        ...
        raise PilotV2116ContinuationError(
            f"V2.11.6 {model_id} current p95 projection drifted"
        )
    return value, path


def verified_v2116_calibration(
    contract: PilotContract,
    *,
    repo_root: str | Path,
    raw_root: str | Path,
    ...
    receipt = verify_v2116_parent_import_receipt(
    ...
    if not isinstance(calibration, Mapping):
        raise PilotV2116ContinuationError("V2.11.6 calibration wrapper is malformed")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("V2.11.6 calibration authority drifted")
    return {
        "receipt": receipt,
        "wrapper": wrapper,
        "q_ref": V2115_Q_REF,
...
def verified_v2116_observed_p95_authority_binding(
    receipt_path: str | Path,
    *,
    repo_root: str | Path,
    ...
    repository = _real_root(repo_root, name="V2.11.6 repository")
    contract_path = repository / "experiments/pilot_v2_11_6.yaml"
    contract = load_pilot_contract(contract_path)
    if (
        contract.contract_id != V2116_CONTRACT_ID
        or contract.canonical_hash != expected_contract_sha256
    ):
        raise PilotV2116ContinuationError("V2.11.6 contract identity drifted")
    ...
    paid = _Paid()
    paid.git_tag = V2116_SCIENCE_TAG
    ...
    if requested.absolute() != expected_path:
        raise PilotV2116ContinuationError("V2.11.6 authority path drifted")
    authority = verify_v2116_current_authority(
        contract=contract,
        repo_root=repository,
        raw_root=raw,
...
def runner_reservations_for_v2116(
    contract: PilotContract,
    model_id: str,
    *,
    repo_root: str | Path,
    ...
    projection, _ = verified_v2116_projection(
        contract,
        model_id,
        repo_root=repo_root,
        raw_root=raw_root,
    ...
    if not isinstance(selected, Mapping) or set(selected) != {"action", "semantic"}:
        raise PilotV2116ContinuationError("V2.11.6 runner authority denominator drifted")
    for kind in ("action", "semantic"):
        key = f"{projection['served_model']}::{kind}"
        if selected[kind]["reservation"] != projection["projection"][key]:
            raise PilotV2116ContinuationError("V2.11.6 runner/projection drifted")
...
def _ledger_prefix(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(events, list) or not events or not isinstance(runs, Mapping):
        raise PilotV2116ContinuationError("acceptance ledger prefix is malformed")
    return {
...
def require_v2116_provider_keys_absent() -> None:
...
def _expected_pre_science_paths(contract: PilotContract) -> tuple[set[str], set[str]]:
    ...
    files = {
        ...
        f".{V2116_ACCEPTANCE_FILENAME}.pending",
    ...
    *,
    ...
    for model_id, seed in groups:
        ...
        if len(group) != 11 or len(representatives) != 1:
            raise PilotV2116ContinuationError("Experiment D acceptance group drifted")
        projections.append(
    ...
    if len(projections) != 36:
        raise PilotV2116ContinuationError("V2.11.6 projection-unit denominator drifted")
...
def _acceptance_material(
    ...
    *,
    ...
    try:
        ...
        raise PilotV2116ContinuationError(
            f"complete V2.11.6 continuation exceeds a hard cap: {exc}"
        ) from exc
    ...
    if (
        ...
        or total_calls != V2116_REMAINING_PROVIDER_COMPLETIONS
        or total_storage != V2116_REMAINING_PROJECTED_STORAGE_BYTES
        or not math.isclose(
            total_cost,
            V2116_REMAINING_PROJECTED_COST_USD,
            rel_tol=0.0,
            abs_tol=1e-12,
        )
    ):
        raise PilotV2116ContinuationError(
            "V2.11.6 full remaining projection differs from preregistration"
        )
    ...
    if len(config_hashes) != V2116_REMAINING_CELL_COUNT:
        raise PilotV2116ContinuationError("V2.11.6 config denominator drifted")
    parent_specs = tuple(contract.expand(stage="parent-import"))
    ...
    return {
        "parent_import": {
            "path": (V2116_RAW_ROOT / "parent-import/parent_import_receipt.json").as_posix(),
        ...
        "budget_projection": {
            ...
            "projected_cumulative_cost_usd": V2116_PROJECTED_CUMULATIVE_COST_USD,
            "projected_cumulative_hosted_completions": (
                V2116_PROJECTED_CUMULATIVE_COMPLETIONS
            ),
            "projected_cumulative_storage_bytes": (
                V2116_PROJECTED_CUMULATIVE_STORAGE_BYTES
            ),
...
def _verified_parent_import_budget_actual(
    ...
    parent_specs = tuple(contract.expand(stage="parent-import"))
    if len(parent_specs) != 1:
        raise PilotV2116ContinuationError("parent-import budget denominator drifted")
    expected = orch._v2116_parent_import_projection(parent_specs[0]).to_dict()
...
def _audit_acceptance_denominator(
    ...
    if (
        len(specs) != V2116_LEDGER_CELL_COUNT
        ...
        raise PilotV2116ContinuationError("V2.11.6 ITT ledger denominator drifted")
    parent_specs = tuple(contract.expand(stage="parent-import"))
    if len(parent_specs) != 1 or rows[parent_specs[0].run_id].get("status") != "complete":
        raise PilotV2116ContinuationError("parent import is not terminal complete")
    scientific = tuple(
        spec
        for stage_id in ("experiment-d", "experiment-b", "cross-model")
        for spec in contract.expand(stage=stage_id)
    ...
    if len(scientific) != 86:
        raise PilotV2116ContinuationError("scientific continuation denominator drifted")
...
def _acceptance_receipt(
    ...
    *,
    ...
    return _seal(
        {
            "schema_version": V2116_ACCEPTANCE_SCHEMA_VERSION,
            "status": "go",
            "go": True,
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            "release": _release_binding(contract, paid),
            "raw_namespace": V2116_RAW_ROOT.as_posix(),
...
def _verify_acceptance_marker(
    ...
    *,
    ...
    if (
        ...
        raise PilotV2116ContinuationError("accepted ledger event prefix is absent")
    marker = events[count]
    expected = {
        "receipt_schema_version": V2116_ACCEPTANCE_SCHEMA_VERSION,
    ...
    if marker.get("event_type") != "acceptance_receipt_bound" or marker.get(
        ...
        raise PilotV2116ContinuationError("acceptance ledger marker drifted")
...
def _verify_acceptance_prefix_state(
    ...
    *,
    ...
    if (
        ...
        raise PilotV2116ContinuationError("accepted ledger prefix drifted")
    if len(events) == count:
        if (
...
def _verify_current_accepted_budget_rows(
    ...
    if not isinstance(rows, Mapping) or len(parent_specs) != 1:
        raise PilotV2116ContinuationError("accepted budget rows are malformed")
...
def _verify_v2116_acceptance_core(
    receipt: Mapping[str, Any],
    *,
    contract: PilotContract,
    repo_root: Path,
    ...
    _verify_seal(receipt, name="V2.11.6 scientific acceptance")
    if (
        set(receipt) != _ACCEPTANCE_TOP_LEVEL_FIELDS
        or receipt.get("schema_version") != V2116_ACCEPTANCE_SCHEMA_VERSION
        or receipt.get("status") != "go"
        or receipt.get("go") is not True
        or receipt.get("contract_id") != contract.contract_id
        or receipt.get("contract_sha256") != contract.canonical_hash
        or receipt.get("release") != _release_binding(contract, paid)
        or receipt.get("raw_namespace") != V2116_RAW_ROOT.as_posix()
        ...
        raise PilotV2116ContinuationError("V2.11.6 acceptance identity drifted")
    material = _acceptance_material(
        contract,
        repo_root=repo_root,
        raw_root=raw_root,
    ...
    for key, value in material.items():
        if receipt.get(key) != value:
            raise PilotV2116ContinuationError(
                f"V2.11.6 acceptance field {key!r} drifted"
            )
    run_snapshot = run_ledger.snapshot()
    budget_snapshot = budget_ledger.snapshot()
    rows = run_snapshot.get("runs")
    ...
    if (
        ...
        raise PilotV2116ContinuationError("accepted ITT denominator no longer matches")
    ...
    if not isinstance(prefixes, Mapping) or set(prefixes) != {
        ...
        raise PilotV2116ContinuationError("acceptance ledger prefixes are absent")
    relative = (V2116_RAW_ROOT / V2116_ACCEPTANCE_FILENAME).as_posix()
...
def verify_v2116_scientific_dispatch_acceptance(
    receipt_path: str | Path,
    *,
    contract: PilotContract,
    repo_root: str | Path,
    ...
    raw = _real_root(raw_root, name="V2.11.6 raw root")
    path = Path(receipt_path).absolute()
    if path != raw / V2116_ACCEPTANCE_FILENAME:
        raise PilotV2116ContinuationError("V2.11.6 acceptance path drifted")
    receipt = _strict_json(path, name="V2.11.6 scientific acceptance")
    with _acceptance_provider_sentinels():
        _verify_v2116_acceptance_core(
            receipt,
            contract=contract,
            repo_root=repository,
            raw_root=raw,
...
def accept_v2116_scientific_dispatch(
    *,
    ...
    expected_contract = repository / "experiments/pilot_v2_11_6.yaml"
    if contract_candidate.absolute() != expected_contract:
        raise PilotV2116ContinuationError("acceptance contract path drifted")
    contract = load_pilot_contract(contract_candidate)
    raw = _real_root(raw_root, name="V2.11.6 raw root")
    if raw != repository.joinpath(*V2116_RAW_ROOT.parts):
        raise PilotV2116ContinuationError("acceptance raw namespace drifted")
    output = raw / V2116_ACCEPTANCE_FILENAME
    if receipt_path is not None and Path(receipt_path).absolute() != output:
        raise PilotV2116ContinuationError("acceptance output path drifted")
    launch = Path(scientific_launch_input_path).absolute()
    if launch != raw / "scientific_launch_input.json":
        raise PilotV2116ContinuationError("scientific launch input path drifted")
    with orch._exclusive_real_stage_lock(
        ...
        budget_ledger = PilotBudgetLedger(
            ...
            tamper_evident=True,
            parent_debit=parent_budget_debit_for_v2116(contract),
        )
        _audit_pre_science_namespace(raw, contract)
        with _acceptance_provider_sentinels():
            if output.exists():
                receipt = _strict_json(
                    output, name="V2.11.6 scientific acceptance"
                )
                run_marked, budget_marked = _verify_v2116_acceptance_core(
                    receipt,
                    contract=contract,
                    repo_root=repository,
                    raw_root=raw,
        ...
        if not isinstance(prefixes, Mapping):
            raise PilotV2116ContinuationError("acceptance prefixes are malformed")
        relative = (V2116_RAW_ROOT / V2116_ACCEPTANCE_FILENAME).as_posix()
        common = {
            "receipt_schema_version": V2116_ACCEPTANCE_SCHEMA_VERSION,
            "receipt_path": relative,
        ...
        return verify_v2116_scientific_dispatch_acceptance(
            output,
            contract=contract,
            repo_root=repository,
            raw_root=raw,
