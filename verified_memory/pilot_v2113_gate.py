"""Zero-provider V2.11.3 reseal and global dispatch gate.
...
V2.11.3 raw namespace, one registered model per authority-import cell, and
...
from .pilot_v2113_parent_import import (
    ...
    V2112_SCIENCE_TAG,
    V2113_ALLOWED_MODELS,
    V2113_CONTRACT_ID,
    V2113_CONTRACT_PATH,
    V2113_DEFAULT_RECEIPT_PATH,
    V2113_RAW_ROOT,
    V2113_SCIENCE_TAG,
    V2113_SOURCE_MANIFEST_CONTENT_SHA256,
    V2113_SOURCE_MANIFEST_FILE_SHA256,
    V2113_SOURCE_MANIFEST_PATH,
    _atomic_json,
    _json_copy,
    _read_regular,
    _sha256,
...
V2113_RESEALED_P95_AUTHORITY_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.3-resealed-observed-p95-authority-v1"
)
V2113_RESEALED_P95_SOURCE_KIND = "v2.11.2-fresh-preflight-v2.11.3"
V2113_PREFLIGHT_IMPORT_GATE_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.3-preflight-authority-import-gate-v1"
)
V2113_GATE_SCHEMA_VERSION = "finevo-pilot-v2.11.3-post-gate-authority-v1"
V2113_POST_GATE_RELATIVE_PATH = (
    V2113_RAW_ROOT / "long-context-preflight/post_gate_authority.json"
)
...
class PilotV2113GateError(RuntimeError):
    """Raised before a V2.11.3 dispatch authority can be consumed."""
...
def _mapping(value: Any, name: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise PilotV2113GateError(f"{name} must be an object")
    return value


def _digest(value: Any, name: str) -> str:
    if not isinstance(value, str) or _SHA256_RE.fullmatch(value) is None:
        raise PilotV2113GateError(f"{name} must be a lowercase SHA-256")
    return value


def _commit(value: Any, name: str) -> str:
    if not isinstance(value, str) or _COMMIT_RE.fullmatch(value) is None:
        raise PilotV2113GateError(f"{name} must be a lowercase git commit")
    return value
...
def _raw_root(repo_root: Path, raw_root: str | Path) -> Path:
    ...
    expected = repo_root.joinpath(*V2113_RAW_ROOT.parts)
    if candidate != expected:
        raise PilotV2113GateError(
            "V2.11.3 observed-p95 authority requires its exact raw namespace"
        )
    return candidate
...
def _relative_to_repo(repo_root: Path, path: str | Path, *, name: str) -> PurePosixPath:
    ...
    if candidate.is_absolute():
        try:
            ...
            raise PilotV2113GateError(f"{name} escaped the repository") from exc
    text = str(candidate)
    ...
    if (
        ...
        raise PilotV2113GateError(f"{name} is not a normalized raw path")
    return relative
...
def _git(repo_root: Path, *args: str) -> str:
    try:
        ...
        raise PilotV2113GateError(
            "V2.11.3 git release identity is unavailable"
        ) from exc
    return result.stdout.strip()


def _verify_release_identity(repo_root: Path, expected_git_commit: str) -> None:
    commit = _commit(expected_git_commit, "V2.11.3 release commit")
    if (
        _git(repo_root, "rev-parse", f"refs/tags/{V2113_SCIENCE_TAG}^{{object}}")
        == commit
    ):
        # An annotated tag object must not equal its peeled commit.
        raise PilotV2113GateError("V2.11.3 science tag is not annotated")
    peeled = _git(repo_root, "rev-parse", f"refs/tags/{V2113_SCIENCE_TAG}^{{commit}}")
    tag_type = _git(repo_root, "cat-file", "-t", f"refs/tags/{V2113_SCIENCE_TAG}")
    if peeled != commit or tag_type != "tag":
        raise PilotV2113GateError("V2.11.3 annotated tag/commit binding drifted")
...
def _contract(
    ...
    contract = selected or load_pilot_contract(
        repo_root.joinpath(*V2113_CONTRACT_PATH.parts)
    )
    try:
        _validate_child(
            contract=contract,
        ...
        raise PilotV2113GateError(str(exc)) from exc
    body = contract.to_dict()
    forward = _mapping(
        body.get("v2113_forward_boundary"),
        "V2.11.3 forward boundary",
    )
    source = _mapping(forward.get("source_manifest"), "V2.11.3 source manifest")
    if source != {
        "path": V2113_SOURCE_MANIFEST_PATH.as_posix(),
        "schema_version": "finevo-pilot-v2.11.3-source-manifest-v1",
        "file_sha256": V2113_SOURCE_MANIFEST_FILE_SHA256,
        "content_sha256": V2113_SOURCE_MANIFEST_CONTENT_SHA256,
    }:
        raise PilotV2113GateError("V2.11.3 contract/source-manifest binding drifted")
    return contract
...
def _contract_binding(repo_root: Path, contract: PilotContract) -> dict[str, Any]:
    raw = _read_regular(
        repo_root,
        V2113_CONTRACT_PATH,
        name="V2.11.3 contract",
    )
    observed = _strict_json(raw, name="V2.11.3 contract")
    if observed != contract.to_dict():
        raise PilotV2113GateError(
            "V2.11.3 contract bytes differ from selected contract"
        )
    return {
        "path": V2113_CONTRACT_PATH.as_posix(),
...
def _load_parent_receipt(
    ...
    *,
    ...
    relative = V2113_DEFAULT_RECEIPT_PATH
    raw = _read_regular(repo_root, relative, name="V2.11.3 parent import receipt")
    value = _strict_json(raw, name="V2.11.3 parent import receipt")
    if supplied is not None and value != _json_copy(dict(supplied)):
        raise PilotV2113GateError(
            "supplied V2.11.3 parent receipt differs from persisted bytes"
        )
    try:
        verified = validate_v2113_parent_import_receipt(
            value,
            contract=contract,
            child_git_commit=expected_git_commit,
            repo_root=repo_root,
        ...
        raise PilotV2113GateError(
            f"V2.11.3 parent import receipt failed: {exc}"
        ) from exc
    return verified, {
...
def v2113_observed_p95_receipt_path(
    raw_root: str | Path,
    profile_id: str,
) -> Path:
    if profile_id not in V2113_ALLOWED_MODELS:
        raise PilotV2113GateError(f"unsupported V2.11.3 p95 profile: {profile_id}")
    return (
        Path(raw_root)
...
def v2113_observed_p95_projection_path(
    raw_root: str | Path,
    profile_id: str,
) -> Path:
    return v2113_observed_p95_receipt_path(raw_root, profile_id).with_name(
        "projection_p95.json"
    )
...
def _runtime_profile(contract: PilotContract, profile_id: str) -> tuple[str, str]:
    if profile_id not in contract.provider_profiles:
        raise PilotV2113GateError("V2.11.3 p95 profile is absent from the contract")
    ...
    if runtime != _RUNTIME_MODELS[profile_id]:
        raise PilotV2113GateError("V2.11.3 runtime model differs from parent authority")
...
def _inherited_source_reservations(
    *,
    ...
    if set(by_kind) != {"action", "semantic"}:
        raise PilotV2113GateError("V2.11.3 source p95 call-kind denominator drifted")
    result: dict[str, Any] = {}
    for call_kind in ("action", "semantic"):
        ...
        if set(source_entry) != {"authority", "reservation"}:
            raise PilotV2113GateError("V2.11.3 source p95 entry shape drifted")
        ...
        try:
            ...
            raise PilotV2113GateError(
                f"V2.11.3 {profile_id}/{call_kind} p95 authority is invalid"
            ) from exc
        if (
            ...
            raise PilotV2113GateError("V2.11.3 inherited p95 provenance drifted")
        result[call_kind] = parsed
    return {runtime: result}


def build_v2113_resealed_observed_p95_authority(
    *,
    repo_root: str | Path,
    contract: PilotContract,
    raw_root: str | Path,
    ...
    root = _strict_root(repo_root, name="V2.11.3 repository")
    ...
    wrappers = preflight_wrappers_from_v2113_receipt(parent)
    ...
    receipt = _seal(
        {
            "schema_version": V2113_RESEALED_P95_AUTHORITY_SCHEMA_VERSION,
            ...
            "raw_root": V2113_RAW_ROOT.as_posix(),
            "git": {"tag": V2113_SCIENCE_TAG, "commit": expected_git_commit},
            "model": {
                "model_id": profile_id,
            ...
            "evidence_use": (
                "Prospective V2.11.3 dispatch-budget authority only; no "
                "V2.11.2 scientific outcome or decoded completion is reused."
            ),
        }
    )
    receipt_path = v2113_observed_p95_receipt_path(raw, profile_id)
    ...
    projection = _seal(
        {
            ...
            "bindings": {
                ...
                "source_kind": V2113_RESEALED_P95_SOURCE_KIND,
    ...
    return {
        ...
        "projection_path": v2113_observed_p95_projection_path(raw, profile_id),
        "receipt": receipt,
...
def _rebuild_pair(
    ...
    *,
    ...
    if set(value) != expected_keys:
        raise PilotV2113GateError("V2.11.3 p95 receipt shape drifted")
    try:
        _verify_seal(
            value,
            schema=V2113_RESEALED_P95_AUTHORITY_SCHEMA_VERSION,
            name="V2.11.3 resealed p95 receipt",
        )
    except PilotV2113ParentImportError as exc:
        raise PilotV2113GateError(str(exc)) from exc
    model = _mapping(value.get("model"), "V2.11.3 p95 model")
    ...
    rebuilt = build_v2113_resealed_observed_p95_authority(
        repo_root=repo_root,
        contract=contract,
        raw_root=raw_root,
        profile_id=profile_id,
    ...
    if value != rebuilt["receipt"]:
        raise PilotV2113GateError(
            "V2.11.3 p95 receipt differs from current parent-import replay"
        )
    return rebuilt


def persist_v2113_resealed_observed_p95_authority(
    *,
    repo_root: str | Path,
    contract: PilotContract,
    raw_root: str | Path,
    ...
    if selected_id is None or (
        ...
        raise PilotV2113GateError("one exact V2.11.3 profile/model id is required")
...
def verify_v2113_resealed_observed_p95_authority(
    receipt: Mapping[str, Any],
    *,
    repo_root: str | Path,
    raw_root: str | Path,
...
def verify_v2113_resealed_observed_p95_projection(
    ...
    *,
    ...
    try:
        _verify_seal(
            ...
            name="V2.11.3 p95 projection",
        )
    except PilotV2113ParentImportError as exc:
        raise PilotV2113GateError(str(exc)) from exc
    if candidate != built["projection"]:
        raise PilotV2113GateError("V2.11.3 p95 projection differs from receipt")
    return candidate


def verified_v2113_observed_p95_authority_binding(
    ...
    *,
    ...
    root = _strict_root(repo_root, name="V2.11.3 repository")
    raw = _raw_root(root, raw_root)
    relative = _relative_to_repo(root, receipt_path, name="V2.11.3 p95 receipt")
    profile_id = relative.parts[-2] if len(relative.parts) >= 2 else ""
    expected = _relative_to_repo(
        root,
        v2113_observed_p95_receipt_path(raw, profile_id),
        name="V2.11.3 p95 receipt",
    )
    if relative != expected:
        raise PilotV2113GateError("V2.11.3 p95 receipt path drifted")
    receipt_raw = _read_regular(root, relative, name="V2.11.3 p95 receipt")
    receipt = _strict_json(receipt_raw, name="V2.11.3 p95 receipt")
    reservations = verify_v2113_resealed_observed_p95_authority(
        receipt,
        repo_root=root,
        raw_root=raw,
        expected_git_commit=expected_git_commit,
    ...
    projection_raw = _read_regular(
        root, projection_relative, name="V2.11.3 p95 projection"
    )
    projection = _strict_json(projection_raw, name="V2.11.3 p95 projection")
    verify_v2113_resealed_observed_p95_projection(
        projection,
        receipt=receipt,
        repo_root=root,
        raw_root=raw,
...
def verified_v2113_observed_p95_projection_binding(
    ...
    *,
    ...
    projection_relative = _relative_to_repo(
        root, projection_path, name="V2.11.3 p95 projection"
    )
    if projection_relative != PurePosixPath(authority["receipt_path"]).with_name(
        "projection_p95.json"
    ):
        raise PilotV2113GateError("V2.11.3 p95 sibling projection path drifted")
    raw = _read_regular(root, projection_relative, name="V2.11.3 p95 projection")
    projection = _strict_json(raw, name="V2.11.3 p95 projection")
    receipt_raw = _read_regular(
        root, PurePosixPath(authority["receipt_path"]), name="V2.11.3 p95 receipt"
    )
    receipt = _strict_json(receipt_raw, name="V2.11.3 p95 receipt")
    payload = verify_v2113_resealed_observed_p95_projection(
        projection,
        receipt=receipt,
        repo_root=root,
        raw_root=raw_root,
    ...
    model = _mapping(receipt["model"], "V2.11.3 p95 model")
    return {
...
def _normalize_model_bindings(
    ...
    *,
    ...
    if set(bindings) != set(V2113_ALLOWED_MODELS):
        raise PilotV2113GateError("V2.11.3 global gate model denominator drifted")
    reservations: dict[str, Any] = {}
    sources: dict[str, Any] = {}
    for model_id in V2113_ALLOWED_MODELS:
        ...
        if set(binding) != {
            ...
            raise PilotV2113GateError("V2.11.3 per-model p95 binding shape drifted")
        runtime, _ = _runtime_profile(contract, model_id)
        rows = _mapping(binding["reservations"], f"{model_id} reservations")
        if set(rows) != {runtime}:
            raise PilotV2113GateError("V2.11.3 p95 runtime denominator drifted")
        ...
        for call_kind in ("action", "semantic"):
            try:
                ...
                raise PilotV2113GateError("V2.11.3 global p95 row is invalid") from exc
        if binding["git_commit"] != expected_git_commit:
            raise PilotV2113GateError("V2.11.3 p95 binding commit drifted")
        if any(
            ...
            raise PilotV2113GateError(
                "V2.11.3 action/semantic preflight provenance disagrees"
            )
        sources[model_id] = {
            "per_model_receipt": {
...
def build_v2113_post_gate_authority(
    *,
    repo_root: str | Path,
    contract: PilotContract,
    expected_git_commit: str,
    ...
    root = _strict_root(repo_root, name="V2.11.3 repository")
    selected = _contract(root, contract, expected_git_commit)
    _digest(ledger_event_chain_head, "V2.11.3 run-ledger event-chain head")
    ...
    capability = capability_wrappers_from_v2113_receipt(persisted_parent)
    calibration = calibration_wrapper_from_v2113_receipt(persisted_parent)
    parent_content = persisted_parent["integrity"]["content_sha256"]
    return _receipt_seal(
        {
            "schema_version": V2113_GATE_SCHEMA_VERSION,
            "contract_id": V2113_CONTRACT_ID,
            "contract_sha256": selected.canonical_hash,
            "release": {"tag": V2113_SCIENCE_TAG, "commit": expected_git_commit},
            "denominator": {
                "registered_model_ids": list(V2113_ALLOWED_MODELS),
                "eligible_model_ids": list(V2113_ALLOWED_MODELS),
            ...
            "scientific_evidence": False,
            "claim_boundary": (
                "V2.11.3 prospective dispatch-budget authority only; all 131 "
                "scientific cells remain fresh V2.11.3 denominator cells."
            ),
        }
    )


def verify_v2113_gate_receipt(
    receipt: Mapping[str, Any],
    *,
    expected_git_commit: str,
    ...
    if expected_contract_sha256 is not None and contract_hash != _digest(
        ...
        raise PilotV2113GateError("V2.11.3 global gate contract hash drifted")
    ...
    if denominator != {
        ...
        raise PilotV2113GateError("V2.11.3 global gate denominator drifted")
    ...
    if boundary != {
        ...
        raise PilotV2113GateError("V2.11.3 global gate is not zero-provider")
    sources = _mapping(receipt.get("authority_sources"), "authority sources")
    if set(sources) != set(V2113_ALLOWED_MODELS):
        raise PilotV2113GateError("V2.11.3 global authority sources drifted")
    rows = _mapping(receipt.get("reservations"), "global reservations")
    if set(rows) != set(_RUNTIME_MODELS.values()):
        raise PilotV2113GateError("V2.11.3 global reservation denominator drifted")
    for model_id, runtime in _RUNTIME_MODELS.items():
        by_kind = _mapping(rows[runtime], f"{model_id} reservations")
        if set(by_kind) != {"action", "semantic"}:
            raise PilotV2113GateError("V2.11.3 global call-kind denominator drifted")
        for kind in ("action", "semantic"):
            try:
                ...
                raise PilotV2113GateError(
                    "V2.11.3 global reservation is invalid"
                ) from exc
            if parsed.sample_count != _SAMPLE_COUNTS[kind]:
                raise PilotV2113GateError("V2.11.3 global sample count drifted")
        ...
        if set(source) != {
            ...
            raise PilotV2113GateError("V2.11.3 global source shape drifted")
        ...
        if parent_gate != {
            ...
            raise PilotV2113GateError("V2.11.3 parent gate lineage drifted")
    return _json_copy(dict(receipt))


def persist_v2113_post_gate_authority(
    *,
    repo_root: str | Path,
    raw_root: str | Path,
    contract: PilotContract,
    ...
    receipt = build_v2113_post_gate_authority(
        repo_root=root,
        contract=selected,
        expected_git_commit=expected_git_commit,
        parent_import_receipt=parent_import_receipt,
    ...
    for model_id, runtime in _RUNTIME_MODELS.items():
        ...
        for call_kind in ("action", "semantic"):
            ...
            try:
                ...
                raise PilotV2113GateError(
                    "V2.11.3 current-release source authority is invalid"
                ) from exc
...
def runner_reservations_from_v2113_gate_binding(
    binding: Mapping[str, Any],
) -> dict[str, dict[str, Any]]:
    """Attach the verified current receipt fields required by the runner."""

    value = _mapping(binding, "V2.11.3 gate authority binding")
    if set(value) != {
        "receipt_path",
        "receipt_file_sha256",
        "receipt_content_sha256",
        ...
        raise PilotV2113GateError("V2.11.3 gate binding fields drifted")
    receipt_path = PurePosixPath(str(value["receipt_path"]))
    if receipt_path != V2113_POST_GATE_RELATIVE_PATH:
        raise PilotV2113GateError("V2.11.3 gate binding path drifted")
    ...
    if set(source) != set(_RUNTIME_MODELS.values()):
        raise PilotV2113GateError("V2.11.3 gate runtime denominator drifted")
    result: dict[str, dict[str, Any]] = {}
    for runtime, raw_by_kind in source.items():
        by_kind = _mapping(raw_by_kind, f"{runtime} gate reservations")
        if set(by_kind) != {"action", "semantic"}:
            raise PilotV2113GateError("V2.11.3 gate call-kind denominator drifted")
        result[runtime] = {}
        for call_kind in ("action", "semantic"):
            entry = _mapping(by_kind[call_kind], "gate source reservation")
            if set(entry) != {"authority", "reservation"}:
                raise PilotV2113GateError("V2.11.3 gate source row drifted")
            ...
            try:
                ...
                raise PilotV2113GateError(
                    "V2.11.3 runner reservation is invalid"
                ) from exc
    return result


def verified_v2113_gate_authority_binding(
    receipt_path: str | Path,
    *,
    repo_root: str | Path,
    expected_git_commit: str,
    ...
    if (
        ...
        raise PilotV2113GateError("V2.11.3 selected contract hash drifted")
    relative = _relative_to_repo(root, receipt_path, name="V2.11.3 global gate")
    if relative != V2113_POST_GATE_RELATIVE_PATH:
        raise PilotV2113GateError("V2.11.3 global gate path drifted")
    raw = _read_regular(root, relative, name="V2.11.3 global gate")
    receipt = verify_v2113_gate_receipt(
        _strict_json(raw, name="V2.11.3 global gate"),
        expected_git_commit=expected_git_commit,
        expected_contract_sha256=selected.canonical_hash,
    )
    ...
    bindings = {
        model_id: verified_v2113_observed_p95_authority_binding(
            v2113_observed_p95_receipt_path(raw_root, model_id),
            repo_root=root,
            raw_root=raw_root,
            expected_git_commit=expected_git_commit,
            contract=selected,
    ...
    rebuilt = build_v2113_post_gate_authority(
        repo_root=root,
        contract=selected,
        expected_git_commit=expected_git_commit,
    ...
    if rebuilt != receipt:
        raise PilotV2113GateError("V2.11.3 global gate differs from current sources")
...
def runner_reservations_from_v2113_gate(
    receipt_path: str | Path,
    *,
    repo_root: str | Path,
    expected_git_commit: str,
    ...
    return runner_reservations_from_v2113_gate_binding(binding)
...
__all__ = [
    ...
    "V2113_PREFLIGHT_IMPORT_GATE_SCHEMA_VERSION",
    "V2113_RESEALED_P95_AUTHORITY_SCHEMA_VERSION",
    "V2113_RESEALED_P95_SOURCE_KIND",
    ...
    "runner_reservations_from_v2113_gate",
    ...
    "verify_v2113_gate_receipt",
    "verify_v2113_resealed_observed_p95_authority",
    "verify_v2113_resealed_observed_p95_projection",
