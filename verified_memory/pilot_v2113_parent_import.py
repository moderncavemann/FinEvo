"""Outcome-blind, zero-provider V2.11.2 authority import for V2.11.3.
...
The complete V2.11.2 denominator and cumulative spend are audited before any
child authority is emitted.  No A-D or cross-model artifact is returned, no
provider module is imported, and no provider client can be constructed here.
"""
...
V2113_SOURCE_MANIFEST_PATH = PurePosixPath(
    "experiments/pilot_v2_11_3_source_manifest.json"
)
V2113_SOURCE_MANIFEST_SCHEMA_VERSION = "finevo-pilot-v2.11.3-source-manifest-v1"
V2113_PARENT_IMPORT_SCHEMA_VERSION = "finevo-pilot-v2.11.3-parent-import-v1"
V2113_CALIBRATION_WRAPPER_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.3-imported-calibration-wrapper-v1"
)
V2113_CAPABILITY_WRAPPER_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.3-imported-capability-wrapper-v1"
)
V2113_PREFLIGHT_WRAPPER_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.3-imported-preflight-authority-v1"
)
V2113_DEFAULT_RECEIPT_PATH = V2113_RAW_ROOT / "parent-import/parent_import_receipt.json"
...
V2113_SOURCE_MANIFEST_FILE_SHA256: str | None = (
    "f05dbac4951e99476c06883e3c1b792e7ccb459c16eb4d78ac15ddf7905598de"
)
V2113_SOURCE_MANIFEST_CONTENT_SHA256: str | None = (
...
V2113_CUMULATIVE_DEBIT = ParentBudgetDebit(
...
V2113_ALLOWED_MODELS = ("gpt52_main", "gpt56_diagnostic")
...
V2113_PARSED_PARENT_SOURCE_ALLOWLIST = (
...
class PilotV2113ParentImportError(RuntimeError):
    """Raised before any imported V2.11.3 authority can be consumed."""
...
def _json_copy(value: Any) -> Any:
    try:
        ...
        raise PilotV2113ParentImportError("value is not canonical JSON") from exc
...
def _strict_json(raw: bytes, *, name: str) -> dict[str, Any]:
    def pairs(rows: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in rows:
            if key in result:
                raise PilotV2113ParentImportError(
    ...
    try:
        ...
        raise PilotV2113ParentImportError(f"{name} is not strict JSON") from exc
    if not isinstance(value, dict):
        raise PilotV2113ParentImportError(f"{name} must contain one object")
    return value
...
def _verify_seal(value: Mapping[str, Any], *, schema: str, name: str) -> str:
    ...
    if (
        ...
        raise PilotV2113ParentImportError(f"{name} seal drifted")
    return str(claimed)
...
def _strict_root(value: str | Path, *, name: str) -> Path:
    ...
    try:
        ...
        raise PilotV2113ParentImportError(f"{name} is unavailable") from exc
...
def _read_regular(root: Path, relative: PurePosixPath, *, name: str) -> bytes:
    ...
    for part in relative.parts[:-1]:
        ...
        try:
            ...
            raise PilotV2113ParentImportError(f"missing {name}: {relative}") from exc
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise PilotV2113ParentImportError(f"{name} path is not guarded")
    ...
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise PilotV2113ParentImportError(f"{name} must be a regular file")
    ...
    try:
        ...
        raise PilotV2113ParentImportError(f"cannot read {name}") from exc
    if (
        ...
        raise PilotV2113ParentImportError(f"{name} changed during guarded read")
    return raw
...
def _read_parent_json(
    ...
    *,
    ...
    if relative.as_posix() not in V2113_PARSED_PARENT_SOURCE_ALLOWLIST:
        raise PilotV2113ParentImportError(
...
def _git(root: Path, *args: str) -> str:
    try:
        ...
        raise PilotV2113ParentImportError("git release verification failed") from exc
...
def _verify_event_ledger(
    ...
    *,
    ...
    if (
        value.get("schema_version") != schema
        or value.get("contract_hash") != V2112_CONTRACT_SHA256
        ...
        raise PilotV2113ParentImportError(f"{name} identity drifted")
    previous = "0" * 64
    for index, event in enumerate(events):
        if not isinstance(event, Mapping):
            raise PilotV2113ParentImportError(f"{name} event is malformed")
        ...
        if (
            ...
            raise PilotV2113ParentImportError(f"{name} event chain drifted")
        previous = str(digest)
...
def _load_parent_contract(root: Path) -> tuple[PilotContract, dict[str, Any]]:
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 contract identity drifted")
    try:
        contract = load_pilot_contract(root.joinpath(*V2112_CONTRACT_PATH.parts))
    except Exception as exc:
        raise PilotV2113ParentImportError("V2.11.2 contract failed to load") from exc
    if contract.canonical_hash != V2112_CONTRACT_SHA256:
        raise PilotV2113ParentImportError("V2.11.2 parsed contract hash drifted")
...
def _verify_raw_inventory(root: Path) -> dict[str, Any]:
    ...
    if summary != expected:
        raise PilotV2113ParentImportError("V2.11.2 raw inventory drifted")
...
def _verify_attestation_and_launch(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 attestation or launch input drifted")
    return {
...
def _verify_run_ledger(
    ...
    if _sha256(raw) != V2112_RUN_LEDGER_FILE_SHA256:
        raise PilotV2113ParentImportError("V2.11.2 run ledger bytes drifted")
    ...
    if len(specs) != 136 or set(runs) != set(specs):
        raise PilotV2113ParentImportError("V2.11.2 ITT denominator drifted")
    ...
    for run_id, row in runs.items():
        if not isinstance(row, Mapping) or row.get("spec") != specs[run_id]:
            raise PilotV2113ParentImportError("V2.11.2 run/spec binding drifted")
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 terminal denominator drifted")
    return ledger, {
...
def _verify_budget_ledger(
    ...
    if _sha256(raw) != V2112_BUDGET_LEDGER_FILE_SHA256:
        raise PilotV2113ParentImportError("V2.11.2 budget ledger bytes drifted")
    _verify_event_ledger(
        ledger,
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 attempt or cumulative debit drifted")
    return ledger, {
        **_binding(relative, raw, ledger),
        "internal_sha256": V2112_BUDGET_LEDGER_SHA256,
        "event_count": V2112_BUDGET_LEDGER_EVENT_COUNT,
...
def _verify_stage_receipts(root: Path) -> dict[str, Any]:
    ...
    for stage_id, expected in _STAGE_RECEIPTS.items():
        ...
        if (
            ...
            raise PilotV2113ParentImportError(f"V2.11.2 {stage_id} receipt drifted")
...
def _deep_verify_post_gate(
    ...
    *,
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 post-gate bytes drifted")
    try:
    ...
    for model_id in V2113_ALLOWED_MODELS:
        ...
        if not checkpoint_raw or not exactness_raw or not journal_raw:
            raise PilotV2113ParentImportError("empty V2.11.2 preflight source")

    science_ids: dict[str, list[str]] = {
        model_id: [] for model_id in V2113_ALLOWED_MODELS
    }
    for stage_id in (
        "experiment-c",
        "experiment-a",
    ...
    if (
        ...
        raise PilotV2113ParentImportError("V2.11.2 post-gate ledger prefix is absent")
    pre_science_ids = {
        spec.run_id
    ...
    if not isinstance(budget_rows, Mapping):
        raise PilotV2113ParentImportError("V2.11.2 budget rows are malformed")
...
def _audit_parent_release(
    *,
    ...
    if _sha256(parent_raw) != V2112_PARENT_IMPORT_FILE_SHA256:
        raise PilotV2113ParentImportError("V2.11.2 parent import bytes drifted")
    try:
    ...
    if parent_receipt.get("integrity", {}).get(
        ...
        raise PilotV2113ParentImportError("V2.11.2 reusable wrappers drifted")
...
def build_v2113_source_manifest(
    *,
    parent_science_root: str | Path,
    evidence_repo_root: str | Path,
) -> dict[str, Any]:
    """Build the deterministic tracked manifest from frozen V2.11.2 bytes."""
    ...
    return _seal(
        {
            "schema_version": V2113_SOURCE_MANIFEST_SCHEMA_VERSION,
            "parent_release": {
            ...
            "parsed_source_allowlist": list(V2113_PARSED_PARENT_SOURCE_ALLOWLIST),
            "reusable_authority_allowlist": list(V2113_REUSABLE_AUTHORITY_KINDS),
            "forbidden_import_prefixes": list(V2113_FORBIDDEN_IMPORT_PREFIXES),
            "calibration_source": {
            ...
            "cumulative_budget_debit": V2113_CUMULATIVE_DEBIT.to_dict(),
            "import_policy": {
                **_ZERO_PROVIDER_POLICY,
            ...
            "observation_boundary": {
                ...
                "fresh_v2_11_3_scientific_cells": 131,
...
def _load_tracked_manifest(repo_root: Path) -> dict[str, Any]:
    if (
        V2113_SOURCE_MANIFEST_FILE_SHA256 is None
        or V2113_SOURCE_MANIFEST_CONTENT_SHA256 is None
    ):
        raise PilotV2113ParentImportError(
            "V2.11.3 source manifest hashes are not sealed"
        )
    raw, value = _read_json(
        repo_root,
        V2113_SOURCE_MANIFEST_PATH,
        name="tracked V2.11.3 source manifest",
    )
    content = _verify_seal(
        value,
        schema=V2113_SOURCE_MANIFEST_SCHEMA_VERSION,
        name="tracked V2.11.3 source manifest",
    )
    canonical = (
    ...
    if (
        raw != canonical
        or _sha256(raw) != V2113_SOURCE_MANIFEST_FILE_SHA256
        or content != V2113_SOURCE_MANIFEST_CONTENT_SHA256
    ):
        raise PilotV2113ParentImportError(
            "tracked V2.11.3 source manifest hash/canonicalization drifted"
        )
    return value


def write_v2113_source_manifest_draft(
    path: str | Path,
    ...
    _verify_seal(
        manifest,
        schema=V2113_SOURCE_MANIFEST_SCHEMA_VERSION,
        name="V2.11.3 source manifest draft",
    )
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise PilotV2113ParentImportError(
            f"refusing to overwrite V2.11.3 source manifest: {target}"
        )
...
def load_v2113_source_manifest(*, repo_root: str | Path) -> dict[str, Any]:
    return _load_tracked_manifest(_strict_root(repo_root, name="V2.11.3 repository"))


def validate_v2113_source_manifest(
    value: Mapping[str, Any],
    *,
    parent_science_root: str | Path,
    evidence_repo_root: str | Path,
    ...
    _verify_seal(
        selected,
        schema=V2113_SOURCE_MANIFEST_SCHEMA_VERSION,
        name="V2.11.3 source manifest",
    )
    expected = build_v2113_source_manifest(
        parent_science_root=parent_science_root,
        evidence_repo_root=evidence_repo_root,
    )
    if selected != expected:
        raise PilotV2113ParentImportError(
            "V2.11.3 source manifest differs from immutable V2.11.2 replay"
        )
    return selected
...
def _validate_child(
    *,
    ...
    if (
        getattr(contract, "contract_id", None) != V2113_CONTRACT_ID
        ...
        raise PilotV2113ParentImportError(
            "V2.11.3 import requires its exact release-bound contract"
        )
    return {
...
def _build_child_wrappers(
    *,
    ...
    calibration = _seal(
        {
            "schema_version": V2113_CALIBRATION_WRAPPER_SCHEMA_VERSION,
    ...
    for model_id in V2113_ALLOWED_MODELS:
        ...
        preflights[model_id] = _seal(
            {
                "schema_version": V2113_PREFLIGHT_WRAPPER_SCHEMA_VERSION,
                "child_release": _json_copy(child),
                "parent_release": _parent_release_binding(),
                "source_manifest": _source_manifest_binding(),
                "model_id": model_id,
...
def build_v2113_parent_import(
    *,
    ...
    child_root = _strict_root(repo_root, name="V2.11.3 repository")
    ...
    rebuilt_manifest = build_v2113_source_manifest(
        parent_science_root=parent_science_root,
        evidence_repo_root=evidence_root,
    )
    if tracked != rebuilt_manifest:
        raise PilotV2113ParentImportError(
            "tracked V2.11.3 source manifest differs from immutable replay"
        )
    ...
    return _seal(
        {
            "schema_version": V2113_PARENT_IMPORT_SCHEMA_VERSION,
            "child_release": child,
            "source_manifest": _source_manifest_binding(),
            "parent_release": {
            ...
            "cumulative_parent_budget_debit": V2113_CUMULATIVE_DEBIT.to_dict(),
            "import_policy": _json_copy(tracked["import_policy"]),
            "scientific_evidence": False,
            "claim_boundary": (
                "V2.11.3 reuses only calibration, capability/interface, and "
                "fresh-preflight observed-p95 budget authority. All 131 A-D/"
                "cross-model cells are new V2.11.3 cells; all V2.11.2 "
                "scientific outcomes and decoded completions are excluded."
            ),
        }
    )


def validate_v2113_parent_import_receipt(
    ...
    *,
    ...
    _verify_seal(
        value,
        schema=V2113_PARENT_IMPORT_SCHEMA_VERSION,
    ...
    if (
        ...
        or set(capabilities) != set(V2113_ALLOWED_MODELS)
        or not isinstance(preflights, Mapping)
        or set(preflights) != set(V2113_ALLOWED_MODELS)
    ):
        raise PilotV2113ParentImportError("V2.11.3 wrapper denominator drifted")
    _verify_seal(
        calibration,
        schema=V2113_CALIBRATION_WRAPPER_SCHEMA_VERSION,
        name="V2.11.3 calibration wrapper",
    )
    if (
        ...
        raise PilotV2113ParentImportError(
            "V2.11.3 calibration wrapper source boundary drifted"
        )
    for model_id in V2113_ALLOWED_MODELS:
        _verify_seal(
            capabilities[model_id],
            schema=V2113_CAPABILITY_WRAPPER_SCHEMA_VERSION,
            name=f"V2.11.3 {model_id} capability wrapper",
        )
        _verify_seal(
            preflights[model_id],
            schema=V2113_PREFLIGHT_WRAPPER_SCHEMA_VERSION,
            name=f"V2.11.3 {model_id} preflight wrapper",
        )
        ...
        if (
            ...
            raise PilotV2113ParentImportError(
                f"V2.11.3 {model_id} preflight wrapper scope drifted"
            )
    return value


def verify_v2113_parent_import_receipt(
    ...
    *,
    ...
    if commit is None or (
        ...
        raise PilotV2113ParentImportError(
            "V2.11.3 parent receipt requires one exact child commit"
        )
    if raw_root is not None:
        raw = Path(raw_root)
        if not raw.is_absolute():
            raw = root.joinpath(*PurePosixPath(str(raw_root)).parts)
        if raw.absolute() != root.joinpath(*V2113_RAW_ROOT.parts):
            raise PilotV2113ParentImportError(
                "V2.11.3 parent receipt raw namespace drifted"
            )
    if isinstance(receipt, Mapping):
        observed = _json_copy(dict(receipt))
    else:
        path = Path(receipt).absolute()
        expected = root.joinpath(*V2113_DEFAULT_RECEIPT_PATH.parts)
        if path != expected:
            raise PilotV2113ParentImportError(
                "V2.11.3 parent receipt path differs from raw namespace"
            )
        raw = _read_regular(
            root,
        ...
        observed = _strict_json(raw, name="V2.11.3 parent import receipt")
    ...
    if parent_science_root is None:
        return observed
    expected = build_v2113_parent_import(
        repo_root=root,
        contract=contract,
        child_git_commit=commit,
        parent_science_root=parent_science_root,
    ...
    if observed != expected:
        raise PilotV2113ParentImportError(
            "V2.11.3 parent import receipt differs from exact source replay"
        )
    return observed
...
def _atomic_json(
    ...
    *,
    ...
    try:
        ...
        raise PilotV2113ParentImportError(
            f"immutable V2.11.3 write failed: {exc}"
        ) from exc


def persist_v2113_parent_import(
    *,
    repo_root: str | Path,
    contract: PilotContract,
    child_git_commit: str | None = None,
    ...
    source = parent_science_root or source_repo_root
    if (
        commit is None
        or source is None
        or (
        ...
        or (
            parent_science_root is not None
            and source_repo_root is not None
            and Path(parent_science_root).absolute()
            != Path(source_repo_root).absolute()
        )
    ):
        raise PilotV2113ParentImportError(
            "V2.11.3 parent import requires one exact commit/source checkout"
        )
    if raw_root is not None:
        raw = Path(raw_root)
        if not raw.is_absolute():
        ...
        if raw.absolute() != root.joinpath(*V2113_RAW_ROOT.parts):
            raise PilotV2113ParentImportError(
                "V2.11.3 parent import raw namespace drifted"
            )
    receipt = build_v2113_parent_import(
        repo_root=root,
        contract=contract,
        child_git_commit=commit,
        parent_science_root=source,
        evidence_repo_root=evidence_repo_root,
    )
    path = (
        root.joinpath(*V2113_DEFAULT_RECEIPT_PATH.parts)
        if destination is None
        else Path(destination).absolute()
    )
    if path != root.joinpath(*V2113_DEFAULT_RECEIPT_PATH.parts):
        raise PilotV2113ParentImportError(
            "V2.11.3 receipt destination differs from raw namespace"
        )
...
def parent_budget_debit_for_v2113(contract: Any = None) -> ParentBudgetDebit | None:
    if getattr(contract, "contract_id", None) != V2113_CONTRACT_ID:
        return None
    return V2113_CUMULATIVE_DEBIT


def calibration_wrapper_from_v2113_receipt(
    receipt: Mapping[str, Any],
    *,
    ...
    if contract is not None or expected_git_commit is not None or repo_root is not None:
        if contract is None or expected_git_commit is None or repo_root is None:
            raise PilotV2113ParentImportError(
                "V2.11.3 wrapper validation requires root, contract, and commit"
            )
        validate_v2113_parent_import_receipt(
            receipt,
            contract=contract,
    ...
    if not isinstance(wrapper, Mapping):
        raise PilotV2113ParentImportError("V2.11.3 calibration wrapper is absent")
    _verify_seal(
        wrapper,
        schema=V2113_CALIBRATION_WRAPPER_SCHEMA_VERSION,
        name="V2.11.3 calibration wrapper",
...
def capability_wrappers_from_v2113_receipt(
    receipt: Mapping[str, Any],
    *,
    contract: PilotContract | None = None,
    expected_git_commit: str | None = None,
    ...
    if not isinstance(wrappers, Mapping) or set(wrappers) != set(V2113_ALLOWED_MODELS):
        raise PilotV2113ParentImportError("V2.11.3 capability wrappers drifted")
    result: dict[str, dict[str, Any]] = {}
    for model_id in V2113_ALLOWED_MODELS:
        wrapper = wrappers[model_id]
        if not isinstance(wrapper, Mapping):
            raise PilotV2113ParentImportError("capability wrapper is malformed")
        _verify_seal(
            wrapper,
            schema=V2113_CAPABILITY_WRAPPER_SCHEMA_VERSION,
            name=f"V2.11.3 {model_id} capability wrapper",
...
def preflight_wrappers_from_v2113_receipt(
    receipt: Mapping[str, Any],
    *,
    contract: PilotContract | None = None,
    expected_git_commit: str | None = None,
    ...
    if not isinstance(wrappers, Mapping) or set(wrappers) != set(V2113_ALLOWED_MODELS):
        raise PilotV2113ParentImportError("V2.11.3 preflight wrappers drifted")
    result: dict[str, dict[str, Any]] = {}
    for model_id in V2113_ALLOWED_MODELS:
        wrapper = wrappers[model_id]
        if not isinstance(wrapper, Mapping):
            raise PilotV2113ParentImportError("preflight wrapper is malformed")
        _verify_seal(
            wrapper,
            schema=V2113_PREFLIGHT_WRAPPER_SCHEMA_VERSION,
            name=f"V2.11.3 {model_id} preflight wrapper",
        ...
        if canonical_sha256(wrapper.get("reservations")) != wrapper.get(
            ...
            raise PilotV2113ParentImportError(
                f"V2.11.3 {model_id} preflight reservation hash drifted"
            )
        result[model_id] = _json_copy(wrapper)
    return result
...
__all__ = [
    ...
    "V2113_PARENT_IMPORT_SCHEMA_VERSION",
    "V2113_PREFLIGHT_WRAPPER_SCHEMA_VERSION",
    "V2113_RAW_ROOT",
    "V2113_SCIENCE_TAG",
