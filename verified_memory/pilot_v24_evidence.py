"""Lane-separated evidence publication for the prospective FinEvo V2.4 pilot.
...
def _validate_v24_contract_matrix(contract: PilotContract) -> None:
    if contract.contract_id != PILOT_V24_CONTRACT_ID:
        raise PilotEvidenceError("V2.4 evidence adapter received another contract")
    if tuple(contract.stage_ids) != _V24_STAGE_IDS:
        raise PilotEvidenceError(
            "V2.4 evidence stages differ from the fixed local-first matrix"
        )
    ...
    if (
        ...
        raise PilotEvidenceError("V2.4 evidence requires five unique main seeds")
    ...
    for lane_id, lane in _V24_LANES.items():
        ...
        if observed_order != PILOT_V24_STAGE_ORDER:
            raise PilotEvidenceError(
                f"V2.4 {lane_id} lane does not preserve C -> A -> D -> B"
            )
        for canonical in PILOT_V24_STAGE_ORDER:
            ...
            if (
                ...
                raise PilotEvidenceError(
                    f"V2.4 {lane_id}/{canonical} registered matrix drifted"
                )
    if any(
        ...
        raise PilotEvidenceError(
            "V2.4 narrative intervention must remain deferred and unregistered"
        )
        ...
        [
            {
                ...
                "metric": "not registered in the V2.4 core matrix",
                "artifact": "aggregate.json#/narrative",
                "status": "deferred-unregistered",
                "boundary": ("make no V2.4 narrative or real-news-understanding claim"),
            },
            {
                "lane": "cross-lane",
            ...
            {
                "lane": "all",
                "claim": "Complete V2.4 preregistered ITT denominator",
...
def aggregate_v24_evidence(
    ...
    *,
    ...
    """Build the pure V2.4 aggregate without reading or writing artifacts."""
    ...
    return {
        "schema_version": PILOT_V24_EVIDENCE_SCHEMA_VERSION,
...
def _require_publishable_terminal_denominator(
    ...
    if aggregate.get("publication_status") == "incomplete":
        ...
        raise PilotEvidenceError(
            "V2.4 immutable evidence publication requires all 211 ITT cells "
...
def _report_markdown(
    ...
    lines = [
        "# FinEvo V2.4 local-first mechanism pilot evidence report",
        "",
...
def _write_v24_package(
    ...
    *,
    ...
    if any(root.iterdir()):
        raise PilotEvidenceError(
            f"temporary V2.4 package directory is not empty: {root}"
        )
    ...
    if copied.canonical_hash != contract.canonical_hash:
        raise PilotEvidenceError("copied V2.4 contract failed hash revalidation")
    ...
    for row in checksums["files"]:
        ...
        if (
            ...
            raise PilotEvidenceError("V2.4 package checksum self-verification failed")
    ...
    if (
        ...
        raise PilotEvidenceError(
            "V2.4 raw evidence plus reviewer package exceeds storage cap"
        )
...
def build_pilot_v24_evidence_package(
    *,
    ...
    """Validate and publish the V2.4 reviewer package without provider calls."""
    ...
    if contract.status != "frozen":
        raise PilotEvidenceError(
            "V2.4 evidence publication requires the frozen science contract"
        )
    raw = Path(raw_root).resolve()
    if not raw.is_dir():
        raise PilotEvidenceError(f"V2.4 pilot raw root does not exist: {raw}")
