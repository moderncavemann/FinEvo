"""Zero-provider scientific-dispatch acceptance for FinEvo V2.11.4.
...
V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_SCHEMA_VERSION = (
    "finevo-pilot-v2.11.4-scientific-dispatch-acceptance-v1"
)
V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME = "scientific_dispatch_acceptance.json"
V2114_SCIENTIFIC_STAGE_IDS = (
    "experiment-c",
    "experiment-a",
    "experiment-d",
    "experiment-b",
...
V2114_OPERATIONAL_STAGE_IDS = (
    "parent-import",
    "capability-gate",
    "long-context-preflight",
)
V2114_EXPECTED_STAGE_CELL_COUNTS = {
    "parent-import": 1,
    "capability-gate": 2,
...
V2114_EXPECTED_LEDGER_CELLS = 136
V2114_EXPECTED_OPERATIONAL_CELLS = 5
V2114_EXPECTED_SCIENTIFIC_CELLS = 131
V2114_EXPECTED_PROVIDER_CELLS = 126
V2114_EXPECTED_OFFLINE_CELLS = 5
V2114_EXPECTED_D_GROUPS = 5
V2114_EXPECTED_D_CELLS_PER_GROUP = 11
V2114_EXPECTED_PROJECTION_UNITS = 81
V2114_EXPECTED_ACTION_CALLS = 4_848
V2114_EXPECTED_SEMANTIC_CALLS = 968
V2114_EXPECTED_PROVIDER_CALLS = 5_816
V2114_EXPECTED_FRESH_STORAGE_BYTES = 1_830_000_000
V2114_EXPECTED_ACCEPTED_RUN_EVENTS = 7
V2114_EXPECTED_ACCEPTED_BUDGET_EVENTS = 12
V2114_ACCEPTANCE_LEDGER_EVENT_TYPE = "acceptance_receipt_bound"
...
class PilotV2114AcceptanceError(RuntimeError):
    """Raised before V2.11.4 can acquire scientific dispatch authority."""
...
def _duplicate_rejecting_object(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise PilotV2114AcceptanceError(f"duplicate JSON key: {key!r}")
        result[key] = value
    return result
...
def _read_regular(path: Path, *, name: str) -> bytes:
    ...
    for part in absolute.parts[1:]:
        ...
        try:
            ...
            raise PilotV2114AcceptanceError(
    ...
    try:
        ...
        raise PilotV2114AcceptanceError(f"{name} is missing: {path}") from exc
    ...
    try:
        ...
        raise PilotV2114AcceptanceError(f"{name} cannot be read: {path}") from exc
    ...
    try:
        value = json.loads(
            ...
            parse_constant=lambda token: (_ for _ in ()).throw(
                PilotV2114AcceptanceError(
                    f"{name} contains non-finite JSON token {token!r}"
                )
            ),
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PilotV2114AcceptanceError(f"{name} is not strict JSON") from exc
    if not isinstance(value, dict):
        raise PilotV2114AcceptanceError(f"{name} root must be an object")
    return value
...
def _relative(repo_root: Path, path: Path, *, name: str) -> str:
    ...
    if any(part in {"", ".", ".."} for part in relative.parts):
        raise PilotV2114AcceptanceError(f"{name} path is not normalized")
...
def _exact_roots(
    ...
    raw = raw.absolute()
    expected = repository.joinpath(*V2114_RAW_ROOT.parts)
    if raw != expected:
        raise PilotV2114AcceptanceError(
            "V2.11.4 acceptance requires its exact ignored raw namespace"
        )
    ...
    for part in raw.parts[1:]:
        ...
        try:
            ...
            raise PilotV2114AcceptanceError(
                f"V2.11.4 raw namespace component is missing: {current}"
            ) from exc
        if stat.S_ISLNK(metadata.st_mode):
            raise PilotV2114AcceptanceError(
                f"V2.11.4 raw namespace contains a symlink: {current}"
            )
    if not stat.S_ISDIR(raw.lstat().st_mode):
        raise PilotV2114AcceptanceError("V2.11.4 raw namespace must be a directory")
    return repository, raw


def _require_contract(contract: PilotContract, repo_root: Path) -> None:
    contract_path = repo_root.joinpath(*V2114_CONTRACT_PATH.parts)
    ...
    if (
        ...
        != (*V2114_OPERATIONAL_STAGE_IDS, *V2114_SCIENTIFIC_STAGE_IDS)
        or {
        ...
        raise PilotV2114AcceptanceError(
            "acceptance requires the exact frozen V2.11.4 contract denominator"
        )
    amendment = contract.v2114_authority_normalization_amendment
    forward = contract.v2114_forward_boundary
    if not isinstance(amendment, Mapping) or not isinstance(forward, Mapping):
        raise PilotV2114AcceptanceError("V2.11.4 acceptance amendment is missing")
    ...
    if (
        ...
        or matrix.get("ledger_cells") != V2114_EXPECTED_LEDGER_CELLS
        or matrix.get("operational_cells") != V2114_EXPECTED_OPERATIONAL_CELLS
        or matrix.get("scientific_cells") != V2114_EXPECTED_SCIENTIFIC_CELLS
        or matrix.get("provider_backed_scientific_cells")
        != V2114_EXPECTED_PROVIDER_CELLS
        or matrix.get("offline_scientific_cells") != V2114_EXPECTED_OFFLINE_CELLS
        or matrix.get("fresh_action_calls") != V2114_EXPECTED_ACTION_CALLS
        or matrix.get("fresh_semantic_calls") != V2114_EXPECTED_SEMANTIC_CALLS
        or matrix.get("fresh_scientific_provider_calls")
        != V2114_EXPECTED_PROVIDER_CALLS
        or fresh.get("registered_scientific_cells") != V2114_EXPECTED_SCIENTIFIC_CELLS
        or fresh.get("registered_provider_calls") != V2114_EXPECTED_PROVIDER_CALLS
        or fresh.get("matrix_shrink") != "forbidden"
        or fresh.get("failed_seed_replacement") != "forbidden"
    ):
        raise PilotV2114AcceptanceError(
            "V2.11.4 frozen matrix declaration differs from acceptance constants"
        )
...
def _expected_pre_science_file_allowlist(contract: PilotContract) -> set[str]:
    ...
    for spec in contract.expand():
        if spec.stage_id not in V2114_OPERATIONAL_STAGE_IDS:
            continue
...
def _audit_pre_science_namespace(
    ...
    the V2.11.4 raw tree.  All legitimate science output is created only after
    this zero-call acceptance has been bound into both ledgers.
    """

    stage_roots = set(V2114_SCIENTIFIC_STAGE_IDS)
    legacy_tokens = (
...
def _ledger_prefix(snapshot: Mapping[str, Any], path: Path) -> dict[str, Any]:
    ...
    if not isinstance(events, list) or not events:
        raise PilotV2114AcceptanceError("acceptance requires ledger event chains")
    ...
    if not isinstance(head, str) or not isinstance(ledger_sha256, str):
        raise PilotV2114AcceptanceError("ledger event-chain binding is malformed")
    return {
...
def _accepted_budget_baseline(
    ...
    operational_ids = {
        spec.run_id
        for stage_id in V2114_OPERATIONAL_STAGE_IDS
...
def _verify_ledger_prefix(
    ...
    *,
    ...
    if (
        ...
        raise PilotV2114AcceptanceError(f"{name} accepted event prefix drifted")
...
def _verify_unmarked_ledger_identity(
    ...
    *,
    ...
    if (
        ...
        raise PilotV2114AcceptanceError(f"{name} identity inputs are malformed")
    if len(events) != count:
        return
...
def _verify_acceptance_event_binding(
    ...
    *,
    ...
    expected_payload = {
        "receipt_schema_version": V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_SCHEMA_VERSION,
    ...
    for name, snapshot, prefix, state_hash_field, expected_state_hash in (
        ...
        marker_events = [
            ...
            and event.get("event_type") == V2114_ACCEPTANCE_LEDGER_EVENT_TYPE
        ]
        if (
            len(marker_events) != 1
            or marker_events[0] is not marker
            or marker.get("event_index") != index
            or marker.get("event_type") != V2114_ACCEPTANCE_LEDGER_EVENT_TYPE
...
def _verify_current_budget_rows(
    ...
    for stage_id in V2114_OPERATIONAL_STAGE_IDS:
        for spec in contract.expand(stage=stage_id):
            projection = (
                orch._v2114_parent_import_projection(spec)
                if spec.execution_mode == "parent_authority_import"
                else orch._v2114_operational_import_projection(spec)
            )
    ...
    if not set(operational).issubset(runs):
        raise PilotV2114AcceptanceError("budget ledger lost an operational row")
...
def _open_ledgers(
    ...
    _read_regular(run_path, name="V2.11.4 run ledger")
    _read_regular(budget_path, name="V2.11.4 budget ledger")
    try:
        ...
        raise PilotV2114AcceptanceError(
            f"V2.11.4 ledger validation failed: {exc}"
        ) from exc
...
def _expected_denominator(contract: PilotContract) -> dict[str, Any]:
    ...
    scientific_specs = tuple(
        spec
        for stage_id in V2114_SCIENTIFIC_STAGE_IDS
        for spec in contract.expand(stage=stage_id)
    )
    offline_specs = tuple(
        spec
    ...
    return {
        ...
        "stage_cell_counts": dict(V2114_EXPECTED_STAGE_CELL_COUNTS),
...
def _audit_denominator(
    ...
    if (
        len(specs) != V2114_EXPECTED_LEDGER_CELLS
        or not isinstance(runs, Mapping)
        or set(runs) != {spec.run_id for spec in specs}
        or any(runs[spec.run_id].get("spec") != spec.to_dict() for spec in specs)
    ):
        raise PilotV2114AcceptanceError("V2.11.4 run-ledger denominator drifted")
    ...
    if (
        len(operational_specs) != V2114_EXPECTED_OPERATIONAL_CELLS
        or len(scientific_specs) != V2114_EXPECTED_SCIENTIFIC_CELLS
        or len(provider_specs) != V2114_EXPECTED_PROVIDER_CELLS
        or len(offline_specs) != V2114_EXPECTED_OFFLINE_CELLS
        or any(
        ...
        raise PilotV2114AcceptanceError(
            "V2.11.4 acceptance requires five complete operational cells and "
    ...
    *,
    ...
    for stage_id in V2114_OPERATIONAL_STAGE_IDS:
        ...
        if (
            ...
            != V2114_EXPECTED_STAGE_CELL_COUNTS[stage_id]
        ):
            raise PilotV2114AcceptanceError(
...
def _audit_authorities(
    ...
    *,
    ...
    sources: dict[str, str] = {}
    parent_path = repo_root.joinpath(*V2114_DEFAULT_RECEIPT_PATH.parts)
    try:
        ...
        raise PilotV2114AcceptanceError(
            f"V2.11.4 parent receipt failed verification: {exc}"
        ) from exc
    ...
    for model_id in V2114_ALLOWED_MODELS:
        receipt_path = v2114_observed_p95_receipt_path(raw_root, model_id)
        projection_path = v2114_observed_p95_projection_path(raw_root, model_id)
        try:
    ...
    global_relative = V2114_POST_GATE_RELATIVE_PATH.as_posix()
    try:
        dedicated = verified_v2114_gate_authority_binding(
            global_relative,
            repo_root=repo_root,
            expected_git_commit=paid.head_commit,
        ...
        raise PilotV2114AcceptanceError(
            f"V2.11.4 global observed-p95 consumer failed verification: {exc}"
        ) from exc
    if generic != dedicated:
        raise PilotV2114AcceptanceError(
            "generic and dedicated V2.11.4 authority bindings differ"
        )
    global_path = repo_root.joinpath(*V2114_POST_GATE_RELATIVE_PATH.parts)
    ...
    *,
    ...
    with observed_p95_authority_repo_context(repo_root):
        for spec in provider_specs:
            ...
            except PilotV2114AcceptanceError:
                raise
            except Exception as exc:
                raise PilotV2114AcceptanceError(
    ...
    if len(config_hashes) != V2114_EXPECTED_PROVIDER_CELLS:
        raise PilotV2114AcceptanceError(
    ...
    for model_id, seed in sorted(
        ...
        try:
            plan = orch.build_v2114_experiment_d_group_plan(
                contract,
                group_specs,
        ...
        if receipt.get("cell_count") != V2114_EXPECTED_D_CELLS_PER_GROUP:
            raise PilotV2114AcceptanceError(
                "Experiment D group does not contain exactly eleven cells"
            )
        groups.append(receipt)
    if len(groups) != V2114_EXPECTED_D_GROUPS:
        raise PilotV2114AcceptanceError(
    ...
    return (
        ...
        },
        {
            ...
            "cells_per_group": V2114_EXPECTED_D_CELLS_PER_GROUP,
            "groups": groups,
...
def _audit_projections(
    ...
    *,
    ...
    if len(values) != V2114_EXPECTED_PROJECTION_UNITS or len(
    ...
    calls_by_model = {
        model_id: {"action": 0, "semantic": 0} for model_id in V2114_ALLOWED_MODELS
    }
    for model_id, projection in projections:
        by_kind = projection.basis.get("calls_by_kind", {})
        if not isinstance(by_kind, Mapping):
            raise PilotV2114AcceptanceError("projection call-kind basis is malformed")
    ...
    if not isinstance(forward, Mapping) or not isinstance(
        ...
        raise PilotV2114AcceptanceError(
            "V2.11.4 forward matrix is unavailable during projection audit"
        )
    expected_by_model = forward["matrix"]["fresh_calls_by_model"]
    if (
        calls
        != {
            "action": V2114_EXPECTED_ACTION_CALLS,
            "semantic": V2114_EXPECTED_SEMANTIC_CALLS,
        }
        or sum(calls.values()) != V2114_EXPECTED_PROVIDER_CALLS
        ...
        != V2114_EXPECTED_FRESH_STORAGE_BYTES
    ):
        raise PilotV2114AcceptanceError(
    ...
    try:
        ...
        raise PilotV2114AcceptanceError(
            f"complete V2.11.4 projected matrix exceeds a hard cap: {exc}"
        ) from exc
    ...
    if (
        ...
        raise PilotV2114AcceptanceError("projected matrix exceeds global hard caps")
    return {
...
def audit_v2114_scientific_dispatch(
    contract: PilotContract,
    *,
    repo_root: str | Path,
    raw_root: str | Path,
    ...
    run_before = _file_sha256(run_path, name="V2.11.4 run ledger")
    budget_before = _file_sha256(budget_path, name="V2.11.4 budget ledger")
    run_snapshot = run_ledger.snapshot()
    budget_snapshot = budget_ledger.snapshot()
    if (
        len(run_snapshot.get("events", ())) != V2114_EXPECTED_ACCEPTED_RUN_EVENTS
        or len(budget_snapshot.get("events", ()))
        != V2114_EXPECTED_ACCEPTED_BUDGET_EVENTS
    ):
        raise PilotV2114AcceptanceError(
    ...
    if (
        _file_sha256(run_path, name="V2.11.4 run ledger") != run_before
        or _file_sha256(budget_path, name="V2.11.4 budget ledger") != budget_before
    ):
        raise PilotV2114AcceptanceError("acceptance audit mutated a scientific ledger")
    receipt = {
        "schema_version": V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_SCHEMA_VERSION,
        "status": "go",
        "go": True,
        "contract_id": contract.contract_id,
        "contract_sha256": contract.canonical_hash,
        "release": material["release"],
        "raw_namespace": _relative(repository, raw, name="V2.11.4 raw root"),
...
def _persist_exact_receipt(path: Path, value: Mapping[str, Any]) -> None:
    if path.is_symlink():
        raise PilotV2114AcceptanceError("acceptance receipt must not be a symlink")
    if path.exists():
...
def _require_recoverable_marker_state(
    ...
    for name, snapshot, prefix_count in (
        ("run", run_snapshot, V2114_EXPECTED_ACCEPTED_RUN_EVENTS),
        ("budget", budget_snapshot, V2114_EXPECTED_ACCEPTED_BUDGET_EVENTS),
    ):
        events = snapshot.get("events")
        ...
        if (
            ...
            != V2114_ACCEPTANCE_LEDGER_EVENT_TYPE
        ):
            raise PilotV2114AcceptanceError(
...
def accept_v2114_scientific_dispatch(
    *,
    ...
    """Audit and atomically persist the unique V2.11.4 acceptance receipt."""
    ...
    if contract_candidate.absolute() != repository.joinpath(*V2114_CONTRACT_PATH.parts):
        raise PilotV2114AcceptanceError(
            "acceptance requires experiments/pilot_v2_11_4.yaml"
        )
    contract = load_pilot_contract(contract_candidate)
    output = (
        raw / V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME
        if receipt_path is None
        else Path(receipt_path).absolute()
    )
    expected = raw / V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME
    if output != expected:
        raise PilotV2114AcceptanceError(
    ...
    with orch._exclusive_real_stage_lock(
        ...
        if not output.exists():
            audited = audit_v2114_scientific_dispatch(
                contract,
                repo_root=repository,
                raw_root=raw,
        ...
        receipt = _verify_v2114_scientific_dispatch_acceptance_core(
            output,
            contract=contract,
            repo_root=repository,
            raw_root=raw,
        ...
        return verify_v2114_scientific_dispatch_acceptance(
            output,
            contract=contract,
            repo_root=repository,
            raw_root=raw,
...
def _verify_v2114_scientific_dispatch_acceptance_core(
    ...
    *,
    ...
    if (
        ...
        or receipt.get("raw_namespace") != V2114_RAW_ROOT.as_posix()
        ...
        or configs.get("provider_config_count") != V2114_EXPECTED_PROVIDER_CELLS
        or not isinstance(d_groups, Mapping)
        or d_groups.get("group_count") != V2114_EXPECTED_D_GROUPS
        or d_groups.get("cells_per_group") != V2114_EXPECTED_D_CELLS_PER_GROUP
        or not isinstance(projection, Mapping)
        or projection.get("projection_unit_count") != V2114_EXPECTED_PROJECTION_UNITS
        or projection.get("fresh_provider_calls") != V2114_EXPECTED_PROVIDER_CALLS
    ...
    if (
        ...
        or run_prefix.get("event_count") != V2114_EXPECTED_ACCEPTED_RUN_EVENTS
        or not isinstance(budget_prefix, Mapping)
        or set(budget_prefix) != _LEDGER_PREFIX_FIELDS
        or budget_prefix.get("event_count") != V2114_EXPECTED_ACCEPTED_BUDGET_EVENTS
    ):
        raise PilotV2114AcceptanceError(
    ...
    if not isinstance(sources, Mapping) or not sources:
        raise PilotV2114AcceptanceError("acceptance source bindings are missing")
    for relative, expected_hash in sources.items():
        candidate = PurePosixPath(str(relative))
        if candidate.is_absolute() or any(
            part in {"", ".", ".."} for part in candidate.parts
        ):
            raise PilotV2114AcceptanceError("acceptance source path is not normalized")
        observed = _file_sha256(
            repository.joinpath(*candidate.parts), name=f"accepted source {relative}"
        )
        if observed != expected_hash:
            raise PilotV2114AcceptanceError(f"accepted source file drifted: {relative}")
    ...
    if budget_snapshot.get("caps") != projection.get("hard_caps"):
        raise PilotV2114AcceptanceError("current budget caps differ from acceptance")
    _verify_current_budget_rows(contract, budget_snapshot, projection)
    return _json_copy(receipt)


def verify_v2114_scientific_dispatch_acceptance(
    receipt_path: str | Path,
    *,
    contract: PilotContract,
    repo_root: str | Path,
    ...
    return _verify_v2114_scientific_dispatch_acceptance_core(
        receipt_path,
        contract=contract,
        repo_root=repo_root,
        raw_root=raw_root,
...
__all__ = [
    ...
    "audit_v2114_scientific_dispatch",
