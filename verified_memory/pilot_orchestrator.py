...
from .pilot_contract import (
    ...
    PILOT_CONTRACT_ID_V2_11_2 as V2112_CONTRACT_ID,
    PILOT_CONTRACT_ID_V2_11_1 as V2111_CONTRACT_ID,
    PILOT_CONTRACT_ID_V2_11 as V211_CONTRACT_ID,
    PILOT_CONTRACT_ID_V2_10_2 as V2102_CONTRACT_ID,
    PILOT_CONTRACT_ID_V2_10_1 as V2101_CONTRACT_ID,
    PILOT_CONTRACT_ID_V2_10 as V210_CONTRACT_ID,
...
from .pilot_v210_parent_import import (
    V29_RAW_STORAGE_BYTES as V210_PARENT_RAW_STORAGE_BYTES,
    V29_SCIENCE_COMMIT as V210_PARENT_COMMIT,
    V29_SCIENCE_TAG as V210_PARENT_TAG,
...
from .pilot_v2102_parent_import import (
    V2101_PARENT_RAW_STORAGE_BYTES as V2102_PARENT_RAW_STORAGE_BYTES,
...
from .pilot_v2111_bootstrap import (
...
from .pilot_v2112_bootstrap import (
...
from .pilot_v2113_parent_import import (
...
from .pilot_v2113_gate import (
    V2113_PREFLIGHT_IMPORT_GATE_SCHEMA_VERSION,
    PilotV2113GateError,
    ...
    verified_v2113_observed_p95_projection_binding,
...
PILOT_V211_POST_GATE_AUTHORITY_FILENAME = "post_gate_authority.json"
PILOT_V211_POST_GATE_STAGE_ID = "long-context-preflight"
PILOT_V27_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION = (
    "finevo-pilot-v2.7-imported-run-envelope-v1"
)
...
PILOT_V210_IMPORTED_QREF_SCHEMA_VERSION = (
    "finevo-pilot-v2.10-imported-qref-resolution-v1"
)
PILOT_V210_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION = (
    "finevo-pilot-v2.10-imported-run-envelope-v1"
)
PILOT_V210_STAGE0_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10-stage0-commit-intent-v1"
)
PILOT_V210_PARENT_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10-parent-import-commit-intent-v1"
)
PILOT_V210_PARENT_FAILURE_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10-parent-import-failure-intent-v1"
)
PILOT_V2101_IMPORTED_QREF_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.1-imported-qref-resolution-v1"
)
PILOT_V2101_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.1-imported-run-envelope-v1"
)
PILOT_V2101_STAGE0_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.1-stage0-commit-intent-v1"
)
PILOT_V2101_PARENT_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.1-parent-import-commit-intent-v1"
)
PILOT_V2101_PARENT_FAILURE_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.1-parent-import-failure-intent-v1"
)
PILOT_V2102_IMPORTED_QREF_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.2-imported-qref-resolution-v1"
)
PILOT_V2102_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.2-imported-run-envelope-v1"
)
PILOT_V2102_STAGE0_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.2-stage0-commit-intent-v1"
)
PILOT_V2102_PARENT_COMMIT_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.2-parent-import-commit-intent-v1"
)
PILOT_V2102_PARENT_FAILURE_INTENT_SCHEMA_VERSION = (
    "finevo-pilot-v2.10.2-parent-import-failure-intent-v1"
)
...
CAPABILITY_EXECUTION_MODES = frozenset(
    {
        "capability_probe",
...
V211_FAMILY_CONTRACT_IDS = frozenset(
    {
        V211_CONTRACT_ID,
        V2111_CONTRACT_ID,
        V2112_CONTRACT_ID,
...
# a local-first continuation.  V2.11 imports calibration and debit only: it
# must obtain fresh p95 evidence and retain its registered cross-model lane.
INHERITED_P95_PARENT_CONTRACT_IDS = frozenset(
    PARENT_IMPORT_CONTRACT_IDS - V211_FAMILY_CONTRACT_IDS
)
LOCAL_FIRST_PARENT_CONTRACT_IDS = INHERITED_P95_PARENT_CONTRACT_IDS
...
def _contract_core_stage_ids(contract: PilotContract) -> tuple[str, ...]:
    return tuple(
        stage_id for stage_id in contract.stage_ids if _is_core_stage(stage_id)
    )
...
class PilotRunLedger:
        ...
        for index, row in enumerate(events):
            ...
            if index == 0:
                ...
                for run_id, spec_sha256 in registered.items():
                    ...
                    if not isinstance(
                        current, Mapping
                    ) or spec_sha256 != canonical_sha256(current.get("spec")):
                        raise PilotOrchestrationError(
            ...
            elif event_type == "acceptance_receipt_bound":
                required = {
                ...
                if (
                    ...
                    or payload.get("accepted_run_event_count") != index
                    or payload.get("accepted_run_event_chain_head")
                    != row.get("previous_event_sha256")
                    or not isinstance(payload.get("accepted_budget_event_count"), int)
                    or isinstance(payload.get("accepted_budget_event_count"), bool)
                    or int(payload["accepted_budget_event_count"]) < 1
                    or any(
                        not isinstance(payload.get(key), str)
                        or len(str(payload[key])) != 64
                        or any(
                            character not in "0123456789abcdef"
                            for character in str(payload[key])
                        )
                        for key in (
                            "receipt_content_sha256",
                            "accepted_run_event_chain_head",
                    ...
                    or not isinstance(events[index - 1].get("payload"), Mapping)
                    or payload.get("runs_sha256")
                    != events[index - 1]["payload"].get("runs_sha256")
                ):
                    raise PilotOrchestrationError(
                        "pilot run ledger acceptance receipt binding is malformed"
                    )
            else:
                raise PilotOrchestrationError(
    ...
    def bind_acceptance_receipt(
        ...
        *,
        ...
        """Append one immutable pre-dispatch receipt marker.

        The receipt itself binds the event-chain prefixes that existed before
        this marker.  The marker then binds the receipt's canonical content
        hash, avoiding a circular dependency while proving that the accepted
        bytes existed before any later scientific run-ledger event.
        """

        if not self.tamper_evident:
            raise PilotOrchestrationError(
                "acceptance receipt binding requires a tamper-evident run ledger"
            )
        ...
        payload = {
            ...
            "accepted_budget_event_chain_head": str(accepted_budget_event_chain_head),
            "runs_sha256": canonical_sha256(self._state["runs"]),
        }
        if existing:
            if len(existing) != 1 or existing[0].get("payload") != payload:
                raise PilotOrchestrationError(
                    "pilot run ledger acceptance receipt binding drifted"
                )
            return
        if (
            isinstance(accepted_run_event_count, bool)
            or accepted_run_event_count != len(events)
            or not events
            or events[-1].get("event_sha256") != accepted_run_event_chain_head
        ):
            raise PilotOrchestrationError(
                "acceptance receipt does not bind the current run-ledger head"
            )
        self._append_event("acceptance_receipt_bound", payload)
        self._write()
    ...
    def finalize_many(
        ...
        for index, value in enumerate(entries):
            if not isinstance(value, Mapping) or set(value) != {
                ...
                raise ValueError(f"run finalization {index} has an invalid shape")
...
def _parent_budget_debit(
    contract: PilotContract,
    *,
    repo_root: str | Path | None = None,
    parent_repo_root: str | Path | None = None,
):
    if contract.contract_id == V2114_CONTRACT_ID:
        try:
            debit = parent_budget_debit_for_v2114(contract)
        except PilotV2114ParentImportError as exc:
            raise PilotOrchestrationError(
                f"V2.11.4 parent budget debit failed validation: {exc}"
            ) from exc
        if debit is None:
            raise PilotOrchestrationError(
                "V2.11.4 parent budget debit is unexpectedly absent"
            )
        return debit
    if contract.contract_id == V2113_CONTRACT_ID:
        try:
            debit = parent_budget_debit_for_v2113(contract)
        except PilotV2113ParentImportError as exc:
            raise PilotOrchestrationError(
                f"V2.11.3 parent budget debit failed validation: {exc}"
            ) from exc
        if debit is None:
            raise PilotOrchestrationError(
                "V2.11.3 parent budget debit is unexpectedly absent"
            )
        return debit
    if contract.contract_id == V2112_CONTRACT_ID:
        ...
        try:
            return parent_budget_debit_for_v2112(
                contract,
                repo_root=release_root,
                parent_science_root=parent_repo_root,
            )
        except PilotV2112ParentImportError as exc:
            raise PilotOrchestrationError(
                f"V2.11.2 parent budget debit failed validation: {exc}"
            ) from exc
    if contract.contract_id == V2111_CONTRACT_ID:
        release_root = (
            Path(repo_root).resolve()
            if repo_root is not None
            else Path(__file__).resolve().parents[1]
        ...
        try:
            return parent_budget_debit_for_v2111(
                contract,
                repo_root=release_root,
                parent_science_root=parent_repo_root,
            )
            ...
            raise PilotOrchestrationError(
                f"V2.11.1 parent budget debit failed validation: {exc}"
            ) from exc
    if contract.contract_id == V211_CONTRACT_ID:
        ...
        try:
            return parent_budget_debit_for_v211(
                contract,
                repo_root=release_root,
                science_repo_root=parent_repo_root,
            )
        except PilotV211ParentImportError as exc:
            raise PilotOrchestrationError(
                f"V2.11 parent budget debit failed validation: {exc}"
            ) from exc
    if contract.contract_id == V2102_CONTRACT_ID:
        v2102_debit = parent_budget_debit_for_v2102(contract)
        if v2102_debit is not None:
            return v2102_debit
    if contract.contract_id == V2101_CONTRACT_ID:
        v2101_debit = parent_budget_debit_for_v2101(contract)
        if v2101_debit is not None:
            return v2101_debit
    if contract.contract_id == V210_CONTRACT_ID:
        v210_debit = parent_budget_debit_for_v210(contract)
        if v210_debit is not None:
            return v210_debit
...
def _strict_imported_jsonl(path: Path) -> tuple[Mapping[str, Any], ...]:
    """Read a hash-verified parent stream without invoking today's runner validator."""
    ...
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, item in pairs:
            if key in parsed:
                raise PilotOrchestrationError(
                    f"imported JSONL stream contains duplicate key {key!r}: {path}"
                )
            parsed[key] = item
        return parsed
    ...
    def reject_nonfinite(value: str) -> None:
        raise PilotOrchestrationError(
            f"imported JSONL stream contains non-finite value {value}: {path}"
        )
    ...
    try:
        ...
        raise PilotOrchestrationError(
            f"imported JSONL stream is not strict UTF-8 JSON: {path}"
        ) from exc
    if any(not isinstance(row, Mapping) for row in rows):
        raise PilotOrchestrationError(
            f"imported JSONL stream contains a non-object row: {path}"
        )
    return rows


def _v27_import_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Revalidate the current V2.7 receipt and tracked immutable-source map."""

    if contract.contract_id != V27_CONTRACT_ID:
        raise PilotOrchestrationError("V2.7 import authority used by another contract")
    repository = Path(__file__).resolve().parents[1]
    try:
        manifest = load_v27_source_manifest(
            repo_root=repository,
            contract=contract,
        )
        receipt = verify_v27_parent_import_receipt(
        ...
        raise PilotOrchestrationError(
            f"V2.7 immutable Stage-0 import authority failed validation: {exc}"
        ) from exc
    if (
        ...
        or receipt.get("v2_6_terminal_no_go_preserved") is not True
    ):
        raise PilotOrchestrationError(
...
def _v210_import_authority(
    ...
    *,
    ...
    """Revalidate V2.10's exact sixteen-cell V2.9 prerequisite import."""

    if contract.contract_id != V210_CONTRACT_ID:
        raise PilotOrchestrationError("V2.10 import authority used by another contract")
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
        else Path(authority_repo_root).resolve(strict=True)
    )
    source_manifest_path = repository.joinpath(*V210_SOURCE_MANIFEST_PATH.parts)
    try:
        manifest = load_v210_source_manifest(source_manifest_path)
        receipt = verify_v210_parent_import_receipt(
            receipt_path=(raw_root / "parent-import" / "parent_import_receipt.json"),
            child_repo_root=repository,
            contract=contract,
            expected_git_commit=paid.head_commit,
        )
    except PilotV210ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.10 immutable prerequisite authority failed validation: {exc}"
        ) from exc

    rows = manifest.get("imported_complete_cells")
    stage_counts = (
        Counter(row.get("stage_id") for row in rows if isinstance(row, Mapping))
        if isinstance(rows, list)
        else Counter()
    )
    source_parent = receipt.get("source_parent")
    imported_cells = receipt.get("imported_cells")
    p95_authorities = receipt.get("p95_current_release_authorities")
    if (
        ...
        or not isinstance(source_parent, Mapping)
        or source_parent.get("contract_id") != V29_CONTRACT_ID
        or source_parent.get("science_tag") != V210_PARENT_TAG
        or source_parent.get("science_commit") != V210_PARENT_COMMIT
        or source_parent.get("terminal_status") != "complete-with-no-go"
        or source_parent.get("scientific_complete") is not False
        or not isinstance(imported_cells, Mapping)
        or imported_cells.get("count") != 16
        or imported_cells.get("breakdown")
        != {
            "parent-import": 1,
            "q-ref-resolution": 1,
            "stage0-calibration": 14,
        }
        or imported_cells.get("q_ref") != 63.50397933257746
        or imported_cells.get("stage0_selected_profile_id") != "nu-0.5"
        or imported_cells.get("offline_candidate_admission_cells_imported") != 0
        or imported_cells.get("failed_actor_cells_imported") != 0
        or not isinstance(p95_authorities, Mapping)
        or set(p95_authorities) != set(V210_ALLOWED_P95_PROFILES)
        or receipt.get("provider_construction_during_import") is not False
        or receipt.get("provider_calls_during_import") != 0
        or receipt.get("hosted_provider_calls_during_import") != 0
        or receipt.get("hosted_cost_usd_during_import") != 0.0
        or receipt.get("scientific_evidence") is not False
    ):
        raise PilotOrchestrationError(
            "V2.10 parent receipt violates its exact prerequisite/import boundary"
        )
    return manifest, receipt


def _v2101_import_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Revalidate V2.10.1's dedicated exact-V2.9 prerequisite authority."""

    if contract.contract_id != V2101_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.1 import authority used by another contract"
        )
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
    ...
    source_manifest_path = repository.joinpath(*V2101_SOURCE_MANIFEST_PATH.parts)
    try:
        manifest = load_v2101_source_manifest(source_manifest_path)
        receipt = verify_v2101_parent_import_receipt(
            receipt_path=(raw_root / "parent-import" / "parent_import_receipt.json"),
            child_repo_root=repository,
            contract=contract,
            expected_git_commit=paid.head_commit,
        ...
        raise PilotOrchestrationError(
            f"V2.10.1 immutable prerequisite authority failed validation: {exc}"
        ) from exc
    ...
    terminal_parent = receipt.get("terminal_parent")
    source_parent = receipt.get("source_parent")
    imported_cells = receipt.get("imported_cells")
    p95_authorities = receipt.get("p95_current_release_authorities")
    if (
        ...
        or len({row.get("target_run_id") for row in rows}) != 16
        or not isinstance(terminal_parent, Mapping)
        or terminal_parent.get("contract_id") != V210_CONTRACT_ID
        or terminal_parent.get("terminal_status") != "complete-with-no-go"
        or terminal_parent.get("status_counts")
        != {"complete": 1, "integrity-stopped": 210}
        or not isinstance(source_parent, Mapping)
        or source_parent.get("contract_id") != V29_CONTRACT_ID
        or source_parent.get("terminal_status") != "complete-with-no-go"
        or source_parent.get("scientific_complete") is not False
        or source_parent.get("source_path_kind")
        ...
        or imported_cells.get("failed_actor_cells_imported") != 0
        or not isinstance(p95_authorities, Mapping)
        or set(p95_authorities) != set(V2101_ALLOWED_P95_PROFILES)
        or receipt.get("provider_construction_during_import") is not False
        or receipt.get("provider_calls_during_import") != 0
        or receipt.get("hosted_provider_calls_during_import") != 0
        or receipt.get("hosted_cost_usd_during_import") != 0.0
        ...
        raise PilotOrchestrationError(
            "V2.10.1 parent receipt violates its exact prerequisite/import boundary"
        )
    return manifest, receipt


def _v2102_import_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Revalidate V2.10.2's exact nested-V2.9 prerequisite authority."""

    if contract.contract_id != V2102_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.2 import authority used by another contract"
        )
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
    ...
    source_manifest_path = repository.joinpath(*V2102_SOURCE_MANIFEST_PATH.parts)
    try:
        manifest = load_v2102_source_manifest(source_manifest_path)
        receipt = verify_v2102_parent_import_receipt(
            receipt_path=(raw_root / "parent-import" / "parent_import_receipt.json"),
            child_repo_root=repository,
            contract=contract,
            expected_git_commit=paid.head_commit,
        ...
        raise PilotOrchestrationError(
            f"V2.10.2 immutable prerequisite authority failed validation: {exc}"
        ) from exc
    ...
    if (
        ...
        or len({row.get("target_run_id") for row in rows}) != 16
        or not isinstance(terminal_parent, Mapping)
        or terminal_parent.get("contract_id") != V2101_CONTRACT_ID
        or terminal_parent.get("terminal_status") != "complete-with-no-go"
        or terminal_parent.get("status_counts") != {"complete": 26, "failed": 185}
        or terminal_parent.get("implementation_root_cause")
        != "observed-p95-consumer-schema-dispatch-gap"
        or terminal_parent.get("offline_candidate_admission_cells_observed") != 10
        or terminal_parent.get("actor_performance_treatment_outcome_blind") is not True
        or not isinstance(source_parent, Mapping)
        or source_parent.get("contract_id") != V29_CONTRACT_ID
        or source_parent.get("terminal_status") != "complete-with-no-go"
        or source_parent.get("scientific_complete") is not False
        ...
        or imported_cells.get("failed_actor_cells_imported") != 0
        or not isinstance(p95_authorities, Mapping)
        or set(p95_authorities) != set(V2102_ALLOWED_P95_PROFILES)
        or receipt.get("provider_construction_during_import") is not False
        or receipt.get("provider_calls_during_import") != 0
        or receipt.get("hosted_provider_calls_during_import") != 0
        or receipt.get("hosted_cost_usd_during_import") != 0.0
        ...
        raise PilotOrchestrationError(
            "V2.10.2 parent receipt violates its exact prerequisite/import boundary"
        )
    return manifest, receipt


def _load_v27_imported_result(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Load only the immutable streams allowed by the V2.7 import amendment.

    The complete V2.6 run was validated in its own science checkout before the
    byte-exact snapshot was admitted.  Replaying the current full runner
    validator here would apply post-V2.6 semantics to an immutable historical
    result.  We instead reverify the copied manifest and source binding, then
    expose the sealed streams to the dedicated q-ref/Stage-0 readers only.
    """

    if contract.contract_id != V27_CONTRACT_ID:
        raise PilotOrchestrationError("imported V2.6 result requested outside V2.7")
    manifest = (
        dict(source_manifest)
        if source_manifest is not None
        else _v27_import_authority(contract, raw_root=raw_root, paid=paid)[0]
    )
    try:
        binding = source_binding_for_target(manifest, spec)
        run_dir = imported_v26_run_dir(raw_root, spec, manifest)
    except PilotV27Stage0ImportError as exc:
        raise PilotOrchestrationError(
            f"{spec.run_id} has no valid immutable V2.6 source: {exc}"
        ) from exc
    ...
    if not isinstance(declared_manifest, Mapping):
        raise PilotOrchestrationError(
            f"{spec.run_id} imported source lacks a manifest binding"
        )
    verification = verify_manifest(run_dir)
    manifest_path = run_dir / "manifest.json"
    if (
        not verification.valid
        or verification.manifest_sha256 != declared_manifest.get("file_sha256")
        or _file_sha256(manifest_path) != declared_manifest.get("file_sha256")
    ):
        raise PilotOrchestrationError(
            f"{spec.run_id} imported verified-run manifest drifted"
        )
    ...
    if (
        ...
        != manifest.get("v2_6_terminal_parent", {})
        .get("contract", {})
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} imported source run identity is malformed"
        )
    if spec.stage_id == "stage0-calibration":
        decoded_streams = ("actions", "utility_ledger", "errors")
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} is outside the V2.7 imported reader boundary"
        )
    streams: dict[str, tuple[Mapping[str, Any], ...]] = {}
    streams_root = run_dir / "streams"
    for name in decoded_streams:
        path = streams_root / f"{name}.jsonl"
        if not path.exists():
            streams[name] = ()
            continue
        if path.is_symlink() or not path.is_file():
            raise PilotOrchestrationError(
                f"{spec.run_id} imported stream path is not a regular file"
            )
        streams[name] = _strict_imported_jsonl(path)
    summary_rows = streams.pop("summary", ())
    if spec.stage_id == "q-ref-resolution" and len(summary_rows) != 1:
        raise PilotOrchestrationError(
            f"{spec.run_id} imported source lacks one exact run summary"
        )
    ...
    if spec.stage_id == "stage0-calibration":
        ...
        try:
            ...
            journal_path = snapshot_path_for_source_artifact(
                raw_root,
            ...
            journal = verify_provider_call_journal(
                ...
                expected_contract_hash=str(source_contract_sha256),
                require_terminal_dispositions=True,
            )
        except (PilotV27Stage0ImportError, ValueError) as exc:
            raise PilotOrchestrationError(
...
def _load_v28_imported_stage0_result(
    ...
    *,
    ...
    if contract.contract_id != V28_CONTRACT_ID or spec.stage_id != "stage0-calibration":
        raise PilotOrchestrationError("V2.8 imported result reader is Stage-0-only")
    manifest = (
        dict(source_manifest)
        if source_manifest is not None
    ...
    if (
        ...
        or config.get("run_id") != binding.get("source_run_id")
        or details.get("run_spec") != source_spec
        or details.get("contract_sha256") != physical_contract.get("canonical_sha256")
        or manifest_value.get("result", {}).get("complete") is not True
        or manifest_value.get("validation_status", {}).get("status") != "pass"
    ):
        raise PilotOrchestrationError(
    ...
    try:
        ...
        journal = verify_provider_call_journal(
            journal_path,
            expected_run_id=str(binding["source_run_id"]),
            expected_contract_hash=str(physical_contract.get("canonical_sha256")),
            require_terminal_dispositions=True,
        )
...
def _load_v29_imported_stage0_result(
    ...
    *,
    ...
    if contract.contract_id != V29_CONTRACT_ID or spec.stage_id != "stage0-calibration":
        raise PilotOrchestrationError("V2.9 imported result reader is Stage-0-only")
    manifest = (
        dict(source_manifest)
        if source_manifest is not None
...
def _v27_source_binding_payload(
    *,
    ...
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    ...
    return {
        ...
        "parent_import_receipt": {
            ...
            "content_sha256": parent_receipt.get("integrity", {}).get("content_sha256"),
        },
        "source_contract_id": "finevo-pilot-v2.6",
        "source_contract_sha256": source_manifest.get("v2_6_terminal_parent", {})
        .get("contract", {})
        .get("canonical_sha256"),
...
def _v28_source_binding_payload(
    *,
    ...
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    parent_receipt = _read_json(parent_receipt_path)
    nested_contract = source_manifest.get("nested_v2_6_stage0_source", {}).get(
        "contract", {}
    )
    parent_contract = source_manifest.get("v2_7_terminal_parent", {}).get(
        "contract", {}
    )
    return {
        "source_manifest": {
        ...
        "source_authority_contract_sha256": parent_contract.get("canonical_sha256"),
        "physical_source_contract_id": nested_contract.get("contract_id"),
        "physical_source_contract_sha256": nested_contract.get("canonical_sha256"),
        "source_run_id": source_binding.get("source_run_id"),
        "source_spec": source_binding.get("source_spec"),
        "source_artifacts": source_binding.get("source_artifacts"),
        "snapshot_run_root": str(source_run_dir),
...
def _v29_source_binding_payload(
    *,
    ...
    parent_contract = source_manifest.get("v2_8_terminal_parent", {}).get(
        "contract", {}
    )
    return {
        "source_manifest": {
...
def _expected_v27_q_ref_resolution(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    specs = tuple(contract.expand(stage="q-ref-resolution"))
    if len(specs) != 1:
        raise PilotOrchestrationError("V2.7 q-ref import requires one exact cell")
    ...
    source_q_ref_binding = source_binding.get("source_artifacts", {}).get(
        "q_ref_resolution"
    )
    if not isinstance(source_q_ref_binding, Mapping):
        raise PilotOrchestrationError("V2.7 q-ref import lacks its source resolution")
    try:
        from .pilot_v27_stage0_import import snapshot_path_for_source_artifact

        source_q_ref_path = snapshot_path_for_source_artifact(
            raw_root,
            str(source_q_ref_binding.get("path", "")),
        )
    except PilotV27Stage0ImportError as exc:
        raise PilotOrchestrationError(
            f"V2.7 q-ref source path failed validation: {exc}"
        ) from exc
    source_q_ref = _read_json(source_q_ref_path)
    source_q_ref_bindings = source_q_ref.get("bindings")
    expected_environment_source_hash = _file_sha256(DEFAULT_ENV_CONFIG)
    if (
        _file_sha256(source_q_ref_path) != source_q_ref_binding.get("file_sha256")
        or source_q_ref.get("q_ref") != recomputed.get("q_ref")
        or source_q_ref.get("status") != "pass"
        or source_q_ref.get("scientific_evidence") is not False
        or not isinstance(source_q_ref_bindings, Mapping)
        or source_q_ref_bindings.get("environment_source_hash")
        != expected_environment_source_hash
        or recomputed.get("bindings", {}).get("environment_source_hash")
        != expected_environment_source_hash
    ):
        raise PilotOrchestrationError(
            "V2.7 q-ref recomputation differs from its immutable source"
        )
    recomputed.update(
        {
            ...
            "source_import": _v27_source_binding_payload(
                source_manifest=source_manifest,
            ...
            "bindings": {
                **dict(recomputed["bindings"]),
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
                "git_commit": paid.head_commit,
                "source_manifest_sha256": source_q_ref_binding.get("file_sha256"),
            },
        }
    )
...
def _expected_v210_q_ref_resolution(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Reseal the one exact V2.9 q-ref prerequisite without provider work."""

    source_manifest, parent_receipt = _v210_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
    ...
    specs = tuple(contract.expand(stage="q-ref-resolution"))
    if len(specs) != 1 or specs[0].execution_mode != "q_ref_resolution":
        raise PilotOrchestrationError(
            "V2.10 q-ref import requires one exact zero-provider cell"
        )
    spec = specs[0]
    try:
        source = verified_v210_imported_prerequisite_binding(
            raw_root,
            source_manifest,
            spec,
        )
        ...
        raise PilotOrchestrationError(
            f"V2.10 q-ref prerequisite failed validation: {exc}"
        ) from exc
    source_resolution = source.get("q_ref_resolution")
    ...
    if (
        source.get("source_stage_id") != "q-ref-resolution"
        or source.get("target_spec") != spec.to_dict()
        or source.get("q_ref") != 63.50397933257746
        or not isinstance(source_resolution, Mapping)
        or source_resolution.get("status") != "pass"
        or source_resolution.get("q_ref") != 63.50397933257746
        or source_resolution.get("row_count") != 48
        or not isinstance(source_artifacts, Mapping)
        or not isinstance(source_artifacts.get("q_ref_resolution"), Mapping)
        or not isinstance(source_release, Mapping)
        or not isinstance(source_terminal, Mapping)
        or source.get("provider_construction_during_verification") is not False
        or source.get("provider_calls_during_verification") != 0
        or source.get("treatment_effect_evidence") is not False
    ):
        raise PilotOrchestrationError("V2.10 imported q-ref binding is malformed")
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
        else Path(authority_repo_root).resolve(strict=True)
    ...
    return _seal_bound_payload(
        {
            "schema_version": PILOT_V210_IMPORTED_QREF_SCHEMA_VERSION,
            "status": "pass",
            ...
            "evidence_scope": "preregistered_q_ref_resolution",
            "claim_boundary": (
                "Exact V2.9 q-ref prerequisite import and V2.10 reseal only; "
                "no V2.10 A-D treatment-effect or model-performance evidence."
            ),
            "source_import": _json_copy(source),
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
                "git_commit": paid.head_commit,
                "source_contract_id": source_release.get("contract_id"),
                "source_contract_sha256": source_release.get("contract_sha256"),
                "source_run_id": source.get("source_run_id"),
                "source_q_ref_file_sha256": source_artifacts["q_ref_resolution"].get(
                    "file_sha256"
                ),
                "source_q_ref_content_sha256": source_artifacts["q_ref_resolution"].get(
                    "content_sha256"
                ),
                "source_terminal_file_sha256": source_terminal.get("file_sha256"),
                "source_terminal_content_sha256": source_terminal.get("content_sha256"),
                "source_raw_inventory_sha256": source_release.get(
                ...
                "parent_import_receipt_file_sha256": _file_sha256(parent_receipt_path),
                "parent_import_receipt_content_sha256": parent_receipt.get(
                    "integrity", {}
                ).get("content_sha256"),
            },
        }
...
def _expected_v2101_q_ref_resolution(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Reseal the exact V2.9 q-ref under V2.10.1's own authority."""

    source_manifest, parent_receipt = _v2101_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
        authority_repo_root=authority_repo_root,
    ...
    if len(specs) != 1 or specs[0].execution_mode != "q_ref_resolution":
        raise PilotOrchestrationError(
            "V2.10.1 q-ref import requires one exact zero-provider cell"
        )
    spec = specs[0]
    try:
        source = verified_v2101_imported_prerequisite_binding(
            raw_root,
            source_manifest,
            spec,
        )
        ...
        raise PilotOrchestrationError(
            f"V2.10.1 q-ref prerequisite failed validation: {exc}"
        ) from exc
    source_resolution = source.get("q_ref_resolution")
    source_artifacts = source.get("source_artifacts")
    source_release = source.get("source_release")
    ...
    if (
        ...
        or not isinstance(source_artifacts.get("q_ref_resolution"), Mapping)
        or not isinstance(source_release, Mapping)
        or source_release.get("contract_id") != V29_CONTRACT_ID
        or source_release.get("source_path_kind")
        != "byte-exact-v2.9-raw-inside-v2.10-terminal-snapshot"
        or not isinstance(source_terminal, Mapping)
        or source.get("provider_construction_during_verification") is not False
        or source.get("provider_calls_during_verification") != 0
        ...
        raise PilotOrchestrationError("V2.10.1 imported q-ref binding is malformed")
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
        else Path(authority_repo_root).resolve(strict=True)
    ...
    return _seal_bound_payload(
        {
            "schema_version": PILOT_V2101_IMPORTED_QREF_SCHEMA_VERSION,
            "status": "pass",
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            "q_ref": 63.50397933257746,
            ...
            "evidence_scope": "preregistered_q_ref_resolution",
            "claim_boundary": (
                "Exact V2.9 q-ref prerequisite import and V2.10.1 reseal "
                "only; no V2.10.1 A-D treatment-effect or model-performance "
                "evidence."
            ),
            "source_import": _json_copy(source),
            "bindings": {
...
def _expected_v2102_q_ref_resolution(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Reseal the exact V2.9 q-ref under V2.10.2's own authority."""

    source_manifest, parent_receipt = _v2102_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
        authority_repo_root=authority_repo_root,
    ...
    if len(specs) != 1 or specs[0].execution_mode != "q_ref_resolution":
        raise PilotOrchestrationError(
            "V2.10.2 q-ref import requires one exact zero-provider cell"
        )
    spec = specs[0]
    try:
        source = verified_v2102_imported_prerequisite_binding(
            raw_root,
            source_manifest,
            spec,
        )
        ...
        raise PilotOrchestrationError(
            f"V2.10.2 q-ref prerequisite failed validation: {exc}"
        ) from exc
    source_resolution = source.get("q_ref_resolution")
    source_artifacts = source.get("source_artifacts")
    source_release = source.get("source_release")
    ...
    if (
        ...
        raise PilotOrchestrationError("V2.10.2 imported q-ref binding is malformed")
    repository = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
        else Path(authority_repo_root).resolve(strict=True)
    ...
    return _seal_bound_payload(
        {
            "schema_version": PILOT_V2102_IMPORTED_QREF_SCHEMA_VERSION,
            "status": "pass",
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            "q_ref": 63.50397933257746,
            ...
            "evidence_scope": "preregistered_q_ref_resolution",
            "claim_boundary": (
                "Exact V2.9 q-ref prerequisite import and V2.10.2 reseal "
                "only; no V2.10.2 A-D treatment-effect or model-performance "
                "evidence."
            ),
            "source_import": _json_copy(source),
            "bindings": {
...
def _build_v28_qref_equivalence_receipt(
    ...
    *,
    ...
    if contract.contract_id != V28_CONTRACT_ID:
        raise PilotOrchestrationError("V2.8 q-ref equivalence used by another contract")
    ...
    try:
        ...
        source_resolution_path = snapshot_path_for_v27_source_artifact_v28(
            raw_root,
            str(reference.get("q_ref_resolution", {}).get("path", "")),
        )
    except PilotV28Stage0ImportError as exc:
        raise PilotOrchestrationError(
    ...
    if (
        ...
        != source_manifest_binding.get("file_sha256")
        or _file_sha256(source_config_path) != source_config_binding.get("file_sha256")
        or _file_sha256(source_resolution_path)
        != source_resolution_binding.get("file_sha256")
    ):
        raise PilotOrchestrationError("V2.8 q-ref historical source bindings drifted")
    ...
    if (
        source_resolution.get("schema_version") != "finevo-q-ref-resolution-v1"
    ...
    source_streams = {
        name: _strict_imported_jsonl(source_run_dir / "streams" / f"{name}.jsonl")
    ...
    checks = {
        "fresh_config_run_id_matches_contract_cell": (
            current_run_id == contract.expand(stage="q-ref-resolution")[0].run_id
        ),
        "historical_config_run_id_is_distinct": (
            isinstance(historical_run_id, str) and historical_run_id != current_run_id
        ),
        "config_equal_except_run_id": current_config == historical_config,
        "actions_exact": (tuple(result.stream("actions")) == source_streams["actions"]),
        "utility_ledger_exact": (
            tuple(result.stream("utility_ledger")) == source_streams["utility_ledger"]
        ),
        "shock_events_exact": (
            tuple(result.stream("shock_events")) == source_streams["shock_events"]
        ),
        "q_ref_exact": (
            resolution.get("q_ref")
            == source_resolution.get("q_ref")
            == reference.get("q_ref")
        ),
        "row_count_exact": (
            resolution.get("row_count") == source_resolution.get("row_count") == 48
        ),
        "run_contract_exact": (
            resolution.get("run_contract") == source_resolution.get("run_contract")
        ),
        "checks_exact": (resolution.get("checks") == source_resolution.get("checks")),
        "run_summary_exact": (
    ...
    return _seal_bound_payload(
        {
            ...
            "current": {
                ...
                "actions_sha256": canonical_sha256(list(result.stream("actions"))),
                ...
                "source_config_hash": current_bindings["source_config_hash"],
            },
            "historical_reference": {
                "run_id": historical_run_id,
                "source_run_root": str(source_run_dir),
                "source_manifest_file_sha256": source_manifest_binding["file_sha256"],
                "q_ref_resolution": str(source_resolution_path),
                "q_ref_resolution_file_sha256": source_resolution_binding[
                    "file_sha256"
                ],
                "actions_sha256": canonical_sha256(list(source_streams["actions"])),
                "utility_ledger_sha256": canonical_sha256(
                ...
                "source_config_hash": source_bindings["source_config_hash"],
            ...
            "bindings": {
                ...
                "git_tag": paid.git_tag,
                "git_commit": paid.head_commit,
                "source_manifest_file_sha256": _file_sha256(source_manifest_file),
                "source_manifest_content_sha256": source_manifest["integrity"][
                    "content_sha256"
                ],
                "parent_import_receipt_file_sha256": _file_sha256(parent_receipt_path),
                "parent_import_receipt_content_sha256": parent_receipt["integrity"][
                    "content_sha256"
                ],
            },
        }
...
def _build_v29_qref_equivalence_receipts(
    ...
    *,
    ...
    if contract.contract_id != V29_CONTRACT_ID:
        raise PilotOrchestrationError("V2.9 q-ref equivalence used by another contract")
    ...
    fixed_bindings = {
        name: verified_runner.get(name) for name in ("manifest", "config", "provenance")
    }
    stream_bindings = verified_runner.get("streams")
    if any(
        not isinstance(value, Mapping) for value in fixed_bindings.values()
    ...
    for name, binding in fixed_bindings.items():
        ...
        if _file_sha256(artifact_path) != binding.get(
            "file_sha256"
    ...
    for name, expected_rows in expected_stream_rows.items():
        ...
        if (
            stream_path != source_run_dir / "streams" / f"{name}.jsonl"
    ...
    try:
        ...
        if str(exc) != expected_extra:
            raise PilotOrchestrationError(
                f"V2.9 historical q-ref runner manifest failed " f"verification: {exc}"
            ) from exc
        ...
        verification = None
    if verification is not None and verification.manifest_sha256 != fixed_bindings[
        "manifest"
    ].get("file_sha256"):
        raise PilotOrchestrationError(
    ...
    if (
        ...
        or tuple(historical_result.stream("actions")) != historical_streams["actions"]
        ...
        raise PilotOrchestrationError("V2.9 historical q-ref runner identity drifted")
    ...
    historical_budget_id = historical_result.summary.get("api", {}).get("budget_id")
    if not all(
    ...
    comparison = {
        "fresh_config_run_id_matches_contract_cell": (
            current_config_run_id == contract.expand(stage="q-ref-resolution")[0].run_id
        ),
        ...
        "shock_events_exact": (
            tuple(result.stream("shock_events")) == historical_streams["shock_events"]
        ),
        "q_ref_exact": (
        ...
        "row_count_exact": (
            resolution.get("row_count") == historical_resolution.get("row_count") == 48
        ),
        "run_contract_exact": (
            resolution.get("run_contract") == historical_resolution.get("run_contract")
        ),
        "checks_exact": (
        ...
        "summary_projection_exact": (
            ...
            == QREF_RUN_SUMMARY_EQUIVALENCE_SCHEMA_VERSION
            and summary_equivalence.get("comparison", {}).get("leaf_path_count") == 1002
    ...
    audit_equivalence = _seal_bound_payload(
        {
            ...
            "historical_reference": {
                ...
                "actions_sha256": canonical_sha256(list(historical_streams["actions"])),
                "utility_ledger_sha256": canonical_sha256(
                ...
                "summary_file_sha256": _file_sha256(historical_stream_paths["summary"]),
                "q_ref": historical_resolution["q_ref"],
                "ledger_hash": historical_bindings["ledger_hash"],
                "source_config_hash": historical_bindings["source_config_hash"],
                "source_result_reused": False,
            },
            "summary_equivalence": {
                "schema_version": summary_equivalence["schema_version"],
                "content_sha256": summary_equivalence["integrity"]["content_sha256"],
...
def _load_verified_v211_calibration(
    contract: PilotContract,
    *,
    raw_root: Path,
    authority_repo_root: str | Path | None = None,
) -> dict[str, Any]:
    """Consume only the contract-allowlisted V2.10.2 calibration fields."""

    if (
        contract.contract_id != V211_CONTRACT_ID
        or contract.v211_forward_boundary is None
    ):
        raise PilotOrchestrationError(
            "V2.11 calibration import requires the V2.11 forward boundary"
        )
    path = raw_root / "parent-import" / "parent_import_receipt.json"
    if not path.exists():
        raise PilotOrchestrationError(f"required artifact is missing: {path}")
    repository = Path(__file__).resolve().parents[1]
    try:
        receipt = verify_v211_parent_import_receipt(
            path,
            repo_root=repository,
            science_repo_root=authority_repo_root,
        )
    except PilotV211ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11 parent calibration import failed validation: {exc}"
        ) from exc
    ...
    parent = receipt.get("parent_release")
    policy = receipt.get("import_policy")
    if not all(
        isinstance(value, Mapping) for value in (imported, source, parent, policy)
    ):
        raise PilotOrchestrationError("V2.11 parent calibration receipt is malformed")

    boundary_source = boundary["source_manifest"]
    expected_profile = {
        ...
        * calibration["utility_profile"]["consumption_scale_multiplier_of_q_ref"],
        "discount_factor": calibration["utility_profile"]["discount_factor"],
        "budget_tolerance": 1e-8,
        "max_labor_hours": 168.0,
    }
    threshold = imported.get("stage0_absolute_flow_utility_threshold")
    source_bindings = imported.get("source_bindings")
    expected_debit = boundary["parent_budget_debit"]
    expected_policy = boundary["import_policy"]
    checks = {
        "source_manifest": source
        == {
            "path": boundary_source["path"],
            "file_sha256": boundary_source["file_sha256"],
            "content_sha256": boundary_source["content_sha256"],
        },
        "parent_release": (
            parent.get("contract_id") == boundary["parent"]["contract_id"]
            and parent.get("contract_sha256") == boundary["parent"]["contract_sha256"]
            and parent.get("resolved_git_commit")
            == boundary["parent"]["science_commit"]
            and parent.get("science_tag") == boundary["parent"]["science_tag"]
            and parent.get("run_ledger_sha256")
            == boundary["parent"]["run_ledger_internal_sha256"]
            and parent.get("budget_ledger_sha256")
            == boundary["parent"]["budget_ledger_internal_sha256"]
        ),
        "q_ref": imported.get("q_ref") == calibration["q_ref"],
        "selected_utility_profile": (
            imported.get("selected_utility_profile") == expected_profile
        ),
        "absolute_flow_utility_threshold": (
            isinstance(threshold, Mapping)
            and threshold.get("value") == calibration["absolute_flow_utility_threshold"]
            and threshold.get("source_profile") == calibration["utility_profile_id"]
            and threshold.get("treatment_outcomes_inspected") is False
        ),
        "source_bindings": (
            isinstance(source_bindings, Mapping)
            and source_bindings.get("q_ref_content_sha256")
            == boundary["parent"]["q_ref_content_sha256"]
            and source_bindings.get("stage0_selection_content_sha256")
            == boundary["parent"]["stage0_selection_content_sha256"]
        ),
        "cumulative_budget_debit": (
            receipt.get("cumulative_budget_debit") == expected_debit
        ),
        "zero_provider_effect_boundary": (
            policy.get("imported_effect_cells")
            == expected_policy["imported_effect_cells"]
            and policy.get("effect_metrics_observed")
            is expected_policy["effect_metrics_observed"]
            and policy.get("effect_artifact_paths")
            == expected_policy["effect_artifact_paths"]
            and policy.get("imported_p95_authorities")
            == expected_policy["imported_p95_authorities"]
            and policy.get("provider_construction")
            is expected_policy["provider_construction_during_import"]
            and policy.get("provider_calls")
            == expected_policy["provider_calls_during_import"]
            and policy.get("raw_tree_copied") is False
            and policy.get("copied_file_count") == 0
            and policy.get("copied_byte_count") == 0
        ),
    }
    failed = sorted(name for name, passed in checks.items() if not passed)
    if failed:
        raise PilotOrchestrationError(
            "V2.11 imported calibration differs from its frozen contract: " f"{failed}"
        )
    return {
        "receipt": receipt,
        "q_ref": float(calibration["q_ref"]),
        "selected_profile_id": str(calibration["utility_profile_id"]),
        "selected_utility": {
            key: value for key, value in expected_profile.items() if key != "profile_id"
        },
        "absolute_flow_utility_threshold": _json_copy(threshold),
    }


def _load_verified_v2111_calibration(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Replay-verify V2.11 and expose only the child-bound calibration wrapper."""

    if (
        contract.contract_id != V2111_CONTRACT_ID
        or contract.v2111_forward_boundary is None
    ):
        raise PilotOrchestrationError(
            "V2.11.1 calibration import requires its forward boundary"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        receipt = verify_v2111_parent_import_receipt(
            receipt_path,
            repo_root=Path(__file__).resolve().parents[1],
            child_contract_sha256=contract.canonical_hash,
            child_git_tag=paid.git_tag,
            child_git_commit=paid.head_commit,
            parent_science_root=authority_repo_root,
        )
        wrapper = calibration_wrapper_from_v2111_receipt(receipt)
    except PilotV2111ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11.1 parent calibration import failed validation: {exc}"
        ) from exc
    calibration = wrapper.get("calibration")
    if not isinstance(calibration, Mapping):
        raise PilotOrchestrationError("V2.11.1 calibration wrapper is malformed")
    boundary = contract.v2111_forward_boundary["calibration_allowlist"]
    selected = calibration.get("selected_utility_profile")
    threshold = calibration.get("stage0_absolute_flow_utility_threshold")
    if (
        calibration.get("q_ref") != boundary["q_ref"]
        or not isinstance(selected, Mapping)
        or selected.get("profile_id") != boundary["utility_profile_id"]
        or {
            "rho": selected.get("rho"),
            "labor_weight": selected.get("labor_weight"),
            "inverse_frisch": selected.get("inverse_frisch"),
            "consumption_scale_multiplier_of_q_ref": (
                float(selected.get("consumption_scale")) / float(calibration["q_ref"])
            ),
            "discount_factor": selected.get("discount_factor"),
        }
        != dict(boundary["utility_profile"])
        or not isinstance(threshold, Mapping)
        or threshold.get("value") != boundary["absolute_flow_utility_threshold"]
        ...
        raise PilotOrchestrationError(
            "V2.11.1 imported calibration differs from its frozen contract"
        )
    return {
        "receipt": receipt,
        "wrapper": wrapper,
...
def _load_verified_v2112_calibration(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    if (
        contract.contract_id != V2112_CONTRACT_ID
        or contract.v2112_forward_boundary is None
    ):
        raise PilotOrchestrationError(
            "V2.11.2 calibration import requires its forward boundary"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        ...
        wrapper = calibration_wrapper_from_v2112_receipt(receipt)
    except PilotV2112ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11.2 parent calibration import failed validation: {exc}"
        ) from exc
    calibration = wrapper.get("calibration")
    if not isinstance(calibration, Mapping):
        raise PilotOrchestrationError("V2.11.2 calibration wrapper is malformed")
    boundary = contract.v2112_forward_boundary["calibration_allowlist"]
    selected = calibration.get("selected_utility_profile")
    threshold = calibration.get("stage0_absolute_flow_utility_threshold")
    if (
        calibration.get("q_ref") != boundary["q_ref"]
        ...
        raise PilotOrchestrationError(
            "V2.11.2 imported calibration differs from its frozen contract"
        )
    return {
        "receipt": receipt,
        "wrapper": wrapper,
...
def _load_verified_v2113_calibration(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Expose only the current-release calibration wrapper after reseal validation."""

    if (
        contract.contract_id != V2113_CONTRACT_ID
        or contract.v2113_forward_boundary is None
    ):
        raise PilotOrchestrationError(
            "V2.11.3 calibration import requires its forward boundary"
        )
    release_root = (
        Path(__file__).resolve().parents[1]
        if release_repo_root is None
    ...
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        receipt = validate_v2113_parent_import_receipt(
            _read_json(receipt_path),
            contract=contract,
            child_git_commit=paid.head_commit,
            repo_root=release_root,
        )
        wrapper = calibration_wrapper_from_v2113_receipt(
            receipt,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=release_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.3 parent calibration import failed validation: {exc}"
        ) from exc
    calibration = wrapper.get("calibration")
    if not isinstance(calibration, Mapping):
        raise PilotOrchestrationError("V2.11.3 calibration wrapper is malformed")
    boundary = contract.v2113_forward_boundary["calibration_allowlist"]
    selected = calibration.get("selected_utility_profile")
    threshold = calibration.get("stage0_absolute_flow_utility_threshold")
    if (
        calibration.get("q_ref") != boundary["q_ref"]
        ...
        raise PilotOrchestrationError(
            "V2.11.3 imported calibration differs from its frozen contract"
        )
    return {
        "receipt": receipt,
        "wrapper": wrapper,
...
def _load_verified_v2114_calibration(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    if (
        contract.contract_id != V2114_CONTRACT_ID
        or contract.v2114_forward_boundary is None
    ):
        raise PilotOrchestrationError(
            "V2.11.4 calibration import requires its forward boundary"
        )
    release_root = (
        Path(__file__).resolve().parents[1]
        if release_repo_root is None
    ...
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        receipt = validate_v2114_parent_import_receipt(
            _read_json(receipt_path),
            contract=contract,
            child_git_commit=paid.head_commit,
            repo_root=release_root,
        )
        wrapper = calibration_wrapper_from_v2114_receipt(
            receipt,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=release_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.4 parent calibration import failed validation: {exc}"
        ) from exc
    calibration = wrapper.get("calibration")
    if not isinstance(calibration, Mapping):
        raise PilotOrchestrationError("V2.11.4 calibration wrapper is malformed")
    boundary = contract.v2114_forward_boundary["calibration_allowlist"]
    selected = calibration.get("selected_utility_profile")
    threshold = calibration.get("stage0_absolute_flow_utility_threshold")
    if (
        calibration.get("q_ref") != boundary["q_ref"]
        ...
        raise PilotOrchestrationError(
            "V2.11.4 imported calibration differs from its frozen contract"
        )
    return {
        "receipt": receipt,
        "wrapper": wrapper,
...
def _load_verified_q_ref(
    ...
    *,
    ...
    if contract.contract_id == V2114_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.4 imported q-ref requires paid release provenance"
            )
        calibration = _load_verified_v2114_calibration(
            contract,
            raw_root=raw_root,
            paid=paid,
        ...
        return {
            "schema_version": "finevo-pilot-v2.11.4-imported-qref-view-v1",
            "status": "pass",
    ...
    if contract.contract_id == V2113_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.3 imported q-ref requires paid release provenance"
            )
        calibration = _load_verified_v2113_calibration(
            contract,
            raw_root=raw_root,
            paid=paid,
            release_repo_root=authority_repo_root,
        ...
        return {
            "schema_version": "finevo-pilot-v2.11.3-imported-qref-view-v1",
            "status": "pass",
            "q_ref": calibration["q_ref"],
            "scientific_evidence": False,
            "source_parent_import_content_sha256": calibration["receipt"]["integrity"][
    ...
    if contract.contract_id == V2112_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.2 imported q-ref requires paid release provenance"
            )
        calibration = _load_verified_v2112_calibration(
            contract,
            raw_root=raw_root,
            paid=paid,
            authority_repo_root=authority_repo_root,
        ...
        return {
            "schema_version": "finevo-pilot-v2.11.2-imported-qref-view-v1",
            "status": "pass",
            "q_ref": calibration["q_ref"],
            "scientific_evidence": False,
            "source_parent_import_content_sha256": calibration["receipt"]["integrity"][
    ...
    if contract.contract_id == V2111_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.1 imported q-ref requires paid release provenance"
            )
        calibration = _load_verified_v2111_calibration(
            contract,
            raw_root=raw_root,
            paid=paid,
            authority_repo_root=authority_repo_root,
        ...
        return {
            "schema_version": "finevo-pilot-v2.11.1-imported-qref-view-v1",
            "status": "pass",
            "q_ref": calibration["q_ref"],
            "scientific_evidence": False,
            "source_parent_import_content_sha256": calibration["receipt"]["integrity"][
    ...
    if contract.contract_id == V211_CONTRACT_ID:
        calibration = _load_verified_v211_calibration(
            contract,
            raw_root=raw_root,
            authority_repo_root=authority_repo_root,
        )
        return {
            "schema_version": "finevo-pilot-v2.11-imported-qref-view-v1",
            "status": "pass",
            "q_ref": calibration["q_ref"],
            "scientific_evidence": False,
            "source_parent_import_content_sha256": calibration["receipt"]["integrity"][
    ...
    if contract.contract_id == V2102_CONTRACT_ID:
        _verify_bound_payload(
            ...
            schema_version=PILOT_V2102_IMPORTED_QREF_SCHEMA_VERSION,
            paid=paid,
            artifact_name="V2.10.2 imported q_ref resolution",
        )
        effective_paid = paid
        if effective_paid is None:
            bindings = value["bindings"]
            effective_paid = GitProvenance(
                git_tag=str(bindings["git_tag"]),
                head_commit=str(bindings["git_commit"]),
                tag_commit=str(bindings["git_commit"]),
                tag_object_type="tag",
                worktree_clean=True,
                contract_binding={},
            )
        expected = _expected_v2102_q_ref_resolution(
            contract,
            raw_root=raw_root,
            paid=effective_paid,
            authority_repo_root=authority_repo_root,
        ...
        if value != expected:
            raise PilotOrchestrationError(
                "V2.10.2 q_ref resolution differs from its exact V2.9 import"
            )
        return value
    if contract.contract_id == V2101_CONTRACT_ID:
        _verify_bound_payload(
            ...
            schema_version=PILOT_V2101_IMPORTED_QREF_SCHEMA_VERSION,
            paid=paid,
            artifact_name="V2.10.1 imported q_ref resolution",
        )
        effective_paid = paid
        if effective_paid is None:
            bindings = value["bindings"]
        ...
        expected = _expected_v2101_q_ref_resolution(
            contract,
            raw_root=raw_root,
            paid=effective_paid,
            authority_repo_root=authority_repo_root,
        ...
        if value != expected:
            raise PilotOrchestrationError(
                "V2.10.1 q_ref resolution differs from its exact V2.9 import"
            )
        return value
    if contract.contract_id == V210_CONTRACT_ID:
        _verify_bound_payload(
            ...
            schema_version=PILOT_V210_IMPORTED_QREF_SCHEMA_VERSION,
            paid=paid,
            artifact_name="V2.10 imported q_ref resolution",
        )
        effective_paid = paid
        if effective_paid is None:
            bindings = value["bindings"]
        ...
        expected = _expected_v210_q_ref_resolution(
            contract,
            raw_root=raw_root,
            paid=effective_paid,
            authority_repo_root=authority_repo_root,
        ...
        if value != expected:
            raise PilotOrchestrationError(
                "V2.10 q_ref resolution differs from its exact V2.9 import"
            )
        return value
    if contract.contract_id == V27_CONTRACT_ID:
        _verify_bound_payload(
            ...
            paid=paid,
            artifact_name="V2.7 q_ref resolution",
        )
        effective_paid = paid
        if effective_paid is None:
            bindings = value["bindings"]
        ...
        expected = _expected_v27_q_ref_resolution(
            contract,
            raw_root=raw_root,
            paid=effective_paid,
        )
        if value != expected:
            raise PilotOrchestrationError(
                "V2.7 q_ref resolution differs from its imported source replay"
            )
        return value
    ...
    if bindings.get("environment_source_hash") != _file_sha256(environment_source_path):
    ...
    if contract.contract_id == V29_CONTRACT_ID:
        ...
        if (
            value.get("q_ref_summary_equivalence") != expected_summary_equivalence
            or value.get("q_ref_audit_equivalence") != expected_audit_equivalence
        ):
            raise PilotOrchestrationError(
...
def verify_v29_qref_resolution(
    ...
    *,
    ...
    if contract.contract_id != V29_CONTRACT_ID:
        raise PilotOrchestrationError("V2.9 q_ref replay used by another contract")
    if (
        ...
        raise PilotOrchestrationError("V2.9 q_ref replay release binding is malformed")
...
def _v27_stage0_envelope_path(raw_root: Path, spec: PilotRunSpec) -> Path:
    return (
        raw_root
        / "stage0-calibration"
        / "runs"
...
def _v27_stage0_terminal_path(raw_root: Path, spec: PilotRunSpec) -> Path:
    return raw_root / "stage0-calibration" / "summaries" / f"{spec.run_id}.json"
...
def _imported_stage0_label(contract: PilotContract) -> str:
    labels = {
        ...
        V210_CONTRACT_ID: "V2.10",
        V2101_CONTRACT_ID: "V2.10.1",
        V2102_CONTRACT_ID: "V2.10.2",
    }
    try:
        return labels[contract.contract_id]
    except KeyError as exc:
        raise PilotOrchestrationError(
            "imported Stage-0 is available only to "
            "V2.7/V2.8/V2.9/V2.10/V2.10.1/V2.10.2"
        ) from exc
...
def _imported_stage0_envelope_schema(contract: PilotContract) -> str:
    schemas = {
        ...
        V210_CONTRACT_ID: PILOT_V210_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
        V2101_CONTRACT_ID: PILOT_V2101_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
        V2102_CONTRACT_ID: PILOT_V2102_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
    }
    _imported_stage0_label(contract)
    return schemas[contract.contract_id]


def _build_v210_stage0_envelope(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Reseal one exact V2.9 Stage-0 prerequisite as a V2.10 wrapper."""

    if (
        contract.contract_id != V210_CONTRACT_ID
        or spec.stage_id != "stage0-calibration"
        or spec.execution_mode != "actor_run"
    ):
        raise PilotOrchestrationError(
            "V2.10 Stage-0 wrapper requires one exact imported actor cell"
        )
    try:
        source = verified_v210_imported_prerequisite_binding(
            raw_root,
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10 Stage-0 source failed validation: {exc}"
        ) from exc
    ...
    if (
        ...
        or source.get("source_stage_id") != "stage0-calibration"
        or source.get("target_spec") != spec.to_dict()
        or source.get("selected_profile_id") != "nu-0.5"
        or source.get("provider_calls_during_verification") != 0
        or source.get("provider_construction_during_verification") is not False
        or source.get("treatment_effect_evidence") is not False
    ):
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10 Stage-0 source binding is malformed"
        )
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    parent_receipt = _read_json(parent_receipt_path)
    repository = (
    ...
    envelope = _seal_bound_payload(
        {
            "schema_version": PILOT_V210_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            ...
            "reader": {
                ...
                "function": "verified_v210_imported_prerequisite_binding",
                "phase_agnostic": True,
                "allowed_record_streams": ["imported_terminal_metrics"],
                "allowed_input_sha256": canonical_sha256(metrics),
                "source_row_counts": _json_copy(metrics.get("row_counts", {})),
                "summary": _json_copy(metrics),
            },
            "source_import": _json_copy(source),
            "provider_calls_current_attempt": 0,
            "hosted_provider_calls_current_attempt": 0,
            "local_model_calls_current_attempt": 0,
            "source_provider_outputs_reused_only_for_imported_stage0": True,
            ...
            "stage0_selection_observed_before_amendment": True,
            "scientific_evidence": True,
            "claim_boundary": (
                "V2.10 imports and reseals this completed V2.9 Stage-0 "
                "prerequisite without provider dispatch. It is calibration "
                "evidence only and is not V2.10 A-D treatment-effect evidence."
            ),
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
    ...
    return envelope, source, dict(source), Path(run_root)


def _build_v2101_stage0_envelope(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Reseal one exact V2.9 Stage-0 prerequisite for V2.10.1."""

    if (
        contract.contract_id != V2101_CONTRACT_ID
        or spec.stage_id != "stage0-calibration"
        or spec.execution_mode != "actor_run"
    ):
        raise PilotOrchestrationError(
            "V2.10.1 Stage-0 wrapper requires one exact imported actor cell"
        )
    try:
        source = verified_v2101_imported_prerequisite_binding(
            raw_root,
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10.1 Stage-0 source failed validation: {exc}"
        ) from exc
    metrics = source.get("metrics")
    source_artifacts = source.get("source_artifacts")
    source_release = source.get("source_release")
    ...
    if (
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10.1 Stage-0 source binding is malformed"
        )
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    parent_receipt = _read_json(parent_receipt_path)
    repository = (
    ...
    envelope = _seal_bound_payload(
        {
            "schema_version": PILOT_V2101_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            "run_spec": spec.to_dict(),
            "execution_disposition": (
            ...
            "reader": {
                "schema_version": "finevo-pilot-stage0-analysis-v1",
                "function": "verified_v2101_imported_prerequisite_binding",
                "phase_agnostic": True,
                "allowed_record_streams": ["imported_terminal_metrics"],
                "allowed_input_sha256": canonical_sha256(metrics),
                "source_row_counts": _json_copy(metrics.get("row_counts", {})),
            ...
            "scientific_evidence": True,
            "claim_boundary": (
                "V2.10.1 imports and reseals this completed V2.9 Stage-0 "
                "prerequisite without provider dispatch. It is calibration "
                "evidence only and is not V2.10.1 A-D treatment-effect evidence."
            ),
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
...
def _build_v2102_stage0_envelope(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Reseal one exact V2.9 Stage-0 prerequisite for V2.10.2."""

    if (
        contract.contract_id != V2102_CONTRACT_ID
        or spec.stage_id != "stage0-calibration"
        or spec.execution_mode != "actor_run"
    ):
        raise PilotOrchestrationError(
            "V2.10.2 Stage-0 wrapper requires one exact imported actor cell"
        )
    try:
        source = verified_v2102_imported_prerequisite_binding(
            raw_root,
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10.2 Stage-0 source failed validation: {exc}"
        ) from exc
    metrics = source.get("metrics")
    source_artifacts = source.get("source_artifacts")
    source_release = source.get("source_release")
    ...
    if (
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.10.2 Stage-0 source binding is malformed"
        )
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    parent_receipt = _read_json(parent_receipt_path)
    repository = (
    ...
    envelope = _seal_bound_payload(
        {
            "schema_version": PILOT_V2102_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION,
            "contract_id": contract.contract_id,
            "contract_sha256": contract.canonical_hash,
            "run_spec": spec.to_dict(),
            "execution_disposition": (
            ...
            "reader": {
                "schema_version": "finevo-pilot-stage0-analysis-v1",
                "function": "verified_v2102_imported_prerequisite_binding",
                "phase_agnostic": True,
                "allowed_record_streams": ["imported_terminal_metrics"],
                "allowed_input_sha256": canonical_sha256(metrics),
                "source_row_counts": _json_copy(metrics.get("row_counts", {})),
            ...
            "scientific_evidence": True,
            "claim_boundary": (
                "V2.10.2 imports and reseals this completed V2.9 Stage-0 "
                "prerequisite without provider dispatch. It is calibration "
                "evidence only and is not V2.10.2 A-D treatment-effect evidence."
            ),
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
...
def _build_v27_stage0_envelope(
    ...
    *,
    ...
    if contract.contract_id == V2102_CONTRACT_ID:
        return _build_v2102_stage0_envelope(
            contract,
            spec,
            raw_root=raw_root,
            paid=paid,
    ...
    if contract.contract_id == V2101_CONTRACT_ID:
        return _build_v2101_stage0_envelope(
            contract,
            spec,
            raw_root=raw_root,
            paid=paid,
    ...
    if contract.contract_id == V210_CONTRACT_ID:
        return _build_v210_stage0_envelope(
            contract,
            spec,
            raw_root=raw_root,
            paid=paid,
    ...
    if contract.contract_id == V29_CONTRACT_ID:
        result, source_binding, source_run_dir = _load_v29_imported_stage0_result(
            contract,
            spec,
            raw_root=raw_root,
            paid=paid,
        ...
        result, source_binding, source_run_dir = _load_v28_imported_stage0_result(
            contract,
            spec,
            raw_root=raw_root,
            paid=paid,
    ...
    if (
        result.config.get("seed") != spec.environment_seed
        or len(reader_records["actions"]) != 48
        or len(reader_records["utility_ledger"]) != 48
        or reader_records["errors"]
    ):
        raise PilotOrchestrationError(
    ...
    envelope = _seal_bound_payload(
        {
            ...
            "contract_sha256": contract.canonical_hash,
            "run_spec": spec.to_dict(),
            "execution_disposition": ("immutable-parent-import-offline-resummary"),
            "reader": {
                "schema_version": "finevo-pilot-stage0-analysis-v1",
                "function": "summarize_stage0_run",
                "phase_agnostic": True,
                "allowed_record_streams": list(allowed_streams),
                "allowed_input_sha256": canonical_sha256(reader_records),
                "source_row_counts": {
                    name: len(rows) for name, rows in reader_records.items()
                },
                "summary": summary,
            },
            "source_import": source_payload,
            "provider_calls_current_attempt": 0,
            "hosted_provider_calls_current_attempt": 0,
            "local_model_calls_current_attempt": 0,
            "source_provider_outputs_reused_only_for_imported_stage0": True,
            ...
            "scientific_evidence": True,
            "claim_boundary": (
                "Current-method Stage-0 calibration reread only; no provider "
                "redispatch and no A-D treatment-effect evidence."
            ),
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
                ...
                "parent_import_receipt_content_sha256": source_payload[
                    "parent_import_receipt"
                ]["content_sha256"],
                "source_manifest_sha256": source_payload["source_artifacts"][
                    "manifest"
                ]["file_sha256"],
            },
        }
    )
    return envelope, result, source_binding, source_run_dir


def _v27_stage0_terminal_payload(
    envelope: Mapping[str, Any],
    envelope_path: Path,
) -> dict[str, Any]:
    return {
        "metrics": envelope["reader"]["summary"],
        "gate_evidence": {
            "status": "pass",
...
def _materialize_v27_stage0_cell(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    paid: GitProvenance,
    source_manifest: Mapping[str, Any],
) -> tuple[Path, Path, VerifiedRunResult, dict[str, Any], dict[str, Any]]:
    envelope, result, source_binding, _ = _build_v27_stage0_envelope(
        contract,
        spec,
        raw_root=raw_root,
        paid=paid,
    ...
    envelope_path = _v27_stage0_envelope_path(raw_root, spec)
    _persist_exact_json(envelope_path, envelope)
    terminal_path = _v27_stage0_terminal_path(raw_root, spec)
    payload = _v27_stage0_terminal_payload(envelope, envelope_path)
    if terminal_path.exists():
        ...
        if (
            ...
            or terminal.get("scientific_evidence") is not True
            or terminal.get("diagnostic_only") is not False
            or terminal.get("evidence_scope") != CURRENT_SCIENTIFIC_SCOPE
        ):
            raise PilotOrchestrationError(
                f"{spec.run_id} imported Stage-0 terminal differs on resume"
            )
    else:
        write_terminal_summary(
            terminal_path,
    ...
    return envelope_path, terminal_path, result, source_binding, envelope


def verify_v27_imported_stage0_terminal(
    contract: PilotContract,
    ...
    *,
    ...
    if contract.contract_id not in {
        ...
        raise PilotOrchestrationError(
            "imported Stage-0 terminal verification requires "
            "V2.7/V2.8/V2.9/V2.10/V2.10.1/V2.10.2"
        )
    contract_label = _imported_stage0_label(contract)
    ...
    matches = tuple(
        candidate
        for candidate in contract.expand(stage="stage0-calibration")
        if candidate.run_id == spec_value.get("run_id")
    )
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "imported Stage-0 terminal release binding is malformed"
        )
    paid = GitProvenance(
        git_tag=git_tag,
        head_commit=resolved_git_commit,
    ...
    root = Path(raw_root).resolve()
    if contract.contract_id == V2102_CONTRACT_ID:
        source_manifest, _ = _v2102_import_authority(
            contract,
            raw_root=root,
            paid=paid,
            authority_repo_root=authority_repo_root,
        )
    elif contract.contract_id == V2101_CONTRACT_ID:
        source_manifest, _ = _v2101_import_authority(
            contract,
            raw_root=root,
            paid=paid,
            authority_repo_root=authority_repo_root,
        )
    elif contract.contract_id == V210_CONTRACT_ID:
        source_manifest, _ = _v210_import_authority(
            contract,
            raw_root=root,
            paid=paid,
            authority_repo_root=authority_repo_root,
        )
    elif contract.contract_id == V29_CONTRACT_ID:
    ...
    if envelope != expected_envelope:
        raise PilotOrchestrationError(
            f"{selected.run_id} imported Stage-0 envelope differs from replay"
        )
    terminal = _load_v2_terminal_summary(
        contract,
        selected,
    ...
    expected_payload = _v27_stage0_terminal_payload(envelope, envelope_path)
    if (
        terminal.get("payload") != expected_payload
        or terminal.get("scientific_evidence") is not True
        or terminal.get("diagnostic_only") is not False
        or terminal.get("evidence_scope") != CURRENT_SCIENTIFIC_SCOPE
    ):
        raise PilotOrchestrationError(
            f"{selected.run_id} imported Stage-0 terminal differs from replay"
        )
    return {
        "metrics": _json_copy(envelope["reader"]["summary"]),
        "envelope_binding": _json_copy(
            expected_payload["gate_evidence"]["imported_run_envelope"]
        ),
        "execution_disposition": envelope["execution_disposition"],
        "provider_calls_current_attempt": 0,
        "scientific_evidence": True,
...
def _expected_v210_family_stage0_selection(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Recompute a current-release selection from fourteen exact V2.9 cells."""

    if contract.contract_id == V2102_CONTRACT_ID:
        label = "V2.10.2"
        envelope_schema = PILOT_V2102_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION
        source_manifest, parent_receipt = _v2102_import_authority(
            contract,
            raw_root=raw_root,
            paid=paid,
        ...
        label = "V2.10.1"
        envelope_schema = PILOT_V2101_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION
        source_manifest, parent_receipt = _v2101_import_authority(
            contract,
            raw_root=raw_root,
            paid=paid,
        ...
        label = "V2.10"
        envelope_schema = PILOT_V210_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION
        source_manifest, parent_receipt = _v210_import_authority(
            contract,
            raw_root=raw_root,
            paid=paid,
        ...
        raise PilotOrchestrationError(
            "current-release Stage-0 selection used by another contract"
        )
    ...
    if len(specs) != 14 or any(
        spec.execution_mode != "actor_run"
        or spec.model_id != "llama33_local_controlled"
        for spec in specs
    ):
        raise PilotOrchestrationError(
            f"{label} Stage-0 import requires the exact 7x2 local actor matrix"
        )
    source_envelopes: list[dict[str, Any]] = []
    cells: list[tuple[PilotRunSpec, Path, Path, dict[str, Any]]] = []
    source_selection: dict[str, Any] | None = None
    source_selection_binding: dict[str, Any] | None = None
    for spec in specs:
        if materialize:
            (
                envelope_path,
            ...
            envelope_path = _v27_stage0_envelope_path(raw_root, spec)
            envelope = _read_json(envelope_path)
            expected, _, source_binding, _ = _build_v27_stage0_envelope(
                contract,
                spec,
                raw_root=raw_root,
                paid=paid,
            ...
            _verify_bound_payload(
                ...
                schema_version=envelope_schema,
                paid=paid,
                artifact_name=(f"{label} {spec.run_id} imported Stage-0 envelope"),
            )
            if envelope != expected:
                raise PilotOrchestrationError(
                    f"{spec.run_id} {label} Stage-0 envelope differs from import"
                )
            terminal_path = _v27_stage0_terminal_path(raw_root, spec)
            terminal = _load_v2_terminal_summary(
                contract,
            ...
            if (
                ...
                != _v27_stage0_terminal_payload(envelope, envelope_path)
                or terminal.get("scientific_evidence") is not True
                or terminal.get("diagnostic_only") is not False
                or terminal.get("evidence_scope") != CURRENT_SCIENTIFIC_SCOPE
            ):
                raise PilotOrchestrationError(
                    f"{spec.run_id} {label} Stage-0 terminal binding drifted"
                )
        summary = envelope["reader"]["summary"]
        by_profile[spec.utility_profile_id].append(summary)
        artifacts = source_binding.get("source_artifacts")
        selection_artifact = (
            artifacts.get("stage0_selection")
            if isinstance(artifacts, Mapping)
            else None
        )
        if not isinstance(selection_artifact, Mapping) or not isinstance(
            selection_artifact.get("snapshot_path"), str
        ):
            raise PilotOrchestrationError(
                f"{spec.run_id} lacks its verified source Stage-0 selection"
            )
        selected_source = _read_json(Path(selection_artifact["snapshot_path"]))
        binding_projection = {
            key: selection_artifact.get(key)
            for key in (
                "file_sha256",
                "content_sha256",
                "snapshot_path",
        ...
        if source_selection is None:
            ...
            source_selection_binding = binding_projection
        elif (
            selected_source != source_selection
            or binding_projection != source_selection_binding
        ):
            raise PilotOrchestrationError(
                f"{label} Stage-0 cells do not share one immutable source selection"
            )
        terminal_value = _read_json(terminal_path)
        source_envelopes.append(
            {
                ...
                "terminal_summary_content_sha256": terminal_value["integrity"][
                    "content_sha256"
                ],
                "source_run_id": source_binding["source_run_id"],
                "source_terminal_file_sha256": source_binding["source_terminal"][
                    "file_sha256"
                ],
                "provider_calls_current_attempt": 0,
            }
        )
        cells.append((spec, envelope_path, terminal_path, dict(summary)))
    if source_selection is None or source_selection_binding is None:
        raise PilotOrchestrationError(
            f"{label} Stage-0 source selection is unavailable"
        )
    ...
    mismatched = [
        ...
        if selection.get(field) != source_selection.get(field)
    ]
    threshold = source_selection.get("absolute_flow_utility_threshold")
    if (
        mismatched
        or selection.get("selected_profile_id") != "nu-0.5"
        or not isinstance(threshold, Mapping)
        or threshold.get("selected_profile_id") != "nu-0.5"
        or threshold.get("row_count") != 96
        or threshold.get("treatment_outcomes_inspected") is not False
        or not isinstance(threshold.get("value"), (int, float))
    ):
        raise PilotOrchestrationError(
            f"{label} Stage-0 recomputation differs from the frozen V2.9 "
            f"selection: {mismatched}"
        )
    selection["absolute_flow_utility_threshold"] = _json_copy(threshold)
    selection.update(
        {
            "contract_sha256": contract.canonical_hash,
            "q_ref_artifact_hash": canonical_sha256(q_ref),
            "evaluator_amendment": {
                "amendment_id": contract.stage0_evaluator_retry_amendment[
                    "amendment_id"
                ],
            ...
            "bindings": {
                ...
                "source_stage0_selection": source_selection_binding,
    ...
    return _seal_bound_payload(selection), tuple(cells)


def _expected_v210_stage0_selection(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    if contract.contract_id != V210_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10 Stage-0 selection used by another contract"
        )
    return _expected_v210_family_stage0_selection(
        contract,
        raw_root=raw_root,
        paid=paid,
...
def _expected_v2101_stage0_selection(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    if contract.contract_id != V2101_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.1 Stage-0 selection used by another contract"
        )
    return _expected_v210_family_stage0_selection(
        contract,
        raw_root=raw_root,
...
def _expected_v2102_stage0_selection(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    if contract.contract_id != V2102_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.2 Stage-0 selection used by another contract"
        )
    return _expected_v210_family_stage0_selection(
        contract,
        raw_root=raw_root,
...
def _expected_v27_stage0_selection(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    contract_label = _imported_stage0_label(contract)
    if contract.contract_id == V2102_CONTRACT_ID:
        return _expected_v2102_stage0_selection(
            contract,
            raw_root=raw_root,
            paid=paid,
            materialize=materialize,
    ...
    if contract.contract_id == V2101_CONTRACT_ID:
        return _expected_v2101_stage0_selection(
            contract,
            raw_root=raw_root,
            paid=paid,
            materialize=materialize,
    ...
    if contract.contract_id == V210_CONTRACT_ID:
        return _expected_v210_stage0_selection(
            contract,
            raw_root=raw_root,
            paid=paid,
            materialize=materialize,
    ...
    calibration_sources: list[
        tuple[PilotRunSpec, VerifiedRunResult, Mapping[str, Any]]
    ] = []
    source_envelopes: list[dict[str, Any]] = []
    cells: list[tuple[PilotRunSpec, Path, Path, dict[str, Any]]] = []
    specs = tuple(contract.expand(stage="stage0-calibration"))
    ...
    for spec in specs:
        if materialize:
            ...
            envelope_path = _v27_stage0_envelope_path(raw_root, spec)
            envelope = _read_json(envelope_path)
            expected, result, source_binding, _ = _build_v27_stage0_envelope(
                contract,
                spec,
                raw_root=raw_root,
                paid=paid,
            ...
            if envelope != expected:
                raise PilotOrchestrationError(
                    f"{spec.run_id} imported Stage-0 envelope differs from source replay"
                )
            terminal_path = _v27_stage0_terminal_path(raw_root, spec)
            terminal = _load_v2_terminal_summary(
                contract,
            ...
            if terminal.get("payload") != _v27_stage0_terminal_payload(
                envelope, envelope_path
            ):
                raise PilotOrchestrationError(
                    f"{spec.run_id} imported Stage-0 terminal binding drifted"
                )
        summary = envelope["reader"]["summary"]
        by_profile[spec.utility_profile_id].append(summary)
        source_manifest_binding = source_binding["source_artifacts"]["manifest"]
        calibration_sources.append(
            (
                ...
                result,
                {
                    ...
                    "manifest_sha256": source_manifest_binding["file_sha256"],
                },
            )
        )
        terminal_value = _read_json(terminal_path)
        source_envelopes.append(
            {
                ...
                "source_run_id": source_binding["source_run_id"],
                "source_manifest_sha256": source_manifest_binding["file_sha256"],
                "provider_calls_current_attempt": 0,
            }
        )
        cells.append((spec, envelope_path, terminal_path, dict(summary)))
...
def _load_verified_stage0_selection(
    ...
    *,
    ...
    paid: GitProvenance | None,
    authority_repo_root: str | Path | None = None,
    allow_diagnostic_fixture: bool = False,
) -> dict[str, Any]:
    path = raw_root / "stage0-calibration" / "stage0_selection.json"
    if allow_diagnostic_fixture:
        if paid is not None:
            raise PilotOrchestrationError(
                "diagnostic Stage-0 selection cannot be used for a paid run"
            )
        value = _read_json(path)
        if isinstance(value.get("bindings"), Mapping):
            raise PilotOrchestrationError(
                "diagnostic Stage-0 selection unexpectedly has release bindings"
            )
    ...
    if contract.contract_id == V2114_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.4 imported Stage-0 selection requires paid provenance"
            )
        calibration = _load_verified_v2114_calibration(
            contract,
            raw_root=raw_root,
        ...
        return {
            "schema_version": (
                "finevo-pilot-v2.11.4-imported-stage0-selection-view-v1"
            ),
            "contract_sha256": contract.canonical_hash,
    ...
    if contract.contract_id == V2113_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.3 imported Stage-0 selection requires paid provenance"
            )
        calibration = _load_verified_v2113_calibration(
            contract,
            raw_root=raw_root,
        ...
        return {
            "schema_version": (
                "finevo-pilot-v2.11.3-imported-stage0-selection-view-v1"
            ),
            "contract_sha256": contract.canonical_hash,
            "selected_profile_id": calibration["selected_profile_id"],
            "selected_utility": _json_copy(calibration["selected_utility"]),
    ...
    if contract.contract_id == V2112_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.2 imported Stage-0 selection requires paid provenance"
            )
        calibration = _load_verified_v2112_calibration(
            contract,
            raw_root=raw_root,
        ...
        return {
            "schema_version": (
                "finevo-pilot-v2.11.2-imported-stage0-selection-view-v1"
            ),
            "contract_sha256": contract.canonical_hash,
            "selected_profile_id": calibration["selected_profile_id"],
            "selected_utility": _json_copy(calibration["selected_utility"]),
    ...
    if contract.contract_id == V2111_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.1 imported Stage-0 selection requires paid provenance"
            )
        calibration = _load_verified_v2111_calibration(
            contract,
            raw_root=raw_root,
        ...
        return {
            "schema_version": (
                "finevo-pilot-v2.11.1-imported-stage0-selection-view-v1"
            ),
            "contract_sha256": contract.canonical_hash,
            "selected_profile_id": calibration["selected_profile_id"],
            "selected_utility": _json_copy(calibration["selected_utility"]),
    ...
    if contract.contract_id == V211_CONTRACT_ID:
        ...
        return {
            "schema_version": ("finevo-pilot-v2.11-imported-stage0-selection-view-v1"),
            "contract_sha256": contract.canonical_hash,
            "selected_profile_id": calibration["selected_profile_id"],
            "selected_utility": _json_copy(calibration["selected_utility"]),
            "absolute_flow_utility_threshold": _json_copy(
    ...
    if contract.contract_id in {
        ...
        expected, _ = _expected_v27_stage0_selection(
            contract,
            raw_root=raw_root,
            paid=effective_paid,
            materialize=False,
...
def verify_v27_stage0_selection(
    contract: PilotContract,
    ...
    *,
    ...
    if contract.contract_id not in {
        ...
        raise PilotOrchestrationError(
            "imported Stage-0 selection replay requires "
            "V2.7/V2.8/V2.9/V2.10/V2.10.1/V2.10.2"
        )
    if (
        git_tag != contract.implementation["required_git_tag"]
    ...
    root = Path(raw_root).resolve(strict=True)
    expected_path = root / "stage0-calibration" / "stage0_selection.json"
    ...
    return _load_verified_stage0_selection(
        contract,
        raw_root=root,
        paid=paid,
        authority_repo_root=authority_repo_root,
    ...
    *,
    ...
    config_kwargs = dict(
        ...
        utility=resolve_utility(
            ...
            authority_repo_root=authority_repo_root,
            allow_diagnostic_fixture=diagnostic_override,
        ),
        ...
        prompt_tier_ceiling_tokens=(
            200_000 if contract.contract_id in V211_FAMILY_CONTRACT_IDS else None
        ),
        scientific_scope=(
    ...
    with observed_p95_authority_repo_context(authority_repo_root):
        return VerifiedRunConfig(**config_kwargs)
...
def _repository_relative_path(
    ...
    *,
    ...
    if (
        ...
        raise PilotOrchestrationError(f"{name} must stay below {required_top}/")
    ...
    *,
    ...
    if contract.contract_id in V211_FAMILY_CONTRACT_IDS:
        if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.11-family post-gate dispatch authority"
            )
        return (
            raw_root
            / "long-context-preflight"
            / PILOT_V211_POST_GATE_AUTHORITY_FILENAME
        )
    if contract.contract_id == V2102_CONTRACT_ID:
        if model_id not in V2102_ALLOWED_P95_PROFILES:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.10.2 resealed dispatch authority"
            )
        return v2102_observed_p95_receipt_path(raw_root, model_id)
    if contract.contract_id == V2101_CONTRACT_ID:
        if model_id not in V2101_ALLOWED_P95_PROFILES:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.10.1 resealed dispatch authority"
            )
        return v2101_observed_p95_receipt_path(raw_root, model_id)
    if contract.contract_id == V210_CONTRACT_ID:
        if model_id not in V210_ALLOWED_P95_PROFILES:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.10 resealed dispatch authority"
            )
        return v210_observed_p95_receipt_path(raw_root, model_id)
    if contract.contract_id == V29_CONTRACT_ID:
    ...
    if contract.contract_id == V27_CONTRACT_ID:
        if model_id not in V27_ALLOWED_P95_PROFILES:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.7 resealed dispatch authority"
            )
        return v27_observed_p95_receipt_path(raw_root, model_id)
...
def _verified_observed_p95_binding(
    ...
    *,
    ...
    if contract.contract_id in {V2113_CONTRACT_ID, V2114_CONTRACT_ID}:
        version = "V2.11.4" if contract.contract_id == V2114_CONTRACT_ID else "V2.11.3"
        gate_verifier = (
            verified_v2114_gate_authority_binding
            if contract.contract_id == V2114_CONTRACT_ID
            else verified_v2113_gate_authority_binding
        )
        if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
            raise PilotOrchestrationError(
                f"{model_id} has no {version} post-gate dispatch authority"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        relative = _repository_relative_path(
            ...
            name=f"{version} post-gate authority receipt",
            repo_root=release_root,
        )
        try:
            binding = gate_verifier(
                relative,
                repo_root=release_root,
                expected_git_commit=paid.head_commit,
                expected_contract_sha256=contract.canonical_hash,
            )
        except (PilotV2113GateError, PilotV2114GateError) as exc:
            raise PilotOrchestrationError(
                f"{version} post-gate authority failed validation: {exc}"
            ) from exc
        ...
        expected_runtime_models = {
            _runtime_model_for_profile(contract.provider_profiles[profile_id])
        ...
        if (
            ...
            or binding.get("receipt_path") != relative
            or binding.get("git_commit") != paid.head_commit
            or not isinstance(reservations, Mapping)
            or set(reservations) != expected_runtime_models
            or runtime_model not in reservations
        ):
            raise PilotOrchestrationError(
                f"{version} {model_id} flat p95 authority identity drifted"
            )
        return _json_copy(binding)
    if contract.contract_id == V2112_CONTRACT_ID:
        if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.11.2 post-gate dispatch authority"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name="V2.11.2 post-gate authority receipt",
            repo_root=release_root,
        )
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 post-gate authority failed validation: {exc}"
            ) from exc
        runtime_model = _runtime_model_for_profile(contract.provider_profiles[model_id])
        reservations = binding.get("reservations")
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} flat p95 authority identity drifted"
            )
        return {
            **{key: value for key, value in binding.items() if key != "reservations"},
            "reservations": {runtime_model: _json_copy(reservations[runtime_model])},
        }
    if contract.contract_id == V2111_CONTRACT_ID:
        if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.11.1 post-gate dispatch authority"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name="V2.11.1 post-gate authority receipt",
            repo_root=release_root,
        )
        try:
        ...
        except PilotV2111GateError as exc:
            raise PilotOrchestrationError(
                f"V2.11.1 post-gate authority failed validation: {exc}"
            ) from exc
        runtime_model = _runtime_model_for_profile(contract.provider_profiles[model_id])
        reservations = binding.get("reservations")
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} flat p95 authority identity drifted"
            )
        return {
            **{key: value for key, value in binding.items() if key != "reservations"},
            "reservations": {runtime_model: _json_copy(reservations[runtime_model])},
    ...
    if contract.contract_id == V211_CONTRACT_ID:
        if model_id not in {"gpt52_main", "gpt56_diagnostic"}:
            raise PilotOrchestrationError(
                f"{model_id} has no V2.11 post-gate dispatch authority"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name="V2.11 post-gate authority receipt",
            repo_root=release_root,
        )
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.11 post-gate authority failed validation: {exc}"
            ) from exc
        runtime_model = _runtime_model_for_profile(contract.provider_profiles[model_id])
        reservations = binding.get("reservations")
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11 {model_id} flat p95 authority identity drifted"
            )
        # The global receipt may authorize both models.  Each runner config
        # receives only its own runtime-model row while retaining the same
        # immutable receipt/file/content binding.
        return {
            **{key: value for key, value in binding.items() if key != "reservations"},
            "reservations": {runtime_model: _json_copy(reservations[runtime_model])},
        }
    if contract.contract_id == V2102_CONTRACT_ID:
        ...
        receipt_path = v2102_observed_p95_receipt_path(raw_root, model_id)
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name=f"{model_id} V2.10.2 p95 receipt",
            repo_root=release_root,
        )
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.10.2 {model_id} p95 authority failed validation: {exc}"
            ) from exc
        ...
        if (
            ...
            or set(binding["reservations"]) != {runtime_model}
        ):
            raise PilotOrchestrationError(
                f"V2.10.2 {model_id} flat p95 authority identity drifted"
            )
        return binding
    if contract.contract_id == V2101_CONTRACT_ID:
        if model_id not in V2101_ALLOWED_P95_PROFILES:
        ...
        receipt_path = v2101_observed_p95_receipt_path(raw_root, model_id)
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name=f"{model_id} V2.10.1 p95 receipt",
            repo_root=release_root,
        )
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.10.1 {model_id} p95 authority failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.10.1 {model_id} flat p95 authority identity drifted"
            )
        return binding
    if contract.contract_id == V210_CONTRACT_ID:
        if model_id not in V210_ALLOWED_P95_PROFILES:
        ...
        receipt_path = v210_observed_p95_receipt_path(raw_root, model_id)
        relative = _repository_relative_path(
            receipt_path,
            required_top="experiment_results",
            name=f"{model_id} V2.10 p95 receipt",
            repo_root=release_root,
        )
        try:
            binding = verified_v210_observed_p95_authority_binding(
                relative,
                repo_root=release_root,
                raw_root=raw_root,
                expected_git_commit=paid.head_commit,
            ...
            raise PilotOrchestrationError(
                f"V2.10 {model_id} p95 authority failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.10 {model_id} flat p95 authority identity drifted"
            )
        return binding
    if contract.contract_id == V29_CONTRACT_ID:
        ...
        if (
            ...
            or binding.get("runtime_model") != _runtime_model_for_profile(profile)
        ):
            raise PilotOrchestrationError(
...
def _persist_observed_p95_authority_receipt(
    ...
    *,
    ...
    if contract.contract_id in INHERITED_P95_PARENT_CONTRACT_IDS:
        raise PilotOrchestrationError(
...
def _verify_v27_importer_p95_profiles(
    contract: PilotContract,
    *,
    ...
    if not isinstance(profiles, Mapping) or set(profiles) != set(allowed_profiles):
        raise PilotOrchestrationError(
    ...
    for model_id in sorted(allowed_profiles):
        ...
        if not isinstance(imported, Mapping) or set(imported) != {
            "receipt",
            "projection",
            "runtime_models",
            "source_kind",
        ...
        if (
            ...
            or set(receipt) != {"path", "file_sha256", "content_sha256"}
        ...
        if (
            imported.get("source_kind") != expected_source_kind
            or imported.get("runtime_models") != sorted(receipt_binding["reservations"])
            or receipt.get("path") != expected_receipt_relative
            or receipt.get("file_sha256") != receipt_binding["receipt_file_sha256"]
            or receipt.get("content_sha256")
            != receipt_binding["receipt_content_sha256"]
            or projection.get("path") != expected_projection_relative
            or projection.get("file_sha256") != _file_sha256(projection_path)
...
def _verify_v210_importer_p95_profiles(
    contract: PilotContract,
    *,
    importer_result: Mapping[str, Any],
    raw_root: Path,
    ...
    """Reverify both current-release flat P95 authorities after V2.9 import."""

    if contract.contract_id != V210_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10 importer p95 verification used by another contract"
        )
    _, receipt = _v210_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
    )
    if (
        importer_result.get("imported_cell_count") != 16
        or importer_result.get("imported_q_ref_cell_count") != 1
        or importer_result.get("imported_stage0_cell_count") != 14
        or importer_result.get("offline_candidate_admission_cells_imported") != 0
        or importer_result.get("provider_construction_during_import") is not False
        or importer_result.get("provider_calls_during_import") != 0
        or importer_result.get("hosted_provider_calls_during_import") != 0
        or importer_result.get("scientific_evidence") is not False
        or importer_result.get("v2_9_terminal_no_go_preserved") is not True
    ):
        raise PilotOrchestrationError(
            "V2.10 importer result violates its exact zero-call boundary"
        )
    declared_profiles = receipt.get("p95_current_release_authorities")
    if not isinstance(declared_profiles, Mapping) or set(declared_profiles) != set(
        V210_ALLOWED_P95_PROFILES
    ):
        raise PilotOrchestrationError(
            "V2.10 parent receipt lacks both exact current-release P95 profiles"
        )
    repository = Path(__file__).resolve().parents[1]
    verified: dict[str, Any] = {}
    for model_id in sorted(V210_ALLOWED_P95_PROFILES):
        receipt_path = v210_observed_p95_receipt_path(raw_root, model_id)
        projection_path = v210_observed_p95_projection_path(raw_root, model_id)
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.10 resealed {model_id} p95 failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        if (
            ...
            or authority.get("git_commit") != paid.head_commit
            or set(authority.get("reservations", {})) != {runtime_model}
            or projection.get("profile_id") != model_id
            or projection.get("served_model") != profile.served_model
            or projection.get("runtime_model") != runtime_model
            or projection.get("git_commit") != paid.head_commit
            or projection.get("reservations") != authority["reservations"]
            or declared_profiles.get(model_id)
            != {"authority": authority, "projection": projection}
        ):
            raise PilotOrchestrationError(
                f"V2.10 resealed {model_id} p95 identity/binding drifted"
            )
        verified[model_id] = {
...
def _verify_v2101_importer_p95_profiles(
    contract: PilotContract,
    *,
    importer_result: Mapping[str, Any],
    raw_root: Path,
    ...
    """Reverify V2.10.1 authorities sourced from exact nested V2.9 bytes."""

    if contract.contract_id != V2101_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.1 importer p95 verification used by another contract"
        )
    _, receipt = _v2101_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
    )
    if (
        ...
        or importer_result.get("hosted_provider_calls_during_import") != 0
        or importer_result.get("scientific_evidence") is not False
        or importer_result.get("v2_10_terminal_no_go_preserved") is not True
        or importer_result.get("v2_9_source_bytes_preserved") is not True
    ):
        raise PilotOrchestrationError(
            "V2.10.1 importer result violates its exact zero-call boundary"
        )
    declared_profiles = receipt.get("p95_current_release_authorities")
    if not isinstance(declared_profiles, Mapping) or set(declared_profiles) != set(
        V2101_ALLOWED_P95_PROFILES
        ...
        raise PilotOrchestrationError(
            "V2.10.1 parent receipt lacks both exact current-release P95 profiles"
        )
    repository = Path(__file__).resolve().parents[1]
    verified: dict[str, Any] = {}
    for model_id in sorted(V2101_ALLOWED_P95_PROFILES):
        receipt_path = v2101_observed_p95_receipt_path(raw_root, model_id)
        projection_path = v2101_observed_p95_projection_path(raw_root, model_id)
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.10.1 resealed {model_id} p95 failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        if (
            ...
            or projection.get("runtime_model") != runtime_model
            or projection.get("git_commit") != paid.head_commit
            or projection.get("source_contract_id") != V29_CONTRACT_ID
            or projection.get("reservations") != authority["reservations"]
            or declared_profiles.get(model_id)
            != {"authority": authority, "projection": projection}
        ):
            raise PilotOrchestrationError(
                f"V2.10.1 resealed {model_id} p95 identity/binding drifted"
            )
        verified[model_id] = {
            "authority": _json_copy(authority),
            "projection": _json_copy(projection),
...
def _verify_v2102_importer_p95_profiles(
    contract: PilotContract,
    *,
    importer_result: Mapping[str, Any],
    raw_root: Path,
    ...
    """Reverify V2.10.2 authorities before any provider construction."""

    if contract.contract_id != V2102_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.2 importer p95 verification used by another contract"
        )
    _, receipt = _v2102_import_authority(
        contract,
        raw_root=raw_root,
        paid=paid,
    )
    if (
        ...
        or importer_result.get("hosted_provider_calls_during_import") != 0
        or importer_result.get("scientific_evidence") is not False
        or importer_result.get("v2_10_1_terminal_no_go_preserved") is not True
        or importer_result.get("v2_9_source_bytes_preserved") is not True
    ):
        raise PilotOrchestrationError(
            "V2.10.2 importer result violates its exact zero-call boundary"
        )
    declared_profiles = receipt.get("p95_current_release_authorities")
    if not isinstance(declared_profiles, Mapping) or set(declared_profiles) != set(
        V2102_ALLOWED_P95_PROFILES
        ...
        raise PilotOrchestrationError(
            "V2.10.2 parent receipt lacks both exact current-release P95 profiles"
        )
    repository = Path(__file__).resolve().parents[1]
    verified: dict[str, Any] = {}
    for model_id in sorted(V2102_ALLOWED_P95_PROFILES):
        receipt_path = v2102_observed_p95_receipt_path(raw_root, model_id)
        projection_path = v2102_observed_p95_projection_path(raw_root, model_id)
        try:
            ...
            raise PilotOrchestrationError(
                f"V2.10.2 resealed {model_id} p95 failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.10.2 resealed {model_id} p95 identity/binding drifted"
            )
        verified[model_id] = {
            "authority": _json_copy(authority),
            "projection": _json_copy(projection),
    ...
    *,
    ...
    if contract.contract_id in {V2113_CONTRACT_ID, V2114_CONTRACT_ID}:
        ...
        transformer = (
            runner_reservations_from_v2114_gate_binding
            if contract.contract_id == V2114_CONTRACT_ID
            else runner_reservations_from_v2113_gate_binding
        )
        try:
            runner_reservations = transformer(receipt_binding)
        except (PilotV2113GateError, PilotV2114GateError) as exc:
            raise PilotOrchestrationError(
                f"{version} runner p95 authority transformation failed"
            ) from exc
        runtime_model = _runtime_model_for_profile(contract.provider_profiles[model_id])
        selected = runner_reservations.get(runtime_model)
        if not isinstance(selected, Mapping) or set(selected) != {
            "action",
            "semantic",
        }:
            raise PilotOrchestrationError(
                f"{version} {model_id} runner reservation denominator drifted"
            )
        for call_kind in ("action", "semantic"):
            entry = selected[call_kind]
            ...
            if (
                ...
                raise PilotOrchestrationError(
                    f"{version} {model_id} {call_kind} runner reservation "
                    "differs from the sealed projection"
                )
    ...
    if contract.contract_id in {
        ...
        declared_fields = (
            amendment["p95_runner_binding_repair"]["runner_binding_required_fields"]
            if amendment is not None
            else ()
        )
        sha_fields = (
            receipt_binding.get("receipt_file_sha256"),
            receipt_binding.get("receipt_content_sha256"),
        )
        if (
            set(declared_fields) != required_flat_fields
            or set(receipt_binding) != required_flat_fields
            or not isinstance(receipt_binding.get("receipt_path"), str)
            or not receipt_binding["receipt_path"]
            or receipt_binding.get("git_commit") != paid.head_commit
            or any(
            ...
            or not isinstance(receipt_binding.get("reservations"), Mapping)
        ):
            raise PilotOrchestrationError(
                f"{contract.contract_id} runner p95 binding is not the exact current-release "
                "flat contract"
            )
        if contract.contract_id == V2102_CONTRACT_ID:
            amendment = contract.p95_consumer_adapter_retry_amendment
            repair = (
                amendment.get("consumer_adapter_repair")
                if isinstance(amendment, Mapping)
                else None
            )
            if (
                not isinstance(repair, Mapping)
                or repair.get("producer_schema_version")
                != "finevo-pilot-v2.10.2-resealed-observed-p95-authority-v1"
                or repair.get("generic_consumer_registry_required") is not True
                or repair.get("dedicated_receipt_and_projection_verifier_required")
                is not True
                or repair.get("exact_current_release_path_required") is not True
                or repair.get("mapping_only_current_release_input") != "reject"
                or repair.get("unknown_schema_policy")
                != "stop-before-provider-construction"
                or repair.get("sibling_projection_tamper_policy")
                != "stop-before-provider-construction"
                or repair.get("receipt_replacement_race_policy")
                != "guarded-read-binding-mismatch-stop"
                or repair.get("provider_construction_before_verification") is not False
                or repair.get("validation_before_provider_construction") is not True
            ):
                raise PilotOrchestrationError(
                    "V2.10.2 consumer-adapter repair contract drifted"
                )
    profile = contract.provider_profiles[model_id]
    runtime_model = _runtime_model_for_profile(profile)
    ...
    if (
        ...
        raise PilotOrchestrationError("observed p95 receipt model reservations drifted")
        ...
        by_kind[call_kind] = {
            "authority": {
                **_json_copy(entry["authority"]),
                "source_authority_receipt_path": (receipt_binding["receipt_path"]),
    ...
    **context: Any,
    ...
    if structured is not None:
        ...
        if details is not None:
            failure.update(bounded_failure_details(_json_copy(details)))
    return failure
...
def _preflight_config(
    ...
    *,
    ...
    authority_repo_root: str | Path | None = None,
    contract_bootstrap_reservations: Mapping[str, Mapping[str, Any]] | None = None,
) -> VerifiedRunConfig:
    v211_family = contract.contract_id in V211_FAMILY_CONTRACT_IDS
    if v211_family:
        if raw_root is None:
            raise PilotOrchestrationError(
                "V2.11-family long-context preflight requires its verified parent "
                "calibration root"
            )
        calibration = (
            _load_verified_v2114_calibration(
                contract,
                raw_root=raw_root,
                paid=paid,
                release_repo_root=authority_repo_root,
            ...
            if contract.contract_id == V2114_CONTRACT_ID
                ...
                _load_verified_v2113_calibration(
                    contract,
                    raw_root=raw_root,
                    paid=paid,
                    release_repo_root=authority_repo_root,
                ...
                if contract.contract_id == V2113_CONTRACT_ID
                    ...
                    _load_verified_v2112_calibration(
                        contract,
                        raw_root=raw_root,
                        paid=paid,
                    )
                    if contract.contract_id == V2112_CONTRACT_ID
                        ...
                        if contract.contract_id == V2111_CONTRACT_ID
                        else _load_verified_v211_calibration(
                            contract,
                            raw_root=raw_root,
                        )
                    )
    ...
    return VerifiedRunConfig(
        ...
        semantic_proposal_interval=3,
        max_rule_proposals_per_agent=4,
        freeze_new_proposals_after=episode_length if v211_family else None,
        semantic_parse_failure_policy="record-and-skip",
        temperature=0.0,
        top_p=1.0,
        action_max_tokens=(
        ...
        prompt_tier_ceiling_tokens=200_000 if v211_family else None,
        scientific_scope=(
            "preregistered_mechanism_micro_pilot"
        ...
        contract_bootstrap_reservations=(contract_bootstrap_reservations or {}),
        preflight_measurement_role=(
            "closed_loop_preflight" if contract_bootstrap_reservations else None
        ),
    )
...
class _CheckpointPreflightResult:
    ...
    def __init__(self, checkpoint: PilotCheckpoint) -> None:
        ...
        action_parse_modes = {
            (int(row["decision_t"]), str(row["agent_id"])): row.get(
                "parse_disposition", {}
            ).get("parse_mode")
            for row in payload["provider_calls"]
            if row.get("call_kind") == "action"
        }
        actions = []
        for step in payload["prefix_steps"]:
            decision_t = int(step["decision_t"])
            for agent_id, decision in step["decisions"].items():
                normalized = dict(decision)
                parse_mode = action_parse_modes.get((decision_t, str(agent_id)))
                if parse_mode is not None:
                    normalized["parse_mode"] = parse_mode
                actions.append({"decision": normalized})
        self.summary = {
            "scientific_evidence": False,
...
def _preflight_checks(
    ...
    *,
    ...
    expected_actions = 24 if v211_long_context else 12
    checks = {
        (
            "action_parse_success_24_of_24"
            if v211_long_context
            else "action_parse_success_12_of_12"
        ): len(actions)
        == expected_actions
        and (
            not v211_long_context
            or all(
                row.get("decision", {}).get("parse_mode") == "exact_json"
                for row in actions
            )
        ),
    ...
    if checkpoint_preflight:
        expected_proposals = 8 if v211_long_context else 4
        expected_calls = 32 if v211_long_context else 16
        checks[
            (
                "proposal_outcomes_accounted_8_of_8"
                if v211_long_context
                else "proposal_outcomes_accounted_4_of_4"
            )
        ] = len(proposals) == expected_proposals and checks.pop(
            "proposal_outcomes_accounted"
        )
        checks[
            (
                "provider_calls_accounted_32_of_32"
                if v211_long_context
                else "provider_calls_accounted_16_of_16"
            )
        ] = (len(api_rows) == expected_calls)
    return checks
    ...
    *,
    ...
    if isinstance(capability_rows, Sequence) and not isinstance(
        ...
        if imported_rows is None:
            # V2.11.2 inherits the normalized V2.11.1 capability wrapper.
            # Its 30 immutable usage records are already in projection-ready
            # response_model/call_kind/usage form under ``usage_rows``.
            imported_rows = capability.get("usage_rows")
...
def _dispatch_projection_rows(
    contract: PilotContract,
    capability: Mapping[str, Any],
    preflight_result: Any,
) -> list[dict[str, Any]]:
    """Return the release-authorized usage denominator for dispatch P95.

    V2.11.2's recovery amendment makes the newly collected long-context
    preflight the sole normal-science reservation authority.  Its inherited
    V2.11.1 capability rows remain capability-audit evidence only.  Older
    releases retain their original combined capability/preflight semantics.
    """
    ...
    return _usage_projection_rows(
        capability,
        preflight_result,
        capability_call_kind_map=_capability_projection_call_kind_map(contract),
    )
...
def _capability_projection_call_kind_map(
    ...
    if not isinstance(policy, Mapping):
        raise PilotOrchestrationError("preflight bootstrap policy is malformed")
...
def _v2112_reseal_long_context_exactness(
    receipt: Mapping[str, Any],
) -> dict[str, Any]:
    """Rebind unchanged V2.11 exactness semantics to the V2.11.2 release.

    The underlying checkpoint is still replayed by the audited V2.11
    verifier.  Only the receipt schema and its self-hash change, preventing a
    historical V2.11/V2.11.1 receipt from being accepted as fresh V2.11.2
    evidence.
    """
    ...
    if (
        ...
        or claimed != canonical_sha256(value)
    ):
        raise PilotOrchestrationError(
            "V2.11.2 exactness reseal requires a valid V2.11 verifier receipt"
        )
    value["schema_version"] = V2112_PREFLIGHT_EXACTNESS_SCHEMA_VERSION
...
def _bootstrap_target_context(
    ...
    *,
    ...
    if len(targets) != 1 or targets[0].execution_mode != "closed_loop_preflight":
        raise PilotOrchestrationError(
...
def _v2112_bootstrap_source_context(
    contract: PilotContract,
    *,
    model_id: str,
    target: PilotRunSpec,
    ...
    """Rebuild the immutable V2.11.1 wrapper and exact bootstrap kwargs."""

    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        source = verified_v2112_capability_source(
            receipt_path,
            model_id=model_id,
            repo_root=repo_root,
            child_contract_sha256=contract.canonical_hash,
        ...
        manifest = load_v2112_source_manifest(repo_root=repo_root)
    except PilotV2112ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11.2 bootstrap source failed validation: {exc}"
        ) from exc
    root_hint = manifest.get("parent_release", {}).get("root_hint")
    if not isinstance(root_hint, str) or not root_hint:
        raise PilotOrchestrationError("V2.11.2 bootstrap source root hint is malformed")
    try:
        parent_root = (repo_root / root_hint).resolve(strict=True)
    except OSError as exc:
        raise PilotOrchestrationError(
            "V2.11.2 immutable V2.11.1 source root is unavailable"
        ) from exc
    if not parent_root.is_dir():
        raise PilotOrchestrationError(
            "V2.11.2 immutable V2.11.1 source root is not a directory"
        )
    source_capability_path = parent_root / source["source_path"]
    ...
    source_parent_receipt = _read_json(source_parent_receipt_path)
    source_release = source["source_release"]
    kwargs = {
        "source_parent_receipt": source_parent_receipt,
        "model_id": model_id,
        "source_contract_sha256": source_release["contract_sha256"],
        "source_capability_spec": source["run_spec"],
        "target_contract_sha256": contract.canonical_hash,
        "target_preflight_spec": target.to_dict(),
        "provider_profile": contract.provider_profiles[model_id].to_dict(),
        "source_parent_receipt_path": source_parent_receipt_path,
        "source_parent_receipt_file_sha256": _file_sha256(source_parent_receipt_path),
        "source_capability_path": source_capability_path,
        "source_capability_file_sha256": source["source_file_sha256"],
        "source_git_tag": source_release["git_tag"],
        "source_git_commit": source_release["resolved_git_commit"],
        "target_git_tag": paid.git_tag,
        "target_git_commit": paid.head_commit,
        "authorized_config_sha256": config_sha256,
    }
    return source, kwargs


def _persist_v2112_bootstrap_projection(
    contract: PilotContract,
    capability_spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Seal the V2.11.1-to-V2.11.2 operational preflight authority."""

    if (
        contract.contract_id != V2112_CONTRACT_ID
        or contract.v2112_recovery_amendment is None
        or capability_spec.execution_mode != "capability_authority_import"
    ):
        raise PilotOrchestrationError(
            "V2.11.2 bootstrap requires one imported capability source"
        )
    targets = contract.expand(
    ...
    if len(targets) != 1 or targets[0].execution_mode != "closed_loop_preflight":
        raise PilotOrchestrationError(
            "V2.11.2 bootstrap lacks one exact target preflight cell"
        )
    target = targets[0]
    provisional = _preflight_config(
        contract,
    ...
    source, kwargs = _v2112_bootstrap_source_context(
        contract,
        model_id=capability_spec.model_id,
        target=target,
        raw_root=raw_root,
        repo_root=repo_root,
        paid=paid,
    ...
    except PilotV2112BootstrapError as exc:
        raise PilotOrchestrationError(
            f"V2.11.2 bootstrap source failed validation: {exc}"
        ) from exc
    path = (
    ...
    try:
        ...
        raise PilotOrchestrationError(
            f"V2.11.2 persisted bootstrap failed validation: {exc}"
        ) from exc
    return projection, path


def _load_v2112_bootstrap_projection(
    contract: PilotContract,
    *,
    ...
    if (
        contract.contract_id != V2112_CONTRACT_ID
        or len(capability_specs) != 1
        or capability_specs[0].execution_mode != "capability_authority_import"
    ):
        raise PilotOrchestrationError(
            "V2.11.2 preflight lacks one imported capability source"
        )
    ...
    source, kwargs = _v2112_bootstrap_source_context(
        contract,
        model_id=preflight_spec.model_id,
        target=preflight_spec,
        raw_root=raw_root,
        repo_root=repo_root,
        paid=paid,
        config_sha256=config_sha256,
    ...
    except (OSError, ValueError, PilotV2112BootstrapError) as exc:
        raise PilotOrchestrationError(
            f"V2.11.2 bootstrap load failed validation: {exc}"
        ) from exc
    return projection, path, reservations


def _persist_v2111_bootstrap_projection(
    contract: PilotContract,
    capability_spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Seal the distinct contract-envelope authority for one preflight cell."""

    if (
        contract.contract_id != V2111_CONTRACT_ID
        or contract.v2111_preflight_bootstrap_amendment is None
        or capability_spec.execution_mode != "capability_authority_import"
    ):
        raise PilotOrchestrationError(
            "V2.11.1 bootstrap requires one imported capability source"
        )
    targets = contract.expand(
        stage="long-context-preflight",
        model=capability_spec.model_id,
    ...
    if len(targets) != 1 or targets[0].execution_mode != "closed_loop_preflight":
        raise PilotOrchestrationError(
            "V2.11.1 bootstrap lacks one exact target preflight cell"
        )
    target = targets[0]
    provisional = _preflight_config(
        contract,
    ...
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        source = verified_v2111_capability_source(
            receipt_path,
            model_id=capability_spec.model_id,
            repo_root=repo_root,
            child_contract_sha256=contract.canonical_hash,
        ...
        kwargs = {
            "model_id": capability_spec.model_id,
            "source_contract_sha256": source["parent_release"]["contract_sha256"],
            "source_capability_spec": source["spec"],
            "target_contract_sha256": contract.canonical_hash,
            "target_preflight_spec": target.to_dict(),
            "provider_profile": contract.provider_profiles[
                capability_spec.model_id
            ].to_dict(),
            "source_capability_path": source["source"]["resolved_path"],
            "source_capability_file_sha256": source["source"]["file_sha256"],
            "source_git_tag": source["parent_release"]["git_tag"],
            "source_git_commit": source["parent_release"]["resolved_git_commit"],
            "target_git_tag": paid.git_tag,
            "target_git_commit": paid.head_commit,
            "authorized_config_sha256": config_sha256,
        }
    ...
    except (PilotV2111ParentImportError, PilotV2111BootstrapError) as exc:
        raise PilotOrchestrationError(
            f"V2.11.1 bootstrap source failed validation: {exc}"
        ) from exc
    path = (
        raw_root
        / capability_spec.stage_id
    ...
    except PilotV2111BootstrapError as exc:
        raise PilotOrchestrationError(
            f"V2.11.1 persisted bootstrap failed validation: {exc}"
        ) from exc
    return projection, path


def _load_v2111_bootstrap_projection(
    contract: PilotContract,
    *,
    preflight_spec: PilotRunSpec,
    raw_root: Path,
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.1 preflight lacks one imported capability source"
        )
    capability_spec = capability_specs[0]
    provisional = _preflight_config(
        contract,
    ...
    try:
        ...
        kwargs = {
            ...
            "source_capability_spec": source["spec"],
            "target_contract_sha256": contract.canonical_hash,
            "target_preflight_spec": preflight_spec.to_dict(),
            "provider_profile": contract.provider_profiles[
        ...
        raise PilotOrchestrationError(
            f"V2.11.1 bootstrap load failed validation: {exc}"
        ) from exc
    return projection, path, reservations
...
def _execute_evaluator_capability_import_stage(
    ...
    *,
    ...
    if not specs or any(
        spec.stage_id != "capability-gate" or spec.execution_mode != "capability_probe"
        for spec in specs
    ):
        raise PilotOrchestrationError(
            "evaluator amendment import is restricted to capability-gate probes"
        )
    account_import_budget = contract.preflight_bootstrap_amendment is not None
    projections = (
        {
    ...
    for spec in specs:
        ...
        if run_ledger.is_terminal(spec.run_id):
            ...
            if account_import_budget and (
                ...
                or existing_budget.get("reservation") != projection.to_dict()
                or existing_budget.get("status") != "complete"
                or not isinstance(existing_budget.get("actual"), Mapping)
            ):
                raise PilotOrchestrationError(
                    f"evaluator import budget row differs on resume: " f"{spec.run_id}"
                )
        ...
        terminal_path = raw_root / spec.stage_id / "summaries" / f"{spec.run_id}.json"
        terminal_payload = {
            "metrics": {},
            "gate_evidence": gate,
        ...
        if not run_ledger.is_terminal(spec.run_id) and account_import_budget:
            ...
            run_ledger.finalize(
                ...
                artifact=(str(terminal_path) if budget_status == "complete" else None),
                failure=budget_failure,
            )
            if budget_status != "complete":
                raise PilotOrchestrationError(
                    f"capability import budget reconciliation failed: " f"{spec.run_id}"
                )
...
def _execute_capability_preflight(
    ...
    *,
    ...
    """Run a capability gate and/or its registered closed-loop preflight."""
    ...
    v211_long_context = (
        contract.contract_id in V211_FAMILY_CONTRACT_IDS and split_preflight_only
    )
    ...
    if split_preflight_only:
        ...
        if source_capability.get("schema_version") == CAPABILITY_IMPORT_SCHEMA_VERSION:
            ...
            contract.contract_id == V2112_CONTRACT_ID
            and source_capability.get("schema_version")
            == V2112_CAPABILITY_WRAPPER_SCHEMA_VERSION
        ):
            ...
            try:
                ...
                expected_wrapper = capability_wrappers_from_v2112_receipt(
                    parent_receipt
                )[spec.model_id]
                source = verified_v2112_capability_source(
                    parent_receipt,
                ...
                raise PilotOrchestrationError(
                    f"V2.11.2 closed-loop imported capability replay failed: {exc}"
                ) from exc
            if source_capability != expected_wrapper:
                raise PilotOrchestrationError(
                    "V2.11.2 closed-loop imported capability wrapper drifted"
                )
            inherited = source["payload"].get("capability")
            if not isinstance(inherited, Mapping):
                raise PilotOrchestrationError(
                    "V2.11.2 inherited capability payload is malformed"
                )
            capability_payload = source_capability
            capability = _json_copy(inherited)
            capability["pass"] = capability.get("capability_pass") is True
        elif (
            contract.contract_id == V2111_CONTRACT_ID
            and source_capability.get("schema_version")
            == V2111_CAPABILITY_WRAPPER_SCHEMA_VERSION
            ...
            try:
                parent_receipt = verify_v2111_parent_import_receipt(
                    raw_root / "parent-import" / "parent_import_receipt.json",
                    repo_root=Path(__file__).resolve().parents[1],
                    child_contract_sha256=contract.canonical_hash,
                    child_git_tag=paid.git_tag,
                ...
                expected_wrapper = capability_wrappers_from_v2111_receipt(
                    parent_receipt
                )[spec.model_id]
                source = verified_v2111_capability_source(
                    parent_receipt,
                ...
                raise PilotOrchestrationError(
                    f"closed-loop imported capability replay failed: {exc}"
                ) from exc
            if source_capability != expected_wrapper:
                raise PilotOrchestrationError(
                    "closed-loop imported capability wrapper drifted"
                )
            capability_payload = source_capability
            capability = source["payload"]
        else:
            capability_payload = source_capability
            if (
        ...
        if contract.preflight_bootstrap_amendment is not None:
            ...
            bootstrap_reservations = runner_reservations_from_bootstrap_projection(
                ...
                source_capability_file_sha256=_file_sha256(bootstrap_capability_path),
                git_tag=paid.git_tag,
                git_commit=paid.head_commit,
                authorized_config_sha256=bootstrap_config_sha256,
            )
        elif contract.contract_id == V2112_CONTRACT_ID:
            (
                bootstrap_projection,
                bootstrap_projection_path,
                bootstrap_reservations,
            ) = _load_v2112_bootstrap_projection(
                contract,
                preflight_spec=spec,
                raw_root=raw_root,
                repo_root=Path(__file__).resolve().parents[1],
            ...
            (
                ...
                bootstrap_projection_path,
                bootstrap_reservations,
            ) = _load_v2111_bootstrap_projection(
                contract,
                preflight_spec=spec,
                raw_root=raw_root,
                repo_root=Path(__file__).resolve().parents[1],
        ...
        capability = run_capability_gate(
            ...
            task_output_contracts=(
                (
                    {
                        "actor-action": contract.task_output_contracts[
                            "capability-choice"
                        ].to_dict(),
                        "semantic-proposal": contract.task_output_contracts[
                            "capability-proposal"
                        ].to_dict(),
                    }
                    if contract.contract_id in V211_FAMILY_CONTRACT_IDS
    ...
    continue_v211_after_content_no_go = (
        v211_long_context and capability.get("interface_gate", {}).get("pass") is True
    )
    if not capability["pass"] and not continue_v211_after_content_no_go:
    ...
    if split_capability_only:
        ...
        if bootstrap is not None:
            ...
            receipt["bootstrap_projection"] = str(capability_bootstrap_path)
        _atomic_json(run_dir / "gate_receipt.json", receipt)
        terminal = write_terminal_summary(
            raw_root / spec.stage_id / "summaries" / f"{spec.run_id}.json",
            contract=contract,
    ...
    if contract.schema_version.endswith("-v2"):
        ...
        if v211_long_context:
            ...
            checkpoint_receipt = verify_v211_long_context_preflight_checkpoint(
                checkpoint,
                call_journal_path=preflight_journal_path,
            )
            if contract.contract_id == V2112_CONTRACT_ID:
                checkpoint_receipt = _v2112_reseal_long_context_exactness(
                    checkpoint_receipt
                )
    ...
    checks = _preflight_checks(
        ...
        v211_long_context=v211_long_context,
    )
    catalog_path = (
    ...
    if bootstrap_projection is not None:
        ...
        source_bindings.update(
            {
                "source_bootstrap_projection": str(bootstrap_projection_path),
        ...
        if contract.contract_id in {V2111_CONTRACT_ID, V2112_CONTRACT_ID}:
            source_bindings["source_preflight_bootstrap_amendment_sha256"] = (
                canonical_sha256(
                    contract.to_dict()[
                        (
                            "v2112_recovery_amendment"
                            if contract.contract_id == V2112_CONTRACT_ID
                            else "v2111_preflight_bootstrap_amendment"
                        )
                    ]
                )
            )
        else:
            amendment_control = preflight_amendment_control_path(raw_root=raw_root)
            ...
            source_bindings.update(
                {
                    "source_preflight_amendment_control": str(amendment_control),
                    ...
                    "source_preflight_amendment_control_content_sha256": (
                        amendment_control_value["integrity"]["content_sha256"]
                    ),
                }
            )
        ...
        source_bindings.update(
            {
                ...
                "source_checkpoint_exactness_content_sha256": (
                    ...
                    if v211_long_context
                    else checkpoint_receipt["integrity"]["content_sha256"]
                ),
            }
        )
    ...
    interface_pass = all(checks.values())
    combined_go = capability_pass and interface_pass
    receipt = {
...
def _load_v25_inherited_projection(
    ...
    *,
    ...
    if (
        not isinstance(bindings, Mapping)
        or bindings.get("source_kind") != "v2.3-historical-parent-import-v2.5"
        or bindings.get("source_authority_receipt") != str(receipt_path)
    ):
        raise PilotOrchestrationError(
...
def _load_v26_inherited_projection(
    ...
    *,
    ...
    if (
        not isinstance(bindings, Mapping)
        or bindings.get("source_kind") != "v2.5-terminal-parent-import-v2.6"
        or bindings.get("source_authority_receipt") != str(receipt_path)
    ):
        raise PilotOrchestrationError(
...
def _load_v27_resealed_projection(
    contract: PilotContract,
    model_id: str,
    *,
    raw_root: Path,
    ...
    if paid is None:
        raise PilotOrchestrationError(
            "V2.7 resealed p95 projection requires release provenance"
        )
    if model_id not in V27_ALLOWED_P95_PROFILES:
        raise PilotOrchestrationError(
            f"{model_id} is not an allowed V2.7 resealed p95 profile"
        )
    _v27_import_authority(contract, raw_root=raw_root, paid=paid)
    receipt_path = v27_observed_p95_receipt_path(raw_root, model_id)
    path = v27_observed_p95_projection_path(raw_root, model_id)
    repository = Path(__file__).resolve().parents[1]
    try:
        payload = verify_v27_resealed_observed_p95_projection(
            path,
        ...
        raise PilotOrchestrationError(
            f"V2.7 {model_id} resealed p95 projection failed validation: {exc}"
        ) from exc
    _verify_bound_payload(
        payload,
        contract=contract,
        schema_version=PILOT_PROJECTION_SCHEMA_VERSION,
        paid=paid,
        artifact_name=f"{model_id} V2.7 resealed p95 projection",
    )
    profile = contract.provider_profiles[model_id]
    receipt_relative = _repository_relative_path(
        receipt_path,
        required_top="experiment_results",
        name="V2.7 resealed p95 receipt",
    )
    bindings = payload.get("bindings")
    if (
        payload.get("model_id") != model_id
        ...
        or bindings.get("source_kind") != V27_RESEALED_OBSERVED_P95_SOURCE_KIND
        or bindings.get("source_authority_receipt") != receipt_relative
    ):
        raise PilotOrchestrationError(
            "V2.7 resealed p95 projection source binding mismatch"
        )
    receipt_binding = _verified_observed_p95_binding(
        contract,
        model_id,
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.7 resealed p95 receipt differs from its child projection"
        )
    return payload, path
...
def _load_v29_imported_projection(
    ...
    *,
    ...
    if (
        ...
        or _bound_content_sha256(payload) != projection_binding.get("content_sha256")
        ...
        raise PilotOrchestrationError(f"V2.9 imported {model_id} p95 binding drifted")
    for call_kind in ("action", "semantic"):
        reservation = reservations[runtime_model].get(call_kind)
        projected = payload_projection.get(f"{profile.served_model}::{call_kind}")
        if (
...
def _load_v210_resealed_projection(
    contract: PilotContract,
    model_id: str,
    *,
    raw_root: Path,
    ...
    """Load the current-release V2.10 projection paired to its flat authority."""

    if paid is None:
        raise PilotOrchestrationError(
            "V2.10 resealed p95 projection requires release provenance"
        )
    if contract.contract_id != V210_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10 resealed p95 projection used by another contract"
        )
    if model_id not in V210_ALLOWED_P95_PROFILES:
        raise PilotOrchestrationError(f"{model_id} is not an allowed V2.10 p95 profile")
    release_root = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
        else Path(authority_repo_root).resolve(strict=True)
    ...
    projection = _read_json(projection_path)
    try:
        verified = verify_v210_resealed_observed_p95_projection(
            projection,
            receipt=receipt,
            repo_root=release_root,
            raw_root=raw_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10 {model_id} p95 projection failed validation: {exc}"
        ) from exc
    profile = contract.provider_profiles[model_id]
    bindings = verified.get("bindings")
    expected_keys = {
        f"{profile.served_model}::action",
        f"{profile.served_model}::semantic",
    }
    if (
        verified.get("model_id") != model_id
        or verified.get("served_model") != profile.served_model
        or set(verified.get("projection", {})) != expected_keys
        or not isinstance(bindings, Mapping)
        or bindings.get("contract_sha256") != contract.canonical_hash
        or bindings.get("git_tag") != paid.git_tag
        or bindings.get("git_commit") != paid.head_commit
    ):
        raise PilotOrchestrationError(
            f"V2.10 {model_id} resealed p95 projection identity drifted"
        )
    return verified, projection_path


def _load_v2101_resealed_projection(
    contract: PilotContract,
    model_id: str,
    *,
    raw_root: Path,
    ...
    """Load V2.10.1's own projection over exact nested V2.9 values."""

    if paid is None:
        raise PilotOrchestrationError(
            "V2.10.1 resealed p95 projection requires release provenance"
        )
    if contract.contract_id != V2101_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.1 resealed p95 projection used by another contract"
        )
    if model_id not in V2101_ALLOWED_P95_PROFILES:
        raise PilotOrchestrationError(
            f"{model_id} is not an allowed V2.10.1 p95 profile"
        )
    release_root = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
    ...
    projection = _read_json(projection_path)
    try:
        verified = verify_v2101_resealed_observed_p95_projection(
            projection,
            receipt=receipt,
            repo_root=release_root,
            raw_root=raw_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10.1 {model_id} p95 projection failed validation: {exc}"
        ) from exc
    profile = contract.provider_profiles[model_id]
    bindings = verified.get("bindings")
    expected_keys = {
    ...
    if (
        ...
        or bindings.get("git_tag") != paid.git_tag
        or bindings.get("git_commit") != paid.head_commit
        or bindings.get("source_kind") != V2101_RESEALED_P95_SOURCE_KIND
    ):
        raise PilotOrchestrationError(
            f"V2.10.1 {model_id} resealed p95 projection identity drifted"
        )
    return verified, projection_path


def _load_v2102_resealed_projection(
    contract: PilotContract,
    model_id: str,
    *,
    raw_root: Path,
    ...
    """Load V2.10.2's consumer-adapter-repaired current-release projection."""

    if paid is None:
        raise PilotOrchestrationError(
            "V2.10.2 resealed p95 projection requires release provenance"
        )
    if contract.contract_id != V2102_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.10.2 resealed p95 projection used by another contract"
        )
    if model_id not in V2102_ALLOWED_P95_PROFILES:
        raise PilotOrchestrationError(
            f"{model_id} is not an allowed V2.10.2 p95 profile"
        )
    release_root = (
        Path(__file__).resolve().parents[1]
        if authority_repo_root is None
    ...
    projection = _read_json(projection_path)
    try:
        verified = verify_v2102_resealed_observed_p95_projection(
            projection,
            receipt=receipt,
            repo_root=release_root,
            raw_root=raw_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10.2 {model_id} p95 projection failed validation: {exc}"
        ) from exc
    profile = contract.provider_profiles[model_id]
    bindings = verified.get("bindings")
    expected_keys = {
    ...
    if (
        ...
        or bindings.get("git_tag") != paid.git_tag
        or bindings.get("git_commit") != paid.head_commit
        or bindings.get("source_kind") != V2102_RESEALED_P95_SOURCE_KIND
    ):
        raise PilotOrchestrationError(
            f"V2.10.2 {model_id} resealed p95 projection identity drifted"
        )
    return verified, projection_path
...
def _load_verified_projection(
    ...
    *,
    ...
    if contract.contract_id in {V2113_CONTRACT_ID, V2114_CONTRACT_ID}:
        is_v2114 = contract.contract_id == V2114_CONTRACT_ID
        version = "V2.11.4" if is_v2114 else "V2.11.3"
        if paid is None:
            raise PilotOrchestrationError(
                f"{version} resealed p95 verification requires paid provenance"
            )
        repository = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        receipt_path = (
            ...
            if is_v2114
            else v2113_observed_p95_receipt_path(raw_root, model_id)
        )
        projection_path = (
            v2114_observed_p95_projection_path(raw_root, model_id)
            if is_v2114
            else v2113_observed_p95_projection_path(raw_root, model_id)
        )
        authority_verifier = (
            verified_v2114_observed_p95_authority_binding
            if is_v2114
            else verified_v2113_observed_p95_authority_binding
        )
        projection_verifier = (
            verified_v2114_observed_p95_projection_binding
            if is_v2114
            else verified_v2113_observed_p95_projection_binding
        )
        try:
            ...
            projection = projection_verifier(
                projection_path,
                receipt_path=receipt_path,
                repo_root=repository,
                raw_root=raw_root,
            ...
            global_binding = _verified_observed_p95_binding(
                contract,
                model_id,
                raw_root=raw_root,
                paid=paid,
            ...
            raise PilotOrchestrationError(
                f"{version} resealed {model_id} p95 failed validation: {exc}"
            ) from exc
        profile = contract.provider_profiles[model_id]
        runtime_model = _runtime_model_for_profile(profile)
        ...
        if (
            ...
            or projection.get("runtime_model") != runtime_model
            or projection.get("git_commit") != paid.head_commit
            or projection.get("source_contract_id") != V2112_CONTRACT_ID
            or projection.get("reservations") != authority["reservations"]
            or global_binding.get("reservations", {}).get(runtime_model)
            != authority["reservations"].get(runtime_model)
            or payload.get("model_id") != model_id
            or payload.get("served_model") != profile.served_model
            or payload.get("bindings", {}).get("contract_sha256")
            != contract.canonical_hash
            or payload.get("bindings", {}).get("git_commit") != paid.head_commit
        ):
            raise PilotOrchestrationError(
                f"{version} resealed {model_id} p95 identity/binding drifted"
            )
        return _json_copy(payload), projection_path
    if contract.contract_id == V2102_CONTRACT_ID:
        return _load_v2102_resealed_projection(
            contract,
            model_id,
            raw_root=raw_root,
            paid=paid,
    ...
    if contract.contract_id == V2101_CONTRACT_ID:
        return _load_v2101_resealed_projection(
            contract,
            model_id,
            raw_root=raw_root,
            paid=paid,
    ...
    if contract.contract_id == V210_CONTRACT_ID:
        return _load_v210_resealed_projection(
            contract,
            model_id,
            raw_root=raw_root,
            paid=paid,
    ...
    capability_artifact = _read_json(capability_path)
    capability = capability_artifact
    if (
        contract.contract_id == V2112_CONTRACT_ID
        and capability_artifact.get("schema_version")
        == V2112_CAPABILITY_WRAPPER_SCHEMA_VERSION
    ):
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.2 preflight projection verification requires paid "
                "release provenance"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
        ...
        try:
            ...
            expected_wrapper = capability_wrappers_from_v2112_receipt(parent_receipt)[
                model_id
            ]
            verified_source = verified_v2112_capability_source(
                parent_receipt,
                model_id=model_id,
                repo_root=release_root,
                child_contract_sha256=contract.canonical_hash,
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 preflight capability replay failed: {exc}"
            ) from exc
        if capability_artifact != expected_wrapper:
            raise PilotOrchestrationError(
                "V2.11.2 preflight capability wrapper drifted"
            )
        inherited = verified_source["payload"].get("capability")
        if not isinstance(inherited, Mapping):
            raise PilotOrchestrationError(
                "V2.11.2 inherited capability payload is malformed"
            )
        ...
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.1 preflight projection verification requires paid "
                "release provenance"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
        ...
        try:
            ...
            expected_wrapper = capability_wrappers_from_v2111_receipt(parent_receipt)[
                model_id
            ]
            verified_source = verified_v2111_capability_source(
                parent_receipt,
                model_id=model_id,
                repo_root=release_root,
                child_contract_sha256=contract.canonical_hash,
            ...
            raise PilotOrchestrationError(
                f"V2.11.1 preflight capability replay failed: {exc}"
            ) from exc
        if capability_artifact != expected_wrapper:
            raise PilotOrchestrationError(
                "V2.11.1 preflight capability wrapper drifted"
            )
        capability = verified_source["payload"]
    elif capability_artifact.get("schema_version") == CAPABILITY_IMPORT_SCHEMA_VERSION:
        try:
            validate_capability_import(
                capability_artifact,
                contract,
        ...
        capability_artifact.get("contract_sha256") != contract.canonical_hash
        or capability_artifact.get("run_spec") != capability_specs[0].to_dict()
    ):
    ...
    if contract.preflight_bootstrap_amendment is not None:
        ...
        bootstrap_target_spec, bootstrap_config_sha256 = _bootstrap_target_context(
            contract,
            capability_specs[0],
            paid=paid,
        )
        if bootstrap_target_spec != preflight_specs[0]:
            raise PilotOrchestrationError("preflight bootstrap target cell drifted")
        ...
        expected_git_commit = (
            paid.head_commit if paid is not None else str(bindings.get("git_commit"))
        )
        try:
            validate_capability_bootstrap_projection(
                bootstrap_projection,
        ...
        if (
            ...
            or bindings.get("source_bootstrap_projection") != str(bootstrap_path)
            or bindings.get("source_bootstrap_projection_file_sha256")
            != _file_sha256(bootstrap_path)
            or bindings.get("source_bootstrap_projection_content_sha256")
            != bootstrap_projection["integrity"]["content_sha256"]
            or bindings.get("source_preflight_amendment_control") != str(control_path)
        ...
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.2 preflight bootstrap verification requires paid "
                "release provenance"
            )
        (
            bootstrap_projection,
            ...
            contract,
            preflight_spec=preflight_specs[0],
            raw_root=raw_root,
            repo_root=(
                Path(__file__).resolve().parents[1]
                if authority_repo_root is None
        ...
        if (
            ...
            or bindings.get("source_bootstrap_projection_content_sha256")
            != bootstrap_projection["integrity"]["content_sha256"]
            or bindings.get("source_preflight_bootstrap_amendment_sha256")
            != canonical_sha256(contract.to_dict()["v2112_recovery_amendment"])
        ):
            raise PilotOrchestrationError(
                "V2.11.2 preflight bootstrap source binding mismatch"
            )
    elif contract.contract_id == V2111_CONTRACT_ID:
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11.1 preflight bootstrap verification requires paid "
                "release provenance"
            )
        (
            bootstrap_projection,
        ...
        if (
            ...
            != bootstrap_projection["integrity"]["content_sha256"]
            or bindings.get("source_preflight_bootstrap_amendment_sha256")
            != canonical_sha256(contract.v2111_preflight_bootstrap_amendment)
        ):
            raise PilotOrchestrationError(
                "V2.11.1 preflight bootstrap source binding mismatch"
            )
    if contract.schema_version.endswith("-v2"):
        checkpoint_path = run_dir / "preflight_checkpoint.json"
        ...
        if contract.preflight_bootstrap_amendment is not None:
            ...
            if (
                checkpoint_run_config.get("contract_bootstrap_reservations")
                != expected_bootstrap_authority
                or checkpoint_run_config.get("preflight_measurement_role")
                != "closed_loop_preflight"
                or checkpoint_run_config.get("preflight_p95_reservations") != {}
            ):
                raise PilotOrchestrationError(
                    "preflight checkpoint bootstrap authority differs from "
                    "its capability projection"
                )
        elif contract.contract_id in {V2111_CONTRACT_ID, V2112_CONTRACT_ID}:
            if (
                checkpoint_run_config.get("contract_bootstrap_reservations")
                != bootstrap_reservations
                or checkpoint_run_config.get("preflight_measurement_role")
                ...
                raise PilotOrchestrationError(
                    "V2.11 family preflight checkpoint bootstrap authority "
                    "differs from its contract-envelope projection"
                )
        elif (
        ...
        if contract.contract_id in V211_FAMILY_CONTRACT_IDS:
            expected_exactness = verify_v211_long_context_preflight_checkpoint(
                checkpoint,
                call_journal_path=journal_path,
            )
            if contract.contract_id == V2112_CONTRACT_ID:
                expected_exactness = _v2112_reseal_long_context_exactness(
                    expected_exactness
                )
            if (
                exactness != expected_exactness
                ...
                != exactness.get("receipt_hash")
            ):
                raise PilotOrchestrationError(
                    "V2.11 preflight checkpoint/exactness source binding mismatch"
                )
        else:
            _verify_bound_payload(
                exactness,
    ...
    if (
        ...
        or contract.contract_id in V211_FAMILY_CONTRACT_IDS
    ):
        ...
        if (
            ...
            or set(receipt_reservations[runtime_model]) != {"action", "semantic"}
            or any(
                receipt_reservations[runtime_model][call_kind].get("reservation")
                != recomputed[f"{payload['served_model']}::{call_kind}"]
                for call_kind in ("action", "semantic")
            )
        ):
            raise PilotOrchestrationError(
...
def _execute_q_ref(
    ...
    *,
    ...
    if contract.contract_id == V29_CONTRACT_ID:
        ...
        resolution["hosted_provider_calls_current_attempt"] = 0
        resolution["hosted_cost_usd_current_attempt"] = 0.0
        resolution["q_ref_audit_equivalence"] = _build_v28_qref_equivalence_receipt(
            contract,
            result,
            resolution,
            raw_root=raw_root,
...
def _v211_post_gate_authority_path(raw_root: Path) -> Path:
    return (
        raw_root
        / PILOT_V211_POST_GATE_STAGE_ID
        / PILOT_V211_POST_GATE_AUTHORITY_FILENAME
    )


def _verify_v211_family_post_gate_receipt(
    contract: PilotContract,
    value: Mapping[str, Any],
    *,
    paid: GitProvenance,
    ...
    try:
        if contract.contract_id == V2114_CONTRACT_ID:
            return verify_v2114_gate_receipt(
                value,
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
        if contract.contract_id == V2113_CONTRACT_ID:
            return verify_v2113_gate_receipt(
                value,
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
        if contract.contract_id == V2112_CONTRACT_ID:
            return verify_v2112_gate_receipt(
                value,
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
        if contract.contract_id == V2111_CONTRACT_ID:
            return verify_v2111_gate_receipt(
                value,
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
        if contract.contract_id == V211_CONTRACT_ID:
            return verify_v211_gate_receipt(
                value,
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
        ...
        raise PilotOrchestrationError(
            f"{contract.contract_id} post-gate receipt failed validation: {exc}"
        ) from exc
    raise PilotOrchestrationError(
        "V2.11-family post-gate verification requires a family contract"
    )


def _v211_post_gate_inputs(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Rebuild the exact provider-free inputs for the V2.11 global gate."""

    if contract.contract_id != V211_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.11 post-gate inputs require the V2.11 contract"
        )
    model_ids = ("gpt52_main", "gpt56_diagnostic")
    capability_artifacts: dict[str, dict[str, Any]] = {}
    preflight_artifacts: dict[str, dict[str, Any]] = {}
    ...
    for model_id in model_ids:
        ...
        preflight_specs = contract.expand(
            stage=PILOT_V211_POST_GATE_STAGE_ID,
            model=model_id,
        )
        if len(capability_specs) != 1 or len(preflight_specs) != 1:
            raise PilotOrchestrationError(
                f"V2.11 {model_id} lacks one exact capability/preflight cell"
            )
        capability_spec = capability_specs[0]
        preflight_spec = preflight_specs[0]
        gate_run_ids.extend((capability_spec.run_id, preflight_spec.run_id))
        ...
        capability = _read_json(capability_path)
        capability_artifacts[model_id] = {
            "run_id": capability_spec.run_id,
        ...
        checkpoint_path = preflight_dir / "preflight_checkpoint.json"
        exactness_path = preflight_dir / "preflight_checkpoint_exactness.json"
        checkpoint = _read_json(checkpoint_path)
        exactness = _read_json(exactness_path)
        checkpoint_run_id = checkpoint.get("run_config", {}).get("run_id")
        if not isinstance(checkpoint_run_id, str):
            raise PilotOrchestrationError(
                f"V2.11 {model_id} checkpoint lacks its runner run ID"
            )
        preflight_artifacts[model_id] = {
            "ledger_run_id": preflight_spec.run_id,
        ...
        local_gate = _read_json(preflight_dir / "gate_receipt.json")
        preflight_checks = local_gate.get("preflight_checks")
        if (
            not isinstance(preflight_checks, Mapping)
            or not preflight_checks
            or any(not isinstance(value, bool) for value in preflight_checks.values())
            or local_gate.get("capability_pass") is not (capability.get("pass") is True)
        ):
            raise PilotOrchestrationError(
                f"V2.11 {model_id} local preflight gate is incomplete"
            )
        capability_pass = capability.get("pass") is True
        interface_pass = all(preflight_checks.values())
        model_status = (
            "capability-no-go"
            if not capability_pass
            else "eligible" if interface_pass else "interface-no-go"
        )
        if local_gate.get("go") is not (model_status == "eligible"):
            raise PilotOrchestrationError(
                f"V2.11 {model_id} local gate decision drifted"
            )
        expected_ledger_status = (
            "complete" if model_status == "eligible" else "capability-no-go"
        )
        if run_ledger.status(preflight_spec.run_id) != expected_ledger_status:
            raise PilotOrchestrationError(
                f"V2.11 {model_id} preflight ledger status differs from its gate"
            )
        model_statuses[model_id] = model_status

    parent_specs = contract.expand(stage="parent-import")
    if len(parent_specs) != 1:
        raise PilotOrchestrationError(
            "V2.11 post-gate authority requires one parent-import cell"
        )
    parent_run_id = parent_specs[0].run_id
    pre_science_run_ids = {parent_run_id, *gate_run_ids}
    budget_snapshot = budget_ledger.snapshot()
    budget_rows = budget_snapshot.get("runs")
    if not isinstance(budget_rows, Mapping):
        raise PilotOrchestrationError("V2.11 budget ledger rows are malformed")
    pre_science_storage = 0
    for run_id in sorted(pre_science_run_ids):
        ...
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11 pre-science budget actual is missing for {run_id}"
            )
        pre_science_storage += int(actual["storage_bytes"])

    science_run_ids_by_model: dict[str, list[str]] = {
    ...
    for stage_id in (
        ...
        for spec in contract.expand(stage=stage_id):
            if spec.model_id not in science_run_ids_by_model:
                raise PilotOrchestrationError(
                    f"V2.11 science cell uses an unregistered model: {spec.model_id}"
                )
            science_run_ids_by_model[spec.model_id].append(spec.run_id)

    run_snapshot = run_ledger.snapshot()
    events = run_snapshot.get("events")
    if not isinstance(events, list) or not events:
        raise PilotOrchestrationError(
            "V2.11 run ledger lacks its tamper-evident event chain"
        )
    inputs = {
        "contract_sha256": contract.canonical_hash,
        "release_tag": paid.git_tag,
        "release_commit": paid.head_commit,
        "parent_import_run_id": parent_run_id,
        "capability_artifacts": capability_artifacts,
        "preflight_artifacts": preflight_artifacts,
        "model_terminal_statuses": model_statuses,
        "pre_science_actual_storage_bytes": pre_science_storage,
        "ledger_event_chain_head": events[-1]["event_sha256"],
        "science_run_ids_by_model": science_run_ids_by_model,
        "source_manifest_hashes": {
            "file_sha256": V211_SOURCE_MANIFEST_FILE_SHA256,
            "content_sha256": V211_SOURCE_MANIFEST_CONTENT_SHA256,
        },
    }
    return inputs, model_statuses


def _build_v211_post_gate_with_inferred_statuses(
    inputs: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, str]]:
    """Let the full 48/14-sample gate determine each terminal model status."""

    capability_artifacts = inputs.get("capability_artifacts")
    if not isinstance(capability_artifacts, Mapping):
        raise PilotOrchestrationError(
            "V2.11 capability artifacts are absent from the post-gate inputs"
        )
    model_ids = ("gpt52_main", "gpt56_diagnostic")
    options: list[tuple[str, ...]] = []
    for model_id in model_ids:
        envelope = capability_artifacts.get(model_id)
        ...
        if not isinstance(payload, Mapping) or not isinstance(
            ...
            raise PilotOrchestrationError(
                f"V2.11 {model_id} capability decision is malformed"
            )
        options.append(
            ("eligible", "interface-no-go")
            if payload["pass"] is True
            else ("capability-no-go",)
        )

    successes: list[tuple[dict[str, Any], dict[str, str]]] = []
    failures: list[str] = []
    for candidate_values in product(*options):
        statuses = dict(zip(model_ids, candidate_values, strict=True))
        candidate = dict(inputs)
        candidate["model_terminal_statuses"] = statuses
        try:
            receipt = build_v211_post_gate_authority(**candidate)
        except PilotV211GateError as exc:
            failures.append(str(exc))
            continue
        successes.append((receipt, statuses))
    if len(successes) != 1:
        detail = (
            "no terminal-status assignment validated"
            if not successes
            else "multiple terminal-status assignments validated"
        )
        if failures:
            detail += f"; first failure: {failures[0]}"
        raise PilotOrchestrationError(
            f"V2.11 post-gate status inference failed: {detail}"
        )
    return successes[0]


def _persist_v211_post_gate_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Validate, terminalize model-scoped no-go rows, and seal the global gate."""

    path = _v211_post_gate_authority_path(raw_root)
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise PilotOrchestrationError(
                "V2.11 post-gate authority must be a regular non-symlink file"
            )
        try:
            existing = verify_v211_gate_receipt(
                _read_json(path),
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
            ...
            raise PilotOrchestrationError(
                f"V2.11 resumed post-gate authority failed validation: {exc}"
            ) from exc
        events = run_ledger.snapshot().get("events")
        bound_head = existing["bindings"]["ledger_event_chain_head"]
        if not isinstance(events, list) or bound_head not in {
            event.get("event_sha256") for event in events if isinstance(event, Mapping)
        }:
            raise PilotOrchestrationError(
                "V2.11 resumed post-gate ledger prefix is absent"
            )
        resumed_inputs, _ = _v211_post_gate_inputs(
            contract,
            raw_root=raw_root,
            paid=paid,
        ...
        resumed_inputs["ledger_event_chain_head"] = bound_head
        rebuilt, _ = _build_v211_post_gate_with_inferred_statuses(resumed_inputs)
        try:
            verify_v211_gate_receipt(
                rebuilt,
                expected_contract_sha256=contract.canonical_hash,
            ...
            raise PilotOrchestrationError(
                "V2.11 resumed post-gate source recomputation failed "
                f"validation: {exc}"
            ) from exc
        if rebuilt != existing:
            raise PilotOrchestrationError(
                "V2.11 resumed post-gate authority differs from current "
                "sealed sources"
            )
        return path, existing

    inputs, _ = _v211_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    draft, draft_statuses = _build_v211_post_gate_with_inferred_statuses(inputs)

    science_specs_by_model = {
        model_id: tuple(
            spec
            for stage_id in (
                "experiment-c",
    ...
    for model_id, decision in draft["model_decisions"].items():
        if decision["eligible_for_science_dispatch"] is True:
            continue
        run_ledger.stop_pending(
            science_specs_by_model[model_id],
            status="capability-no-go",
            failure={
                "error_type": "CapabilityOrInterfaceNoGo",
                "message": (
                    f"{model_id} did not pass the frozen V2.11 "
                    f"{decision['terminal_status']} gate"
                ),
                "source_stage": PILOT_V211_POST_GATE_STAGE_ID,
                "model_id": model_id,
            },
        )

    if draft.get("go") is not True:
        remaining_science = tuple(
            spec
            for model_specs in science_specs_by_model.values()
        ...
        run_ledger.stop_pending(
            ...
            failure={
                "error_type": "PostGateProjectionNoGo",
                "message": "V2.11 full-matrix projection did not fit hard caps",
                "source_stage": PILOT_V211_POST_GATE_STAGE_ID,
                "reasons": list(draft.get("reasons", [])),
            },
        )

    final_inputs, _ = _v211_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    receipt, final_statuses = _build_v211_post_gate_with_inferred_statuses(final_inputs)
    if final_statuses != draft_statuses:
        raise PilotOrchestrationError(
            "V2.11 model decisions changed while sealing the global gate"
        )
    try:
        verify_v211_gate_receipt(
            receipt,
        ...
        raise PilotOrchestrationError(
            f"V2.11 final post-gate authority failed validation: {exc}"
        ) from exc
    _persist_exact_json(path, receipt)
    return path, receipt


def _v2111_post_gate_inputs(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Rebuild the zero-provider V2.11.1 global-gate inputs."""

    if contract.contract_id != V2111_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.11.1 post-gate inputs require the exact V2.11.1 contract"
        )
    repo_root = Path(__file__).resolve().parents[1]
    model_ids = ("gpt52_main", "gpt56_diagnostic")
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        ...
        inherited_capability_bindings = {
            model_id: verified_v2111_inherited_capability_binding(
                parent_receipt,
                model_id=model_id,
                repo_root=repo_root,
                child_contract_sha256=contract.canonical_hash,
        ...
        raise PilotOrchestrationError(
            f"V2.11.1 post-gate parent replay failed: {exc}"
        ) from exc
    parent_relative = _repository_relative_path(
        parent_receipt_path,
        required_top="experiment_results",
    ...
    parent_import_receipt_binding = {
        ...
        "file_sha256": _file_sha256(parent_receipt_path),
        "content_sha256": parent_receipt["integrity"]["content_sha256"],
    }

    fresh_preflight_artifacts: dict[str, dict[str, Any]] = {}
    local_statuses: dict[str, str] = {}
    pre_science_run_ids: set[str] = set()
    parent_specs = contract.expand(stage="parent-import")
    if len(parent_specs) != 1:
        raise PilotOrchestrationError(
            "V2.11.1 post-gate requires one exact parent-import cell"
        )
    pre_science_run_ids.add(parent_specs[0].run_id)
    pre_science_run_ids.update(
        spec.run_id for spec in contract.expand(stage="capability-gate")
    )
    ...
    for model_id in model_ids:
        ...
        if len(preflight_specs) != 1:
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} lacks one exact long-context preflight"
            )
        spec = preflight_specs[0]
        pre_science_run_ids.add(spec.run_id)
        run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
        checkpoint_path = run_dir / "preflight_checkpoint.json"
        exactness_path = run_dir / "preflight_checkpoint_exactness.json"
        checkpoint = _read_json(checkpoint_path)
        ...
        if not isinstance(checkpoint_run_id, str):
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} checkpoint lacks its runner run ID"
            )
        fresh_preflight_artifacts[model_id] = {
            "ledger_run_id": spec.run_id,
            "checkpoint_run_id": checkpoint_run_id,
        ...
        local_gate = _read_json(run_dir / "gate_receipt.json")
        checks = local_gate.get("preflight_checks")
        inherited_payload = inherited_capability_bindings[model_id].get("payload")
        inherited_capability = (
            inherited_payload.get("capability")
            if isinstance(inherited_payload, Mapping)
            else None
        )
        if (
            ...
            or not isinstance(local_gate.get("capability_pass"), bool)
            or not isinstance(inherited_capability, Mapping)
            or not isinstance(
                inherited_capability.get("capability_pass"),
                bool,
            )
        ):
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} local preflight gate is incomplete"
            )
        if local_gate["capability_pass"] is not inherited_capability["capability_pass"]:
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} local capability decision differs "
                "from the replay-verified inherited capability wrapper"
            )
        local_go = local_gate["capability_pass"] is True and all(checks.values())
        expected_ledger_status = "complete" if local_go else "capability-no-go"
        if (
            local_gate.get("go") is not local_go
            or run_ledger.status(spec.run_id) != expected_ledger_status
        ):
            raise PilotOrchestrationError(
                f"V2.11.1 {model_id} local preflight status drifted"
            )
        local_statuses[model_id] = "eligible" if local_go else "interface-no-go"

    budget_snapshot = budget_ledger.snapshot()
    budget_rows = budget_snapshot.get("runs")
    parent_debit = budget_snapshot.get("parent_debit")
    if not isinstance(budget_rows, Mapping) or not isinstance(
        parent_debit,
        Mapping,
    ):
        raise PilotOrchestrationError("V2.11.1 pre-science budget ledger is malformed")
    child_pre_science_storage = 0
    for run_id in sorted(pre_science_run_ids):
        row = budget_rows.get(run_id)
        actual = row.get("actual") if isinstance(row, Mapping) else None
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.1 pre-science budget actual is missing for {run_id}"
            )
        child_pre_science_storage += int(actual["storage_bytes"])
    ...
    for stage_id in (
        ...
        for spec in contract.expand(stage=stage_id):
            if spec.model_id not in science_run_ids_by_model:
                raise PilotOrchestrationError(
                    "V2.11.1 science cell uses an unregistered model"
                )
            science_run_ids_by_model[spec.model_id].append(spec.run_id)
    ...
    if not isinstance(events, list) or not events:
        raise PilotOrchestrationError(
            "V2.11.1 run ledger lacks its tamper-evident event chain"
        )
    return {
        "contract_sha256": contract.canonical_hash,
        "release_tag": paid.git_tag,
        "release_commit": paid.head_commit,
        "parent_import_receipt_binding": parent_import_receipt_binding,
        "parent_budget_debit": _json_copy(parent_debit),
        "inherited_capability_bindings": inherited_capability_bindings,
        "fresh_preflight_artifacts": fresh_preflight_artifacts,
        "model_terminal_statuses": local_statuses,
        "current_attempt_pre_science_storage_bytes": (child_pre_science_storage),
        "ledger_event_chain_head": events[-1]["event_sha256"],
        "science_run_ids_by_model": science_run_ids_by_model,
        "source_manifest_hashes": {
...
def _build_v2111_post_gate_with_inferred_statuses(
    inputs: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, str]]:
    """Infer the unique V2.11.1 terminal model-status assignment."""
    ...
    for candidate_values in product(
        ...
        candidate["model_terminal_statuses"] = statuses
        try:
            receipt = build_v2111_post_gate_authority(**candidate)
        except PilotV2111GateError as exc:
            failures.append(str(exc))
            continue
        successes.append((receipt, statuses))
    if len(successes) != 1:
        ...
        if failures:
            detail += f"; first failure: {failures[0]}"
        raise PilotOrchestrationError(
            f"V2.11.1 post-gate status inference failed: {detail}"
        )
    return successes[0]


def _persist_v2111_post_gate_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Seal V2.11.1 observed-p95 authority and terminalize no-go cells."""

    path = _v211_post_gate_authority_path(raw_root)
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise PilotOrchestrationError(
                "V2.11.1 post-gate authority must be a regular non-symlink file"
            )
        try:
            existing = verify_v2111_gate_receipt(
                _read_json(path),
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
            ...
            raise PilotOrchestrationError(
                f"V2.11.1 resumed post-gate authority failed validation: {exc}"
            ) from exc
        events = run_ledger.snapshot().get("events")
        bound_head = existing["bindings"]["ledger_event_chain_head"]
        if not isinstance(events, list) or bound_head not in {
            ...
            raise PilotOrchestrationError(
                "V2.11.1 resumed post-gate ledger prefix is absent"
            )
        resumed_inputs = _v2111_post_gate_inputs(
            contract,
            raw_root=raw_root,
            paid=paid,
            budget_ledger=budget_ledger,
        ...
        resumed_inputs["ledger_event_chain_head"] = bound_head
        rebuilt, _ = _build_v2111_post_gate_with_inferred_statuses(resumed_inputs)
        if rebuilt != existing:
            raise PilotOrchestrationError(
                "V2.11.1 resumed post-gate authority differs from current "
                "sealed sources"
            )
        return path, existing

    inputs = _v2111_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    draft, draft_statuses = _build_v2111_post_gate_with_inferred_statuses(inputs)
    science_specs_by_model = {
        model_id: tuple(
            spec
            for stage_id in (
    ...
    for model_id, decision in draft["model_decisions"].items():
        ...
        run_ledger.stop_pending(
            ...
            failure={
                "error_type": "CapabilityOrInterfaceNoGo",
                "message": (
                    f"{model_id} did not pass the frozen V2.11.1 "
                    f"{decision['terminal_status']} gate"
                ),
                "source_stage": PILOT_V211_POST_GATE_STAGE_ID,
                "model_id": model_id,
    ...
    if draft.get("go") is not True:
        run_ledger.stop_pending(
            ...
            failure={
                "error_type": "PostGateProjectionNoGo",
                "message": ("V2.11.1 full-matrix projection did not fit hard caps"),
                "source_stage": PILOT_V211_POST_GATE_STAGE_ID,
                "reasons": list(draft.get("reasons", [])),
            },
        )
    final_inputs = _v2111_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    receipt, final_statuses = _build_v2111_post_gate_with_inferred_statuses(
        final_inputs
    )
    if final_statuses != draft_statuses:
        raise PilotOrchestrationError(
            "V2.11.1 model decisions changed while sealing the global gate"
        )
    try:
        verify_v2111_gate_receipt(
            receipt,
        ...
        raise PilotOrchestrationError(
            f"V2.11.1 final post-gate authority failed validation: {exc}"
        ) from exc
    _persist_exact_json(path, receipt)
    return path, receipt


def _v2112_post_gate_inputs(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Rebuild only fresh V2.11.2 gate evidence plus inherited capability."""

    if contract.contract_id != V2112_CONTRACT_ID:
        raise PilotOrchestrationError(
            "V2.11.2 post-gate inputs require the exact V2.11.2 contract"
        )
    repo_root = Path(__file__).resolve().parents[1]
    model_ids = ("gpt52_main", "gpt56_diagnostic")
    parent_receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        ...
        raise PilotOrchestrationError(
            f"V2.11.2 post-gate parent replay failed: {exc}"
        ) from exc
    parent_relative = _repository_relative_path(
        parent_receipt_path,
        required_top="experiment_results",
    ...
    if len(parent_specs) != 1:
        raise PilotOrchestrationError(
            "V2.11.2 post-gate requires one exact parent-import cell"
        )
    pre_science_run_ids.add(parent_specs[0].run_id)
    pre_science_run_ids.update(
        spec.run_id for spec in contract.expand(stage="capability-gate")
    ...
    for model_id in model_ids:
        ...
        if len(preflight_specs) != 1:
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} lacks one exact fresh preflight"
            )
        spec = preflight_specs[0]
        pre_science_run_ids.add(spec.run_id)
        run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
        ...
        if not isinstance(checkpoint_run_id, str):
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} checkpoint lacks its runner run ID"
            )
        fresh_preflight_artifacts[model_id] = {
            "ledger_run_id": spec.run_id,
            "checkpoint_run_id": checkpoint_run_id,
        ...
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} local preflight gate is incomplete"
            )
        if local_gate["capability_pass"] is not inherited_capability["capability_pass"]:
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} capability decision differs from "
                "the inherited capability wrapper"
            )
        local_go = local_gate["capability_pass"] is True and all(checks.values())
        expected_ledger_status = "complete" if local_go else "capability-no-go"
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 {model_id} local preflight status drifted"
            )
        local_statuses[model_id] = (
            "eligible"
            if local_go
            else (
                "capability-no-go"
                if local_gate["capability_pass"] is not True
                else "interface-no-go"
            )
        )
    ...
    if not isinstance(budget_rows, Mapping) or not isinstance(
        ...
        raise PilotOrchestrationError("V2.11.2 pre-science budget ledger is malformed")
    child_pre_science_storage = 0
    for run_id in sorted(pre_science_run_ids):
        row = budget_rows.get(run_id)
        actual = row.get("actual") if isinstance(row, Mapping) else None
        if (
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 pre-science budget actual is missing for {run_id}"
            )
        child_pre_science_storage += int(actual["storage_bytes"])
    ...
    for stage_id in (
        ...
        for spec in contract.expand(stage=stage_id):
            if spec.model_id not in science_run_ids_by_model:
                raise PilotOrchestrationError(
                    "V2.11.2 science cell uses an unregistered model"
                )
            science_run_ids_by_model[spec.model_id].append(spec.run_id)
    ...
    if not isinstance(events, list) or not events:
        raise PilotOrchestrationError(
            "V2.11.2 run ledger lacks its tamper-evident event chain"
        )
    return {
        "contract_sha256": contract.canonical_hash,
        "release_tag": paid.git_tag,
...
def _build_v2112_post_gate_with_inferred_statuses(
    inputs: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, str]]:
    """Infer status from sealed 24/8 fresh samples plus capability audit."""
    ...
    for candidate_values in product(
        ...
        candidate["model_terminal_statuses"] = statuses
        try:
            receipt = build_v2112_post_gate_authority(**candidate)
        except PilotV2112GateError as exc:
            failures.append(str(exc))
            continue
        successes.append((receipt, statuses))
    if len(successes) != 1:
        ...
        if failures:
            detail += f"; first failure: {failures[0]}"
        raise PilotOrchestrationError(
            f"V2.11.2 post-gate status inference failed: {detail}"
        )
    return successes[0]


def _persist_v2112_post_gate_authority(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    ...
    """Seal V2.11.2 authority or terminalize all 131 science cells."""

    path = _v211_post_gate_authority_path(raw_root)
    if path.exists():
        if path.is_symlink() or not path.is_file():
            raise PilotOrchestrationError(
                "V2.11.2 post-gate authority must be a regular file"
            )
        try:
            existing = verify_v2112_gate_receipt(
                _read_json(path),
                expected_contract_sha256=contract.canonical_hash,
                expected_git_commit=paid.head_commit,
            )
            ...
            raise PilotOrchestrationError(
                f"V2.11.2 resumed post-gate authority failed: {exc}"
            ) from exc
        events = run_ledger.snapshot().get("events")
        bound_head = existing["bindings"]["ledger_event_chain_head"]
        if not isinstance(events, list) or bound_head not in {
            ...
            raise PilotOrchestrationError(
                "V2.11.2 resumed post-gate ledger prefix is absent"
            )
        resumed_inputs = _v2112_post_gate_inputs(
            contract,
            raw_root=raw_root,
            paid=paid,
            budget_ledger=budget_ledger,
        ...
        resumed_inputs["ledger_event_chain_head"] = bound_head
        rebuilt, _ = _build_v2112_post_gate_with_inferred_statuses(resumed_inputs)
        if rebuilt != existing:
            raise PilotOrchestrationError(
                "V2.11.2 resumed post-gate differs from sealed sources"
            )
        return path, existing

    inputs = _v2112_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    draft, draft_statuses = _build_v2112_post_gate_with_inferred_statuses(inputs)
    science_specs = tuple(
        spec
        for stage_id in (
            "experiment-c",
    ...
    if draft.get("go") is not True:
        run_ledger.stop_pending(
            ...
            failure={
                "error_type": "V2112PostGateNoGo",
                "message": (
                    "V2.11.2 global fresh-preflight gate did not authorize "
                    "scientific dispatch"
                ),
                "source_stage": PILOT_V211_POST_GATE_STAGE_ID,
                "reasons": list(draft.get("reasons", [])),
    ...
    final_inputs = _v2112_post_gate_inputs(
        contract,
        raw_root=raw_root,
        paid=paid,
        budget_ledger=budget_ledger,
    ...
    receipt, final_statuses = _build_v2112_post_gate_with_inferred_statuses(
        final_inputs
    )
    if final_statuses != draft_statuses:
        raise PilotOrchestrationError(
            "V2.11.2 model decisions changed while sealing the global gate"
        )
    try:
        verify_v2112_gate_receipt(
            receipt,
        ...
        raise PilotOrchestrationError(
            f"V2.11.2 final post-gate authority failed: {exc}"
        ) from exc
    _persist_exact_json(path, receipt)
    return path, receipt
...
def _capability_source_stage(
    ...
    source_modes = {"capability_probe", "capability_authority_import"}
    candidates = [
        prerequisite
        for prerequisite in contract.stage(preflight_stage).prerequisites
        if _stage_execution_modes(contract, prerequisite) & source_modes
...
def _preflight_stage_for_model(
    ...
    split = [
        ...
        and _stage_execution_modes(contract, stage_id)
        & {"closed_loop_preflight", "preflight_authority_import"}
    ]
    if len(split) == 1:
        return split[0]
    if split:
        raise PilotOrchestrationError(
            f"model {model_id} has multiple projection-producing preflight stages"
        )
    combined = [
        stage_id
        for stage_id in contract.stage_ids
...
def _load_v2_terminal_summary(
    ...
    *,
    ...
    if (
        provenance.get("git_tag") != contract.implementation["required_git_tag"]
...
def _v2_capability_semantic_go(
    ...
    *,
    ...
    artifact_value = _read_json(
        Path(_v2_source_file_binding(raw_root, artifact)["path"])
    )
    ...
    if (
        spec.execution_mode == "preflight_authority_import"
        and contract.contract_id == V2114_CONTRACT_ID
    ):
        if paid is None:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 preflight import lacks provenance"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        wrapper = payload.get("preflight_authority")
        resealed = payload.get("resealed_authority")
        if not isinstance(wrapper, Mapping) or not isinstance(resealed, Mapping):
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 preflight authority is malformed"
            )
        try:
            parent = verify_v2114_parent_import_receipt(
                raw_root / "parent-import" / "parent_import_receipt.json",
                repo_root=release_root,
                raw_root=raw_root,
                contract=contract,
            ...
            expected_wrapper = preflight_wrappers_from_v2114_receipt(
                parent,
                contract=contract,
                expected_git_commit=paid.head_commit,
                repo_root=release_root,
            )[spec.model_id]
            authority_path = v2114_observed_p95_receipt_path(raw_root, spec.model_id)
            projection_path = v2114_observed_p95_projection_path(
                raw_root, spec.model_id
            )
            ...
            projection = verified_v2114_observed_p95_projection_binding(
                projection_path,
        ...
        except (PilotV2114ParentImportError, PilotV2114GateError) as exc:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 preflight replay failed: {exc}"
            ) from exc
        expected_gate = _verify_v2114_preflight_import_gate(
            gate,
            contract=contract,
            spec=spec,
            authority_receipt=authority_path,
            projection_path=projection_path,
        )
        wrapper_path = run_dir / "preflight_authority.json"
        ...
        semantic_go = (
            ...
            and _read_json(wrapper_path) == wrapper
            and resealed
            == {
                "receipt": str(authority_path),
                "projection": str(projection_path),
                "receipt_content_sha256": authority["receipt_content_sha256"],
                "projection_content_sha256": projection["projection_content_sha256"],
            }
            and set(authority.get("reservations", {})) == {runtime_model}
            and projection.get("reservations") == authority["reservations"]
            and summary.get("scientific_evidence") is False
            and summary.get("evidence_scope") == "preregistered_capability_gate"
        )
        if (status == "complete") is not semantic_go:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 preflight status differs from gate"
            )
        return semantic_go
    if (
        spec.execution_mode == "preflight_authority_import"
        and contract.contract_id == V2113_CONTRACT_ID
    ):
        if paid is None:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 preflight import lacks provenance"
            )
        release_root = (
            Path(__file__).resolve().parents[1]
            if authority_repo_root is None
        ...
        if not isinstance(wrapper, Mapping) or not isinstance(resealed, Mapping):
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 preflight authority is malformed"
            )
        try:
            parent = verify_v2113_parent_import_receipt(
                raw_root / "parent-import" / "parent_import_receipt.json",
                repo_root=release_root,
                raw_root=raw_root,
                contract=contract,
            ...
            expected_wrapper = preflight_wrappers_from_v2113_receipt(
                parent,
                contract=contract,
                expected_git_commit=paid.head_commit,
                repo_root=release_root,
            )[spec.model_id]
            authority_path = v2113_observed_p95_receipt_path(raw_root, spec.model_id)
            projection_path = v2113_observed_p95_projection_path(
                raw_root, spec.model_id
            )
            authority = verified_v2113_observed_p95_authority_binding(
                authority_path,
                repo_root=release_root,
                raw_root=raw_root,
                expected_git_commit=paid.head_commit,
            ...
            projection = verified_v2113_observed_p95_projection_binding(
                projection_path,
                receipt_path=authority_path,
                repo_root=release_root,
                raw_root=raw_root,
        ...
        except (PilotV2113ParentImportError, PilotV2113GateError) as exc:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 preflight replay failed: {exc}"
            ) from exc
        expected_gate = _verify_v2113_preflight_import_gate(
            gate,
            contract=contract,
            spec=spec,
            authority_receipt=authority_path,
        ...
        if (status == "complete") is not semantic_go:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 preflight status differs from gate"
            )
        return semantic_go
    capability = payload.get("capability")
    if not isinstance(capability, Mapping):
        raise PilotOrchestrationError(f"{spec.run_id} capability evidence is malformed")
    if (
        spec.execution_mode == "capability_authority_import"
        and contract.contract_id == V2114_CONTRACT_ID
    ):
        if paid is None:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability import lacks provenance"
            )
        if capability.get("schema_version") != V2114_CAPABILITY_WRAPPER_SCHEMA_VERSION:
            raise PilotOrchestrationError(
                f"{spec.run_id} has the wrong V2.11.4 wrapper schema"
            )
        try:
            release_root = (
                Path(__file__).resolve().parents[1]
            ...
            parent_receipt = verify_v2114_parent_import_receipt(
                raw_root / "parent-import" / "parent_import_receipt.json",
                repo_root=release_root,
                raw_root=raw_root,
                contract=contract,
            ...
            expected_wrapper = capability_wrappers_from_v2114_receipt(
                parent_receipt,
                contract=contract,
                expected_git_commit=paid.head_commit,
                repo_root=release_root,
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability replay failed: {exc}"
            ) from exc
        imported = capability.get("capability")
        semantic_go = (
            capability == expected_wrapper
            and isinstance(imported, Mapping)
            and imported.get("capability_pass") is True
            and imported.get("interface_pass") is True
        )
        expected_gate = {
            "capability_pass": semantic_go,
            "capability_status": ("imported-pass" if semantic_go else "imported-no-go"),
            "go": semantic_go,
            "historical_source_calls": 30,
            "provider_construction_current_attempt": False,
            "provider_calls_current_attempt": 0,
            "interface_pass": semantic_go,
            "preflight_run": None,
            "reason": None if semantic_go else "inherited-capability-no-go",
            "imported_effect_cells": 0,
        }
        if (
            gate != expected_gate
            or _read_json(run_dir / "capability.json") != capability
            or summary.get("scientific_evidence") is not False
            or summary.get("evidence_scope") != "preregistered_task_capability_gate"
        ):
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability terminal drifted"
            )
        if (status == "complete") is not semantic_go:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability status differs from gate"
            )
        return semantic_go
    if (
        spec.execution_mode == "capability_authority_import"
        ...
        if paid is None:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability import lacks provenance"
            )
        if capability.get("schema_version") != V2113_CAPABILITY_WRAPPER_SCHEMA_VERSION:
            raise PilotOrchestrationError(
                f"{spec.run_id} has the wrong V2.11.3 wrapper schema"
            )
        try:
            release_root = (
                Path(__file__).resolve().parents[1]
            ...
            parent_receipt = verify_v2113_parent_import_receipt(
                raw_root / "parent-import" / "parent_import_receipt.json",
                repo_root=release_root,
                raw_root=raw_root,
                contract=contract,
            ...
            expected_wrapper = capability_wrappers_from_v2113_receipt(
                parent_receipt,
                contract=contract,
                expected_git_commit=paid.head_commit,
                repo_root=release_root,
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability replay failed: {exc}"
            ) from exc
        imported = capability.get("capability")
        semantic_go = (
            capability == expected_wrapper
        ...
        if (
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability terminal drifted"
            )
        if (status == "complete") is not semantic_go:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability status differs from gate"
            )
        return semantic_go
    if spec.execution_mode == "capability_authority_import":
        if paid is None or contract.contract_id not in {
            V2111_CONTRACT_ID,
            V2112_CONTRACT_ID,
        }:
            raise PilotOrchestrationError(
                f"{spec.run_id} is not a valid V2.11-family capability import"
            )
        if contract.contract_id == V2112_CONTRACT_ID:
            if (
            ...
            try:
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.2 capability replay failed: {exc}"
                ) from exc
            bootstrap_filename = V2112_BOOTSTRAP_PROJECTION_FILENAME
            additional_gate = {
                "historical_failed_preflight_calls_used": 0,
            }
        else:
            if (
                capability.get("schema_version")
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} has the wrong V2.11.1 wrapper schema"
                )
            try:
                parent_receipt = verify_v2111_parent_import_receipt(
                    raw_root / "parent-import" / "parent_import_receipt.json",
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} inherited capability replay failed: {exc}"
                ) from exc
            bootstrap_filename = V2111_BOOTSTRAP_PROJECTION_FILENAME
            additional_gate = {}
        imported = capability.get("capability")
        semantic_go = (
            capability == expected_wrapper
        ...
        expected_gate = {
            ...
            **additional_gate,
            ...
            "preflight_run": None,
            "reason": None if semantic_go else "inherited-capability-no-go",
            "bootstrap_projection": str(run_dir / bootstrap_filename),
        }
        if (
            gate != expected_gate
            or _read_json(run_dir / "capability.json") != capability
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} imported capability terminal drifted"
            )
        if (status == "complete") is not semantic_go:
            raise PilotOrchestrationError(
                f"{spec.run_id} imported capability status differs from gate"
            )
        return semantic_go
    try:
        ...
        if capability_schema == CAPABILITY_V3_SCHEMA_VERSION:
        ...
        elif capability_schema == CAPABILITY_V5_SCHEMA_VERSION:
            ...
            capability_schema == V2112_CAPABILITY_WRAPPER_SCHEMA_VERSION
            and contract.contract_id == V2112_CONTRACT_ID
            and spec.execution_mode == "closed_loop_preflight"
        ):
            base_wrapper = {
                key: value
                for key, value in capability.items()
                if key not in {"preflight_go", "preflight_checks"}
            }
            parent_receipt = verify_v2112_parent_import_receipt(
                raw_root / "parent-import" / "parent_import_receipt.json",
                repo_root=(
            ...
            if base_wrapper != expected_wrapper:
                raise PilotV2112ParentImportError(
                    "preflight capability wrapper differs from parent replay"
                )
        elif (
            capability_schema == V2111_CAPABILITY_WRAPPER_SCHEMA_VERSION
            and contract.contract_id == V2111_CONTRACT_ID
            and spec.execution_mode == "closed_loop_preflight"
        ):
            base_wrapper = {
                key: value
    ...
    if spec.execution_mode == "capability_probe":
        ...
        if contract.preflight_bootstrap_amendment is not None and semantic_go:
            ...
            bootstrap_target, bootstrap_config_sha256 = _bootstrap_target_context(
                contract,
                spec,
                paid=paid,
            )
            ...
            try:
                validate_capability_bootstrap_projection(
                    ...
                    git_commit=str(summary["provenance"]["resolved_git_commit"]),
                    authorized_config_sha256=bootstrap_config_sha256,
                )
            except PilotPreflightAmendmentError as exc:
                raise PilotOrchestrationError(
        ...
        if (
            ...
            or (
                ...
                and gate.get("bootstrap_projection") != str(expected_bootstrap_path)
            )
            or (expected_bootstrap_path is None and "bootstrap_projection" in gate)
        ):
            raise PilotOrchestrationError(
        ...
        checks = _preflight_checks(
            ...
            v211_long_context=(contract.contract_id in V211_FAMILY_CONTRACT_IDS),
        )
        if capability_schema in {
            V2111_CAPABILITY_WRAPPER_SCHEMA_VERSION,
            V2112_CAPABILITY_WRAPPER_SCHEMA_VERSION,
        }:
            imported_capability = capability.get("capability")
            capability_pass = (
                isinstance(imported_capability, Mapping)
                and imported_capability.get("capability_pass") is True
                and imported_capability.get("interface_pass") is True
            )
        else:
            capability_pass = capability.get("pass") is True
        semantic_go = capability_pass and all(checks.values())
        capability_stage = _capability_source_stage(
            contract,
            spec.stage_id,
            spec.model_id,
        ...
        elif contract.preflight_bootstrap_amendment is not None:
            expected_bootstrap_path = _capability_bootstrap_projection_path(
                raw_root=raw_root,
                spec=source_spec,
            )
        ...
        if (
            ...
            or gate.get("preflight_checkpoint_exactness") != str(exactness_path)
            or (
                expected_bootstrap_path is not None
                and gate.get("bootstrap_projection") != str(expected_bootstrap_path)
            )
    ...
    *,
    ...
    if stage_id == "q-ref-resolution":
        ...
        for path in stage_root.rglob("*.json"):
            if path != stage_receipt and path.is_file() and not path.is_symlink():
                paths.add(path)
        ...
        if contract.contract_id in {
            ...
            paths.update(
                path
                for path in stage_root.glob("runs/*/imported_run_envelope.json")
                if path.is_file() and not path.is_symlink()
            )
            paths.update(
                path
                for path in stage_root.glob("summaries/*.json")
                if path.is_file() and not path.is_symlink()
            )
    ...
    if (
        ...
        preflight_control = preflight_amendment_control_path(raw_root=raw_root)
    ...
    if (
        ...
        if contract.contract_id in {V2113_CONTRACT_ID, V2114_CONTRACT_ID}:
            paths.update(
                path
                for path in (
                    raw_root / PILOT_V211_POST_GATE_STAGE_ID / "imported_observed_p95"
                ).glob("*/*.json")
                if path.is_file() and not path.is_symlink()
            )
        post_gate = _v211_post_gate_authority_path(raw_root)
        if post_gate.exists():
            paths.add(post_gate)
...
def _v2_stage_receipt_bindings(
    ...
    *,
    ...
    deduplicated = {str(item["path"]): item for item in source_files}
    sources = [deduplicated[path] for path in sorted(deduplicated)]
    go_models: list[str] = []
    missing_terminal_family_post_gate = (
        contract.contract_id in V211_FAMILY_CONTRACT_IDS
        and stage_id == PILOT_V211_POST_GATE_STAGE_ID
        and not _v211_post_gate_authority_path(raw_root).exists()
        and all(row.get("status") in TERMINAL_RUN_STATUSES for row in rows.values())
    )
    if (
        _is_capability_stage(contract, stage_id)
        and not missing_terminal_family_post_gate
    ):
        for spec in specs:
    ...
    if (
        ...
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11-family post-gate model binding requires release provenance"
            )
        post_gate_path = _v211_post_gate_authority_path(raw_root)
        if post_gate_path.exists():
            post_gate = _verify_v211_family_post_gate_receipt(
                contract,
                _read_json(post_gate_path),
                paid=paid,
            )
            globally_eligible = [
                str(model_id)
                for model_id in post_gate["denominator"]["eligible_model_ids"]
            ]
            if not set(globally_eligible) <= set(go_models):
                raise PilotOrchestrationError(
                    "V2.11-family global gate authorized a locally " "ineligible model"
                )
            go_models = globally_eligible
        elif all(row.get("status") in TERMINAL_RUN_STATUSES for row in rows.values()):
            # A provider, budget, or integrity failure can terminalize the
            # entire gate before a post-gate authority is sealable.  The
            # failure receipt must remain publishable and must authorize no
            # model; trying to read the necessarily absent authority here
            # would mask the primary failure with a secondary missing-file
            # error.
            go_models = []
        else:
            raise PilotOrchestrationError(
                "V2.11-family nonterminal post-gate stage lacks its authority"
            )
    ...
    bindings = {
        ...
        "ledger_event_chain_head": events[-1]["event_sha256"],
        "stage_specs_sha256": canonical_sha256([spec.to_dict() for spec in specs]),
...
def _v2_control_gate_ok(
    ...
    *,
    ...
    if (
        ...
        if paid is None:
            raise PilotOrchestrationError(
                "V2.11-family post-gate control requires release provenance"
            )
        path = _v211_post_gate_authority_path(raw_root)
        if not path.exists():
            return False
        receipt = _verify_v211_family_post_gate_receipt(
            contract,
            _read_json(path),
            paid=paid,
        )
        eligible = set(receipt["denominator"]["eligible_model_ids"])
        local_go: set[str] = set()
        for spec in contract.expand(stage=stage_id):
            gate_path = raw_root / stage_id / "runs" / spec.run_id / "gate_receipt.json"
            if gate_path.exists() and _read_json(gate_path).get("go") is True:
                local_go.add(spec.model_id)
        if not eligible <= local_go:
            raise PilotOrchestrationError(
                "V2.11-family global gate authorized a locally " "ineligible model"
            )
        run_snapshot = _read_json(raw_root / "run_ledger.json")
        events = run_snapshot.get("events")
        bound_head = receipt["bindings"]["ledger_event_chain_head"]
        if not isinstance(events, list) or bound_head not in {
            event.get("event_sha256") for event in events if isinstance(event, Mapping)
        }:
            raise PilotOrchestrationError(
                "V2.11-family post-gate ledger event-chain prefix is absent"
            )
        return receipt.get("go") is True
    if stage_id == "parent-import":
        ...
        if contract.contract_id == V2114_CONTRACT_ID:
        ...
        elif contract.contract_id == V2113_CONTRACT_ID:
            _load_verified_v2113_calibration(
                contract,
                raw_root=raw_root,
                paid=paid,
        ...
        elif contract.contract_id == V211_CONTRACT_ID:
            _load_verified_v211_calibration(
                contract,
                raw_root=raw_root,
            )
        elif contract.contract_id == V2102_CONTRACT_ID:
            try:
                verify_v2102_parent_import_receipt(
                    receipt_path=path,
                ...
                raise PilotOrchestrationError(
                    f"V2.10.2 parent import receipt failed validation: {exc}"
                ) from exc
        elif contract.contract_id == V2101_CONTRACT_ID:
            try:
                verify_v2101_parent_import_receipt(
                ...
                raise PilotOrchestrationError(
                    f"V2.10.1 parent import receipt failed validation: {exc}"
                ) from exc
        elif contract.contract_id == V210_CONTRACT_ID:
            try:
                verify_v210_parent_import_receipt(
                ...
                raise PilotOrchestrationError(
                    f"V2.10 parent import receipt failed validation: {exc}"
                ) from exc
        elif contract.contract_id == V29_CONTRACT_ID:
            try:
                verify_v29_parent_import_receipt(
            ...
            try:
                ...
                raise PilotOrchestrationError(
                    f"V2.7 parent import receipt failed validation: {exc}"
                ) from exc
        elif contract.contract_id == V26_CONTRACT_ID:
            try:
                verify_v26_parent_import_receipt(
    ...
    if stage_id == "q-ref-resolution":
        try:
            value = _load_verified_q_ref(
                ...
                raw_root=raw_root,
                paid=paid,
                authority_repo_root=release_repo_root,
            )
...
def _v2_recomputed_stage_fields(
    ...
    *,
    ...
    go = (
        ...
        and control_go
        and (
            not v211_post_gate_stage
            or set(go_models) == set(contract.models_for_stage(stage_id))
        )
    )
    hard_stop_count = sum(
    ...
    execution_progression_go = (
        (terminal and hard_stop_count == 0 and control_go and bool(go_models))
        if v211_post_gate_stage
        else (
            terminal
            and hard_stop_count == 0
            and (
                _is_core_stage(stage_id)
                or stage_id == "cross-model-sentinels"
                or (_is_capability_stage(contract, stage_id) and bool(go_models))
                or go
            )
        )
    )
...
def _verify_v2_stage_receipt(
    ...
    *,
    ...
    if (
        fields["hard_stop_cell_count"] == 0
        and receipt.get("status") != fields["semantic_status"]
    ):
        raise PilotOrchestrationError(
    ...
    *,
    ...
    for prerequisite in stage.prerequisites:
        ...
        if v2 and _is_capability_stage(contract, prerequisite):
            ...
            if (
                contract.contract_id in V211_FAMILY_CONTRACT_IDS
                and prerequisite == PILOT_V211_POST_GATE_STAGE_ID
            ):
                target_specs = tuple(
                    spec
                    for spec in contract.expand(stage=stage_id)
                    if not relevant_models or spec.model_id in relevant_models
                )
                target_already_terminal = bool(target_specs) and all(
                    ledger is not None and ledger.is_terminal(spec.run_id)
                    for spec in target_specs
                )
                if receipt.get("execution_progression_go") is not True:
                    if not target_already_terminal:
                        raise PilotOrchestrationError(
                            "V2.11-family post-gate authority is a global "
                            "execution no-go"
                        )
                    continue
                if (
                    ...
                    and not target_already_terminal
                ):
                    raise PilotOrchestrationError(
                        f"V2.11-family post-gate authority has no eligible model for "
                        f"{stage_id}"
                    )
                continue
    ...
    if contract.schema_version.endswith("-v2"):
        if output.exists():
            if output.is_symlink() or not output.is_file():
                raise PilotOrchestrationError(
                    f"V2 stage receipt must be a regular file: {stage_id}"
                )
            ...
            if (
                ...
                or (
                    ...
                    and existing.get("go_models") != declared_go_models
                )
                or existing.get("artifacts") != dict(artifacts or {})
                or existing.get("failure") != (dict(failure) if failure else None)
                or existing.get("diagnostic_only") is not diagnostic
                or existing.get("scientific_evidence")
                is not (False if diagnostic else None)
            ):
                raise PilotOrchestrationError(
                    f"immutable V2 stage receipt arguments drifted for {stage_id}"
                )
            return output
        ...
        if declared_go_models is not None and declared_go_models != fields["go_models"]:
            raise PilotOrchestrationError(
                f"V2 {stage_id} go_models differ from sealed source recomputation"
            )
        if fields["hard_stop_cell_count"] == 0 and status != fields["semantic_status"]:
            raise PilotOrchestrationError(
        ...
        payload = {
            ...
            "stage_id": stage_id,
            "status": status,
            **{key: value for key, value in fields.items() if key != "semantic_status"},
            "bindings": bindings,
...
def _experiment_c_sensitivity_path(
    ...
    if not _is_experiment_c_stage(stage_id):
        raise PilotOrchestrationError(f"{stage_id!r} is not an Experiment C stage")
    ...
    for spec in expected_specs:
        ...
        result = load_verified_run_artifacts(
            ...
            **(
                {}
                if authority_repo_root is None
                else {"authority_repo_root": authority_repo_root}
            ),
        )
        if (
...
def _d_narrative_causal_bindings(
    ...
    return {
        ...
        "narrative_pulse_contract": dict(narratives["narrative_pulse_contract"]),
...
def _d_group_projection(
    ...
    *,
    ...
    if contract.contract_id == V2114_CONTRACT_ID:
        ...
        build_v2113_experiment_d_group_plan(contract, group_specs)
    branch_arms = {
        "matched-a",
        "matched-b",
        "no-memory",
    ...
    continuation_count = sum(spec.arm_id in branch_arms for spec in group_specs)
    narrative_count = sum(spec.arm_id == "narrative-content" for spec in group_specs)
    if (
        not group_specs
    ...
    calls = {"action": 4 * 6 * (1 + continuation_count + narrative_count)}
    ...
    return RunProjection(
        ...
        cost_usd=totals["cost_usd"],
        completions=(sum(calls.values()) if hosted_completion_cap_counted else 0),
...
V2113_D_BRANCH_ARM_TO_TREATMENT = {
    "matched-a": "matched-a",
    "matched-b": "matched-b",
    "no-memory": "no-memory",
...
class V2113ExperimentDGroupPlan:
    """Pure exact V2.11.3 D dispatch plan shared by audit and execution."""

    representative: PilotRunSpec
    prefix_config: VerifiedRunConfig | None
    continuation_specs: Mapping[str, PilotRunSpec]
    narrative_specs: Mapping[str, PilotRunSpec]

    @property
    def registered_treatments(self) -> tuple[str, ...]:
        return tuple(DEFAULT_TREATMENTS)

    def to_receipt(self) -> dict[str, Any]:
        specs = tuple(self.continuation_specs.values()) + tuple(
            self.narrative_specs.values()
        )
        return {
            "stage_id": self.representative.stage_id,
            "model_id": self.representative.model_id,
            "environment_seed": self.representative.environment_seed,
            "cell_count": len(specs),
            "continuation_arms": sorted(
                spec.arm_id for spec in self.continuation_specs.values()
            ),
            "treatments": list(self.registered_treatments),
            "narrative_ids": sorted(self.narrative_specs),
            "run_ids_sha256": canonical_sha256(sorted(spec.run_id for spec in specs)),
            "prefix_config_sha256": (
                None
                if self.prefix_config is None
                else canonical_sha256(self.prefix_config.to_dict())
            ),
        }


def build_v2113_experiment_d_group_plan(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    base_config: VerifiedRunConfig | None = None,
) -> V2113ExperimentDGroupPlan:
    """Require one exact 7-continuation + 4-narrative V2.11.3 D group."""

    entries = tuple(specs)
    if contract.contract_id != V2113_CONTRACT_ID or len(entries) != 11:
        raise PilotOrchestrationError(
            "V2.11.3 Experiment D requires exactly eleven registered cells"
        )
    stage_ids = {spec.stage_id for spec in entries}
    model_ids = {spec.model_id for spec in entries}
    seeds = {spec.environment_seed for spec in entries}
    run_ids = {spec.run_id for spec in entries}
    if (
        stage_ids != {"experiment-d"}
        or model_ids != {"gpt52_main"}
        or len(seeds) != 1
        or len(run_ids) != len(entries)
        or any(spec.execution_mode != "checkpoint_continuation" for spec in entries)
    ):
        raise PilotOrchestrationError(
            "V2.11.3 Experiment D group identity or run denominator drifted"
        )

    continuation_by_arm: dict[str, PilotRunSpec] = {}
    narrative_specs: dict[str, PilotRunSpec] = {}
    for spec in entries:
        if spec.arm_id in V2113_D_BRANCH_ARM_TO_TREATMENT:
            if spec.arm_id in continuation_by_arm or spec.narrative_id != "none":
                raise PilotOrchestrationError(
                    "V2.11.3 Experiment D continuation arm is duplicated or narrated"
                )
            continuation_by_arm[spec.arm_id] = spec
        elif spec.arm_id == "narrative-content":
            narrative_id = str(spec.narrative_id)
            if narrative_id in narrative_specs:
                raise PilotOrchestrationError(
                    "V2.11.3 Experiment D narrative cell is duplicated"
                )
            narrative_specs[narrative_id] = spec
        else:
            raise PilotOrchestrationError(
                "V2.11.3 Experiment D contains an unregistered arm"
            )
    if set(continuation_by_arm) != set(V2113_D_BRANCH_ARM_TO_TREATMENT):
        raise PilotOrchestrationError(
            "V2.11.3 Experiment D continuation-arm denominator drifted"
        )
    if (
        set(narrative_specs) != set(DEFAULT_NARRATIVES)
        or {key: row["text"] for key, row in contract.narratives.items()}
        != DEFAULT_NARRATIVES
        or contract.stop_go["experiment_d"].get("narrative_fixture_hash")
        != canonical_sha256(DEFAULT_NARRATIVES)
    ):
        raise PilotOrchestrationError(
            "V2.11.3 Experiment D narrative denominator or fixture drifted"
        )

    representative = continuation_by_arm["matched-a"]
    prefix_config = None
    if base_config is not None:
        if (
            base_config.run_id != representative.run_id
            or base_config.seed != representative.environment_seed
            or base_config.num_agents != 4
            or base_config.episode_length != 12
        ):
            raise PilotOrchestrationError(
                "V2.11.3 Experiment D base runner config identity drifted"
            )
        action_grid = contract.stop_go["experiment_d"]["action_grid"]
        if not math.isclose(
            float(base_config.labor_step),
            float(action_grid["labor_step_hours"]),
            rel_tol=0.0,
            abs_tol=0.0,
        ) or not math.isclose(
            float(base_config.consumption_step),
            float(action_grid["consumption_step"]),
            rel_tol=0.0,
            abs_tol=0.0,
        ):
            raise PilotOrchestrationError(
                "V2.11.3 Experiment D runner action grid differs from frozen bins"
            )
        group_id = (
    ...
    continuation_specs = {
        V2113_D_BRANCH_ARM_TO_TREATMENT[arm_id]: continuation_by_arm[arm_id]
        for arm_id in V2113_D_BRANCH_ARM_TO_TREATMENT
    }
    return V2113ExperimentDGroupPlan(
        representative=representative,
        prefix_config=prefix_config,
        continuation_specs=continuation_specs,
        narrative_specs=narrative_specs,
    )


V2114_D_BRANCH_ARM_TO_TREATMENT = {
    "matched-a": "matched-a",
    "matched-b": "matched-b",
    "no-memory": "no-memory",
    "shuffled-episodic": "shuffled-episodic",
...
@dataclass(frozen=True)
class V2114ExperimentDGroupPlan:
    """Pure exact V2.11.4 D dispatch plan shared by audit and execution."""
...
def build_v2114_experiment_d_group_plan(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    base_config: VerifiedRunConfig | None = None,
) -> V2114ExperimentDGroupPlan:
    """Require one exact 7-continuation + 4-narrative V2.11.4 D group."""

    entries = tuple(specs)
    if contract.contract_id != V2114_CONTRACT_ID or len(entries) != 11:
        raise PilotOrchestrationError(
            "V2.11.4 Experiment D requires exactly eleven registered cells"
        )
    stage_ids = {spec.stage_id for spec in entries}
    model_ids = {spec.model_id for spec in entries}
    seeds = {spec.environment_seed for spec in entries}
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.4 Experiment D group identity or run denominator drifted"
        )

    continuation_by_arm: dict[str, PilotRunSpec] = {}
    narrative_specs: dict[str, PilotRunSpec] = {}
    for spec in entries:
        if spec.arm_id in V2114_D_BRANCH_ARM_TO_TREATMENT:
            if spec.arm_id in continuation_by_arm or spec.narrative_id != "none":
                raise PilotOrchestrationError(
                    "V2.11.4 Experiment D continuation arm is duplicated or narrated"
                )
            continuation_by_arm[spec.arm_id] = spec
        elif spec.arm_id == "narrative-content":
            narrative_id = str(spec.narrative_id)
            if narrative_id in narrative_specs:
                raise PilotOrchestrationError(
                    "V2.11.4 Experiment D narrative cell is duplicated"
                )
            narrative_specs[narrative_id] = spec
        else:
            raise PilotOrchestrationError(
                "V2.11.4 Experiment D contains an unregistered arm"
            )
    if set(continuation_by_arm) != set(V2114_D_BRANCH_ARM_TO_TREATMENT):
        raise PilotOrchestrationError(
            "V2.11.4 Experiment D continuation-arm denominator drifted"
        )
    if (
        set(narrative_specs) != set(DEFAULT_NARRATIVES)
        or {key: row["text"] for key, row in contract.narratives.items()}
        ...
        raise PilotOrchestrationError(
            "V2.11.4 Experiment D narrative denominator or fixture drifted"
        )
    ...
    if base_config is not None:
        if (
            ...
            raise PilotOrchestrationError(
                "V2.11.4 Experiment D base runner config identity drifted"
            )
        action_grid = contract.stop_go["experiment_d"]["action_grid"]
        if not math.isclose(
            float(base_config.labor_step),
            ...
            raise PilotOrchestrationError(
                "V2.11.4 Experiment D runner action grid differs from frozen bins"
            )
        group_id = (
            f"{contract.contract_id}--experiment-d--gpt52_main--"
            f"checkpoint-group--s{representative.environment_seed}"
    ...
    continuation_specs = {
        V2114_D_BRANCH_ARM_TO_TREATMENT[arm_id]: continuation_by_arm[arm_id]
        for arm_id in V2114_D_BRANCH_ARM_TO_TREATMENT
    }
...
def _demote_d_publication(
    *,
    ...
    if len(stage_ids) != 1:
        raise PilotOrchestrationError("Experiment D demotion requires one exact stage")
    stage_id = next(iter(stage_ids))
    for spec in specs:
        summary = raw_root / stage_id / "summaries" / f"{spec.run_id}.json"
...
def _execute_d_seed(
    ...
    *,
    ...
    reserve_path = group_dir / D_PUBLICATION_FAILURE_RESERVE_FILENAME
    if all(run_ledger.is_terminal(spec.run_id) for spec in specs):
        if any(run_ledger.status(spec.run_id) != "complete" for spec in specs):
    ...
    projection = (
        RunProjection(
            run_id=(f"development-fake-{stage_id}-{model_id}-d-s{seed}"),
            stage_bucket=representative.budget_bucket,
            cost_usd=0.0,
            completions=0,
            storage_bytes=80_000_000,
    ...
    if existing is not None:
        ...
        nonterminal_specs = tuple(
            spec for spec in specs if not run_ledger.is_terminal(spec.run_id)
        )
    ...
    try:
        ...
        if contract.contract_id == V2114_CONTRACT_ID:
            exact_plan = build_v2114_experiment_d_group_plan(
                contract,
                specs,
                base_config=base,
            )
            ...
            exact_plan = build_v2113_experiment_d_group_plan(
                contract,
                specs,
                base_config=base,
            )
            ...
            narrative_specs = dict(exact_plan.narrative_specs)
            registered_treatments = exact_plan.registered_treatments
            branch_map = dict(V2113_D_BRANCH_ARM_TO_TREATMENT)
        else:
            ...
            if set(continuation_specs) != set(registered_treatments) or len(
                continuation_specs
            ) + len(narrative_specs) != len(specs):
                raise PilotOrchestrationError(
        ...
        d_journal_paths = tuple(Path(journal_target(spec)["path"]) for spec in specs)
        ...
        def terminal_payload(spec: PilotRunSpec) -> dict[str, Any]:
            if spec.arm_id == "narrative-content":
                ...
                return {
                    ...
                    "provider_call_journal": narrative_branch["provider_call_journal"],
                }
        ...
        if scientific_run:
            ...
            for spec in specs:
                artifact_path = (
                    raw_root / stage_id / "diagnostic_summaries" / f"{spec.run_id}.json"
                )
                _atomic_json(
                    artifact_path,
                    {
            ...
            if scientific_run:
                ...
                for spec in specs:
                    completed_artifacts[spec.run_id] = write_terminal_summary(
                        raw_root / stage_id / "summaries" / f"{spec.run_id}.json",
                        contract=contract,
                        run_spec=spec,
                        resolved_git_commit=paid.head_commit,
                        git_tag=paid.git_tag,
            ...
            [
                {
                    ...
                    "artifact": (
                        ...
                        if ledger_status == "complete"
                            ...
                            str(reconciliation_failure_receipt)
        ...
        if budget_reconciled:
            ...
            failure_receipt = _write_execution_failure_receipt(
                group_dir / "failure_receipt",
                scope=(f"finevo-pilot/{stage_id}/" "post-reconciliation-publication"),
                error=exc,
                contract=contract,
                projection=projection,
                budget=budget,
            ...
            if post_failure_actual["storage_bytes"] > actual["storage_bytes"]:
                raise PilotOrchestrationError(
        ...
        actual = _actual_budget_values(
            group_dir,
            budget,
            additional_paths=tuple(path for path in d_journal_paths if path.exists()),
            completion_cap_counted=bool(
                projection.basis.get(
                    "hosted_completion_cap_counted",
                    True,
    ...
    *,
    ...
    for stage_id in core_stage_ids:
        ...
        for model_id, seed in groups:
            ...
            if all(run_ledger.is_terminal(spec.run_id) for spec in group_specs):
                continue
            representative = next(
...
def _reconcile_v27_qref_no_go_budget(
    contract: PilotContract,
    spec: PilotRunSpec,
    *,
    raw_root: Path,
    ...
    """Resume only the missing budget finalization for a sealed q-ref no-go."""
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.7 q-ref no-go resume lacks its exact terminal receipt chain"
        )
    if any(
        not run_ledger.is_terminal(candidate.run_id) for candidate in contract.expand()
    ):
        raise PilotOrchestrationError(
            "V2.7 q-ref no-go resume has an unterminated descendant"
        )
    ...
    if (
        ...
        or budget_row.get("status") not in {"reserved", "integrity-stopped"}
    ):
        raise PilotOrchestrationError(
            "V2.7 q-ref no-go budget row is not safely reconcilable"
        )
    budget_ledger.finalize(
        spec.run_id,
        status="integrity-stopped",
        cost_usd=0.0,
        completions=0,
        storage_bytes=_directory_size(raw_root / "q-ref-resolution"),
        failure=failure,
    )
    return _read_json(receipt)


def _execute_v27_q_ref_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if (
        ...
        or len(specs) != 1
        or specs[0].stage_id != "q-ref-resolution"
        or specs[0].execution_mode != "q_ref_resolution"
    ):
        raise PilotOrchestrationError(
            "V2.7 q-ref import requires one exact zero-provider contract cell"
        )
    spec = specs[0]
    projection = RunProjection(
        run_id=spec.run_id,
        ...
        storage_bytes=5_000_000,
        basis={
            "method": "v2.7-imported-qref-reseal",
            "provider_calls": 0,
            "hosted_completion_cap_counted": False,
        },
    )
    ...
    if run_ledger.is_terminal(spec.run_id):
        if existing_budget is None:
            raise PilotOrchestrationError(
                "V2.7 q-ref is terminal without an own-stage budget "
                "reservation; upstream no-go has no dispatch authority"
            )
        if existing_budget.get("reservation") != projection.to_dict():
            raise PilotOrchestrationError("V2.7 q-ref budget reservation drifted")
        if run_ledger.status(spec.run_id) != "complete":
            return _reconcile_v27_qref_no_go_budget(
                contract,
                spec,
                raw_root=raw_root,
                paid=paid,
    ...
    elif existing_budget is None:
        budget_ledger.reserve(projection)
    elif existing_budget.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError("V2.7 q-ref budget reservation drifted")
    try:
        _assert_prerequisites(
            contract,
        ...
        resolution = _expected_v27_q_ref_resolution(
            contract,
            raw_root=raw_root,
            paid=paid,
        )
        resolution_path = raw_root / "q-ref-resolution" / "q_ref_resolution.json"
        _persist_exact_json(resolution_path, resolution)
        terminal_path = (
            raw_root / "q-ref-resolution" / "summaries" / f"{spec.run_id}.json"
        )
        terminal_payload = {
            "metrics": {"q_ref": resolution["q_ref"]},
            "gate_evidence": {
                "status": resolution["status"],
                "q_ref_resolution": {
                    "path": str(resolution_path),
                    "file_sha256": _file_sha256(resolution_path),
                    "content_sha256": resolution["integrity"]["content_sha256"],
                },
                "provider_calls_current_attempt": 0,
            },
            "q_ref_resolution": {
                ...
                "row_count": resolution["row_count"],
                "source_manifest": str(
                    Path(resolution["source_import"]["snapshot_run_root"])
                    / "manifest.json"
                ),
                "source_manifest_sha256": resolution["source_import"][
                    "source_artifacts"
                ]["manifest"]["file_sha256"],
                "resolution_artifact": str(resolution_path),
            },
            "provider_calls": 0,
            "claim_boundary": (
                "Deterministic imported q_ref scale resolution only; no "
                "treatment-effect or model-performance evidence."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if (
                ...
                or terminal.get("scientific_evidence") is not False
                or terminal.get("diagnostic_only") is not False
                or terminal.get("evidence_scope") != "preregistered_q_ref_resolution"
            ):
                raise PilotOrchestrationError("V2.7 q-ref terminal differs on resume")
        else:
            write_terminal_summary(
                terminal_path,
                contract=contract,
        ...
        if run_ledger.is_terminal(spec.run_id):
            if run_ledger.status(spec.run_id) != "complete":
                raise PilotOrchestrationError(
                    "V2.7 q-ref cell is already terminal with no progression authority"
                )
            row = run_ledger.snapshot()["runs"][spec.run_id]
            if row.get("artifact") != str(terminal_path):
                raise PilotOrchestrationError(
                    "V2.7 q-ref ledger artifact differs on resume"
                )
        else:
            run_ledger.finalize(
                spec.run_id,
        ...
        receipt = _write_stage_receipt(
            ...
            status="complete",
            artifacts={
                "q_ref_resolution": str(resolution_path),
                "provider_calls_current_attempt": 0,
            },
            paid=paid,
        )
        ...
        budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
        if isinstance(budget_row, Mapping) and budget_row.get("status") != "complete":
            budget_ledger.finalize(
                spec.run_id,
                status="integrity-stopped",
                cost_usd=0.0,
        ...
        raise PilotOrchestrationError(
            f"V2.7 q-ref import failed; receipt={receipt}"
        ) from exc


def _execute_v210_family_q_ref_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Import/reseal one frozen V2.9 q-ref prerequisite with zero calls."""
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "current-release q-ref import requires one exact zero-provider contract cell"
        )
    label = {
        V210_CONTRACT_ID: "V2.10",
        V2101_CONTRACT_ID: "V2.10.1",
    ...
    projection = RunProjection(
        ...
        storage_bytes=5_000_000,
        basis={
            "method": f"{label.lower()}-exact-v2.9-qref-import-reseal",
            "provider_calls": 0,
            "hosted_completion_cap_counted": False,
        },
    )
    ...
    if run_ledger.is_terminal(spec.run_id):
        if existing_budget is None:
            raise PilotOrchestrationError(
                f"{label} q-ref is terminal without an own-stage budget "
                "reservation; upstream no-go has no dispatch authority"
            )
        if existing_budget.get("reservation") != projection.to_dict():
            raise PilotOrchestrationError(f"{label} q-ref budget reservation drifted")
        if run_ledger.status(spec.run_id) != "complete":
            return _reconcile_v27_qref_no_go_budget(
                contract,
                spec,
    ...
    try:
        ...
        resolution = (
            _expected_v2102_q_ref_resolution(
                contract,
                raw_root=raw_root,
                paid=paid,
            )
            if contract.contract_id == V2102_CONTRACT_ID
                ...
                if contract.contract_id == V2101_CONTRACT_ID
                else _expected_v210_q_ref_resolution(
                    contract,
                    raw_root=raw_root,
                    paid=paid,
                )
        ...
        source_artifacts = source_import["source_artifacts"]
        terminal_payload = {
            "metrics": {"q_ref": resolution["q_ref"]},
            "gate_evidence": {
                "status": resolution["status"],
                "execution_disposition": (
                    (
                        ...
                        if contract.contract_id == V2102_CONTRACT_ID
                            ...
                            "immutable-v2.9-prerequisite-import-offline-v2.10.1-reseal"
                            if contract.contract_id == V2101_CONTRACT_ID
                            else "immutable-v2.9-prerequisite-import-offline-reseal"
                        )
                    )
                ),
                "q_ref_resolution": {
            ...
            "q_ref_resolution": {
                "q_ref": resolution["q_ref"],
                "row_count": resolution["row_count"],
                "source_resolution": source_artifacts["q_ref_resolution"][
                    "snapshot_path"
                ],
                "source_resolution_sha256": source_artifacts["q_ref_resolution"][
                    "file_sha256"
                ],
                "resolution_artifact": str(resolution_path),
            },
        ...
        if terminal_path.exists():
            ...
            if (
                ...
                raise PilotOrchestrationError(
                    f"{label} q-ref terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.is_terminal(spec.run_id):
            if run_ledger.status(spec.run_id) != "complete":
                raise PilotOrchestrationError(
                    f"{label} q-ref cell is terminal without progression authority"
                )
            row = run_ledger.snapshot()["runs"][spec.run_id]
            if row.get("artifact") != str(terminal_path):
                raise PilotOrchestrationError(
                    f"{label} q-ref ledger artifact differs on resume"
                )
        else:
            run_ledger.finalize(
                spec.run_id,
        ...
        if Counter(row["status"] for row in rows.values()) != {
            "complete": 2,
            "scheduled": 209,
        }:
            raise PilotOrchestrationError(
                f"{label} q-ref success must leave exactly two imported "
                "prerequisites complete and 209 fresh cells scheduled"
            )
        receipt = _write_stage_receipt(
            contract,
            "q-ref-resolution",
            ...
            artifacts={
                "q_ref_resolution": str(resolution_path),
                "source_run_id": source_import["source_run_id"],
                "provider_calls_current_attempt": 0,
            },
            paid=paid,
        )
        ...
        raise PilotOrchestrationError(
            f"{label} q-ref import failed; receipt={receipt}"
        ) from exc


def _execute_v210_q_ref_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if contract.contract_id != V210_CONTRACT_ID:
        raise PilotOrchestrationError("V2.10 q-ref executor used by another contract")
    return _execute_v210_family_q_ref_import_stage(
        contract,
        specs,
        raw_root=raw_root,
        paid=paid,
...
def _execute_v2101_q_ref_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if contract.contract_id != V2101_CONTRACT_ID:
        raise PilotOrchestrationError("V2.10.1 q-ref executor used by another contract")
    return _execute_v210_family_q_ref_import_stage(
        contract,
        specs,
        raw_root=raw_root,
...
def _execute_v2102_q_ref_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if contract.contract_id != V2102_CONTRACT_ID:
        raise PilotOrchestrationError("V2.10.2 q-ref executor used by another contract")
    return _execute_v210_family_q_ref_import_stage(
        contract,
        specs,
        raw_root=raw_root,
...
def _persist_v27_stage0_commit_intent(
    contract: PilotContract,
    *,
    raw_root: Path,
    paid: GitProvenance,
    cells: Sequence[tuple[PilotRunSpec, Path, Path, Mapping[str, Any]]],
    selection_path: Path,
    projections: Mapping[str, RunProjection],
    entry_completed_run_ids: Sequence[str],
) -> Path:
    ...
    intent_schema = {
        ...
        V210_CONTRACT_ID: PILOT_V210_STAGE0_COMMIT_INTENT_SCHEMA_VERSION,
        V2101_CONTRACT_ID: PILOT_V2101_STAGE0_COMMIT_INTENT_SCHEMA_VERSION,
        V2102_CONTRACT_ID: PILOT_V2102_STAGE0_COMMIT_INTENT_SCHEMA_VERSION,
    }[contract.contract_id]
    path = raw_root / "stage0-calibration" / "stage0_commit_intent.json"
    run_ids = {spec.run_id for spec, _, _, _ in cells}
    origin_entry = sorted(str(run_id) for run_id in entry_completed_run_ids)
    if path.exists():
        existing = _read_json(path)
        _verify_bound_payload(
            existing,
        ...
        if (
            not isinstance(existing_origin, list)
            or any(not isinstance(run_id, str) for run_id in existing_origin)
    ...
    value = _seal_bound_payload(
        {
            ...
            "entry_completed_run_ids": origin_entry,
            "planned_run_finalizations": sorted(
                [
                    {
                        "run_id": spec.run_id,
                        "status": "complete",
                        ...
                        "terminal_summary": str(terminal_path),
                        "terminal_summary_file_sha256": _file_sha256(terminal_path),
                        "budget_reservation": projections[spec.run_id].to_dict(),
                    }
                    for spec, envelope_path, terminal_path, _ in cells
                ],
                key=lambda row: row["run_id"],
            ),
            "selection": {
                ...
                "content_sha256": _read_json(selection_path)["integrity"][
                    "content_sha256"
                ],
            },
            "provider_calls_current_attempt": 0,
            "commit_protocol": {
                "run_ledger_group_finalization": "atomic-finalize-many",
                "budget_rows": "idempotent-per-run-finalization",
                "resume_required_after_partial_publication": True,
                "terminal_no_go_rewrite_forbidden_after_run_commit": True,
            },
            "bindings": {
                "contract_sha256": contract.canonical_hash,
                "git_tag": paid.git_tag,
...
def _reconcile_v27_stage0_no_go_budget(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Finish a Stage-0 no-go receipt/budget transaction without new work."""

    rows = run_ledger.snapshot()["runs"]
    stage_rows = [rows.get(spec.run_id) for spec in specs]
    if any(not isinstance(row, Mapping) for row in stage_rows):
        raise PilotOrchestrationError(
            "V2.7 Stage-0 no-go resume has a missing ledger row"
        )
    statuses = {str(row["status"]) for row in stage_rows}
    failures = [row.get("failure") for row in stage_rows if isinstance(row, Mapping)]
    if (
        len(statuses) != 1
        or next(iter(statuses)) not in {"integrity-stopped", "budget-stopped"}
        or any(row.get("artifact") is not None for row in stage_rows)
        or not failures
        or not isinstance(failures[0], Mapping)
        or any(failure != failures[0] for failure in failures)
        or any(
        ...
        raise PilotOrchestrationError(
            "V2.7 Stage-0 no-go ledger is not an exact resumable transaction"
        )
    terminal_status = next(iter(statuses))
    failure = dict(failures[0])
    budget_rows = budget_ledger.snapshot()["runs"]
    for spec in specs:
        budget_row = budget_rows.get(spec.run_id)
        if (
            not isinstance(budget_row, Mapping)
            or budget_row.get("reservation") != projections[spec.run_id].to_dict()
            or budget_row.get("status") not in {"reserved", terminal_status}
        ):
            raise PilotOrchestrationError(
                "V2.7 Stage-0 no-go budget rows are not safely reconcilable"
            )
    receipt = _write_stage_receipt(
        contract,
        "stage0-calibration",
    ...
    first_run_id = min(projections)
    for spec in specs:
        storage_bytes = 0
        envelope_parent = _v27_stage0_envelope_path(raw_root, spec).parent
        if envelope_parent.exists():
            storage_bytes += _directory_size(envelope_parent)
        terminal_path = _v27_stage0_terminal_path(raw_root, spec)
        if terminal_path.is_file():
            storage_bytes += terminal_path.stat().st_size
        if spec.run_id == first_run_id:
            for control_path in (
                raw_root / "stage0-calibration" / "stage0_selection.json",
                raw_root / "stage0-calibration" / "stage0_commit_intent.json",
                receipt,
            ):
                if control_path.is_file():
                    storage_bytes += control_path.stat().st_size
        budget_ledger.finalize(
            spec.run_id,
            status=terminal_status,
            cost_usd=0.0,
...
def _reserve_imported_stage0_budget_matrix(
    specs: Sequence[PilotRunSpec],
    *,
    budget_ledger: PilotBudgetLedger,
    projections: Mapping[str, RunProjection],
    ...
    """Preflight and idempotently reserve the complete zero-call Stage-0 matrix.

    The imported Stage-0 cells form one transaction.  Checking the aggregate
    matrix before the first per-row write prevents a genuine cap violation from
    being mistaken for a recoverable mid-loop interruption.  Re-entering this
    helper after a transient write failure is safe: existing rows must match
    byte-for-byte, and only missing rows are admitted through the same budget
    ledger cap checks.
    """

    ordered = tuple(projections[spec.run_id] for spec in specs)
    if not prevalidated and isinstance(budget_ledger, PilotBudgetLedger):
        _assert_projection_matrix_fits(budget_ledger, ordered)
    for projection in ordered:
        existing = budget_ledger.snapshot()["runs"].get(projection.run_id)
        if existing is None:
            budget_ledger.reserve(projection)
        elif existing.get("reservation") != projection.to_dict():
            raise PilotOrchestrationError(
                f"{projection.run_id} imported Stage-0 budget reservation drifted"
            )
...
def _execute_imported_stage0_stage(
    ...
    *,
    ...
    if (
        ...
        or len(specs) != 14
        or {spec.run_id for spec in specs} != {spec.run_id for spec in expected}
    ...
    entry_completed_run_ids = {
        spec.run_id
        for spec in specs
        if isinstance(entry_snapshot.get(spec.run_id), Mapping)
        and entry_snapshot[spec.run_id].get("status") == "complete"
    }
    import_projections = {
        spec.run_id: RunProjection(
            run_id=spec.run_id,
            ...
            storage_bytes=2_000_000,
            basis={
                "method": (f"{contract_label.lower()}-imported-stage0-envelope"),
                "provider_calls": 0,
                "hosted_completion_cap_counted": False,
            },
        )
    ...
    entry_terminal_ids = {
        spec.run_id for spec in specs if run_ledger.is_terminal(spec.run_id)
    }
    entry_budget_rows = budget_ledger.snapshot()["runs"]
    reservation_matrix_prevalidated = False
    if entry_terminal_ids:
        ...
        present_budget_ids = {
            spec.run_id for spec in specs if spec.run_id in entry_budget_rows
        }
        ...
        if present_budget_ids != expected_run_ids or any(
            entry_budget_rows[spec.run_id].get("reservation")
            != import_projections[spec.run_id].to_dict()
            for spec in specs
        ):
            raise PilotOrchestrationError(
    ...
    try:
        ...
        if isinstance(budget_ledger, PilotBudgetLedger):
            _assert_projection_matrix_fits(
                budget_ledger,
                tuple(import_projections[spec.run_id] for spec in specs),
            )
        reservation_matrix_prevalidated = True
        _reserve_imported_stage0_budget_matrix(
            specs,
            budget_ledger=budget_ledger,
            projections=import_projections,
            prevalidated=True,
        ...
        selection_path = raw_root / "stage0-calibration" / "stage0_selection.json"
        _persist_exact_json(selection_path, selection)

        # This is the same full-matrix budget gate as a fresh Stage-0 run,
        # except the fourteen imported cells themselves require zero calls.
        ...
        by_run = {
            spec.run_id: (terminal, envelope) for spec, envelope, terminal, _ in cells
        }
        commit_intent = _persist_v27_stage0_commit_intent(
            contract,
            raw_root=raw_root,
            paid=paid,
            cells=cells,
            ...
            projections=import_projections,
            entry_completed_run_ids=sorted(entry_completed_run_ids),
        )
        first_run_id = sorted(import_projections)[0]
        storage_by_run: dict[str, int] = {}
        for spec in specs:
            terminal_path, envelope_path = by_run[spec.run_id]
            storage_bytes = _directory_size(envelope_path.parent)
            storage_bytes += terminal_path.stat().st_size
            if spec.run_id == first_run_id:
                storage_bytes += selection_path.stat().st_size
                storage_bytes += commit_intent.stat().st_size
            if storage_bytes > import_projections[spec.run_id].storage_bytes:
                raise PilotBudgetError(
                    f"{spec.run_id} imported Stage-0 prepared artifacts "
                    "exceed their storage reservation"
                )
            storage_by_run[spec.run_id] = storage_bytes
            if run_ledger.is_terminal(spec.run_id):
                row = run_ledger.snapshot()["runs"][spec.run_id]
                if row.get("artifact") != str(terminal_path):
                    raise PilotOrchestrationError(
                        f"{spec.run_id} imported ledger artifact differs on resume"
                    )
        pending_finalizations = [
            {
                "run_id": spec.run_id,
                "status": "complete",
                "artifact": str(by_run[spec.run_id][0]),
                "failure": None,
            }
            for spec in specs
            if not run_ledger.is_terminal(spec.run_id)
        ...
        if pending_finalizations:
            run_ledger.finalize_many(pending_finalizations)
        ...
        receipt = _write_stage_receipt(
            ...
            artifacts={
                ...
                "imported_run_envelope_count": 14,
                "provider_calls_current_attempt": 0,
                "remaining_projection_count": len(projections),
            },
            paid=paid,
        )
        storage_by_run[first_run_id] += receipt.stat().st_size
        for spec in specs:
            storage_bytes = storage_by_run[spec.run_id]
            if storage_bytes > import_projections[spec.run_id].storage_bytes:
                raise PilotBudgetError(
                    f"{spec.run_id} imported Stage-0 published artifacts "
                    "exceed their storage reservation"
                )
            budget_ledger.finalize(
                spec.run_id,
        ...
        # Once the atomic run-ledger group commit has landed, a receipt or
        # budget write failure is a resumable partial publication.  The sealed
        # commit intent records the entry state and exact planned artifacts;
        # never rewrite those completed rows into a terminal no-go.
        completed_run_ids = {
            spec.run_id
            for spec in specs
            if run_ledger.is_terminal(spec.run_id)
        ...
        if completed_run_ids:
            disposition = (
                "immutable-resume-verification-failure"
                if completed_run_ids == entry_completed_run_ids
                else "current-attempt-partial-publication"
            )
            budget_statuses = {
                spec.run_id: (
                    budget_ledger.snapshot()["runs"].get(spec.run_id, {}).get("status")
                )
                for spec in specs
            }
            intent_path = raw_root / "stage0-calibration" / "stage0_commit_intent.json"
            raise PilotOrchestrationError(
                ...
                f"budget_statuses={canonical_sha256(budget_statuses)}"
            ) from exc
        failure = _exception_failure(exc)
        terminal_status = (
            "budget-stopped"
        ...
        if reservation_matrix_prevalidated:
            # A transient failure may occur after only a prefix of the
            # per-row ledger writes.  Complete the exact zero-call reservation
            # matrix before terminalizing the ITT denominator so --resume can
            # deterministically reconcile all fourteen rows.  This repeats the
            # aggregate cap check and uses the normal ledger reservation path;
            # it neither bypasses a cap nor constructs a provider.
            try:
                _reserve_imported_stage0_budget_matrix(
                    specs,
                    budget_ledger=budget_ledger,
                ...
                recovery_failure = _exception_failure(recovery_exc)
                raise PilotOrchestrationError(
                    f"{contract_label} Stage-0 zero-call reservation recovery "
                    "failed before denominator terminalization; "
                    f"original_failure_sha256={canonical_sha256(failure)}; "
                    "recovery_failure_sha256="
                    f"{canonical_sha256(recovery_failure)}"
                ) from recovery_exc
        run_ledger.stop_pending(
            specs,
            status=terminal_status,
        ...
        for spec in specs:
            ...
            envelope_parent = _v27_stage0_envelope_path(raw_root, spec).parent
            storage_bytes = (
                _directory_size(envelope_parent) if envelope_parent.exists() else 0
            )
            terminal_path = _v27_stage0_terminal_path(raw_root, spec)
            if terminal_path.is_file():
                storage_bytes += terminal_path.stat().st_size
...
def _transactional_parent_commit_schema(contract: PilotContract) -> str:
    ...
    return {
        ...
        V210_CONTRACT_ID: PILOT_V210_PARENT_COMMIT_INTENT_SCHEMA_VERSION,
        V2101_CONTRACT_ID: PILOT_V2101_PARENT_COMMIT_INTENT_SCHEMA_VERSION,
        V2102_CONTRACT_ID: PILOT_V2102_PARENT_COMMIT_INTENT_SCHEMA_VERSION,
    }[contract.contract_id]
...
def _transactional_parent_failure_schema(contract: PilotContract) -> str:
    ...
    return {
        ...
        V210_CONTRACT_ID: PILOT_V210_PARENT_FAILURE_INTENT_SCHEMA_VERSION,
        V2101_CONTRACT_ID: PILOT_V2101_PARENT_FAILURE_INTENT_SCHEMA_VERSION,
        V2102_CONTRACT_ID: PILOT_V2102_PARENT_FAILURE_INTENT_SCHEMA_VERSION,
    }[contract.contract_id]
...
def _transactional_parent_import_projection(
    ...
    _transactional_parent_label(contract)
    source_storage = (
        V2102_PARENT_RAW_STORAGE_BYTES
        if contract.contract_id == V2102_CONTRACT_ID
        else (
            V210_PARENT_RAW_STORAGE_BYTES
            if contract.contract_id in {V210_CONTRACT_ID, V2101_CONTRACT_ID}
            else (
    ...
    method = (
        "v2.10.2-byte-exact-v2.9-via-v2.10.1-parent-snapshot-plus-consumer-adapter-reseal"
        if contract.contract_id == V2102_CONTRACT_ID
        else (
            "v2.10.1-byte-exact-v2.9-via-v2.10-parent-snapshot-plus-current-flat-p95-reseal"
            if contract.contract_id == V2101_CONTRACT_ID
            else (
                "v2.10-byte-exact-v2.9-parent-snapshot-plus-current-flat-p95-reseal"
                if contract.contract_id == V210_CONTRACT_ID
                else (
...
def _persist_v29_parent_import_for_orchestrator(
    *,
    ...
    if contract.contract_id != V29_CONTRACT_ID or child_git_tag != V29_SCIENCE_TAG:
        raise PilotOrchestrationError("V2.9 parent importer release identity drifted")
...
def _persist_v210_parent_import_for_orchestrator(
    *,
    contract: PilotContract,
    repo_root: Path,
    raw_root: Path,
    ...
    """Adapt the V2.10 source/evidence API to the parent-stage transaction."""

    required_tag = str(contract.implementation["required_git_tag"])
    if contract.contract_id != V210_CONTRACT_ID or child_git_tag != required_tag:
        raise PilotOrchestrationError("V2.10 parent importer release identity drifted")
    source_manifest_path = repo_root.joinpath(*V210_SOURCE_MANIFEST_PATH.parts)
    try:
        source_manifest = load_v210_source_manifest(source_manifest_path)
        return persist_v210_parent_import(
            parent_repo_root=parent_repo_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10 parent import failed validation: {exc}"
        ) from exc


def _persist_v2101_parent_import_for_orchestrator(
    *,
    contract: PilotContract,
    repo_root: Path,
    raw_root: Path,
    ...
    """Adapt the dedicated V2.10.1 source API to the parent transaction."""

    required_tag = str(contract.implementation["required_git_tag"])
    if contract.contract_id != V2101_CONTRACT_ID or child_git_tag != required_tag:
        raise PilotOrchestrationError(
            "V2.10.1 parent importer release identity drifted"
        )
    source_manifest_path = repo_root.joinpath(*V2101_SOURCE_MANIFEST_PATH.parts)
    try:
        source_manifest = load_v2101_source_manifest(source_manifest_path)
        return persist_v2101_parent_import(
            parent_repo_root=parent_repo_root,
            evidence_repo_root=repo_root,
            child_repo_root=repo_root,
            child_raw_root=raw_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10.1 parent import failed validation: {exc}"
        ) from exc


def _persist_v2102_parent_import_for_orchestrator(
    *,
    contract: PilotContract,
    repo_root: Path,
    raw_root: Path,
    ...
    """Adapt V2.10.2's exact V2.10.1 lineage to the parent transaction."""

    required_tag = str(contract.implementation["required_git_tag"])
    if contract.contract_id != V2102_CONTRACT_ID or child_git_tag != required_tag:
        raise PilotOrchestrationError(
            "V2.10.2 parent importer release identity drifted"
        )
    source_manifest_path = repo_root.joinpath(*V2102_SOURCE_MANIFEST_PATH.parts)
    try:
        source_manifest = load_v2102_source_manifest(source_manifest_path)
        return persist_v2102_parent_import(
            parent_repo_root=parent_repo_root,
            evidence_repo_root=repo_root,
            child_repo_root=repo_root,
            child_raw_root=raw_root,
        ...
        raise PilotOrchestrationError(
            f"V2.10.2 parent import failed validation: {exc}"
        ) from exc
...
def _v27_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    ...
    return RunProjection(
        ...
        cost_usd=0.0,
        completions=0,
        storage_bytes=V26_RAW_STORAGE_BYTES + 5_000_000,
        basis={
            "method": "v2.7-byte-exact-parent-snapshot-plus-reseal",
            "source_snapshot_storage_bytes": V26_RAW_STORAGE_BYTES,
            "artifact_headroom_bytes": 5_000_000,
            "provider_calls": 0,
            "hosted_completion_cap_counted": False,
        },
...
def _finalize_v27_parent_import_budget(
    ledger: PilotBudgetLedger,
    projection: RunProjection,
    *,
    raw_root: Path,
    ...
    *,
    ...
    if terminal_path.exists():
        ...
        if (
            ...
            or terminal.get("scientific_evidence") is not False
            or terminal.get("diagnostic_only") is not False
            or terminal.get("evidence_scope") != "preregistered_parent_authority_import"
        ):
            raise PilotOrchestrationError(
...
def _v27_parent_commit_intent_path(*, raw_root: Path) -> Path:
    return raw_root / "parent-import" / "parent_import_commit_intent.json"
...
def _assert_v27_parent_success_ledger_boundary(
    ...
    *,
    ...
    expected_terminal = str(
        raw_root / "parent-import" / "summaries" / f"{spec.run_id}.json"
    )
...
def _reconcile_v27_parent_import_no_go_budget(
    ...
    *,
    ...
    if (
        not isinstance(row, Mapping)
        or row.get("status") != "integrity-stopped"
        or row.get("artifact") != str(failure_path)
        or not isinstance(row.get("failure"), Mapping)
    ):
        raise PilotOrchestrationError(
...
def _v211_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    """Reserve only compact V2.11 receipt/ledger metadata, never parent raw."""
    ...
    return RunProjection(
        ...
        basis={
            "method": "v2.11-compact-parent-calibration-receipt-only",
            "artifact_headroom_bytes": 1_000_000,
...
def _v211_parent_terminal_payload(
    result: Mapping[str, Any],
    ...
    return {
        "metrics": {},
        "gate_evidence": {
            "importer_result": _json_copy(result),
            "q_ref": calibration["q_ref"],
            "selected_profile_id": calibration["selected_profile_id"],
        ...
        "imported_p95_authorities": [],
        "claim_boundary": (
            "V2.10.2 calibration prerequisites and cumulative budget debit "
            "only; no imported A-D effect or observed-p95 authority."
        ),
    }


def _v2111_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
        cost_usd=0.0,
        ...
        storage_bytes=1_000_000,
        basis={
            "method": ("v2.11.1-compact-parent-calibration-capability-receipt-only"),
            "artifact_headroom_bytes": 1_000_000,
            "provider_construction": False,
            "provider_calls": 0,
            "imported_effect_cells": 0,
...
def _v2112_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
        cost_usd=0.0,
        ...
        storage_bytes=1_000_000,
        basis={
            "method": ("v2.11.2-compact-parent-calibration-capability-receipt-only"),
            "artifact_headroom_bytes": 1_000_000,
            "provider_construction": False,
            "provider_calls": 0,
            "imported_effect_cells": 0,
...
def _v2113_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
        cost_usd=0.0,
        ...
        storage_bytes=1_000_000,
        basis={
            "method": "v2.11.3-compact-parent-authority-replay",
            "artifact_headroom_bytes": 1_000_000,
            "provider_construction": False,
            "provider_calls": 0,
            "imported_effect_cells": 0,
            "imported_scientific_cells": 0,
            "imported_capability_wrappers": 2,
...
def _execute_v2113_parent_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Replay V2.11.2 and import only non-effect authorities, with zero calls."""

    if contract.contract_id != V2113_CONTRACT_ID:
        raise PilotOrchestrationError(
            "dedicated V2.11.3 parent executor requires its exact contract"
        )
    if (
        len(specs) != 1
        ...
        raise PilotOrchestrationError(
            "V2.11.3 parent import requires one exact zero-call cell"
        )
    if parent_repo_root is None:
        raise PilotOrchestrationError(
            "V2.11.3 parent-import requires --parent-repo-root pointing to "
            "the immutable V2.11.2 science checkout"
        )
    boundary = contract.v2113_forward_boundary
    expanded = tuple(contract.expand())
    rows = run_ledger.snapshot().get("runs")
    if (
        not isinstance(boundary, Mapping)
        or int(boundary["matrix"]["ledger_cells"]) != 136
        or int(boundary["matrix"]["scientific_cells"]) != 131
        or int(boundary["import_policy"]["imported_effect_cells"]) != 0
        or int(boundary["import_policy"]["v2112_scientific_cells_reused"]) != 0
        or len(expanded) != 136
        or not isinstance(rows, Mapping)
        or len(rows) != 136
        or set(rows) != {candidate.run_id for candidate in expanded}
    ):
        raise PilotOrchestrationError(
            "V2.11.3 parent import requires the exact fresh 136-cell registration"
        )

    spec = specs[0]
    projection = _v2113_parent_import_projection(spec)
    budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
    if budget_row is None:
        budget_ledger.reserve(projection)
    elif budget_row.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError(
            "V2.11.3 parent-import budget reservation drifted"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    terminal_path = raw_root / "parent-import" / "summaries" / f"{spec.run_id}.json"
    entry_status = run_ledger.status(spec.run_id)
    if entry_status not in {"scheduled", "complete"}:
        row = run_ledger.snapshot()["runs"][spec.run_id]
        failure = row.get("failure")
        if not isinstance(failure, Mapping):
            raise PilotOrchestrationError(
                "V2.11.3 terminal parent no-go lacks failure evidence"
            )
        _propagate_stage_no_go(
            contract,
            source_stage="parent-import",
    ...
    try:
        result = persist_v2113_parent_import(
            ...
            contract=contract,
            git_commit=paid.head_commit,
            source_repo_root=parent_repo_root,
            evidence_repo_root=repo_root,
        )
        verified = verify_v2113_parent_import_receipt(
            receipt_path,
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
        ...
        calibration = calibration_wrapper_from_v2113_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        )
        capabilities = capability_wrappers_from_v2113_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        )
        preflights = preflight_wrappers_from_v2113_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        )
        import_policy = verified.get("import_policy")
        terminal_parent = verified.get("terminal_parent_denominator")
        if (
            not isinstance(import_policy, Mapping)
            or import_policy.get("provider_construction_during_import") is not False
            or import_policy.get("provider_calls_during_import") != 0
            or import_policy.get("hosted_provider_calls_during_import") != 0
            or import_policy.get("imported_effect_cells") != 0
            or import_policy.get("historical_scientific_cells_imported") != 0
            or import_policy.get("imported_scientific_run_summaries") != 0
            or import_policy.get("decoded_completion_reuse") is not False
            or not isinstance(terminal_parent, Mapping)
            or terminal_parent.get("status_counts") != {"complete": 10, "failed": 126}
            or set(capabilities) != {"gpt52_main", "gpt56_diagnostic"}
            or set(preflights) != {"gpt52_main", "gpt56_diagnostic"}
            or calibration.get("scientific_evidence") is not False
        ):
            raise PilotOrchestrationError(
                "V2.11.3 parent importer violated its zero-provider/effect boundary"
            )
        payload = {
            "metrics": {},
            "gate_evidence": {
                "importer_result": _json_copy(result),
                "parent_status_counts": _json_copy(terminal_parent["status_counts"]),
                "capability_wrapper_content_sha256": {
                    model_id: wrapper["integrity"]["content_sha256"]
                    for model_id, wrapper in capabilities.items()
                },
                "preflight_wrapper_content_sha256": {
                    model_id: wrapper["integrity"]["content_sha256"]
                    for model_id, wrapper in preflights.items()
                },
            },
            "provider_construction": False,
            "provider_calls": 0,
            ...
            "claim_boundary": (
                "V2.11.2 calibration, capability/interface, and observed-p95 "
                "dispatch-budget authority only. Its 131 scientific cells and "
                "all decoded completions remain excluded from V2.11.3."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    "V2.11.3 parent terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "complete":
            raise
        failure = {
            "error_type": "V2113ParentImportIntegrityError",
            "cause_type": type(exc).__name__,
            "message": str(exc),
            "provider_construction": False,
            "provider_calls": 0,
        ...
        raise PilotOrchestrationError(
            f"V2.11.3 parent import failed; receipt={stage_receipt}"
        ) from exc


def _v2113_operational_import_projection(spec: PilotRunSpec) -> RunProjection:
    if spec.execution_mode == "capability_authority_import":
        method = "v2.11.3-current-release-capability-wrapper"
        historical_calls = {"historical_source_calls": 30}
    elif spec.execution_mode == "preflight_authority_import":
        method = "v2.11.3-current-release-p95-authority-reseal"
        historical_calls = {
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} is not a V2.11.3 operational import cell"
        )
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
...
def _v2113_operational_no_go(
    ...
    *,
    ...
    error_type: str,
    cause: BaseException,
    run_status: str = "integrity-stopped",
) -> dict[str, Any]:
    """Seal a zero-provider operational no-go and its complete descendants."""
    ...
    for spec in specs:
        ...
        projection = _v2113_operational_import_projection(spec)
        budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
        if budget_row is None:
            budget_ledger.reserve(projection)
        ...
        if budget_row.get("status") == "reserved":
            ...
            if spec.execution_mode == "preflight_authority_import":
                external_paths.extend(
                    (
                        v2113_observed_p95_receipt_path(raw_root, spec.model_id),
                        v2113_observed_p95_projection_path(raw_root, spec.model_id),
                    )
                )
            budget_ledger.finalize(
                spec.run_id,
                status=(
                    "complete"
                    if run_ledger.status(spec.run_id) == "capability-no-go"
                    else run_ledger.status(spec.run_id)
                ),
                cost_usd=0.0,
                completions=0,
                storage_bytes=_directory_size(run_dir)
                + sum(path.stat().st_size for path in external_paths if path.is_file()),
                failure=failure,
            )
    _propagate_stage_no_go(
        contract,
...
def _execute_v2112_parent_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Import only V2.11.1 calibration/capability wrappers, with zero calls."""

    if contract.contract_id != V2112_CONTRACT_ID:
        raise PilotOrchestrationError(
            "dedicated V2.11.2 parent executor requires its exact contract"
        )
    if (
        len(specs) != 1
        or specs[0].stage_id != "parent-import"
        ...
        raise PilotOrchestrationError(
            "V2.11.2 parent import requires one exact zero-call cell"
        )
    if parent_repo_root is None:
        raise PilotOrchestrationError(
            "V2.11.2 parent-import requires --parent-repo-root pointing to "
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.2 parent import requires the exact 136-cell registration"
        )

    spec = specs[0]
    projection = _v2112_parent_import_projection(spec)
    budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
    if budget_row is None:
        budget_ledger.reserve(projection)
    elif budget_row.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError(
            "V2.11.2 parent-import budget reservation drifted"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    terminal_path = raw_root / "parent-import" / "summaries" / f"{spec.run_id}.json"
    entry_status = run_ledger.status(spec.run_id)
    if entry_status not in {"scheduled", "complete"}:
        ...
        if not isinstance(failure, Mapping):
            raise PilotOrchestrationError(
                "V2.11.2 terminal parent no-go lacks failure evidence"
            )
        _propagate_stage_no_go(
            contract,
            source_stage="parent-import",
    ...
    try:
        result = persist_v2112_parent_import(
            repo_root=repo_root,
            child_contract_sha256=contract.canonical_hash,
            child_git_tag=paid.git_tag,
            child_git_commit=paid.head_commit,
        ...
        wrappers = capability_wrappers_from_v2112_receipt(verified)
        if (
            result.get("provider_construction_during_import") is not False
            or result.get("provider_calls_during_import") != 0
            or result.get("imported_effect_cells") != 0
            or result.get("imported_preflight_samples") != 0
            or result.get("imported_checkpoint_artifacts") != []
            or result.get("imported_p95_authorities") != []
            or result.get("raw_tree_copied") is not False
            or set(wrappers) != {"gpt52_main", "gpt56_diagnostic"}
        ):
            raise PilotOrchestrationError(
                "V2.11.2 parent importer violated its zero-provider boundary"
            )
        calibration = _load_verified_v2112_calibration(
            contract,
            raw_root=raw_root,
        ...
        payload = {
            ...
            "gate_evidence": {
                ...
                "historical_failed_preflight_calls_audit_only": 64,
            },
            "provider_construction": False,
            "provider_calls": 0,
            "imported_effect_cells": 0,
            ...
            "imported_p95_authorities": [],
            "claim_boundary": (
                "V2.11.1 calibration and capability wrappers only; its failed "
                "preflight journals remain budget/failure audit and provide no "
                "sample, checkpoint, p95, or treatment-effect authority."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    "V2.11.2 parent terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "complete":
            raise
        failure = {
            "error_type": "V2112ParentImportIntegrityError",
            "cause_type": type(exc).__name__,
            "message": str(exc),
            "provider_construction": False,
            "provider_calls": 0,
        ...
        raise PilotOrchestrationError(
            f"V2.11.2 parent import failed; receipt={stage_receipt}"
        ) from exc


def _execute_v2111_parent_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Import V2.11 calibration/capability evidence with zero provider calls."""

    if contract.contract_id != V2111_CONTRACT_ID:
        raise PilotOrchestrationError(
            "dedicated V2.11.1 parent executor requires its exact contract"
        )
    if (
        len(specs) != 1
        or specs[0].stage_id != "parent-import"
        ...
        raise PilotOrchestrationError(
            "V2.11.1 parent import requires one exact zero-call contract cell"
        )
    if parent_repo_root is None:
        raise PilotOrchestrationError(
            "V2.11.1 parent-import requires --parent-repo-root pointing to "
            "the immutable V2.11 science checkout"
        )
    boundary = contract.v2111_forward_boundary
    if not isinstance(boundary, Mapping):
        raise PilotOrchestrationError("V2.11.1 forward boundary is unavailable")
    expanded = tuple(contract.expand())
    rows = run_ledger.snapshot().get("runs")
    expected_count = int(boundary["matrix"]["ledger_cells"])
    if (
        expected_count != 136
        or len(expanded) != expected_count
        or not isinstance(rows, Mapping)
        or len(rows) != expected_count
        or set(rows) != {candidate.run_id for candidate in expanded}
    ):
        raise PilotOrchestrationError(
            "V2.11.1 parent import requires the exact 136-cell registration"
        )

    spec = specs[0]
    projection = _v2111_parent_import_projection(spec)
    budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
    if budget_row is None:
        budget_ledger.reserve(projection)
    elif budget_row.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError(
            "V2.11.1 parent-import budget reservation drifted"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    terminal_path = raw_root / "parent-import" / "summaries" / f"{spec.run_id}.json"
    entry_status = run_ledger.status(spec.run_id)
    if entry_status not in {"scheduled", "complete"}:
        ...
        if not isinstance(failure, Mapping):
            raise PilotOrchestrationError(
                "V2.11.1 terminal parent no-go lacks failure evidence"
            )
        _propagate_stage_no_go(
            contract,
            source_stage="parent-import",
    ...
    try:
        result = persist_v2111_parent_import(
            repo_root=repo_root,
            child_contract_sha256=contract.canonical_hash,
            child_git_tag=paid.git_tag,
            child_git_commit=paid.head_commit,
        ...
        if (
            ...
            raise PilotOrchestrationError(
                "V2.11.1 parent importer violated its zero-provider boundary"
            )
        calibration = _load_verified_v2111_calibration(
            contract,
            raw_root=raw_root,
        ...
        payload = {
            ...
            "imported_p95_authorities": [],
            "claim_boundary": (
                "V2.11 calibration and capability/interface wrappers only; "
                "no imported A-D effect, preflight sample, or p95 authority."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if (
                ...
                raise PilotOrchestrationError(
                    "V2.11.1 parent terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "complete":
            raise
        failure = {
            "error_type": "V2111ParentImportIntegrityError",
            "cause_type": type(exc).__name__,
            "message": str(exc),
            "provider_construction": False,
            "provider_calls": 0,
        ...
        raise PilotOrchestrationError(
            f"V2.11.1 parent import failed; receipt={stage_receipt}"
        ) from exc


def _execute_v2113_capability_import_stage_impl(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Materialize two current-release capability wrappers without a provider."""

    if (
        contract.contract_id != V2113_CONTRACT_ID
        or {spec.model_id for spec in specs} != {"gpt52_main", "gpt56_diagnostic"}
        or any(
            spec.stage_id != "capability-gate"
            or spec.execution_mode != "capability_authority_import"
            for spec in specs
        )
    ):
        raise PilotOrchestrationError(
            "V2.11.3 capability import requires its exact two registered cells"
        )
    _assert_prerequisites(
        contract,
        "capability-gate",
    ...
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        receipt = verify_v2113_parent_import_receipt(
            receipt_path,
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
        ...
        wrappers = capability_wrappers_from_v2113_receipt(
            receipt,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.3 capability source failed validation: {exc}"
        ) from exc
    ...
    artifacts: dict[str, Any] = {
        "provider_construction": False,
        "provider_calls_current_attempt": 0,
        "historical_source_calls": 60,
        "imported_effect_cells": 0,
        "scientific_evidence": False,
        "models": {},
    }
    for spec in specs:
        wrapper = wrappers[spec.model_id]
        projection = _v2113_operational_import_projection(spec)
        budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
        if budget_row is None:
            budget_ledger.reserve(projection)
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability reservation drifted"
            )
        run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
        capability_path = run_dir / "capability.json"
        gate_path = run_dir / "gate_receipt.json"
        terminal_path = raw_root / spec.stage_id / "summaries" / f"{spec.run_id}.json"
        imported = wrapper.get("capability")
        if not isinstance(imported, Mapping):
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability wrapper is malformed"
            )
        semantic_go = (
            imported.get("capability_pass") is True
            and imported.get("interface_pass") is True
        )
        gate = {
            "capability_pass": semantic_go,
        ...
        _persist_exact_json(capability_path, wrapper)
        _persist_exact_json(gate_path, gate)
        payload = {
            "metrics": {},
            ...
            "provider_calls": 0,
            "claim_boundary": (
                "Direct V2.11.2 capability/interface authority only; no "
                "V2.11.2 treatment outcome or decoded completion is reused."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.3 capability terminal differs"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        status = "complete" if semantic_go else "capability-no-go"
        ...
        elif run_ledger.status(spec.run_id) != status:
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.3 capability ledger status drifted"
            )
        if budget_row is None or budget_row.get("status") == "reserved":
            budget_ledger.finalize(
                spec.run_id,
                status="complete" if status == "capability-no-go" else status,
                cost_usd=0.0,
                completions=0,
                storage_bytes=_directory_size(run_dir) + terminal_path.stat().st_size,
            )
        if semantic_go:
            go_models.append(spec.model_id)
        artifacts["models"][spec.model_id] = {
    ...
    status = (
        "complete"
        if set(go_models) == {"gpt52_main", "gpt56_diagnostic"}
        else "complete-with-no-go"
    )
    failure: dict[str, Any] | None = None
    if status != "complete":
        failure = {
            "error_type": "V2113CapabilitySemanticNoGo",
            "cause_type": "CapabilityOrInterfaceNoGo",
            "message": "one or more imported capability/interface gates are no-go",
            "provider_construction": False,
            "provider_calls": 0,
        }
        _propagate_stage_no_go(
...
def _execute_v2113_capability_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Execute the capability import with one immutable fail-closed boundary."""
    ...
    existing_stage_receipt = _stage_receipt_path(raw_root, "capability-gate")
    if existing_stage_receipt.exists() and all(
        run_ledger.is_terminal(spec.run_id) for spec in specs
    ):
        return _verify_v2_stage_receipt(
            contract,
            "capability-gate",
            _read_json(existing_stage_receipt),
            raw_root=raw_root,
            ledger=run_ledger,
            paid=paid,
            authority_repo_root=repo_root,
    ...
    try:
        return _execute_v2113_capability_import_stage_impl(
            contract,
            specs,
            raw_root=raw_root,
            repo_root=repo_root,
        ...
        return _v2113_operational_no_go(
            contract,
            specs,
            ...
            budget_ledger=budget_ledger,
            artifacts=artifacts,
            error_type="V2113CapabilityAuthorityImportError",
            cause=exc,
        )


def _verify_v2113_preflight_import_gate(
    value: Mapping[str, Any],
    *,
    contract: PilotContract,
    spec: PilotRunSpec,
    ...
    if not isinstance(integrity, Mapping):
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.11.3 preflight gate lacks integrity"
        )
    body = _json_copy(candidate)
    body["integrity"].pop("content_sha256", None)
    expected = {
        "schema_version": V2113_PREFLIGHT_IMPORT_GATE_SCHEMA_VERSION,
        ...
        "projection_p95": str(projection_path),
        "integrity": {
    ...
    if (
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.11.3 preflight import gate drifted"
        )
    return candidate


_RESEALED_SOURCE_RECEIPT_AUTHORITY_FIELDS = {
    "source_authority_receipt_path": "path",
...
def _resealed_reservations_match_source_wrapper(
    *,
    source_wrapper: Mapping[str, Any],
    resealed_reservations: Mapping[str, Any],
) -> bool:
    """Verify a receipt-enriched reseal against its exact source wrapper.

    The reseal deliberately adds four current-verifier provenance fields to
    each action/semantic authority.  Those fields are not present in the
    imported source wrapper, so whole-object equality is invalid.  Remove only
    that fixed allowlist for the source comparison, then validate every removed
    value independently against the wrapper's bound source receipt.
    """

    source_reservations = source_wrapper.get("reservations")
    source_receipt = source_wrapper.get("source_gate_receipt")
    if not isinstance(source_reservations, Mapping) or not isinstance(
        source_receipt, Mapping
    ):
        return False
    normalized = _json_copy(resealed_reservations)
    if not isinstance(normalized, dict) or set(normalized) != {"action", "semantic"}:
        return False
    for call_kind in ("action", "semantic"):
        entry = normalized.get(call_kind)
        if not isinstance(entry, dict) or set(entry) != {"authority", "reservation"}:
            return False
        authority = entry.get("authority")
        if not isinstance(authority, dict):
            return False
        for resealed_field, receipt_field in (
            _RESEALED_SOURCE_RECEIPT_AUTHORITY_FIELDS.items()
        ):
            if authority.pop(resealed_field, None) != source_receipt.get(receipt_field):
                return False
    return normalized == source_reservations


def _execute_v2113_preflight_authority_import_stage_impl(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Reseal two parent P95 authorities without constructing a provider."""

    model_ids = {"gpt52_main", "gpt56_diagnostic"}
    if (
        contract.contract_id != V2113_CONTRACT_ID
        or {spec.model_id for spec in specs} != model_ids
        or any(
            spec.stage_id != PILOT_V211_POST_GATE_STAGE_ID
            or spec.execution_mode != "preflight_authority_import"
            for spec in specs
        )
    ):
        raise PilotOrchestrationError(
            "V2.11.3 preflight import requires its exact two registered cells"
        )
    existing_stage_receipt = _stage_receipt_path(
        raw_root, PILOT_V211_POST_GATE_STAGE_ID
    )
    if existing_stage_receipt.exists() and all(
        run_ledger.is_terminal(spec.run_id) for spec in specs
    ):
        existing = _verify_v2_stage_receipt(
            contract,
            PILOT_V211_POST_GATE_STAGE_ID,
            _read_json(existing_stage_receipt),
            raw_root=raw_root,
        ...
        # Terminal ITT outcomes are immutable.  Resume re-verifies and
        # returns their receipt; it never re-runs a reseal cell or a failed
        # global authority construction.
        return existing
    _assert_prerequisites(
        contract,
        PILOT_V211_POST_GATE_STAGE_ID,
    ...
    try:
        ...
        wrappers = preflight_wrappers_from_v2113_receipt(
            parent,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.3 preflight source failed validation: {exc}"
        ) from exc
    if set(wrappers) != model_ids:
        raise PilotOrchestrationError("V2.11.3 preflight wrapper denominator drifted")
    ...
    go_models: list[str] = []
    per_model_bindings: dict[str, Any] = {}
    try:
        for spec in specs:
            projection = _v2113_operational_import_projection(spec)
            budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
            if budget_row is None:
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.3 preflight reservation drifted"
                )
            entry_status = run_ledger.status(spec.run_id)
            if entry_status not in {"scheduled", "complete"}:
                raise PilotOrchestrationError(
                    f"{spec.run_id} is a terminal V2.11.3 preflight no-go; "
                    "redispatch/reseal is forbidden"
                )
            run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
            wrapper_path = run_dir / "preflight_authority.json"
            gate_path = run_dir / "gate_receipt.json"
            ...
            reseal = persist_v2113_resealed_observed_p95_authority(
                repo_root=repo_root,
                raw_root=raw_root,
                contract=contract,
                expected_git_commit=paid.head_commit,
            ...
            authority_path = Path(str(reseal["receipt"]))
            projection_path = Path(str(reseal["projection"]))
            gate = _verify_v2113_preflight_import_gate(
                reseal["gate_receipt"],
                contract=contract,
                spec=spec,
                authority_receipt=authority_path,
            ...
            authority_binding = verified_v2113_observed_p95_authority_binding(
                authority_path,
                repo_root=repo_root,
                raw_root=raw_root,
                expected_git_commit=paid.head_commit,
            ...
            projection_binding = verified_v2113_observed_p95_projection_binding(
                projection_path,
                receipt_path=authority_path,
                repo_root=repo_root,
                raw_root=raw_root,
            ...
            wrapper = wrappers[spec.model_id]
            if (
                reseal.get("provider_construction_during_reseal") is not False
                or reseal.get("provider_calls_during_reseal") != 0
                or reseal.get("hosted_provider_calls_during_reseal") != 0
                or reseal.get("scientific_evidence") is not False
                or set(authority_binding.get("reservations", {})) != {runtime_model}
                or projection_binding.get("reservations")
                != authority_binding["reservations"]
                or projection_binding.get("payload", {})
                .get("bindings", {})
                .get("contract_sha256")
                != contract.canonical_hash
            ...
            _persist_exact_json(wrapper_path, wrapper)
            _persist_exact_json(gate_path, gate)
            payload = {
                "metrics": {},
                "preflight_authority": _json_copy(wrapper),
                "gate_evidence": gate,
                "resealed_authority": {
                    "receipt": str(authority_path),
                    "projection": str(projection_path),
                    "receipt_content_sha256": reseal["receipt_content_sha256"],
                    "projection_content_sha256": reseal["projection_content_sha256"],
                },
                "provider_calls": 0,
                "claim_boundary": (
                    "V2.11.2 fresh-preflight observed-p95 dispatch-budget "
                    "authority only; no scientific outcome is imported."
                ),
            }
            if terminal_path.exists():
                terminal = _load_v2_terminal_summary(
                ...
                if terminal.get("payload") != payload:
                    raise PilotOrchestrationError(
                        f"{spec.run_id} V2.11.3 preflight terminal differs"
                    )
            else:
                write_terminal_summary(
                    terminal_path,
            ...
            if budget_row is None or budget_row.get("status") == "reserved":
                budget_ledger.finalize(
                    ...
                    completions=0,
                    storage_bytes=(
                        _directory_size(run_dir)
                        + authority_path.stat().st_size
                        + projection_path.stat().st_size
                        + terminal_path.stat().st_size
                    ),
                )
            go_models.append(spec.model_id)
            per_model_bindings[spec.model_id] = authority_binding
            artifacts["models"][spec.model_id] = {
                "preflight_authority": str(wrapper_path),
                "gate_receipt": str(gate_path),
                "observed_p95_receipt": str(authority_path),
                "projection_p95": str(projection_path),
                "terminal_summary": str(terminal_path),
                "provider_calls_current_attempt": 0,
                "historical_provider_calls": 32,
        ...
        if not isinstance(events, list) or not events:
            raise PilotOrchestrationError(
                "V2.11.3 preflight gate lacks a run-ledger event chain"
            )
        post_gate_path, post_gate = persist_v2113_post_gate_authority(
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
            expected_git_commit=paid.head_commit,
            parent_import_receipt=parent,
            per_model_authority_bindings=per_model_bindings,
            ledger_event_chain_head=str(events[-1]["event_sha256"]),
        )
        verify_v2113_gate_receipt(
            post_gate,
            expected_git_commit=paid.head_commit,
        ...
        artifacts["post_gate_authority"] = {
            "path": str(post_gate_path),
            "file_sha256": _file_sha256(post_gate_path),
            "receipt_sha256": post_gate["receipt_sha256"],
            "go": True,
            "eligible_model_ids": post_gate["denominator"]["eligible_model_ids"],
        ...
        stage_receipt = _write_stage_receipt(
            ...
            ledger=run_ledger,
            status="complete",
            go_models=post_gate["denominator"]["eligible_model_ids"],
            artifacts=artifacts,
            paid=paid,
            authority_repo_root=repo_root,
        )
        ...
        return _v2113_operational_no_go(
            contract,
            specs,
            stage_id=PILOT_V211_POST_GATE_STAGE_ID,
            raw_root=raw_root,
            repo_root=repo_root,
            paid=paid,
            run_ledger=run_ledger,
...
def _execute_v2113_preflight_authority_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Execute P95 resealing behind one immutable zero-provider boundary."""
    ...
    try:
        return _execute_v2113_preflight_authority_import_stage_impl(
            contract,
            specs,
            raw_root=raw_root,
            repo_root=repo_root,
...
def _v2114_parent_import_projection(spec: PilotRunSpec) -> RunProjection:
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
        cost_usd=0.0,
        ...
        storage_bytes=1_000_000,
        basis={
            "method": "v2.11.4-compact-parent-authority-replay",
            "artifact_headroom_bytes": 1_000_000,
            "provider_construction": False,
            "provider_calls": 0,
            "imported_effect_cells": 0,
...
def _execute_v2114_parent_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if contract.contract_id != V2114_CONTRACT_ID:
        raise PilotOrchestrationError(
            "dedicated V2.11.4 parent executor requires its exact contract"
        )
    if (
        len(specs) != 1
        or specs[0].stage_id != "parent-import"
        ...
        raise PilotOrchestrationError(
            "V2.11.4 parent import requires one exact zero-call cell"
        )
    if parent_repo_root is None or authority_repo_root is None:
        raise PilotOrchestrationError(
            "V2.11.4 parent-import requires --parent-repo-root pointing to "
            "the immutable V2.11.3 terminal checkout and --authority-repo-root "
            "pointing to the immutable V2.11.2 authority checkout"
        )
    ...
    if (
        ...
        or int(boundary["matrix"]["scientific_cells"]) != 131
        or int(boundary["import_policy"]["imported_effect_cells"]) != 0
        or int(boundary["import_policy"]["v2113_scientific_cells_reused"]) != 0
        or len(expanded) != 136
        or not isinstance(rows, Mapping)
        or len(rows) != 136
        or set(rows) != {candidate.run_id for candidate in expanded}
        ...
        raise PilotOrchestrationError(
            "V2.11.4 parent import requires the exact fresh 136-cell registration"
        )

    spec = specs[0]
    projection = _v2114_parent_import_projection(spec)
    budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
    if budget_row is None:
        budget_ledger.reserve(projection)
    elif budget_row.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError(
            "V2.11.4 parent-import budget reservation drifted"
        )
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    terminal_path = raw_root / "parent-import" / "summaries" / f"{spec.run_id}.json"
    entry_status = run_ledger.status(spec.run_id)
    if entry_status not in {"scheduled", "complete"}:
        ...
        if not isinstance(failure, Mapping):
            raise PilotOrchestrationError(
                "V2.11.4 terminal parent no-go lacks failure evidence"
            )
        _propagate_stage_no_go(
            contract,
            source_stage="parent-import",
    ...
    try:
        result = persist_v2114_parent_import(
            ...
            contract=contract,
            git_commit=paid.head_commit,
            source_repo_root=authority_repo_root,
            lineage_repo_root=parent_repo_root,
            evidence_repo_root=repo_root,
        )
        verified = verify_v2114_parent_import_receipt(
            receipt_path,
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
        ...
        calibration = calibration_wrapper_from_v2114_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        )
        capabilities = capability_wrappers_from_v2114_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        )
        preflights = preflight_wrappers_from_v2114_receipt(
            verified,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        ...
        if (
            ...
            or not isinstance(terminal_parent, Mapping)
            or terminal_parent.get("status_counts")
            != {"complete": 3, "integrity-stopped": 133}
            or set(capabilities) != {"gpt52_main", "gpt56_diagnostic"}
            or set(preflights) != {"gpt52_main", "gpt56_diagnostic"}
            or calibration.get("scientific_evidence") is not False
        ):
            raise PilotOrchestrationError(
                "V2.11.4 parent importer violated its zero-provider/effect boundary"
            )
        payload = {
            "metrics": {},
            "gate_evidence": {
            ...
            "claim_boundary": (
                "V2.11.3 remains an immutable zero-call terminal no-go. "
                "V2.11.4 replays only V2.11.2 calibration, capability/interface, "
                "and observed-p95 dispatch-budget authority; no V2.11.2 or "
                "V2.11.3 scientific outcome or decoded completion is imported."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    "V2.11.4 parent terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "complete":
            raise
        failure = {
            "error_type": "V2114ParentImportIntegrityError",
            "cause_type": type(exc).__name__,
            "message": str(exc),
            "provider_construction": False,
            "provider_calls": 0,
        ...
        raise PilotOrchestrationError(
            f"V2.11.4 parent import failed; receipt={stage_receipt}"
        ) from exc


def _v2114_operational_import_projection(spec: PilotRunSpec) -> RunProjection:
    if spec.execution_mode == "capability_authority_import":
        method = "v2.11.4-current-release-capability-wrapper"
        historical_calls = {"historical_source_calls": 30}
    elif spec.execution_mode == "preflight_authority_import":
        method = "v2.11.4-current-release-p95-authority-reseal"
        historical_calls = {
            "historical_provider_calls": 32,
            "historical_calls_already_in_parent_debit": True,
            "fresh_samples": 0,
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} is not a V2.11.4 operational import cell"
        )
    return RunProjection(
        run_id=spec.run_id,
        stage_bucket=spec.budget_bucket,
...
def _v2114_operational_no_go(
    ...
    *,
    ...
    for spec in specs:
        ...
        projection = _v2114_operational_import_projection(spec)
        budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
        if budget_row is None:
            budget_ledger.reserve(projection)
            budget_row = budget_ledger.snapshot()["runs"][spec.run_id]
        if budget_row.get("status") == "reserved":
            ...
            if spec.execution_mode == "preflight_authority_import":
                external_paths.extend(
                    (
                        v2114_observed_p95_receipt_path(raw_root, spec.model_id),
                        v2114_observed_p95_projection_path(raw_root, spec.model_id),
                    )
                )
            budget_ledger.finalize(
                spec.run_id,
...
def _execute_v2114_capability_import_stage_impl(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.4 capability import requires its exact two registered cells"
        )
    _assert_prerequisites(
        contract,
        "capability-gate",
    ...
    receipt_path = raw_root / "parent-import" / "parent_import_receipt.json"
    try:
        receipt = verify_v2114_parent_import_receipt(
            receipt_path,
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
        ...
        wrappers = capability_wrappers_from_v2114_receipt(
            receipt,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.4 capability source failed validation: {exc}"
        ) from exc
    ...
    for spec in specs:
        ...
        if budget_row is None:
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability reservation drifted"
            )
        run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
        capability_path = run_dir / "capability.json"
        gate_path = run_dir / "gate_receipt.json"
        ...
        if not isinstance(imported, Mapping):
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability wrapper is malformed"
            )
        semantic_go = (
            imported.get("capability_pass") is True
            and imported.get("interface_pass") is True
        ...
        if terminal_path.exists():
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.4 capability terminal differs"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "scheduled":
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.4 capability ledger status drifted"
            )
        if budget_row is None or budget_row.get("status") == "reserved":
            budget_ledger.finalize(
                spec.run_id,
    ...
    if status != "complete":
        failure = {
            "error_type": "V2114CapabilitySemanticNoGo",
            "cause_type": "CapabilityOrInterfaceNoGo",
            "message": "one or more imported capability/interface gates are no-go",
            "provider_construction": False,
            "provider_calls": 0,
...
def _execute_v2114_capability_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    try:
        return _execute_v2114_capability_import_stage_impl(
            contract,
            specs,
            raw_root=raw_root,
            repo_root=repo_root,
        ...
        return _v2114_operational_no_go(
            contract,
            specs,
            stage_id="capability-gate",
            raw_root=raw_root,
            ...
            budget_ledger=budget_ledger,
            artifacts=artifacts,
            error_type="V2114CapabilityAuthorityImportError",
            cause=exc,
        )


def _verify_v2114_preflight_import_gate(
    value: Mapping[str, Any],
    *,
    contract: PilotContract,
    spec: PilotRunSpec,
    ...
    if not isinstance(integrity, Mapping):
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.11.4 preflight gate lacks integrity"
        )
    body = _json_copy(candidate)
    body["integrity"].pop("content_sha256", None)
    expected = {
    ...
    if (
        ...
        raise PilotOrchestrationError(
            f"{spec.run_id} V2.11.4 preflight import gate drifted"
        )
    return candidate


def _execute_v2114_preflight_authority_import_stage_impl(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.4 preflight import requires its exact two registered cells"
        )
    existing_stage_receipt = _stage_receipt_path(
        raw_root, PILOT_V211_POST_GATE_STAGE_ID
    )
    ...
    try:
        ...
        wrappers = preflight_wrappers_from_v2114_receipt(
            parent,
            contract=contract,
            expected_git_commit=paid.head_commit,
            repo_root=repo_root,
        ...
        raise PilotOrchestrationError(
            f"V2.11.4 preflight source failed validation: {exc}"
        ) from exc
    if set(wrappers) != model_ids:
        raise PilotOrchestrationError("V2.11.4 preflight wrapper denominator drifted")
    ...
    try:
        for spec in specs:
            ...
            if budget_row is None:
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.4 preflight reservation drifted"
                )
            entry_status = run_ledger.status(spec.run_id)
            if entry_status not in {"scheduled", "complete"}:
                raise PilotOrchestrationError(
                    f"{spec.run_id} is a terminal V2.11.4 preflight no-go; "
                    "redispatch/reseal is forbidden"
                )
            run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
            wrapper_path = run_dir / "preflight_authority.json"
            ...
            reseal = persist_v2114_resealed_observed_p95_authority(
                repo_root=repo_root,
                raw_root=raw_root,
                contract=contract,
                expected_git_commit=paid.head_commit,
            ...
            authority_path = Path(str(reseal["receipt"]))
            projection_path = Path(str(reseal["projection"]))
            gate = _verify_v2114_preflight_import_gate(
                reseal["gate_receipt"],
                contract=contract,
                spec=spec,
                authority_receipt=authority_path,
            ...
            authority_binding = verified_v2114_observed_p95_authority_binding(
                authority_path,
                repo_root=repo_root,
                raw_root=raw_root,
                expected_git_commit=paid.head_commit,
            ...
            projection_binding = verified_v2114_observed_p95_projection_binding(
                projection_path,
                receipt_path=authority_path,
                repo_root=repo_root,
                raw_root=raw_root,
            ...
            if (
                ...
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.4 resealed authority drifted"
                )
            _persist_exact_json(wrapper_path, wrapper)
            _persist_exact_json(gate_path, gate)
            payload = {
            ...
            if terminal_path.exists():
                ...
                if terminal.get("payload") != payload:
                    raise PilotOrchestrationError(
                        f"{spec.run_id} V2.11.4 preflight terminal differs"
                    )
            else:
                write_terminal_summary(
                    terminal_path,
        ...
        if not isinstance(events, list) or not events:
            raise PilotOrchestrationError(
                "V2.11.4 preflight gate lacks a run-ledger event chain"
            )
        post_gate_path, post_gate = persist_v2114_post_gate_authority(
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
            expected_git_commit=paid.head_commit,
        ...
        return _v2114_operational_no_go(
            ...
            budget_ledger=budget_ledger,
            artifacts=artifacts,
            error_type="V2114PreflightAuthorityImportError",
            cause=exc,
        )


def _execute_v2114_preflight_authority_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    try:
        return _execute_v2114_preflight_authority_import_stage_impl(
            contract,
            specs,
            raw_root=raw_root,
            repo_root=repo_root,
...
def _execute_v2112_capability_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Materialize the two direct V2.11.1 wrappers without a provider."""
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.2 capability import requires its exact two registered cells"
        )
    _assert_prerequisites(
        contract,
        "capability-gate",
    ...
    try:
        ...
        wrappers = capability_wrappers_from_v2112_receipt(receipt)
    except PilotV2112ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11.2 capability source failed validation: {exc}"
        ) from exc
    ...
    artifacts: dict[str, Any] = {
        ...
        "provider_calls_current_attempt": 0,
        "historical_source_calls": 60,
        "historical_failed_preflight_calls_used_for_gate": 0,
        "scientific_evidence": False,
        "models": {},
    }
    for spec in specs:
        ...
        projection = RunProjection(
            ...
            storage_bytes=1_000_000,
            basis={
                "method": "v2.11.2-direct-v2.11.1-capability-wrapper",
                "provider_construction": False,
                "provider_calls_current_attempt": 0,
                "historical_source_calls": 30,
                "imported_preflight_samples": 0,
        ...
        if budget_row is None:
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} imported capability reservation drifted"
            )
        run_dir = raw_root / spec.stage_id / "runs" / spec.run_id
        capability_path = run_dir / "capability.json"
        gate_path = run_dir / "gate_receipt.json"
        terminal_path = raw_root / spec.stage_id / "summaries" / f"{spec.run_id}.json"
        imported = wrapper["capability"]
        semantic_go = (
            imported.get("capability_pass") is True
            and imported.get("interface_pass") is True
        )
        ...
        _persist_exact_json(capability_path, wrapper)
        _, bootstrap_path = _persist_v2112_bootstrap_projection(
            contract,
            spec,
            raw_root=raw_root,
            repo_root=repo_root,
        ...
        payload = {
            ...
            "provider_calls": 0,
            "claim_boundary": (
                "Direct V2.11.1 capability/interface evidence only; failed "
                "preflight samples, checkpoints and P95 remain excluded."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    f"{spec.run_id} V2.11.2 capability terminal differs"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "scheduled":
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} V2.11.2 capability ledger status drifted"
            )
        if budget_row is None or budget_row.get("status") == "reserved":
            budget_ledger.finalize(
                spec.run_id,
...
def _execute_v2111_capability_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Materialize two inherited capability cells without constructing a provider."""
    ...
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11.1 capability import requires its exact two registered cells"
        )
    _assert_prerequisites(
        contract,
        "capability-gate",
    ...
    try:
        ...
        wrappers = capability_wrappers_from_v2111_receipt(receipt)
    except PilotV2111ParentImportError as exc:
        raise PilotOrchestrationError(
            f"V2.11.1 capability import source failed validation: {exc}"
        ) from exc
    ...
    for spec in specs:
        ...
        projection = RunProjection(
            ...
            storage_bytes=1_000_000,
            basis={
                "method": "v2.11.1-inherited-capability-wrapper",
                "provider_construction": False,
                "provider_calls_current_attempt": 0,
                "historical_source_calls": 30,
                "hosted_completion_cap_counted": False,
        ...
        _persist_exact_json(capability_path, wrapper)
        _, bootstrap_path = _persist_v2111_bootstrap_projection(
            contract,
            spec,
            raw_root=raw_root,
            repo_root=repo_root,
        ...
        payload = {
            ...
            "provider_calls": 0,
            "claim_boundary": (
                "Inherited preregistered capability/interface evidence only; "
                "not treatment-effect, preflight, or model-performance evidence."
            ),
        }
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
            ...
            if terminal.get("payload") != payload:
                raise PilotOrchestrationError(
                    f"{spec.run_id} imported capability terminal differs on resume"
                )
        else:
            write_terminal_summary(
                terminal_path,
        ...
        if run_ledger.status(spec.run_id) == "scheduled":
            ...
            raise PilotOrchestrationError(
                f"{spec.run_id} imported capability ledger status drifted"
            )
        if budget_row is None or budget_row.get("status") == "reserved":
            budget_ledger.finalize(
                spec.run_id,
...
def _execute_v211_parent_import_stage(
    contract: PilotContract,
    specs: Sequence[PilotRunSpec],
    *,
    raw_root: Path,
    ...
    """Execute V2.11's one-cell, zero-provider parent authority import."""

    if contract.contract_id != V211_CONTRACT_ID:
        raise PilotOrchestrationError(
            "dedicated V2.11 parent executor requires the V2.11 contract"
        )
    if (
        len(specs) != 1
        or specs[0].stage_id != "parent-import"
        ...
        raise PilotOrchestrationError(
            "V2.11 parent import requires one exact zero-call contract cell"
        )
    if parent_repo_root is None:
        raise PilotOrchestrationError(
            "V2.11 parent-import requires --parent-repo-root pointing to "
            "the immutable V2.10.2 science checkout"
        )
    boundary = contract.v211_forward_boundary
    if not isinstance(boundary, Mapping):
        raise PilotOrchestrationError("V2.11 forward boundary is unavailable")
    expected_count = int(boundary["matrix"]["ledger_cells"])
    expanded = tuple(contract.expand())
    rows = run_ledger.snapshot().get("runs")
    if (
        ...
        raise PilotOrchestrationError(
            "V2.11 parent import requires the exact 136-cell ITT registration"
        )

    spec = specs[0]
    projection = _v211_parent_import_projection(spec)
    budget_row = budget_ledger.snapshot()["runs"].get(spec.run_id)
    if budget_row is None:
        budget_ledger.reserve(projection)
    elif budget_row.get("reservation") != projection.to_dict():
        raise PilotOrchestrationError("V2.11 parent-import budget reservation drifted")

    entry_status = run_ledger.status(spec.run_id)
    if entry_status != "scheduled" and entry_status != "complete":
        row = run_ledger.snapshot()["runs"][spec.run_id]
        failure = row.get("failure")
        if not isinstance(failure, Mapping):
            raise PilotOrchestrationError(
                "V2.11 terminal parent no-go lacks failure evidence"
            )
        _propagate_stage_no_go(
            contract,
            source_stage="parent-import",
    ...
    try:
        ...
        result = persist_v211_parent_import(
            repo_root=repo_root,
            science_repo_root=parent_repo_root,
            destination=receipt_path,
        )
        ...
        if expected != verified:
            raise PilotOrchestrationError(
                "V2.11 persisted parent receipt differs from its source replay"
            )
        zero_boundary = {
            "provider_construction": False,
            "provider_calls": 0,
        ...
        if any(result.get(key) != value for key, value in zero_boundary.items()):
            raise PilotOrchestrationError(
                "V2.11 parent importer violated its zero-provider/effect boundary"
            )
        calibration = _load_verified_v211_calibration(
            contract,
            raw_root=raw_root,
        ...
        payload = _v211_parent_terminal_payload(result, calibration)
        if terminal_path.exists():
            terminal = _load_v2_terminal_summary(
                contract,
                spec,
            ...
            if (
                ...
                raise PilotOrchestrationError("V2.11 parent terminal differs on resume")
        else:
            write_terminal_summary(
                terminal_path,
                contract=contract,
        ...
        if run_ledger.status(spec.run_id) == "complete":
            # A complete row is immutable. A later resume-verification error
            # must be surfaced without rewriting its denominator state.
            raise
        failure = {
            "error_type": "V211ParentImportIntegrityError",
            "cause_type": type(exc).__name__,
            "message": str(exc),
            "provider_construction": False,
            "provider_calls": 0,
        ...
        raise PilotOrchestrationError(
            f"V2.11 parent import failed; receipt={stage_receipt}"
        ) from exc
...
def _execute_v24_parent_import_stage(
    ...
    *,
    ...
    if contract.contract_id == V2102_CONTRACT_ID:
        contract_label = "V2.10.2"
        persist_parent_import = _persist_v2102_parent_import_for_orchestrator
        failure_schema = PILOT_V2102_PARENT_FAILURE_INTENT_SCHEMA_VERSION
    elif contract.contract_id == V2101_CONTRACT_ID:
        contract_label = "V2.10.1"
        persist_parent_import = _persist_v2101_parent_import_for_orchestrator
        failure_schema = PILOT_V2101_PARENT_FAILURE_INTENT_SCHEMA_VERSION
    elif contract.contract_id == V210_CONTRACT_ID:
        contract_label = "V2.10"
        persist_parent_import = _persist_v210_parent_import_for_orchestrator
        failure_schema = PILOT_V210_PARENT_FAILURE_INTENT_SCHEMA_VERSION
    elif contract.contract_id == V29_CONTRACT_ID:
        contract_label = "V2.9"
        ...
        raise PilotOrchestrationError(
            ...
            "V2.4/V2.5/V2.6/V2.7/V2.8/V2.9/V2.10/V2.10.1/V2.10.2 contracts"
        )
    ...
    entry_run_status = (
        ...
        else ("complete" if run_ledger.is_terminal(spec.run_id) else "scheduled")
    )
    ...
    if run_ledger.is_terminal(spec.run_id):
        ...
        if transactional_parent:
            verifier = (
                _verify_v2102_importer_p95_profiles
                if contract.contract_id == V2102_CONTRACT_ID
                else (
                    _verify_v2101_importer_p95_profiles
                    if contract.contract_id == V2101_CONTRACT_ID
                    else (
                        _verify_v210_importer_p95_profiles
                        if contract.contract_id == V210_CONTRACT_ID
                        else (
    ...
    try:
        ...
        if transactional_parent:
            intent_path = _v27_parent_commit_intent_path(raw_root=raw_root)
        ...
        if transactional_parent and run_ledger.is_terminal(spec.run_id):
            budget_status = (
                None
                if budget_ledger is None
                else budget_ledger.snapshot()["runs"].get(spec.run_id, {}).get("status")
            )
            raise PilotOrchestrationError(
...
def _execute_stage_locked(
    *,
    ...
    if preflight_amendment_control is not None:
        preflight_control_path = preflight_amendment_control_path(raw_root=root)
        _persist_exact_json(
    ...
    if stage_id == "parent-import":
        if contract.contract_id == V2114_CONTRACT_ID:
            ...
            return _execute_v2114_parent_import_stage(
                contract,
                specs,
                raw_root=root,
                repo_root=repository,
        ...
        if contract.contract_id == V2113_CONTRACT_ID:
            ...
            return _execute_v2113_parent_import_stage(
                contract,
                specs,
                raw_root=root,
                repo_root=repository,
        ...
        if contract.contract_id == V2112_CONTRACT_ID:
            parent_budget_ledger = PilotBudgetLedger(
                ...
                caps=_budget_caps(contract),
                tamper_evident=True,
                parent_debit=_parent_budget_debit(
                    contract,
                    repo_root=repository,
                    parent_repo_root=parent_repo_root,
                ),
            )
            return _execute_v2112_parent_import_stage(
                contract,
                specs,
                raw_root=root,
                repo_root=repository,
        ...
        if contract.contract_id == V2111_CONTRACT_ID:
            ...
            return _execute_v2111_parent_import_stage(
                contract,
                specs,
                raw_root=root,
                repo_root=repository,
        ...
        if contract.contract_id == V211_CONTRACT_ID:
            ...
            return _execute_v211_parent_import_stage(
                contract,
                specs,
                raw_root=root,
                repo_root=repository,
    ...
    if (
        ...
        and stage_id == "q-ref-resolution"
    ):
        q_ref_budget_ledger = PilotBudgetLedger(
            root / "budget_ledger.json",
            contract_hash=contract.canonical_hash,
            caps=_budget_caps(contract),
            tamper_evident=True,
        ...
        qref_executor = (
            _execute_v2102_q_ref_import_stage
            if contract.contract_id == V2102_CONTRACT_ID
            else (
                _execute_v2101_q_ref_import_stage
                if contract.contract_id == V2101_CONTRACT_ID
                else (
                    _execute_v210_q_ref_import_stage
                    if contract.contract_id == V210_CONTRACT_ID
                    else _execute_v27_q_ref_import_stage
                )
            )
        )
        return qref_executor(
            contract,
            specs,
            raw_root=root,
            paid=paid,
            run_ledger=run_ledger,
            budget_ledger=q_ref_budget_ledger,
        )
    budget_ledger = PilotBudgetLedger(
        root / "budget_ledger.json",
        contract_hash=contract.canonical_hash,
    ...
    if contract.contract_id == V2114_CONTRACT_ID and stage_id == "capability-gate":
        return _execute_v2114_capability_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    if contract.contract_id == V2113_CONTRACT_ID and stage_id == "capability-gate":
        return _execute_v2113_capability_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    if (
        ...
        return _execute_v2114_preflight_authority_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    if (
        ...
        # This dedicated zero-provider executor is the only permitted path.
        # In particular, V2.11.3 must never fall through to the generic paid
        # ``closed_loop_preflight`` machinery.
        return _execute_v2113_preflight_authority_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    if contract.contract_id == V2112_CONTRACT_ID and stage_id == "capability-gate":
        return _execute_v2112_capability_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    if contract.contract_id == V2111_CONTRACT_ID and stage_id == "capability-gate":
        return _execute_v2111_capability_import_stage(
            contract,
            specs,
            raw_root=root,
            repo_root=repository,
    ...
    try:
        if (
            ...
            from .pilot_v2114_acceptance import (
                V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME,
                verify_v2114_scientific_dispatch_acceptance,
            )

            verify_v2114_scientific_dispatch_acceptance(
                root / V2114_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME,
                contract=contract,
                repo_root=repository,
                raw_root=root,
                paid=paid,
        ...
        if (
            ...
            # The complete 131-cell matrix must be accepted on the clean tag,
            # before credentials are loaded.  Keep this local import to avoid
            # a module cycle: the acceptance auditor deliberately reuses the
            # pure contract/config/projection helpers in this module.
            from .pilot_v2113_acceptance import (
                V2113_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME,
                verify_v2113_scientific_dispatch_acceptance,
            )

            verify_v2113_scientific_dispatch_acceptance(
                root / V2113_SCIENTIFIC_DISPATCH_ACCEPTANCE_FILENAME,
                contract=contract,
                repo_root=repository,
                raw_root=root,
                paid=paid,
    ...
    for model_id in contract.models_for_stage(stage_id):
        ...
        try:
            if contract.contract_id == PILOT_V21_CONTRACT_ID and profile.transport in {
                "openai",
                "openrouter",
            }:
    ...
    if (
        ...
        # Both registered models must finish the exact 30+32 gate denominator.
        # Capability content no-go is adjudicated only after the long-context
        # sample; provider/integrity failures remain terminal and issue no
        # replacement calls.
        gate_model_go = None
    ...
    if _is_experiment_d_stage(stage_id):
        groups = sorted({(spec.model_id, spec.environment_seed) for spec in specs})
        for model_id, seed in groups:
            group = tuple(
                spec
                for spec in specs
    ...
    v211_post_gate_receipt: Mapping[str, Any] | None = None
    if (
        contract.contract_id in V211_FAMILY_CONTRACT_IDS
        and stage_id == PILOT_V211_POST_GATE_STAGE_ID
    ):
        try:
            persist_post_gate = (
                ...
                if contract.contract_id == V2112_CONTRACT_ID
                    ...
                    if contract.contract_id == V2111_CONTRACT_ID
                    else _persist_v211_post_gate_authority
                )
            )
            post_gate_path, v211_post_gate_receipt = persist_post_gate(
                contract,
                raw_root=root,
                paid=paid,
                budget_ledger=budget_ledger,
            ...
            stage_artifacts["post_gate_authority"] = {
                "path": str(post_gate_path),
                "file_sha256": _file_sha256(post_gate_path),
                "receipt_sha256": v211_post_gate_receipt["receipt_sha256"],
                "go": v211_post_gate_receipt["go"],
                "eligible_model_ids": v211_post_gate_receipt["denominator"][
                    "eligible_model_ids"
                ],
                "provider_calls_during_authority": 0,
            }
            ...
            _propagate_stage_no_go(
                ...
                ledger=run_ledger,
                failure={
                    "error_type": "PostGateAuthorityFailure",
                    "cause_type": type(exc).__name__,
                    "message": str(exc),
                },
                status="integrity-stopped",
    ...
    if (
        ...
        status = (
            ...
            if (
                complete
                and isinstance(v211_post_gate_receipt, Mapping)
                and v211_post_gate_receipt.get("go") is True
                and set(v211_post_gate_receipt["denominator"]["eligible_model_ids"])
                == set(contract.models_for_stage(stage_id))
            )
            else "complete-with-no-go"
        )
    elif _is_capability_stage(contract, stage_id):
        ...
        status = "complete-with-no-go"
    if _is_capability_stage(contract, stage_id) and not (
        contract.contract_id in V211_FAMILY_CONTRACT_IDS
...
def _bootstrap_development_utility(
    ...
    *,
    ...
    selection_path = raw_root / "stage0-calibration" / "stage0_selection.json"
    existing = {
        "manifest": qref_dir / "manifest.json",
        "q_ref_resolution": qref_path,
        "stage0_selection": selection_path,
    }
    if resume and any(path.exists() for path in existing.values()):
        if not all(path.is_file() for path in existing.values()):
            raise PilotOrchestrationError(
                "development utility bootstrap is partial; refusing to "
                "silently regenerate it on resume"
            )
        result = load_verified_run_artifacts(qref_dir)
        resolution = build_q_ref_resolution(
            result,
            contract_hash=contract.canonical_hash,
            environment_source_hash=hashlib.sha256(
        ...
        resolution["source_manifest"] = str(existing["manifest"])
        resolution["diagnostic_only"] = True
        resolution["scientific_evidence"] = False
        if _read_json(qref_path) != resolution:
            raise PilotOrchestrationError(
                "development q_ref bootstrap differs from sealed run artifacts"
            )
        ...
        expected_selection = {
            ...
            "scientific_evidence": False,
            "evidence_boundary": (
                "This fixed-center development fixture only exercises A-D "
                "plumbing; it is not Stage-0 scientific calibration."
            ),
        }
        if _read_json(selection_path) != expected_selection:
            raise PilotOrchestrationError(
                "development Stage-0 selection differs on resume"
            )
        return {
...
def run_development_fake_matrix(
    *,
    ...
    root.mkdir(parents=True, exist_ok=True)
    bootstrap = _bootstrap_development_utility(
        contract,
        raw_root=root,
        resume=resume,
    )
    ...
    if contract.contract_id in V211_FAMILY_CONTRACT_IDS:
        forward_boundary = (
            contract.v2114_forward_boundary
            if contract.contract_id == V2114_CONTRACT_ID
            else (
                contract.v2113_forward_boundary
                if contract.contract_id == V2113_CONTRACT_ID
                else (
                    contract.v2112_forward_boundary
                    if contract.contract_id == V2112_CONTRACT_ID
                    else (
                        contract.v211_forward_boundary
                        if contract.contract_id == V211_CONTRACT_ID
                        else contract.v2111_forward_boundary
                    )
                )
            )
        )
        if forward_boundary is None:  # pragma: no cover - parser
            raise PilotOrchestrationError(
                "V2.11-family development matrix lacks its frozen parent debit"
            )
        development_parent_debit = ParentBudgetDebit.from_dict(
            forward_boundary["parent_budget_debit"]
        )
    else:
    ...
    d_specs = tuple(spec for spec in selected if _is_experiment_d_stage(spec.stage_id))
    actor_specs = tuple(
    ...
    d_groups = sorted(
        {(spec.stage_id, spec.model_id, spec.environment_seed) for spec in d_specs}
    )
