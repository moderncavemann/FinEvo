    ...
    MappingProxyType(
        {
            ...
            "finevo-pilot-v2.11.4-post-gate-authority-v1": (
                "v2.11.4-post-gate-authority"
            ),
            "finevo-pilot-v2.11.5-post-gate-authority-v1": (
                "v2.11.5-post-gate-authority"
            ),
            "finevo-pilot-v2.11.6-continuation-observed-p95-authority-v1": (
                "v2.11.6-continuation-authority"
            ),
            "finevo-pilot-v2.11.7-continuation-observed-p95-authority-v1": (
                "v2.11.7-continuation-authority"
            ),
...
_V2114_POST_GATE_AUTHORITY_PATH = PurePosixPath(
    "experiment_results/pilot-v2.11.4/raw/long-context-preflight/"
    "post_gate_authority.json"
)
_V2115_POST_GATE_AUTHORITY_PATH = PurePosixPath(
    "experiment_results/pilot-v2.11.5/raw/long-context-preflight/"
    "post_gate_authority.json"
)
_V2116_CONTINUATION_AUTHORITY_PATH = PurePosixPath(
    "experiment_results/pilot-v2.11.6/raw/parent-import/current_authority/"
    "post_gate_authority.json"
)
_V2117_CONTINUATION_AUTHORITY_PATH = PurePosixPath(
    "experiment_results/pilot-v2.11.7/raw/parent-import/current_authority/"
    "post_gate_authority.json"
)
...
_V21110_CONTINUATION_AUTHORITY_PATH = PurePosixPath(
...
def _verified_dedicated_observed_p95_binding(
    ...
    *,
    ...
    if adapter_id == "v2.10.1-resealed-with-sibling-projection":
        ...
        return _json_copy(binding)
    elif adapter_id == "v2.11.4-post-gate-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_4_CANONICAL_SHA256
        ...
        if schema_version != V2114_GATE_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.4 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V2114_POST_GATE_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.4 post-gate authority is outside its exact frozen "
                "current-release path"
            )
        if PILOT_CONTRACT_V2_11_4_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.4 frozen contract identity is unavailable"
            )
        try:
            ...
            raise ObservedP95AuthorityError(
                "V2.11.4 post-gate observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        ...
        if (
            ...
            raise ObservedP95AuthorityError(
                "v2.11.4-post-gate-authority receipt bytes or flat binding "
                "changed during dedicated verification"
            )
        return _json_copy(binding)
    elif adapter_id == "v2.11.5-post-gate-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_5_CANONICAL_SHA256
        ...
        if schema_version != V2115_GATE_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.5 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V2115_POST_GATE_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.5 post-gate authority is outside its exact frozen "
                "current-release path"
            )
        if PILOT_CONTRACT_V2_11_5_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.5 frozen contract identity is unavailable"
            )
        try:
        ...
        except PilotV2115GateError as exc:
            raise ObservedP95AuthorityError(
                "V2.11.5 post-gate observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        ...
        if (
            ...
            raise ObservedP95AuthorityError(
                "v2.11.5-post-gate-authority receipt bytes or flat binding "
                "changed during dedicated verification"
            )
        return _json_copy(binding)
    elif adapter_id == "v2.11.6-continuation-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_6_CANONICAL_SHA256
        ...
        if schema_version != V2116_CURRENT_AUTHORITY_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.6 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V2116_CONTINUATION_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.6 continuation authority is outside its exact path"
            )
        if PILOT_CONTRACT_V2_11_6_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.6 frozen contract identity is unavailable"
            )
        try:
            ...
            raise ObservedP95AuthorityError(
                "V2.11.6 continuation observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        integrity = receipt.get("integrity")
        expected_binding = {
        ...
        if (
            ...
            raise ObservedP95AuthorityError(
                "V2.11.6 continuation receipt bytes or binding changed during "
                "dedicated verification"
            )
        return _json_copy(binding)
    elif adapter_id == "v2.11.7-continuation-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_7_CANONICAL_SHA256
        from .pilot_v2117_continuation import (
            PilotV2117ContinuationError,
            V2117_CURRENT_AUTHORITY_SCHEMA_VERSION,
            verified_v2117_observed_p95_authority_binding,
        )

        if schema_version != V2117_CURRENT_AUTHORITY_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.7 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V2117_CONTINUATION_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.7 continuation authority is outside its exact path"
            )
        if PILOT_CONTRACT_V2_11_7_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.7 frozen contract identity is unavailable"
            )
        try:
        ...
        except PilotV2117ContinuationError as exc:
            raise ObservedP95AuthorityError(
                "V2.11.7 continuation observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        integrity = receipt.get("integrity")
        expected_binding = {
        ...
        if (
            ...
            raise ObservedP95AuthorityError(
                "V2.11.7 continuation receipt bytes or binding changed during "
                "dedicated verification"
            )
        return _json_copy(binding)
    elif adapter_id == "v2.11.8-continuation-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_8_CANONICAL_SHA256
        from .pilot_v2118_continuation import (
            PilotV2118ContinuationError,
            V2118_CURRENT_AUTHORITY_SCHEMA_VERSION,
            verified_v2118_observed_p95_authority_binding,
        )

        if schema_version != V2118_CURRENT_AUTHORITY_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.8 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V2118_CONTINUATION_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.8 continuation authority is outside its exact path"
            )
        if PILOT_CONTRACT_V2_11_8_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.8 frozen contract identity is unavailable"
            )
        try:
        ...
        except PilotV2118ContinuationError as exc:
            raise ObservedP95AuthorityError(
                "V2.11.8 continuation observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        integrity = receipt.get("integrity")
        expected_binding = {
        ...
        return _json_copy(binding)
    elif adapter_id == "v2.11.10-continuation-authority":
        from .pilot_contract import PILOT_CONTRACT_V2_11_10_CANONICAL_SHA256
        ...
        if schema_version != V21110_CURRENT_AUTHORITY_SCHEMA_VERSION:
            raise ObservedP95AuthorityError(
                "V2.11.10 observed-p95 producer/consumer schema registry drifted"
            )
        if relative != _V21110_CONTINUATION_AUTHORITY_PATH:
            raise ObservedP95AuthorityError(
                "V2.11.10 continuation authority is outside its exact path"
            )
        if PILOT_CONTRACT_V2_11_10_CANONICAL_SHA256 is None:
            raise ObservedP95AuthorityError(
                "V2.11.10 frozen contract identity is unavailable"
            )
        try:
        ...
        except PilotV21110ContinuationError as exc:
            raise ObservedP95AuthorityError(
                "V2.11.10 continuation observed-p95 authority failed validation: "
                f"{exc}"
            ) from exc
        integrity = receipt.get("integrity")
        expected_binding = {
        ...
        if (
            ...
            raise ObservedP95AuthorityError(
                "V2.11.10 continuation receipt bytes or binding changed during "
                "dedicated verification"
            )
        return _json_copy(binding)
