#!/usr/bin/env python3
"""Render the independently canonicalized V2.11.3 consumer-adapter draft."""
...
from verified_memory.pilot_contract import (
    ...
    PilotContract,
    _v2_11_3_expected_consumer_adapter_amendment,
    _v2_11_3_expected_forward_boundary,
...
TRACKED_CONTRACT_PATH = ROOT / "experiments" / "pilot_v2_11_3.yaml"
...
def _assert_v2112_science_delta(
    parent: Mapping[str, Any], child: Mapping[str, Any]
) -> None:
    """Fail closed if the adapter repair changes a scientific choice."""
    ...
    if drifted:
        raise ValueError(
            "V2.11.3 changed frozen V2.11.2 scientific fields: " + ", ".join(drifted)
        )
    if _science_arms(parent) != _science_arms(child):
        raise ValueError("V2.11.3 changed frozen V2.11.2 scientific arms")
    if _science_stages(parent) != _science_stages(child):
        raise ValueError("V2.11.3 changed frozen V2.11.2 scientific stages")
    ...
    if parent_denominator != child_denominator:
        raise ValueError("V2.11.3 changed the frozen denominator semantics")
    ...
    if actual_operational_modes != expected_operational_modes:
        raise ValueError("V2.11.3 operational authority modes drifted")
    if child["arms"]["closed-loop-preflight"] != (_v2_11_3_expected_preflight_arm()):
        raise ValueError("V2.11.3 preflight authority arm drifted")
...
def _assert_expanded_science_specs_match(
    ...
    if len(parent_specs) != 136 or len(child_specs) != 136:
        raise ValueError("V2.11.2/V2.11.3 denominator is not exactly 136 cells")
    if len({spec.run_id for spec in child_specs}) != 136:
        raise ValueError("V2.11.3 denominator contains duplicate run IDs")
    if _normalized_science_specs(parent) != _normalized_science_specs(child):
        raise ValueError("V2.11.3 expanded scientific run specs drifted")
...
def build_contract(
    ...
    *,
    ...
    """Build V2.11.3 without reopening the immutable V2.11.2 denominator."""
    ...
    parent_contract = load_pilot_contract(
        repo_root / "experiments" / "pilot_v2_11_2.yaml"
    )
    parent = parent_contract.to_dict()
    contract = json.loads(json.dumps(parent))
    contract.pop("v2112_forward_boundary")
    contract.pop("v2112_recovery_amendment")
    contract["contract_id"] = PILOT_CONTRACT_ID_V2_11_3
    ...
    contract["implementation"] = {
        ...
        "required_git_tag": PILOT_CONTRACT_TAG_V2_11_3,
    }
    contract["arms"]["closed-loop-preflight"] = _v2_11_3_expected_preflight_arm()
    contract["model_roles"] = _v2_11_3_expected_model_roles()
    contract["budgets"] = {
    ...
    contract["denominator_policy"]["policy_id"] = "finevo-pilot-v2.11.3-itt"
    contract["stages"] = _v2_11_3_expected_stages()
    contract["non_claims"] = _v2_11_3_expected_non_claims()
    contract["v2113_forward_boundary"] = _v2_11_3_expected_forward_boundary()
    contract["v2113_consumer_adapter_amendment"] = (
        _v2_11_3_expected_consumer_adapter_amendment()
    )
    ...
    _assert_v2112_science_delta(parent, contract)
    contract["integrity"]["declared_sha256"] = canonical_contract_sha256(contract)
    if frozen_candidate:
        pinned = pilot_contract_module.PILOT_CONTRACT_V2_11_3_CANONICAL_SHA256
        if pinned is not None:
            raise FrozenCandidateBootstrapError(
                "V2.11.3 canonical hash is already pinned; use normal frozen mode"
            )
        candidate_hash = contract["integrity"]["declared_sha256"]
        # Candidate mode may satisfy exactly one otherwise-circular release
        # identity check with its own independently computed hash. All other
        # PilotContract validation remains active, and the module-level pin is
        # restored even if validation fails. This candidate is not dispatchable:
        # after pinning its hash, the ordinary frozen render must reproduce it.
        pilot_contract_module.PILOT_CONTRACT_V2_11_3_CANONICAL_SHA256 = candidate_hash
        try:
            parsed = PilotContract.from_dict(contract)
        finally:
            pilot_contract_module.PILOT_CONTRACT_V2_11_3_CANONICAL_SHA256 = pinned
        if (
        ...
        # Normal frozen rendering deliberately retains the production parser's
        # pinned-constant requirement. It cannot bootstrap itself.
        parsed = PilotContract.from_dict(contract)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/pilot_v2_11_3.yaml"),
    )
    ...
    if args.frozen_candidate and output == TRACKED_CONTRACT_PATH.resolve(strict=True):
        raise SystemExit(
