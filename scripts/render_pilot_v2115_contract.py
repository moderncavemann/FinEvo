#!/usr/bin/env python3
"""Render the independently canonicalized V2.11.5 consumer-binding draft."""
...
from verified_memory.pilot_contract import (
    ...
    _v2_11_5_expected_forward_boundary,
...
TRACKED_CONTRACT_PATH = ROOT / "experiments" / "pilot_v2_11_5.yaml"
...
def _assert_v2114_science_delta(
    ...
    if drifted:
        raise ValueError(
            "V2.11.5 changed frozen V2.11.4 scientific fields: " + ", ".join(drifted)
        )
    if _science_arms(parent) != _science_arms(child):
        raise ValueError("V2.11.5 changed frozen V2.11.4 scientific arms")
    if _science_stages(parent) != _science_stages(child):
        raise ValueError("V2.11.5 changed frozen V2.11.4 scientific stages")
    ...
    if parent_denominator != child_denominator:
        raise ValueError("V2.11.5 changed the frozen denominator semantics")
    ...
    if actual_operational_modes != expected_operational_modes:
        raise ValueError("V2.11.5 operational authority modes drifted")
    if child["arms"]["closed-loop-preflight"] != (_v2_11_5_expected_preflight_arm()):
        raise ValueError("V2.11.5 preflight authority arm drifted")
...
def _assert_expanded_science_specs_match(
    ...
    if len(parent_specs) != 136 or len(child_specs) != 136:
        raise ValueError("V2.11.4/V2.11.5 denominator is not exactly 136 cells")
    if len({spec.run_id for spec in child_specs}) != 136:
        raise ValueError("V2.11.5 denominator contains duplicate run IDs")
    if _normalized_science_specs(parent) != _normalized_science_specs(child):
        raise ValueError("V2.11.5 expanded scientific run specs drifted")
...
def _parse_with_bootstrap_design_pin(contract: Mapping[str, Any]) -> PilotContract:
    ...
    pinned = pilot_contract_module.PILOT_CONTRACT_V2_11_5_SCIENCE_DESIGN_SHA256
    if pinned is not None:
        return PilotContract.from_dict(contract)
    if contract.get("status") != "draft":
        raise FrozenCandidateBootstrapError(
            "V2.11.5 science-design pin must be set before frozen rendering"
        )
    candidate = science_design_sha256(contract)
    pilot_contract_module.PILOT_CONTRACT_V2_11_5_SCIENCE_DESIGN_SHA256 = candidate
    try:
        return PilotContract.from_dict(contract)
    finally:
        pilot_contract_module.PILOT_CONTRACT_V2_11_5_SCIENCE_DESIGN_SHA256 = pinned
...
def build_contract(
    ...
    *,
    ...
    """Build V2.11.5 without reopening the V2.11.4 no-go namespace."""
    ...
    parent_contract = load_pilot_contract(
        repo_root / "experiments" / "pilot_v2_11_4.yaml"
    )
    parent = parent_contract.to_dict()
    contract = json.loads(json.dumps(parent))
    contract.pop("v2114_forward_boundary")
    contract.pop("v2114_authority_normalization_amendment")
    contract["contract_id"] = PILOT_CONTRACT_ID_V2_11_5
    ...
    contract["implementation"] = {
        ...
        "required_git_tag": PILOT_CONTRACT_TAG_V2_11_5,
    }
    contract["arms"]["closed-loop-preflight"] = _v2_11_5_expected_preflight_arm()
    contract["model_roles"] = _v2_11_5_expected_model_roles()
    contract["budgets"] = {
    ...
    contract["denominator_policy"]["policy_id"] = "finevo-pilot-v2.11.5-itt"
    contract["stages"] = _v2_11_5_expected_stages()
    contract["non_claims"] = _v2_11_5_expected_non_claims()
    contract["v2115_forward_boundary"] = _v2_11_5_expected_forward_boundary(
        status=status
    )
    contract["v2115_consumer_authority_normalization_amendment"] = (
        _v2_11_5_expected_consumer_authority_normalization_amendment()
    )
    contract["release_requirements"] = {
        "remote": "origin",
        "branch": "main",
        "tag": PILOT_CONTRACT_TAG_V2_11_5,
    ...
    _assert_v2114_science_delta(parent, contract)
    contract["integrity"]["declared_sha256"] = canonical_contract_sha256(contract)

    if frozen_candidate:
        if (
            pilot_contract_module.PILOT_V2_11_5_SOURCE_MANIFEST_FILE_SHA256 is None
            or pilot_contract_module.PILOT_V2_11_5_SOURCE_MANIFEST_CONTENT_SHA256
            is None
        ):
            raise FrozenCandidateBootstrapError(
                "V2.11.5 source manifest must be sealed before frozen candidate"
            )
        pinned = pilot_contract_module.PILOT_CONTRACT_V2_11_5_CANONICAL_SHA256
        if pinned is not None:
            raise FrozenCandidateBootstrapError(
                "V2.11.5 canonical hash is already pinned; use normal frozen mode"
            )
        candidate_hash = contract["integrity"]["declared_sha256"]
        pilot_contract_module.PILOT_CONTRACT_V2_11_5_CANONICAL_SHA256 = candidate_hash
        try:
            parsed = PilotContract.from_dict(contract)
        finally:
            pilot_contract_module.PILOT_CONTRACT_V2_11_5_CANONICAL_SHA256 = pinned
        if (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/pilot_v2_11_5.yaml"),
    )
