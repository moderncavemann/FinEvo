#!/usr/bin/env python3
"""Render the prospective V2.11.8 observed-p95 context recovery contract."""
...
from verified_memory.pilot_contract import (
    ...
    _v2_11_8_expected_recovery_boundary,
...
TRACKED_CONTRACT_PATH = ROOT / "experiments" / "pilot_v2_11_8.yaml"
...
def _assert_v2117_recovery_delta(
    ...
    *,
    ...
    if drifted:
        raise ValueError(
            "V2.11.8 changed frozen V2.11.7 continuation fields: " + ", ".join(drifted)
        )
    ...
    if {key: parent["arms"][key] for key in arm_ids} != {
        ...
        raise ValueError("V2.11.8 changed a remaining V2.11.7 scientific arm")
    if child["arms"]["parent-import"] != _v2_11_8_expected_parent_import_arm():
        raise ValueError("V2.11.8 recovery import semantics drifted")
    ...
    if parent_roles != child_roles:
        raise ValueError("V2.11.8 changed a model role outside its stage allowlist")
    if child["model_roles"] != _v2_11_8_expected_model_roles():
        raise ValueError("V2.11.8 continuation stage allowlists drifted")
    ...
    if parent_denominator != child_denominator:
        raise ValueError("V2.11.8 changed the frozen denominator semantics")

    if _normalized_continuation_specs(parent_contract) != (
        _normalized_continuation_specs(child_contract)
    ):
        raise ValueError("V2.11.8 changed a normalized remaining run spec")
...
def _assert_expanded_continuation_specs_match(
    ...
    if len(parent.expand()) != 87:
        raise ValueError("V2.11.7 no-go denominator is not exactly 87 cells")
    if len(parent_rows) != 86 or len(child_rows) != 86:
        raise ValueError("V2.11.8 must map exactly 86 remaining science cells")
    if len(child_specs) != 87 or len({spec.run_id for spec in child_specs}) != 87:
        raise ValueError("V2.11.8 denominator is not 87 unique cells")
    if parent_rows != child_rows:
        raise ValueError("V2.11.8 normalized continuation specs drifted")
    if canonical_sha256(parent_rows) != (
        "9968bb55b9c56ced90f56826bc8e186f72299e0a8bb40dfdb4fbb1e637af1632"
    ):
        raise ValueError("V2.11.7 normalized continuation digest drifted")
    ...
    if len(import_specs) != 1 or import_specs[0].execution_mode != (
        ...
        raise ValueError("V2.11.8 must contain one zero-provider import cell")
...
def _parse_with_bootstrap_design_pin(contract: Mapping[str, Any]) -> PilotContract:
    ...
    pinned = pilot_contract_module.PILOT_CONTRACT_V2_11_8_SCIENCE_DESIGN_SHA256
    if pinned is not None:
        return PilotContract.from_dict(contract)
    if contract.get("status") != "draft":
        raise FrozenCandidateBootstrapError(
            "V2.11.8 science-design pin must be set before frozen rendering"
        )
    candidate = science_design_sha256(contract)
    pilot_contract_module.PILOT_CONTRACT_V2_11_8_SCIENCE_DESIGN_SHA256 = candidate
    try:
        return PilotContract.from_dict(contract)
    finally:
        pilot_contract_module.PILOT_CONTRACT_V2_11_8_SCIENCE_DESIGN_SHA256 = pinned
...
def build_contract(
    ...
    *,
    ...
    """Build V2.11.8 without reopening the immutable V2.11.7 raw namespace."""
    ...
    parent_contract = load_pilot_contract(
        repo_root / "experiments" / "pilot_v2_11_7.yaml"
    )
    parent = parent_contract.to_dict()
    contract = json.loads(json.dumps(parent))
    contract.pop("v2117_recovery_boundary")
    contract["contract_id"] = PILOT_CONTRACT_ID_V2_11_8
    ...
    contract["implementation"] = {
        ...
        "required_git_tag": PILOT_CONTRACT_TAG_V2_11_8,
    }
    contract["arms"]["parent-import"] = _v2_11_8_expected_parent_import_arm()
    contract["model_roles"] = _v2_11_8_expected_model_roles()
    contract["budgets"] = {
    ...
    contract["denominator_policy"]["policy_id"] = "finevo-pilot-v2.11.8-itt"
    contract["stages"] = _v2_11_8_expected_stages()
    contract["non_claims"] = _v2_11_8_expected_non_claims()
    contract["v2118_recovery_boundary"] = _v2_11_8_expected_recovery_boundary(
        status=status
    )
    contract["release_requirements"] = {
        "remote": "origin",
        "branch": "main",
        "tag": PILOT_CONTRACT_TAG_V2_11_8,
    ...
    if frozen_candidate:
        if (
            pilot_contract_module.PILOT_V2_11_8_SOURCE_MANIFEST_FILE_SHA256 is None
            or pilot_contract_module.PILOT_V2_11_8_SOURCE_MANIFEST_CONTENT_SHA256
            is None
        ):
            raise FrozenCandidateBootstrapError(
                "V2.11.8 source manifest must be sealed before frozen candidate"
            )
        pinned = pilot_contract_module.PILOT_CONTRACT_V2_11_8_CANONICAL_SHA256
        if pinned is not None:
            raise FrozenCandidateBootstrapError(
                "V2.11.8 canonical hash is already pinned; use normal frozen mode"
            )
        candidate_hash = contract["integrity"]["declared_sha256"]
        pilot_contract_module.PILOT_CONTRACT_V2_11_8_CANONICAL_SHA256 = candidate_hash
        try:
            parsed = PilotContract.from_dict(contract)
            _assert_v2117_recovery_delta(parent, contract, parsed_child=parsed)
            _assert_expanded_continuation_specs_match(parent_contract, parsed)
        finally:
            pilot_contract_module.PILOT_CONTRACT_V2_11_8_CANONICAL_SHA256 = pinned
        if (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/pilot_v2_11_8.yaml"),
    )
