...
from verified_memory import pilot_v2113_gate as gate
...
from tests.test_pilot_v2113_gate import FrozenContract
...
def frozen_v2113_release(
    ...
    (repo_root / "experiments" / "pilot_v2_11_3.yaml").write_text(
    ...
    for model_id in ("gpt52_main", "gpt56_diagnostic"):
        gate.persist_v2113_resealed_observed_p95_authority(
            repo_root=repo_root,
            raw_root=raw_root,
            contract=contract,
            model_id=model_id,
    ...
    bindings = {
        model_id: gate.verified_v2113_observed_p95_authority_binding(
            gate.v2113_observed_p95_receipt_path(raw_root, model_id),
            repo_root=repo_root,
            raw_root=raw_root,
            expected_git_commit=release_commit,
            contract=contract,
    ...
    receipt_path, receipt = gate.persist_v2113_post_gate_authority(
        repo_root=repo_root,
        raw_root=raw_root,
        contract=contract,
        expected_git_commit=release_commit,
...
def _scientific_config(
    ...
    return VerifiedRunConfig(
        run_id="v2113-generic-consumer-regression",
        seed=2010922376,
        num_agents=2,
        episode_length=6,
        context_mode="full",
        ...
        pilot_tag=gate.V2113_SCIENCE_TAG,
        allow_scientific_scope=True,
        preflight_p95_reservations=reservations,
    )


def test_v2113_schema_is_registered_without_replacing_parent_adapter() -> None:
    registry = authority.DEDICATED_OBSERVED_P95_BINDING_SCHEMA_REGISTRY

    assert registry[gate.V2113_GATE_SCHEMA_VERSION] == ("v2.11.3-post-gate-authority")
    assert registry["finevo-pilot-v2.11.2-post-gate-authority-v1"] == (
        "v2.11.2-post-gate-authority"
    )


def test_v2113_gate_reaches_generic_binding(
    frozen_v2113_release: dict[str, Any],
) -> None:
    case = frozen_v2113_release
    dedicated = gate.verified_v2113_gate_authority_binding(
...
def test_v2113_gate_reaches_runner_consumers_before_provider(
    frozen_v2113_release: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    case = frozen_v2113_release
    ...
    runner_reservations = gate.runner_reservations_from_v2113_gate_binding(binding)
...
def test_v2113_mapping_only_and_alias_paths_fail_closed(
    frozen_v2113_release: dict[str, Any],
) -> None:
    case = frozen_v2113_release
    with pytest.raises(
    ...
    alias = Path("experiment_results/pilot-v2.11.3/raw/alias/post_gate_authority.json")
...
def test_v2113_wrong_contract_and_binding_substitution_fail_closed(
    frozen_v2113_release: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    case = frozen_v2113_release
    ...
    original = gate.verified_v2113_gate_authority_binding
...
def test_v2113_symlink_receipt_fails_closed(
    frozen_v2113_release: dict[str, Any],
) -> None:
    case = frozen_v2113_release
