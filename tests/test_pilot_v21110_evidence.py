...
def _parent_capability_fixture(parent_rows):
    ...
    for row in parent_rows:
        if row["stage_id"] != "capability-gate":
            continue
        wrapper = deepcopy(row["capability"])
        wrapper["capability"]["historical_source_calls"] = 30
        result[row["model_id"]] = {
            "capability": {
                **wrapper,
                "integrity": {
                    "content_sha256": row["capability"]["capability"][
                        "source_wrapper_content_sha256"
                    ]
                },
            }
...
def _normalized_parent_ac_receipts(current):
    ...
    for stage_id in ("experiment-a", "experiment-c"):
        binding = deepcopy(frozen[stage_id])
        binding["path"] = "/immutable-v2115/" + binding["path"]
        receipts[stage_id] = binding
    return {"receipts": receipts}


def test_external_parent_gates_accept_normalized_scientific_matrix_field():
    current, parent, parent_rows, *_ = _terminal_fixture()

    gates = _external_parent_gates(
        current,
        parent,
        parent_rows,
        _normalized_parent_ac_receipts(current),
    )

    assert gates["experiment_a"]["status"] == "no-go"
    assert gates["experiment_c"]["status"] == "no-go"


@pytest.mark.parametrize("stage_id", ("experiment-a", "experiment-c"))
def test_external_parent_gates_reject_missing_or_flipped_matrix_field(stage_id):
    current, parent, parent_rows, *_ = _terminal_fixture()
    receipts = _normalized_parent_ac_receipts(current)
    receipts["receipts"][stage_id]["scientific_matrix_complete"] = not receipts[
        "receipts"
    ][stage_id]["scientific_matrix_complete"]

    with pytest.raises(PilotEvidenceError, match="receipt differs"):
        _external_parent_gates(current, parent, parent_rows, receipts)
