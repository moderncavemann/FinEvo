...
def test_v210_publish_evidence_selects_lane_separated_adapter(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = []
    package_dir = tmp_path / "evidence" / "current_v2" / "pilot-v2.10"
    ...
    monkeypatch.setattr(
        ...
        "load_pilot_contract",
        lambda _path: SimpleNamespace(
            contract_id=run_pilot.PILOT_V210_CONTRACT_ID
        ),
    )
    monkeypatch.setattr(
        run_pilot,
    ...
    monkeypatch.setattr(
        ...
        "build_pilot_evidence_package",
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("V2.10 must use the lane-separated evidence builder")
        ),
    )
    monkeypatch.setattr(
        run_pilot,
