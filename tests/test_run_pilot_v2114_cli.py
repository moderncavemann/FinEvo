...
from verified_memory.pilot_contract import PILOT_CONTRACT_ID_V2_11_4
...
def _forbidden(*_args: Any, **_kwargs: Any) -> None:
    raise AssertionError("V2.11.4 CLI crossed the requested boundary")


def _contract(status: str = "frozen") -> SimpleNamespace:
    return SimpleNamespace(contract_id=PILOT_CONTRACT_ID_V2_11_4, status=status)


def test_v2114_raw_namespace_is_fresh_and_exact() -> None:
...
@pytest.mark.parametrize(
    ("provided", "message"),
    (
        ((), r"V2\.11\.4 parent-import requires --parent-repo-root"),
        (
            ("parent",),
            r"V2\.11\.4 parent-import requires --authority-repo-root",
        ),
    ),
)
def test_v2114_parent_import_requires_both_immutable_roots_before_dispatch(
    tmp_path: Path,
    ...
    if "parent" in provided:
        argv.extend(["--parent-repo-root", str(tmp_path / "v2113")])
...
def test_v2114_draft_real_stages_stop_before_dispatch(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    stage_id: str,
) -> None:
    ...
    if stage_id == "parent-import":
        argv.extend(
            [
                "--parent-repo-root",
                str(tmp_path / "pilot-v2.11.3-science"),
                "--authority-repo-root",
    ...
    with pytest.raises(
        ...
        match=r"V2\.11\.4 real stages require a frozen contract",
    ):
        run_pilot.execute(run_pilot.build_parser().parse_args(argv))


@pytest.mark.parametrize("stage_id", REGISTERED_STAGES)
def test_v2114_frozen_stage_forwards_dual_roots_only_to_parent_import(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    stage_id: str,
) -> None:
    raw_root = tmp_path / "experiment_results" / "pilot-v2.11.4" / "raw"
    parent_root = tmp_path / "pilot-v2.11.3-science"
...
def test_v2114_authority_root_is_rejected_outside_parent_import(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(run_pilot, "execute_stage", _forbidden)
