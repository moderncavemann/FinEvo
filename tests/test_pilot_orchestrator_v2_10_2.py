...
def _projection_and_flat_binding(contract, model_id: str):
        ...
        runtime_model: {
            kind: {
                "authority": {
                    ...
                    "pilot_tag": "pilot-v2.10.2-science",
                },
    ...
    return (
        ...
        {
            "receipt_path": (
                "experiment_results/pilot-v2.10.2/raw/parent-import/"
...
def test_v2102_denominator_and_cli_use_the_fresh_contract() -> None:
    contract = _contract()
    counts = Counter(spec.stage_id for spec in contract.expand())

    assert contract.contract_id == orchestrator.V2102_CONTRACT_ID
...
def test_v2102_parent_qref_stage0_transactions_are_zero_provider(
    tmp_path: Path,
    ...
    def forbidden(*_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("V2.10.2 prerequisite transaction constructed a provider")
    ...
    monkeypatch.setattr(
        orchestrator,
        "_persist_v2102_parent_import_for_orchestrator",
        lambda **_kwargs: {
    ...
    qref = orchestrator._seal_bound_payload(
        {
            "schema_version": (orchestrator.PILOT_V2102_IMPORTED_QREF_SCHEMA_VERSION),
            "status": "pass",
    ...
    orchestrator._execute_v2102_q_ref_import_stage(
        contract,
        ...
        for spec in contract.expand(stage="stage0-calibration"):
            ...
            orchestrator._persist_exact_json(
                ...
                orchestrator._seal_bound_payload(
                    {
                        "schema_version": (
                            orchestrator.PILOT_V2102_IMPORTED_RUN_ENVELOPE_SCHEMA_VERSION
                        ),
                        "contract_id": contract.contract_id,
                        "contract_sha256": contract.canonical_hash,
...
def test_v2102_p95_tamper_stops_before_provider(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    field: str,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: pytest.fail(
            "provider constructed after malformed V2.10.2 p95 binding"
        ),
    )
...
def test_v2102_development_fake_matrix_is_39_diagnostic_cells(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def forbidden(*_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("V2.10.2 fake matrix attempted a live provider")
...
@pytest.mark.parametrize(
    ...
    (
        (CONTRACT_PATH, "pilot-v2.10.2-science"),
        (V2101_CONTRACT_PATH, "pilot-v2.10.1-science"),
    ),
)
