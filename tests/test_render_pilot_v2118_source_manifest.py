...
import scripts.render_pilot_v2118_source_manifest as renderer
...
def test_unpinned_source_bootstrap_is_draft_only(
    ...
    document = {
        "contract_id": "finevo-pilot-v2.11.8",
        "status": "frozen",
    }
    ...
    with pytest.raises(ValueError, match="only an unpinned V2.11.8 draft"):
...
def test_source_renderer_emits_canonical_bytes_and_binds_both_roots(
    ...
    manifest = {
        ...
        "failed_release": {"contract_id": "finevo-pilot-v2.11.7"},
    ...
    monkeypatch.setattr(renderer, "build_v2118_source_manifest", build)
    contract_path = tmp_path / "contract.json"
    repo_root = tmp_path / "child"
    failed_root = tmp_path / "v2117-no-go"
...
def test_check_mode_is_read_only_and_fails_on_byte_drift(
    ...
    output = tmp_path / "pilot_v2_11_8_source_manifest.json"
    ...
    with pytest.raises(SystemExit, match="tracked V2.11.8 source manifest drifted"):
        renderer.main(argv)
