...
SOURCE_MANIFEST_PATH = ROOT / "experiments" / "pilot_v2_11_5_source_manifest.json"
...
def _forbidden_provider(*_args: Any, **_kwargs: Any) -> None:
    raise AssertionError("V2.11.5 operational reseal constructed a provider")


def test_v2115_consumer_normalization_separates_source_and_current_generation() -> None:
    ...
    source = enriched["gpt52_main"][runtime]
    current = deepcopy(source)
    for call_kind in ("action", "semantic"):
        authority = current[call_kind]["authority"]
        for field in (
        ...
        authority.update(
            {
                ...
                "pilot_tag": "pilot-v2.11.5-science",
                "source_projection_file_sha256": "b" * 64,
    ...
    assert not orchestrator._v2115_cross_generation_reservations_match(
        source_runtime_reservations=source,
        current_runtime_reservations=drifted,
    )
    drifted = deepcopy(current)
    drifted["semantic"]["reservation"]["sample_count"] = 9
    assert not orchestrator._v2115_cross_generation_reservations_match(
        source_runtime_reservations=source,
        current_runtime_reservations=drifted,
    )

    loader_source = inspect.getsource(orchestrator._load_verified_projection)
    assert (
        'global_binding.get("reservations", {}).get(runtime_model)\n'
        '            != authority["reservations"].get(runtime_model)'
        in loader_source
    )
    assert "_v2115_cross_generation_reservations_match" in loader_source


def test_v2115_preflight_reseal_success_is_zero_provider_and_keeps_science_fresh(
    tmp_path: Path,
    ...
    monkeypatch.setattr(
        orchestrator,
        "_verify_v2115_preflight_import_gate",
    ...
    monkeypatch.setattr(
        orchestrator, "persist_v2115_post_gate_authority", persist_post_gate
    )
    monkeypatch.setattr(orchestrator, "verify_v2115_gate_receipt", lambda *_a, **_k: {})
    ...
    result = orchestrator._execute_v2115_preflight_authority_import_stage(
        contract,
