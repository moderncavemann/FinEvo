...
@pytest.fixture
def v2118_adapter_case(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> dict[str, Any]:
    receipt = {
        "schema_version": continuation.V2118_CURRENT_AUTHORITY_SCHEMA_VERSION,
...
def test_v2118_schema_is_registered_without_replacing_v2117_adapter() -> None:
    registry = authority.DEDICATED_OBSERVED_P95_BINDING_SCHEMA_REGISTRY

    assert registry[continuation.V2118_CURRENT_AUTHORITY_SCHEMA_VERSION] == (
...
def test_v2118_generic_adapter_uses_exact_path_contract_and_verifier(
    v2118_adapter_case: dict[str, Any],
) -> None:
    case = v2118_adapter_case
...
def test_v2118_mapping_only_and_alias_paths_fail_closed(
    v2118_adapter_case: dict[str, Any],
) -> None:
    case = v2118_adapter_case
    with pytest.raises(
    ...
    alias = Path(
        "experiment_results/pilot-v2.11.8/raw/alias/post_gate_authority.json"
    )
...
def test_v2118_missing_frozen_contract_identity_fails_closed(
    v2118_adapter_case: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    case = v2118_adapter_case
...
def test_v2118_verifier_failure_is_wrapped_fail_closed(
    v2118_adapter_case: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    case = v2118_adapter_case
    ...
    def reject(*_args: Any, **_kwargs: Any) -> None:
        raise continuation.PilotV2118ContinuationError("fixture drift")
...
def test_v2118_binding_substitution_fails_closed(
    v2118_adapter_case: dict[str, Any],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    case = v2118_adapter_case
